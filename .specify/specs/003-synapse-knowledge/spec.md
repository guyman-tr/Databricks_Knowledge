...
- **FR-018 (ALTER ↔ wiki name-level parity)**: Count-only matching between wiki Elements and ALTER COLUMN statements is NOT sufficient. Every ALTER COLUMN name (after stripping backticks and lowercasing) MUST match a wiki Element Name (lowercased), and every wiki element MUST have a corresponding ALTER COLUMN. A count-match-with-name-mismatch (e.g., wiki has `ColumnA` but ALTER has `ColumnB`) causes the description to land on the wrong UC column — this is a HARD FAIL. Verified by `tools/audit_wiki_alter_comment_parity.py`. (2026-03-30: deploy audit found count-matching files with misaligned column names.)
- **FR-019 (UTF-8 file encoding on all writes)**: ALL file writes (`.md`, `.review-needed.md`, `.lineage.md`, `.alter.sql`) MUST use explicit UTF-8 encoding — Python: `open(path, "w", encoding="utf-8")` or `Path.write_text(..., encoding="utf-8")`. MUST NOT rely on system default encoding (Windows CP1252/CP1255 corrupts em-dash tier tags and special characters). The `_uc_comment_sanitize.py` sanitizer is a runtime safety net, not a substitute for correct encoding at write time. (2026-03-30: mojibake root-caused to system-default encoding on file writes.)
- **FR-020 (Backtick-quote ALL column names in ALTER)**: Every `ALTER COLUMN` identifier in generated `.alter.sql` MUST be backtick-quoted (`` `ColumnName` ``), not just those with special characters. Standard alphanumeric names wrapped in backticks are harmless; omitting them on names with digits, hyphens, slashes, or reserved words causes runtime failures. This is a zero-cost safety measure — always quote. See `11w-write-objects.mdc` rule 9. (2026-03-30: `Dim_Channel.Organic/Paid`, `Dealing_dbo.BNY-eToro_Units` failures.)
...
- **SC-011**: For each deployed schema, `audit_wiki_alter_comment_parity.py` reports zero name-only mismatches (FR-018) — every ALTER COLUMN name matches a wiki Element Name after normalization
- **SC-012**: All Python scripts under `tools/` and `knowledge/synapse/Wiki/` that write files use explicit `encoding="utf-8"` in every `open()` / `write_text()` call (FR-019). Verifiable by grep
- **SC-013**: Every `ALTER COLUMN` in newly generated `.alter.sql` files uses backtick quoting on the column identifier (FR-020). Verifiable by `audit_alter_uc_mapping.py`
