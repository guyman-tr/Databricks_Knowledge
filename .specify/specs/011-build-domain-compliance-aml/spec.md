...
- **FR-011** (NEVER-DROP): System MUST validate every node in the final scope against `system.information_schema.tables` via `user-databricks_sql` MCP and classify each into one of `{UC, synapse_only, hybrid_synapse_uc, external_system, manual_only}`. The classification result is **additive annotation, not a filter**. Anchors that fail UC presence (e.g., `BI_DB_dbo.BI_DB_AML_BI_Alerts_New`, `BI_DB_dbo.BI_DB_RiskAlertManagementTool`, the Actimize-fed CDD score, the `BackOffice.SetRiskClassification_*` SP family) are kept in the skill with `locality: synapse_only | external_system | manual_only` and an explicit bridge strategy. The output goes to `knowledge/skills/_uc_object_map.action_required.md` (for any genuinely-missing anchors that require DE follow-up) AND to a new `## Locality classification` section appended to `_compliance_staleness.md`. Rationale: at this phase a substantial portion of AML/Compliance LOGIC and OUTPUT lives only in Synapse; pretending the gap doesn't exist defeats the purpose of the skill. See `knowledge/skills/_AUTHORITY_HIERARCHY.md` "Locality is orthogonal to authority" and `.specify/templates/domain-build-template.md` Phase A.6.
- **FR-012**: System MUST author whatever file shape Phase B chose, conforming to spec 007 FR-009 (DataPlatform DE skill-creator schema; `lint_skill.py` exit 0; cross-reference to `_AUTHORITY_HIERARCHY.md` from the SKILL.md). Any sub-skill whose Phase A.6 produced ≥1 non-UC anchor MUST populate `external_references:` in frontmatter AND include a `## External Data Sources` body section explaining locality, source system, role, and bridge strategy per non-UC entry.
