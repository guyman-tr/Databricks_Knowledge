<!--
Sync Impact Report — v1.13.0 → v1.14.0 (MINOR)

Version change: 1.13.0 → 1.14.0
Bump rationale: Codified anti-AI-slop guardrail after an incident in which a
  skill artifact (knowledge/skills/_shared/valid-users-filter-contract.md) and
  four super-domain SKILL.md callouts asserted editorialized codepoint-to-name
  mappings that did not match the dictionary tables: `PlayerLevelID = 4`
  labeled as "Popular Investor" when DWH_dbo.Dim_PlayerLevel.Name = 'Internal';
  `LabelID = 26` labeled as "BonusOnly" when DWH_dbo.Dim_Label.Name = 'ILQ';
  `LabelID = 30` labeled as "internal" when DWH_dbo.Dim_Label.Name = 'Dealing'.
  Root cause was a single editorialized column comment in Dim_Customer.alter.sql
  that propagated downstream. Principle XI makes verbatim dictionary names
  NON-NEGOTIABLE for any codepoint-to-meaning claim in any produced artifact.

Modified principles:
  - Added: XI. No Unsubstantiated Facts (NON-NEGOTIABLE). Every factual claim
    in any produced artifact (wiki COMMENT, ALTER, SKILL.md, footer, scope
    note, contract row, table interpretation) must trace to a Tier 1–3 source
    or be flagged `[UNVERIFIED]` and surfaced in the review sidecar. For
    codepoint-to-name mappings specifically (PlayerLevelID=N, LabelID=N,
    AccountTypeID=N, CountryID=N, etc.), the human-readable label MUST be
    the verbatim `Name` (or equivalent) column from the corresponding
    `Dim_*` / `Dictionary.*` table — queried live or read from the
    canonical SSDT DDL — never an AI-paraphrased colloquial name.

Quality Gates updated:
  - Added: every codepoint→meaning assertion in any artifact must cite the
    dictionary source. Pre-publish check: grep new content for the pattern
    "<col>ID = <n>" or "<col>ID IN (<n>)" — every match MUST be accompanied
    by the verbatim `Dim_<Table>.Name` or a `[UNVERIFIED]` flag.

Pipeline rules requiring updates: none — Principle XI binds on the agent at
  write-time, not on a phase. The skill-template, glossary, and review-sidecar
  workflow already accommodate `[UNVERIFIED]` flags.

Specs requiring updates: none

Follow-up TODOs:
  - Audit the existing knowledge/skills tree for residual "Popular Investor /
    BonusOnly" propagations (grep already identified ~11 files); each must be
    re-checked against the live Dim_PlayerLevel / Dim_Label dictionaries
    before its next regeneration.
  - Re-author the Dim_Customer.alter.sql IsValidCustomer / IsCreditReportValidCB
    column comments to use the verbatim dictionary names.

Previous report (v1.12.0 → v1.13.0 (MINOR)):

Sync Impact Report — v1.12.0 → v1.13.0 (MINOR)

Version change: 1.12.0 → 1.13.0
Bump rationale: Codified eToro DataPlatform DE skill-creator schema as the
  binding contract for any skill artifact produced by this SpecKit. The DE
  team enforces a CI/CD-checked format for SKILL.md files at workspace level
  (/Workspace/.assistant/skills/*). Skills generated by this SpecKit that
  drift from that schema cannot be promoted to workspace-level deployment
  and silently fail retrieval-time ACL/quality gates. Adding Principle X
  makes DE-schema compliance NON-NEGOTIABLE for every skill-producing spec
  (currently 007-package-agent-domains, future skill specs).

Modified principles:
  - Added: X. Skill Artifacts Conform to eToro DE Schema (NON-NEGOTIABLE).
    Every SKILL.md authored or generated by this SpecKit MUST conform to
    .specify/memory/skill-schema.md (mirror of /Workspace/.assistant/skills/
    skill-creator/SKILL.md). Pre-commit lint via tools/skills/lint_skill.py
    must exit 0. Pre-creation overlap check against /Workspace/.assistant/
    skills/* is mandatory.

Quality Gates updated:
  - Added: every new or modified SKILL.md must pass tools/skills/lint_skill.py
    (mirrors DE CI checks SEC-001, QUAL-001..005, DOMAIN-001..007,
    AUTHOR-001..002, hygiene). Lint exit 0 is a hard merge gate.
  - Added: SKILL.md `last_validated_at` must be ≤90 days old at deployment
    time (DE check DOMAIN-003 carry-through).

Templates updated:
  - .specify/templates/skill-template.md → NEW. Single-domain (shape A) and
    cross-domain (shape B) variants in one template. Pre-commit checklist
    inlined as a comment block.

Pipeline rules requiring updates: none — skill-producing specs (007 and any
  future spec that emits SKILL.md) inherit Principle X automatically through
  the constitution + template + linter combination.

Specs requiring updates:
  - .specify/specs/007-package-agent-domains/spec.md → append FR-009
    (skill artifact format conformance) referencing Principle X.

Follow-up TODOs:
  - Retrofit existing knowledge/skills/payments/, revenue-and-fees/, cross/*
    files to DE format if any of them are ever promoted from user-level
    (/Workspace/Users/<email>/.assistant/skills/) to workspace-level
    (/Workspace/.assistant/skills/). User-level deployment is not subject
    to DE CI today and the existing custom frontmatter (keywords, load_after,
    sub_skills) continues to work for the local Cursor router.

Previous report (v1.11.0 → v1.12.0 (MINOR)):
...
### X. Skill Artifacts Conform to eToro DE Schema (NON-NEGOTIABLE)
**Every SKILL.md authored or generated by this SpecKit MUST conform to the eToro DataPlatform skill-creator schema.** That schema is mirrored byte-for-byte at `.specify/memory/skill-schema.md` from the canonical source `/Workspace/.assistant/skills/skill-creator/SKILL.md`. The DE team enforces this schema in CI/CD on the workspace-level skills folder (`/Workspace/.assistant/skills/*`); skills that drift cannot be promoted, silently fail retrieval, or be flagged at merge time.

**Required for every SKILL.md:**
1. **Frontmatter fields** — `id` (kebab-case, matches filename stem), `name`, `description` (≥30 chars, third-person, "Covers...", not "I help..."), `triggers`, `required_tables` (≥1 fully qualified UC name `catalog.schema.table_or_view`), `version` (integer), `owner` (default `"dataplatform"`), `last_validated_at` (ISO date, ≤90 days old at deployment).
2. **Required sections** — `## When to Use`, `## Scope` (with all three lines: `In scope:`, `Out of scope:`, `Last verified: YYYY-MM-DD`), `## Critical Warnings` (numbered list, severity-ordered Tier 1 → Tier 2 → Tier 3).
3. **Pre-creation overlap check** — before writing any new skill, list `/Workspace/.assistant/skills/*` and verify no existing workspace skill already owns the same `required_tables`, `triggers`, or domain. On overlap: STOP, alert the user, and suggest merge / narrow / restructure.
4. **Pre-commit lint** — `tools/skills/lint_skill.py <path>` must exit 0. The linter mirrors the DE CI checks: SEC-001 (no secrets), QUAL-001..005 (description ≥30 chars, body non-empty, ≤500 lines, no absolute paths, scope not vague), DOMAIN-001..007 (Scope tri-line, Last verified ≤90 days, When to Use exists, Critical Warnings numbered + tier-ordered), AUTHOR-001..002 (no backslash paths, third-person description), hygiene (no trailing whitespace in frontmatter, UTF-8, no BOM, ≤100 KB).

**Cross-domain skills:** A skill spanning two or more super-domains uses the `cross-` filename and `id` prefix and lists `required_tables` from each domain. Apart from naming, it follows the exact same DE schema as a single-domain skill — same lint, same CI checks. The cross-domain "shape B" section in `.specify/templates/skill-template.md` is the only structural addition.

**Mirror policy:** `.specify/memory/skill-schema.md` is a byte-for-byte mirror with a provenance header. Local additions (cross-domain conventions, our linter, our overlap-check workflow) live in adjacent files (`.specify/templates/skill-template.md`, `.cursor/commands/speckit.skill.md`, `tools/skills/lint_skill.py`), never inline in the mirror. Re-mirror with `databricks workspace export-dir /Workspace/.assistant/skills/skill-creator <tmp> --profile guyman --overwrite` and replace the body of `skill-schema.md` (preserve the provenance header).

**Authority interaction:** Principle X is independent of the Tier 1–5 source hierarchy in Section II. Tier hierarchy governs what goes INTO a knowledge artifact; Principle X governs the SHAPE of the artifact when that artifact is a SKILL.md. A skill whose body is internally Tier-1-correct but whose frontmatter or section structure violates the DE schema is still non-compliant and must be fixed before merge.

### XI. No Unsubstantiated Facts (NON-NEGOTIABLE)

**Every factual claim in every produced artifact MUST trace to a Tier 1–3 source — or carry an explicit `[UNVERIFIED]` flag.** This binds wiki COMMENTs, ALTER scripts, SKILL.md bodies, scope footers, contract rows, glossary entries, and any inline rule text the SpecKit emits. "Helpful" interpretive paraphrases that sound plausible but were not sourced from upstream wiki, code, live data, Atlassian, or a glossary entry are fabrication. They are AI slop and they ship wrong numbers — silently — for every downstream consumer.

**Codepoint-to-name mappings are the most failure-prone case** and have an absolute rule:

When any artifact names what a coded value means (`PlayerLevelID = N`, `LabelID = N`, `AccountTypeID = N`, `CountryID = N`, `PlayerStatusID = N`, etc.), the human-readable label MUST be the verbatim `Name` (or equivalent) column from the corresponding `Dim_*` / `Dictionary.*` table — queried live or read from the canonical SSDT DDL. Never paraphrase. Never editorialize. Never translate a dictionary code into a colloquial business name "because that's what people call it" unless the colloquial name itself is documented in `knowledge/glossary.md` (Tier 5) or an upstream wiki (Tier 1).

Anti-patterns (forbidden, exemplified by a real incident in v1.13.0 → v1.14.0):

| Asserted | Dictionary truth | Verdict |
|---|---|---|
| `PlayerLevelID = 4` (Popular Investor) | `Dim_PlayerLevel.Name = 'Internal'` | Fabrication |
| `LabelID = 26` (BonusOnly) | `Dim_Label.Name = 'ILQ'` | Fabrication |
| `LabelID = 30` (internal label) | `Dim_Label.Name = 'Dealing'` | Fabrication |
| `IsValidCustomer = 1` excludes "Popular Investors" | SP filters `PlayerLevelID <> 4`; the dictionary name is "Internal"; PIs are tracked by `GuruStatusID`, not `PlayerLevelID`, and are valid customers | Fabrication, propagated 5+ files deep |

Required pattern:

> "`IsValidCustomer = 1` ⇔ `PlayerLevelID <> 4` (`Dim_PlayerLevel.Name = 'Internal'`) AND `LabelID NOT IN (30, 26)` (`Dim_Label.Name IN ('Dealing', 'ILQ')`) AND `CountryID <> 250` (`Dim_Country.Name = 'eToro'` — internal jurisdiction)."

Every code-to-name link is the verbatim dictionary value. The artifact may add ONE non-editorialized clarifier per token (e.g., `'eToro' — internal jurisdiction`) only if that clarifier itself is sourced from the dictionary row (e.g., a `Region` or `IsHighRiskCountry` flag) or from a Tier-1 wiki / glossary entry. Inventing a clarifier because it "sounds about right" is fabrication.

**Procedural rule (binds the writing agent):**

1. Before writing any sentence of the form "X = N means Y" or "the filter excludes Y", **query the dictionary table** (or read its repo DDL — Constitution IX) and quote the `Name` column verbatim. This is non-optional; "I'm pretty sure" is not a source.
2. If the verbatim dictionary value is itself opaque (`Name = 'ILQ'` with no documented expansion in glossary or wiki), the artifact ships with the verbatim value AND a `[UNVERIFIED — meaning of ILQ not in glossary]` flag. Surface to the review sidecar. Never guess an expansion.
3. Business-meaning interpretations of a filter, flag, or aggregate (e.g., "this counts only retail customers", "this excludes the dealing desk") must be traceable to (a) the SP source code's own comment, (b) a Confluence / Jira citation, (c) a glossary entry, or (d) an explicit user statement in the originating session. None of those = `[UNVERIFIED]`.
4. When an earlier artifact is found to contain a Principle-XI violation, **the propagation MUST be unwound**: find every downstream artifact that inherited the bad label (grep for the editorialized string), correct each one against the verbatim dictionary, and note the root-cause artifact + corrected version in the sync impact report.

**Authority interaction:** Principle XI is independent of the Tier 1–5 hierarchy in Section II. The Tier hierarchy governs which source wins when sources conflict; Principle XI governs what counts as a source at all. A claim with no source is not Tier 4 — it is `[UNVERIFIED]` and must be flagged. Principle XI also overrides Principle X's "description ≥30 chars, third-person" floor: if the only verifiable description is short, pad with explicit `[UNVERIFIED]` notes rather than fabricated context to hit the character floor.
...
- **Codepoint-to-name claims must cite the dictionary verbatim (Principle XI).** Before publishing any artifact (wiki COMMENT, ALTER, SKILL.md, footer, glossary entry) that contains a phrase of the form "`<col>ID = <n>`" or "`<col>ID IN (<n>,…)`" with a colloquial label, the writing agent MUST query the corresponding `Dim_*` / `Dictionary.*` table (live MCP or repo DDL) and quote the `Name` column verbatim. Editorialized labels (e.g., calling `PlayerLevelID=4` "Popular Investor" when the dictionary says "Internal") are fabrication and MUST be flagged or removed. If the verbatim value is itself unclear, the artifact ships with the verbatim value plus a `[UNVERIFIED — meaning not in glossary]` flag, surfaced to the review sidecar. When a violation is detected, the propagation MUST be unwound across every downstream artifact.
...
**Version**: 1.14.0 | **Ratified**: 2026-02-25 | **Last Amended**: 2026-05-13
