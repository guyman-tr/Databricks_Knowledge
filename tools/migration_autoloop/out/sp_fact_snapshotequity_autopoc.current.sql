...
case 
when 
DATE_TRUNC('YEAR', CAST(DATEADD(DAY, -1, current_date()) AS DATE)) = CAST(DATEADD(DAY, -1, current_date()) AS DATE) -- if first day of year
then year(DATEADD(DAY, -1, current_date()))-1
else year(DATEADD(DAY, -1, current_date()))
end
)
and right(a.DateRangeID, 4) = 1231;
...
) AND CAST(left(CAST(a.DateRangeID AS STRING),4)+right(CAST(a.DateRangeID AS STRING),4) AS TIMESTAMP)>=V_maxentrydate THEN UPDATE SET 
...
VALUES(  b.CreditID
,b.CID
,COALESCE(CAST(date_format(V_date, 'yyyyMMdd')+right(date_format(V_largedate, 'yyyyMMdd'),4) AS bigint), CAST(date_format(DATEADD(DAY, -1, current_date()), 'yyyyMMdd')||'1231' AS bigint))
...
WHERE DateRangeID <> COALESCE(CAST(date_format(V_date, 'yyyyMMdd')+right(date_format(V_largedate, 'yyyyMMdd'),4) AS bigint), CAST(date_format(DATEADD(DAY, -1, current_date()), 'yyyyMMdd')||'1231' AS bigint))
...
,current_timestamp()
where COALESCE(CAST(date_format(V_date, 'yyyyMMdd')+right(date_format(V_largedate, 'yyyyMMdd'),4) AS bigint), CAST(date_format(DATEADD(DAY, -1, current_date()), 'yyyyMMdd')||'1231' AS bigint)) not in (SELECT DateRangeID from dwh_daily_process.migration_tables.Dim_Range);
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
-- original value 0 /* @@ROWCOUNT not supported */,NULL,NULL,NULL;
/**************************************************************CLOSE YEAR FOR CIDs*********************************************************************************************************/
