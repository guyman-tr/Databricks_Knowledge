...
TRUNCATE TABLE dwh_daily_process.migration_tables.Ext_FCUPNL_BackOfficeCustomer;
    ...
    SELECT ROW_NUMBER() OVER (PARTITION BY a.CID ORDER BY ValidFrom, ValidTo) rank,
           ...
           a.ValidFrom,
           a.ValidTo
    FROM dwh_daily_process.migration_tables.vw_fcupnl_backofficecustomer_target a
        JOIN
        (
            SELECT DISTINCT
                   CID
            FROM dwh_daily_process.migration_tables.vw_fcupnl_backofficecustomer_target
...
INSERT INTO dwh_daily_process.migration_tables.Ext_FCUPNL_GetSpreadedPriceCandle60MinSplitted (`rn`, `InstrumentID`, `AskLast`, `BidLast`, `AskFirst`, `DateFrom`)
...
,current_timestamp() as UpdateDate
 FROM dwh_daily_process.migration_tables.vw_fcupnl_currencypricemaxdate_target;
