...
-- Extract Ext_Dim_Position_Real --------------------------------------->
	INSERT INTO dwh_daily_process.migration_tables.Ext_Dim_Position_Real
		...
		SELECT 
		   ...
		   , MirrorID
		   , IsOpenOpen
		   , CAST(date_format(CAST(Occurred AS DATE), 'yyyyMMdd') AS int) as OpenDateID
		   , 0 as CloseDateID
		   , LotCountDecimal
		   , SpreadedCommission
...
-- Extract Ext_Dim_Position_History_Real ------------------------>
	INSERT INTO dwh_daily_process.migration_tables.Ext_Dim_Position_History_Real
		...
		SELECT
		   ...
		   , MirrorID
		   , IsOpenOpen
		   , CAST(date_format(CAST(OpenOccurred AS DATE), 'yyyyMMdd') AS int) as OpenDateID
		   , CAST(date_format(CAST(CloseOccurred AS DATE), 'yyyyMMdd') AS int) as CloseDateID
		   , LotCountDecimal
		   , SpreadedCommission
		   , EndForexRate
		   , LastOpConversionRate
...
call dwh_daily_process.migration_tables.sp_dim_position_hedgetype_history_autopoc(V_Yesterday);
call dwh_daily_process.migration_tables.sp_dim_position_hedgetype_real_autopoc(V_Yesterday);
...
INNER JOIN dwh_daily_process.migration_tables.Ext_Dim_Position_History_Real b ON a.PositionID = b.PositionID --AND (a.CloseDateID = 19000101 or  a.CloseDateID = 0 ) --AND b.CloseDateID <> 0
 AND ( a.CloseOccurred = TIMESTAMP '1900-01-01 00:00:00' or a.CloseDateID = 0 ) ---------------------------------------------------------------------------->
...
CloseOccurred = CASE WHEN b.CloseOccurred >= CAST ( current_timestamp() as date ) THEN TIMESTAMP '1900-01-01 00:00:00' else b.CloseOccurred end ,
CloseDateID = CASE WHEN b.CloseOccurred >= CAST ( current_timestamp() as date ) THEN 0 ELSE CAST(date_format(CAST(b.CloseOccurred AS DATE), 'yyyyMMdd') AS int) END ,
...
;
	...
	SELECT  b.`PositionID`
	      ...
	      , b.`CommissionOnClose`
	      , b.`OpenOccurred`
	      , CASE WHEN b.CloseOccurred  >=CAST(current_timestamp() as date)  THEN TIMESTAMP '1900-01-01 00:00:00' ELSE b.CloseOccurred  END as CloseOccurred
	    -- , b.[CloseOccurred]
	      , b.`ParentPositionID`
	      ...
	      , b.`TreeID`
	      , b.`OpenDateID`
	      , CASE WHEN b.CloseOccurred >=CAST(current_timestamp() as date)  THEN 0 ELSE CAST(date_format(CAST(b.CloseOccurred AS DATE), 'yyyyMMdd') AS int) END AS CloseDateID
	    --, b.[CloseDateID]
...
OpenMarkup = s.OpenMarkup --20240115 Inbal BML
 -- Ofir - DLT commition
 ,
A_TGT.DLTOpen = s.DLTOpen -- , t.DLTClose = s.DLTClose
 ,
A_TGT.OpenMarkupByUnits = s.OpenMarkupByUnits ,
A_TGT.CommissionVersion = s.CommissionVersion ,
A_TGT.LotCountDecimal = s.LotCountDecimal --20250121 Inbal BML
 ,
A_TGT.OpenPositionReasonID = s.OpenPositionReasonID ,
A_TGT.OpenTotalTaxes = s.OpenTotalTaxes --20250620 Adi Ferber
 ,
A_TGT.OpenTotalFees = s.OpenTotalFees --20250620 Adi Ferber
 /* 2025-09-08     Daniel Kaplan  */
 ,
A_TGT.EstimateCloseFeeForCFD = s.EstimateCloseFeeForCFD ,
A_TGT.EstimateCloseFeeOnOpenByUnits = s.EstimateCloseFeeOnOpenByUnits ,
A_TGT.EstimateCloseFeeOnOpen = s.EstimateCloseFeeOnOpen ,
A_TGT.Close_PnLInDollars = s.Close_PnLInDollars ,
A_TGT.Close_CalculationRate = s.Close_CalculationRate ,
A_TGT.Close_ConversionRate = s.Close_ConversionRate ,
A_TGT.Close_PriceType = s.Close_PriceType ,
A_TGT.CurrentCalculationRate = s.CurrentCalculationRate ,
A_TGT.CurrentConversionRate = s.CurrentConversionRate /* 2025-09-08     Daniel Kaplan  */;
/* 
...
call dwh_daily_process.migration_tables.sp_dim_position_reopen_autopoc(V_Yesterday);
