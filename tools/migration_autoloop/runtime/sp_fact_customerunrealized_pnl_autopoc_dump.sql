...
WHERE DateModified = CAST(date_format(DATE('2026-06-18'), 'yyyyMMdd') AS int);
...
from  (select  CID, MirrorID, InstrumentID, AmountInUnitsDecimal, LastOpConversionRate, InitForexRate, 
         ...
         where DATE('2026-06-18') > OpenOccurred
       union all
	   select CID, MirrorID, InstrumentID, AmountInUnitsDecimal,  LastOpConversionRate, InitForexRate, 
	     ...
	     where  DATE('2026-06-18') > OpenOccurred and DATE('2026-06-18') < CloseOccurred
		) a
...
where DateID=CAST(date_format(DATEADD(day, -1, DATEADD(week, DATEDIFF(week, DATE('2026-06-18'), 0), 0)), 'yyyyMMdd') AS int) and SampleSize>100
;-- [autopoc] removed call dbo.LastRowCount(...);
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
...
where b.DateKey=CAST(date_format(DATE('2026-06-18'), 'yyyyMMdd') AS int) 
...
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
CREATE TABLE IF NOT EXISTS dwh_daily_process.migration_tables.fcupnl_debug_counts (run_date_id INT, captured_at TIMESTAMP, prices_cnt BIGINT, open_positions_cnt BIGINT, open_positions_final_cnt BIGINT, end_conv_cnt BIGINT, unrealized_cnt BIGINT, unrealized_filtered_cnt BIGINT) USING DELTA;INSERT INTO dwh_daily_process.migration_tables.fcupnl_debug_counts SELECT CAST(date_format(DATE('2026-06-18'), 'yyyyMMdd') AS int), current_timestamp(), (SELECT COUNT(*) FROM TEMP_TABLE_Prices), (SELECT COUNT(*) FROM TEMP_TABLE_OpenPositions), (SELECT COUNT(*) FROM TEMP_TABLE_OpenPositionsFinal), (SELECT COUNT(*) FROM TEMP_TABLE_EndConvertionRate), (SELECT COUNT(*) FROM TEMP_TABLE_UnrealizedPnL), (SELECT COUNT(*) FROM TEMP_TABLE_UnrealizedPnL WHERE PnLInDollars IS NOT NULL AND CalculatedNetProfit IS NOT NULL);INSERT INTO dwh_daily_process.migration_tables.Fact_CustomerUnrealized_PnL 
(CID
										, DateModified
										, PositionPnL_old
...
------------------Futures
	...
	, PositionPnLStocksMargin
	from 
	(select CID,CAST(date_format(DATE('2026-06-18'), 'yyyyMMdd') as int) as DateID
...
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
DELETE FROM dwh_daily_process.migration_tables.Fact_CustomerUnrealized_PnL_UserAPI where DateModified = CAST(date_format(DATE('2026-06-18'), 'yyyyMMdd') AS int)
