...
*********************************************************************************************/
   -- exec dwh_dbo.[SP_Fact_CustomerAction] '2025-08-01'
  -- DECLARE @dt as DATE = '2025-08-01'
  --'2021-07-01'
...
;
	...
	where CAST(CreatedAt as TIMESTAMP) >= DATEADD(DAY, -7, V_Yesterday) and  CAST(CreatedAt as TIMESTAMP) <= V_CurrentDate
...
call dwh_daily_process.migration_tables.SP_STS_User_Operations_Data_History_CREATE_SWITCH_SINGLE(V_Yesterday);
...
SELECT 
	...
	`CountryISOCode`,
	`AdditionalData`,
    CAST(date_format(V_Yesterday, 'yyyyMMdd') AS int)  ,
    current_timestamp()
FROM dwh_daily_process.daily_snapshot.STS_Audit_UserOperationsData  
...
;
    ...
    `OpenMarkupByUnits`
		...
		ON a.InstrumentID = b.InstrumentID
		WHERE 
		(OpenOccurred>=DATEADD(HOUR, -1, V_Yesterday)  and OpenOccurred< V_CurrentDate)
		OR (CloseOccurred>= V_Yesterday  and CloseOccurred< V_CurrentDate);  --OPTION	(RECOMPILE)
--------------------------------------------------------------------
...
--TRUNCATE TABLE [DWH_dbo].Ext_FCA_OpenBook_Engagement

--INSERT INTO [DWH_dbo].[Ext_FCA_OpenBook_Engagement]
--            ([EngagementType]
--            ,[Occurred]
--            ,[UserName]
--            ,[PostRootID]
...
-- From [DWH_staging].[Streams0_dbo_Entries] With (Nolock)
-- Where TypeName In ('Like', 'Comment', 'Discussion')
-- And OccurredAt >= @Yesterday
-- And OccurredAt < @CurrentDate
...
SET V_DateID =  date_format(V_Yesterday, 'yyyyMMdd')
...
-- Insert to Fact_CustomerAction_Create_SWITCH_SINGLE --------------
	...
	WHERE ActionTypeID is not null;
--------------------------------------------------------------------
