...
;
	...
	set 
	CloseOccurred = CAST('1900-01-01' AS TIMESTAMP) ,
	CloseDateID = 0
...
;
	...
	SELECT  
	  ...
	  ,CAST('1900-01-01' AS TIMESTAMP) as CloseOccurred
	  ,0 As CloseDateID
	  ,MirrorTypeID
	  ,CloseMirrorActionType
...
;
	...
	SELECT
	  ...
	  ,CAST(date_format(CAST(Occurred AS DATE), 'yyyyMMdd') AS int) as OpenDateID
	    ...
	    ,CAST(date_format(CAST(ModificationDate AS DATE), 'yyyyMMdd') AS int)as CloseDateID
	  ,MirrorTypeID
	  ,CloseMirrorActionType
	  ,CAST(IsActive AS INT)
	  ,IsOpenOpen
...
UpdateDate = current_timestamp();
	...
	,OpenOccurred
	,OpenDateID
	,case when CloseOccurred >= cast(current_timestamp() as date) then CAST('1900-01-01' AS TIMESTAMP) else CloseOccurred end as CloseOccurred
	,case when CloseOccurred >= cast(current_timestamp() as date) then 0 else CAST(date_format(CAST(CloseOccurred AS DATE), 'yyyyMMdd') AS int) end as CloseDateID
	--,[CloseOccurred]
	--,[CloseDateID]
	,MirrorTypeID
	,CloseMirrorActionType
