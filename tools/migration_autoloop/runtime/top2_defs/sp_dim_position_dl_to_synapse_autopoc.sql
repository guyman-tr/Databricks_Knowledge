...
-- exec [DWH_dbo].[SP_Dim_Position_DL_To_Synapse] '20250907'
-- =============================================
/***************************
** Change History
**************************
...
2024-08-22     Katy F         Add CommisionVersion
2025-01-21	   Inbal BML	  Add LotCountDecimal to update of open positions that allready exist in Dim_Position
2025-01-25     Ofir A         add OpenPositionReasonID and ExitOrderType 
2025-06-25     Adi Ferber     add OpenTotalTaxes ,CloseTotalTaxes,OpenTotalFees,CloseTotalFees
2025-07-13	   Eitan L        add switch partition 
2025-09-08     Daniel Kaplan  Add new fields : EstimateCloseFeeForCFD ,EstimateCloseFeeOnOpenByUnits,EstimateCloseFeeOnOpen,Close_PnLInDollars,Close_CalculationRate,
Close_ConversionRate,Close_PriceType,CurrentCalculationRate ,CurrentConversionRate 

--********************************   **************************************************************************************/
    -- SET NOCOUNT ON added to prevent extra result sets from
    -- interfering with SELECT statements.
    
 --  DECLARE @dt Date ='2025-06-02'  
...
-- Extract Ext_Dim_Position_Real --------------------------------------->
	INSERT INTO dwh_daily_process.migration_tables.Ext_Dim_Position_Real
           ...
           /* New columns for DLT (Ofir)*/
           , DLTOpen
           -- , DLTClose
		   , OpenMarkupByUnits
		   , CommissionVersion
		   , OpenPositionReasonID -- ofir
		   , OpenTotalTaxes --20250620 Adi Ferber
		   , OpenTotalFees  --20250620 Adi Ferber
		   /* 2025-09-08     Daniel Kaplan  */
		...
		SELECT 
		   ...
		   , MirrorID
		   , IsOpenOpen
		   , CAST(date_format(DATEADD(DAY, 0, CAST(Occurred AS DATE)), 'yyyyMMdd') AS int) as OpenDateID
		   , 0 as CloseDateID
		   , LotCountDecimal
		   , SpreadedCommission
...
(CASE WHEN SellCurrencyID=1 THEN 1 WHEN BuyCurrencyID=1 THEN 1/cast(`InitForexRate` as decimal (16,8)) else cast(`LastOpConversionRate` as decimal (16,8)) end),0) AS int)  Volume
		   ...
		   , OpenMarkupByUnits
		   , a.CommissionVersion
		   , a.OpenActionType as OpenPositionReasonID 
		   , a.OpenTotalTaxes --20250620 Adi Ferber
		   , a.OpenTotalFees  --20250620 Adi Ferber
		   /* 2025-09-08     Daniel Kaplan  */
...
-- Extract Ext_Dim_Position_History_Real ------------------------>
	INSERT INTO dwh_daily_process.migration_tables.Ext_Dim_Position_History_Real
		   ...
		   , OpenMarkupByUnits
		   , CommissionVersion
		   , ExitOrderType -- Ofir New column
		   , OpenPositionReasonID
		   , OpenTotalTaxes --20250620 Adi Ferber
		   , CloseTotalTaxes --20250620 Adi Ferber
		   , OpenTotalFees  --20250620 Adi Ferber
		   , CloseTotalFees  --20250620 Adi Ferber
		   )
		SELECT
		     PositionID
		   , a.CID
		   ...
		   , MirrorID
		   , IsOpenOpen
		   , CAST(date_format(DATEADD(DAY, 0, CAST(OpenOccurred AS DATE)), 'yyyyMMdd') AS int) as OpenDateID
		   , CAST(date_format(DATEADD(DAY, 0, CAST(CloseOccurred AS DATE)), 'yyyyMMdd') AS int) as CloseDateID
		   , LotCountDecimal
		   , SpreadedCommission
		   , EndForexRate
		   , LastOpConversionRate
...
(CASE WHEN SellCurrencyID=1 THEN 1 WHEN BuyCurrencyID=1 THEN 1/cast(`InitForexRate` as decimal (16,8)) else cast(`LastOpConversionRate` as decimal (16,8)) end),0) AS int)  Volume
		   ...
		   , a.CommissionVersion
		   , ExitOrderType
		   , OpenActionType as OpenPositionReasonIDfOpenTotalFees
		   , OpenTotalTaxes --20250620 Adi Ferber
		   , CloseTotalTaxes--20250620 Adi Ferber
		   , OpenTotalFees  --20250620 Adi Ferber
		   , CloseTotalFees --20250620 Adi Ferber
...
;
	INSERT INTO dwh_daily_process.migration_tables.Ext_Dim_Position_PositionChangeLog
		...
		(
			...
			WHERE Occurred > DATEADD(DAY, 1, CAST(V_Yesterday AS DATE))
...
CloseDateID = CASE WHEN b.CloseOccurred >= CAST ( current_timestamp() as date ) THEN 0 ELSE CAST(date_format(DATEADD(DAY, 0, CAST(b.CloseOccurred AS DATE)), 'yyyyMMdd') AS int) END ,
...
DLTOpen = b.DLTOpen ,
DLTClose = b.DLTClose ,
OpenMarkupByUnits = b.OpenMarkupByUnits ,
CommissionVersion = b.CommissionVersion ,
ExitOrderType = b.ExitOrderType -- ofir
 ,
OpenPositionReasonID = b.OpenPositionReasonID ,
OpenTotalTaxes = b.OpenTotalTaxes --20250620 Adi Ferber
 ,
CloseTotalTaxes = b.CloseTotalTaxes --20250620 Adi Ferber
 ,
OpenTotalFees = b.OpenTotalFees --20250620 Adi Ferber
 ,
CloseTotalFees = b.CloseTotalFees --20250620 Adi Ferber
;
	...
	SELECT  b.`PositionID`
	    ...
	    -- , b.[CloseOccurred]
	      ...
	      , b.`TreeID`
	      , b.`OpenDateID`
	      , CASE WHEN b.CloseOccurred >=CAST(current_timestamp() as date)  THEN 0 ELSE CAST(date_format(DATEADD(DAY, 0, CAST(b.CloseOccurred AS DATE)), 'yyyyMMdd') AS int) END AS CloseDateID
	    --, b.[CloseDateID]
		  ...
		  , b.CommissionVersion
		  , b.ExitOrderType
		  , b.OpenPositionReasonID
		  , b.OpenTotalTaxes --20250620 Adi Ferber
		  , b.CloseTotalTaxes --20250620 Adi Ferber
		  , b.OpenTotalFees  	--20250620 Adi Ferber
		  , b.CloseTotalFees 	--20250620 Adi Ferber
...
END IF;
    ...
    /*---------------------------------------------------
	  2025-07-17    EITAN Lipo    
	  here I add 2 commands to swift partiton 
	*/
TRUNCATE TABLE dwh_daily_process.migration_tables.Dim_Position_SWITCH_SINGLE

	;
	MERGE INTO t A_TGT USING (
SELECT * 
FROM -- [DWH_dbo].[Dim_Position] t   -- 2025-07-13 Before Change 
 dwh_daily_process.migration_tables.Dim_Position_SWITCH_SINGLE t
INNER JOIN dwh_daily_process.migration_tables.Ext_Dim_Position_Real s ON t.PositionID = s.PositionID ---- AND t.[OpenDateID] = s.[OpenDateID] ---<<<<<-
 ...
 , COALESCE(s.DLTOpen, 0) , COALESCE(s.OpenMarkupByUnits, 0) , COALESCE(s.CommissionVersion, 0) , COALESCE(s.LotCountDecimal, 0) --20250121 Inbal BML
 , COALESCE(s.OpenPositionReasonID, 0) , COALESCE(s.OpenTotalTaxes, 0) , COALESCE(s.OpenTotalFees, 0) , COALESCE(s.EstimateCloseFeeForCFD, 0) , COALESCE(s.EstimateCloseFeeOnOpenByUnits, 0) , COALESCE(s.EstimateCloseFeeOnOpen, 0) , COALESCE(s.Close_PnLInDollars, 0) , COALESCE(s.Close_CalculationRate, 0) , COALESCE(s.Close_ConversionRate, 0) , COALESCE(s.Close_PriceType, 0) , COALESCE(s.CurrentCalculationRate, 0) , COALESCE(s.CurrentConversionRate, 0) ) --Back Partition to Dim_Position
...
OpenMarkup = s.OpenMarkup --20240115 Inbal BML
 -- Ofir - DLT commition
 ,
t.DLTOpen = s.DLTOpen -- , t.DLTClose = s.DLTClose
 ,
t.OpenMarkupByUnits = s.OpenMarkupByUnits ,
t.CommissionVersion = s.CommissionVersion ,
t.LotCountDecimal = s.LotCountDecimal --20250121 Inbal BML
 ,
t.OpenPositionReasonID = s.OpenPositionReasonID ,
t.OpenTotalTaxes = s.OpenTotalTaxes --20250620 Adi Ferber
 ,
t.OpenTotalFees = s.OpenTotalFees --20250620 Adi Ferber
 /* 2025-09-08     Daniel Kaplan  */
 ,
t.EstimateCloseFeeForCFD = s.EstimateCloseFeeForCFD ,
t.EstimateCloseFeeOnOpenByUnits = s.EstimateCloseFeeOnOpenByUnits ,
t.EstimateCloseFeeOnOpen = s.EstimateCloseFeeOnOpen ,
t.Close_PnLInDollars = s.Close_PnLInDollars ,
t.Close_CalculationRate = s.Close_CalculationRate ,
t.Close_ConversionRate = s.Close_ConversionRate ,
t.Close_PriceType = s.Close_PriceType ,
t.CurrentCalculationRate = s.CurrentCalculationRate ,
t.CurrentConversionRate = s.CurrentConversionRate /* 2025-09-08     Daniel Kaplan  */;
/* 
	2025-07-13:  End Change
	*/
-----------------------------------------------------------------> 
-- SQL Task Insert from Ext_Dim_Position_Real --------------------->
	...
	SELECT 
	   ...
	   , b.OpenMarkupByUnits
	   , b.CommissionVersion
	   , b.OpenPositionReasonID -- ofir
	   , b.OpenTotalTaxes --20250620 Adi Ferber
	   , b.OpenTotalFees  --20250620 Adi Ferber
	   /* 2025-09-08     Daniel Kaplan  */
	   , b.EstimateCloseFeeForCFD 
	   , b.EstimateCloseFeeOnOpenByUnits
	   , b.EstimateCloseFeeOnOpen 
	   , b.Close_PnLInDollars
	   , b.Close_CalculationRate
	   , b.Close_ConversionRate
	   , b.Close_PriceType
	   , b.CurrentCalculationRate 
	   , b.CurrentConversionRate
	   /* 2025-09-08     Daniel Kaplan  */
...
WHERE OpenMarket_Ask IS NULL 
    ...
    AND p.OpenDateID =CAST(date_format(V_Yesterday, 'yyyyMMdd') AS INT)
