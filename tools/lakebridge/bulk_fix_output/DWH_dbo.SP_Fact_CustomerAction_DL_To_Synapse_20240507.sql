...
CREATE OR REPLACE PROCEDURE dwh_daily_process.migration_tables.SP_Fact_CustomerAction_DL_To_Synapse_20240507(
IN V_dt TIMESTAMP)
LANGUAGE SQL
...
WHERE `DateID` =  CAST(date_format(DATEADD(day, DATEDIFF(0, V_Yesterday), 0), 'yyyyMMdd') AS int) 



;
INSERT INTO dwh_daily_process.migration_tables.STS_User_Operations_Data_History 
(
    `Gcid` ,
    `RealCid` ,
    `DemoCid` ,
