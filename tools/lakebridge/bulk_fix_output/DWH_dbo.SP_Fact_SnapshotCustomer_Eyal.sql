...
CREATE OR REPLACE PROCEDURE dwh_daily_process.migration_tables.SP_Fact_SnapshotCustomer_Eyal(
IN V_date TIMESTAMP)
LANGUAGE SQL
...
Date:        2018-03-11
Description: Update table Fact_SnapshotCustomer_Eyal - Process one to update table Fact_SnapshotCustomer_Eyal
...
----------     ----------   ------------------------------------
 ...
 2022-11-28	    Inbal        Add Columns: PhoneNumber, IsPhoneVerified and PhoneVerificationDateID to Fact_SnapshotCustomer_Eyal
 2022-12-28		Inbal		 Add PlayerStatusSubReasonID to Fact_SnapshotCustomer_Eyal
 2023-02-01		Inbal		 Add PhoneNumber to GDPR
 2024-04-01		Inbal		 Add WeekendFeePrecentage to Fact_SnapshotCustomer_Eyal
 2024-08-13		Nir.H		 Add DltStatusID,DltID to Fact_SnapshotCustomer_Eyal
 2024-09-08		Nir.H		 DltID add default  null
 2024-10-06     Eyal.B       FIX DltID update logic
 2024-10-13     Eyal.B	     FFIX DltID update logic to get the data to Fact_SnapshotCustomer_Eyal
...
;
SET V_ProcessName = 'Fact_SnapshotCustomer_Eyal';

--print @largedate
--print @maxentrydate
--print @date
--select DateRangeID , convert(bigint,left(convert (Varchar(20),DateRangeID),8)+right(convert (Varchar,@daybefore,112),4)) from [DWH_dbo].[Fact_SnapshotCustomer_Eyal]
/****************************************************************Fill Ext_FSC tables*********************************************************************************/
...
FROM dwh_daily_process.migration_tables.Ext_FSC_Real_Customer_Customer t 
	left join
	dwh_daily_process.migration_tables.Fact_SnapshotCustomer_Eyal f
	on 
	t.CID = f.RealCID
...
DROP VIEW IF EXISTS TEMP_TABLE_TodayDepositors;

CREATE OR REPLACE TEMPORARY VIEW TEMP_TABLE_TodayDepositors AS
Select Distinct CID

From dwh_daily_process.migration_tables.Ext_FSC_Real_History_Credit;
...
From 
	TEMP_TABLE_TodayDepositors TD
	Left Join dwh_daily_process.migration_tables.Fact_SnapshotCustomer_Eyal FSC 
...
Where
	COALESCE(CAST(FSC.IsDepositor AS BOOLEAN), FALSE) = 0;
...
AS
	...
	SELECT CID,	PhoneNumber,	IsPhoneVerified,	PhoneVerificationDate,	PhoneVerificationDateID
	FROM (
		  SELECT
			   CID
			  ,PhoneNumber
			  ...
			  ,ROW_NUMBER () OVER (PARTITION BY CID, PhoneVerificationDateID ORDER BY PhoneVerificationDate desc , EndTime DESC ) AS RN
			   FROM dwh_daily_process.migration_tables.Ext_FSC_PhoneVerificationDetails
			   WHERE PhoneVerificationDateID <> 19000101
			) AS PVD
	WHERE RN=1;


--select * from #PhoneVerificationDetails
...
;
DROP VIEW IF EXISTS TEMP_TABLE_DLT;
CREATE OR REPLACE TEMPORARY VIEW TEMP_TABLE_DLT
...
from dwh_daily_process.migration_tables.Fact_SnapshotCustomer_Eyal fsc 
...
DROP VIEW IF EXISTS TEMP_TABLE_DailyCustomerSnapshot;
CREATE OR REPLACE TEMPORARY VIEW TEMP_TABLE_DailyCustomerSnapshot
(CID INT
, GCID INT
, CountryID INT
, LabelID INT
...
select * from TEMP_TABLE_DailyCustomerSnapshot
where CID = 26400844

;
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks) INSERT INTO TEMP_TABLE_DailyCustomerSnapshot SELECT DISTINCT coalesce(TEMP_TABLE_CCTemp.CID, TEMP_TABLE_BCTemp.CID, TEMP_TABLE_DepositorChanges.CID, PVD.CID, iden.RealCID) AS CID , coalesce(TEMP_TABLE_CCTemp.GCID, FSC.GCID,iden.GCID,0) AS GCID , coalesce(TEMP_TABLE_CCTemp.CountryID, FSC.CountryID, iden.CountryID, 0) AS CountryID , coalesce(TEMP_TABLE_CCTemp.LabelID, FSC.LabelID, iden.LabelID, 0) AS LabelID , coalesce(TEMP_TABLE_CCTemp.LanguageID, FSC.LanguageID, iden.LanguageID,0) AS LanguageID , coalesce(TEMP_TABLE_CCTemp.PlayerStatusID, FSC.PlayerStatusID, iden.PlayerStatusID,0) AS PlayerStatusID , coalesce(TEMP_TABLE_CCTemp.CommunicationLanguageID, FSC.CommunicationLanguageID, iden.CommunicationLanguageID,0) AS CommunicationLanguageID , coalesce(TEMP_TABLE_CCTemp.AccountStatusID, FSC.AccountStatusID, iden.AccountStatusID,0) AS AccountStatusID , coalesce(TEMP_TABLE_CCTemp.PlayerLevelID, FSC.PlayerLevelID, iden.PlayerLevelID,0) AS PlayerLevelID , coalesce(TEMP_TABLE_BCTemp.VerificationLevelID, FSC.VerificationLevelID, iden.VerificationLevelID,0) AS VerificationLevelID , coalesce(TEMP_TABLE_BCTemp.RiskStatusID, FSC.RiskStatusID, iden.RiskStatusID,0) AS RiskStatusID , coalesce(TEMP_TABLE_BCTemp.RiskClassificationID, FSC.RiskClassificationID, iden.RiskClassificationID,0) AS RiskClassificationID , coalesce(TEMP_TABLE_BCTemp.RegulationID, FSC.RegulationID, TEMP_TABLE_BCTemp.RegulationIDBackup, iden.RegulationID, 0) AS RegulationID , coalesce(TEMP_TABLE_BCTemp.ManagerID, FSC.AccountManagerID, iden.AccountManagerID,0) AS AccountManagerID , coalesce(TEMP_TABLE_BCTemp.AccountTypeID, FSC.AccountTypeID, iden.AccountTypeID,0) AS AccountTypeID , coalesce(TEMP_TABLE_BCTemp.GuruStatusID, FSC.GuruStatusID, iden.GuruStatusID,0) AS GuruStatusID , coalesce(TEMP_TABLE_DepositorChanges.IsDepositor, FSC.IsDepositor, iden.IsDepositor, 0) AS IsDepositor , coalesce(TEMP_TABLE_CCTemp.PendingClosureStatusID, FSC.PendingClosureStatusID, iden.PendingClosureStatusID, 0) AS PendingClosureStatusID , coalesce(TEMP_TABLE_BCTemp.DocumentStatusID, FSC.DocumentStatusID, iden.DocumentStatusID, 0) AS DocumentStatusID , coalesce(TEMP_TABLE_BCTemp.SuitabilityTestStatusID, FSC.SuitabilityTestStatusID, iden.SuitabilityTestStatusID, 0) AS SuitabilityTestStatusID , coalesce(TEMP_TABLE_BCTemp.MifidCategorizationID, FSC.MifidCategorizationID, iden.MifidCategorizationID, 0) AS MifidCategorizationID , coalesce(CAST(TEMP_TABLE_CCTemp.IsEmailVerified AS INT), CAST(FSC.IsEmailVerified AS INT), CAST(iden.IsEmailVerified AS INT), 0) AS IsEmailVerified , coalesce(TEMP_TABLE_BCTemp.DesignatedRegulationID, FSC.DesignatedRegulationID, iden.DesignatedRegulationID, 0) AS DesignatedRegulationID , coalesce(TEMP_TABLE_BCTemp.EvMatchStatus, FSC.EvMatchStatus, iden.EvMatchStatus, 0) AS EvMatchStatus , coalesce(TEMP_TABLE_CCTemp.RegionID, FSC.RegionID, iden.RegionID, 0) AS RegionID , coalesce(TEMP_TABLE_CCTemp.PlayerStatusReasonID, FSC.PlayerStatusReasonID, iden.PlayerStatusReasonID, 0) AS PlayerStatusReasonID , case when V_date<='20200314' then case when coalesce(TEMP_TABLE_CCTemp.PlayerLevelID, FSC.PlayerLevelID, iden.PlayerLevelID,0) <> 4 And coalesce(TEMP_TABLE_CCTemp.LabelID, FSC.LabelID, iden.LabelID,0) <> 30 And coalesce(TEMP_TABLE_CCTemp.CountryID, FSC.CountryID, iden.CountryID,0) <> 250 and coalesce(TEMP_TABLE_BCTemp.AccountTypeID, FSC.AccountTypeID, iden.AccountTypeID,0) <> 9 then 1 else 0 end ELSE case when coalesce(TEMP_TABLE_CCTemp.PlayerLevelID, FSC.PlayerLevelID, iden.PlayerLevelID,0) <> 4 And coalesce(TEMP_TABLE_CCTemp.LabelID, FSC.LabelID, iden.LabelID,0) NOT IN (30, 26) And coalesce(TEMP_TABLE_CCTemp.CountryID, FSC.CountryID, iden.CountryID,0) <> 250 then 1 else 0 end end as IsValidCustomer , case when V_date<='20200314' then case when (NOT (coalesce(TEMP_TABLE_CCTemp.PlayerLevelID, FSC.PlayerLevelID, iden.PlayerLevelID, iden.PlayerLevelID,0) = 4 AND coalesce(TEMP_TABLE_BCTemp.AccountTypeID, FSC.AccountTypeID, iden.AccountTypeID, iden.AccountTypeID,0) <> 2) AND coalesce(TEMP_TABLE_CCTemp.LabelID, FSC.LabelID, iden.LabelID, iden.LabelID,0) NOT IN ( 26,30) ) then 1 else 0 end ELSE case when (NOT (coalesce(TEMP_TABLE_CCTemp.PlayerLevelID, FSC.PlayerLevelID, iden.PlayerLevelID,0) = 4 AND coalesce(TEMP_TABLE_BCTemp.AccountTypeID, FSC.AccountTypeID, iden.AccountTypeID,0) <> 2) AND coalesce(TEMP_TABLE_CCTemp.LabelID, FSC.LabelID, iden.LabelID,0) NOT IN ( 26,30) AND NOT (coalesce(TEMP_TABLE_CCTemp.CountryID, FSC.CountryID, iden.CountryID,0) = 250 and coalesce(TEMP_TABLE_CCTemp.CID, TEMP_TABLE_BCTemp.CID, TEMP_TABLE_DepositorChanges.CID,iden.RealCID) NOT IN (3400616,10526243))) then 1 else 0 end end as IsCreditReportValidCB , coalesce(TEMP_TABLE_CCTemp.Email, FSC.Email, iden.Email, '') AS Email , coalesce(TEMP_TABLE_CCTemp.City, FSC.City, iden.City, '') AS City , coalesce(TEMP_TABLE_CCTemp.Address, FSC.Address, iden.Address, '') AS Address , coalesce(TEMP_TABLE_CCTemp.Zip, FSC.Zip, iden.Zip, '') AS Zip , coalesce(TEMP_TABLE_CCTemp.AffiliateID, FSC.AffiliateID, iden.AffiliateID, 0) AS AffiliateID , coalesce(PVD.PhoneNumber, FSC.PhoneNumber, iden.PhoneNumber, '') AS PhoneNumber , coalesce(PVD.IsPhoneVerified, FSC.IsPhoneVerified, iden.IsPhoneVerified, 0) AS IsPhoneVerified , coalesce(PVD.PhoneVerificationDateID, FSC.PhoneVerificationDateID, iden.PhoneVerificationDateID, '') AS PhoneVerificationDateID , coalesce(TEMP_TABLE_CCTemp.PlayerStatusSubReasonID, FSC.PlayerStatusSubReasonID, iden.PlayerStatusSubReasonID, 0) AS PlayerStatusSubReasonID , coalesce(TEMP_TABLE_CCTemp.WeekendFeePrecentage, FSC.WeekendFeePrecentage, iden.WeekendFeePrecentage) AS WeekendFeePrecentage , coalesce(iden.DltStatusID,0) as DltStatusID , coalesce(iden.DltID,null) as DltID FROM TEMP_TABLE_CCTemp FULL JOIN TEMP_TABLE_BCTemp ON TEMP_TABLE_CCTemp.CID = TEMP_TABLE_BCTemp.CID FULL JOIN TEMP_TABLE_DepositorChanges ON COALESCE(TEMP_TABLE_CCTemp.CID, TEMP_TABLE_BCTemp.CID) = TEMP_TABLE_DepositorChanges.CID FULL JOIN TEMP_TABLE_PhoneVerificationDetails PVD ON coalesce(TEMP_TABLE_CCTemp.CID, TEMP_TABLE_BCTemp.CID, TEMP_TABLE_DepositorChanges.CID) = PVD.CID LEFT JOIN dwh_daily_process.migration_tables.Fact_SnapshotCustomer_Eyal FSC ON (FSC.RealCID = coalesce(TEMP_TABLE_CCTemp.CID, TEMP_TABLE_BCTemp.CID, TEMP_TABLE_DepositorChanges.CID, PVD.CID)) AND CAST(left(CAST(FSC.DateRangeID AS STRING), 4) + right(CAST(FSC.DateRangeID AS STRING), 4) AS TIMESTAMP) = V_maxentrydate FULL join TEMP_TABLE_DLT iden ON iden.RealCID=coalesce(TEMP_TABLE_CCTemp.CID, TEMP_TABLE_BCTemp.CID, TEMP_TABLE_DepositorChanges.CID, PVD.CID)
...
THEN
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
INSERT INTO dwh_daily_process.migration_tables.Fact_SnapshotCustomer_Eyal 
(RealCID
								   , GCID
								   , DateRangeID
								   , CountryID
...
IF  DATEADD(YEAR, CAST(DATEDIFF(0, current_timestamp()) / 365 AS INT), 0) <>  V_date
THEN
--EXEC SP_Log_Full 'Fact_SnapshotCustomer_Eyal','Update Old Customer',@dateID,1,0

-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
MERGE INTO dwh_daily_process.migration_tables.Fact_SnapshotCustomer_Eyal AS a
...
(COALESCE(CAST(a.DltID AS STRING), null) <> COALESCE(CAST(b.DltID AS STRING), null))
)
...
RealCID,
DateRangeID 
FROM dwh_daily_process.migration_tables.Fact_SnapshotCustomer_Eyal 
...
;
MERGE INTO dwh_daily_process.migration_tables.Fact_SnapshotCustomer_Eyal a_TGT
USING (
SELECT * 
from dwh_daily_process.migration_tables.Fact_SnapshotCustomer_Eyal a
