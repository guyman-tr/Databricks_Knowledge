...
CREATE OR REPLACE PROCEDURE dwh_daily_process.migration_tables.SP_Dictionaries_DL_To_Synapse(
)
LANGUAGE SQL
...
SELECT  
	...
	,sum(case when cpa.CreditDate >= DATEADD(day, DATEDIFF(1, current_timestamp()), 0) and cpa.CreditDate < DATEADD(day, DATEDIFF(0, current_timestamp()), 0) then  1 else 0 end) AS FTDYesterday
	,sum(case when cpa.CreditDate > LAST_DAY(DATEADD(MONTH, -2, current_timestamp())) and cpa.CreditDate < DATEADD(month, CAST(MONTHS_BETWEEN(0, current_timestamp()) AS INT), 0) then  1 else 0 end) AS FTDLastMonth
	,sum(case when cpa.CreditDate >= DATEADD(QUARTER, DATEDIFF(QUARTER, current_timestamp(), 0) -1, 0) and  cpa.CreditDate < DATEADD(QUARTER, DATEDIFF(QUARTER, current_timestamp(), -1) -1, 0) then  1 else 0 end) AS FTDLastQuarter
	,sum(case when cpa.CreditDate >= DATEADD(YEAR, CAST(DATEDIFF(0, current_timestamp()) / 365 AS INT) - 1, 0) and cpa.CreditDate < DATEADD(YEAR, CAST(DATEDIFF(0, current_timestamp()) / 365 AS INT), 0) then  1 else 0 end) AS FTDLastYear
	,sum(case when cpa.CreditDate >=  DATEADD(month, CAST(MONTHS_BETWEEN(0, current_timestamp()) AS INT), 0) then  1 else 0 end) AS FTDThisMonth
	,sum(case when cpa.CreditDate >=  DATEADD(QUARTER, DATEDIFF(QUARTER, current_timestamp(), -1) -1, 0) then  1 else 0 end) AS FTDThisQuarter
	,sum(case when cpa.CreditDate >=  DATEADD(YEAR, CAST(DATEDIFF(0, current_timestamp()) / 365 AS INT), 0) then  1 else 0 end) AS FTDThisYear
...
WHERE  cpa.CreditTypeID=1 AND 
	   ...
	   cpa.CreditDate < DATEADD(day, DATEDIFF(0, current_timestamp()), 0)
...
SELECT
	...
	SUM(CAST(cpa.IsFirstDeposit as INT)) FTDeLifeTime,
	 sum(case when cpa.CreditDate >= DATEADD(day, DATEDIFF(1, current_timestamp()), 0) and cpa.CreditDate < DATEADD(day, DATEDIFF(0, current_timestamp()), 0) then  1 else 0 end) AS FTDeYesterday
	 ,sum(case when cpa.CreditDate > LAST_DAY(DATEADD(MONTH, -2, current_timestamp())) and cpa.CreditDate < DATEADD(month, CAST(MONTHS_BETWEEN(0, current_timestamp()) AS INT), 0) then  1 else 0 end) AS FTDeLastMonth
	,sum(case when cpa.CreditDate >= DATEADD(QUARTER, DATEDIFF(QUARTER, current_timestamp(), 0) -1, 0) and  cpa.CreditDate < DATEADD(QUARTER, DATEDIFF(QUARTER, current_timestamp(), -1) -1, 0) then  1 else 0 end) AS FTDeLastQuarter
	,sum(case when cpa.CreditDate >= DATEADD(YEAR, CAST(DATEDIFF(0, current_timestamp()) / 365 AS INT) - 1, 0) and cpa.CreditDate < DATEADD(YEAR, CAST(DATEDIFF(0, current_timestamp()) / 365 AS INT), 0) then  1 else 0 end) AS FTDeLastYear
	,sum(case when cpa.CreditDate >=  DATEADD(month, CAST(MONTHS_BETWEEN(0, current_timestamp()) AS INT), 0) then  1 else 0 end) AS FTDeThisMonth
	,sum(case when cpa.CreditDate >=  DATEADD(QUARTER, DATEDIFF(QUARTER, current_timestamp(), -1) -1, 0) then  1 else 0 end) AS FTDeThisQuarter
	,sum(case when cpa.CreditDate >=  DATEADD(YEAR, CAST(DATEDIFF(0, current_timestamp()) / 365 AS INT), 0) then  1 else 0 end) AS FTDeThisYear
FROM  dwh_daily_process.daily_snapshot.fiktivo_AffiliateCommission_Credit cpa   
INNER JOIN dwh_daily_process.daily_snapshot.fiktivo_AffiliateCommission_CreditCommission cc  
...
SELECT 
	...
	,sum(case when Registrations.RegistrationDate >= DATEADD(day, DATEDIFF(1, current_timestamp()), 0) and Registrations.RegistrationDate < DATEADD(day, DATEDIFF(0, current_timestamp()), 0) then  1 else 0 end) AS RegistrationYesterday
	,sum(case when Registrations.RegistrationDate > LAST_DAY(DATEADD(MONTH, -2, current_timestamp())) and Registrations.RegistrationDate < DATEADD(month, CAST(MONTHS_BETWEEN(0, current_timestamp()) AS INT), 0) then  1 else 0 end) AS RegistrationLastMonth
	,sum(case when Registrations.RegistrationDate >= DATEADD(QUARTER, DATEDIFF(QUARTER, current_timestamp(), 0) -1, 0) and Registrations.RegistrationDate < DATEADD(QUARTER, DATEDIFF(QUARTER, current_timestamp(), -1) -1, 0) then  1 else 0 end) AS RegistrationLastQuarter
	,sum(case when Registrations.RegistrationDate >= DATEADD(YEAR, CAST(DATEDIFF(0, current_timestamp()) / 365 AS INT) - 1, 0) and Registrations.RegistrationDate < DATEADD(YEAR, CAST(DATEDIFF(0, current_timestamp()) / 365 AS INT), 0) then  1 else 0 end) AS RegistrationLastYear
	,sum(case when Registrations.RegistrationDate >= DATEADD(month, CAST(MONTHS_BETWEEN(0, current_timestamp()) AS INT), 0) then  1 else 0 end) AS RegistrationThisMonth
	,sum(case when Registrations.RegistrationDate >= DATEADD(QUARTER, DATEDIFF(QUARTER, current_timestamp(), -1) -1, 0) then  1 else 0 end) AS RegistrationThisQuarter
	,sum(case when Registrations.RegistrationDate >= DATEADD(YEAR, CAST(DATEDIFF(0, current_timestamp()) / 365 AS INT), 0) then  1 else 0 end) AS RegistrationThisYear
...
AND RC.Tier=1
WHERE Registrations.RegistrationDate < DATEADD(day, DATEDIFF(0, current_timestamp()), 0)
