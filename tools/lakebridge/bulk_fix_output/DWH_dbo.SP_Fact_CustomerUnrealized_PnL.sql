...
CREATE OR REPLACE PROCEDURE dwh_daily_process.migration_tables.SP_Fact_CustomerUnrealized_PnL(
...
where IsFuture = 1
;
call `dbo`.`LastRowCount`( 'futures14' , V_row_count
...
group by a.CID
;
call `dbo`.`LastRowCount`( 'final_NOP_Notional5' , V_row_count
...
;
call `dbo`.`LastRowCount`( 'covarience6' , V_row_count
...
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
INSERT INTO dwh_daily_process.migration_tables.Fact_CustomerUnrealized_PnL 
(CID
										, DateModified
										, PositionPnL_old
...
------------------Futures
	...
	, PositionPnLStocksMargin
	from 
	(select CID,CAST(date_format(V_RepDate, 'yyyyMMdd') as int) as DateID
...
;
call `dbo`.`LastRowCount`( 'Fact_CustomerUnrealized_PnL10' , V_row_count
...
INSERT INTO dwh_daily_process.migration_tables.Fact_CustomerUnrealized_PnL_UserAPI
	 ...
	 `CID`
           ...
           WHERE DateModified = V_dateid
;
call `dbo`.`LastRowCount`( 'Fact_CustomerUnrealized_PnL_UserAPI11' , V_row_count
		   
);
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
ELSE

-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks) INSERT INTO dwh_daily_process.migration_tables.Fact_CustomerUnrealized_PnL (CID , DateModified , PositionPnL_old , PositionPnLStocksReal , PositionPnLCryptoReal , CopyPositionPnL , MenualPositionPnL , StocksPositionPnL , CommissionOnOpen , FullCommissionOnOpen , MirrorStocksPositionPnL , CryptoPositionPnL , ManualCryptoPositionPnL , CopyCryptoPositionPnL , CopyFundPnL , UpdateDate , NOP , Notional , NOP_Crypto , Notional_Crypto , NOP_CFD , Notional_CFD , NOP_Crypto_CFD , Notional_Crypto_CFD , NOP_Stock , Notional_Stock , NOP_Stock_CFD , Notional_Stock_CFD , CommissionByUnits , FullCommissionByUnits , FullCommissionByUnitsStocksReal , FullCommissionByUnitsCryptoReal , GuruCopiesPNL , GuruCopiesPNL_Dit , CommissionByUnitsStocksReal , CommissionByUnitsCryptoReal , FullCommissionByUnitsStocksCFD , FullCommissionByUnitsCryptoCFD ,CommissionByUnitsCrypto_TRS ,CopyCryptoPositionPnL_TRS ,CryptoPositionPnL_TRS ,FullCommissionByUnitsCrypto_TRS ,ManualCryptoPositionPnL_TRS ,NOP_Crypto_TRS ,Notional_Crypto_TRS ,PositionPnL , MirrorRealFuturesPositionPnL , ManualRealFuturesPositionPnL , NOP_FuturesReal , Notional_FuturesReal , PositionPnLFuturesReal , FullCommissionByUnitsFuturesReal , CommissionByUnitsFuturesReal , NOP_StocksMargin , PositionPnLStocksMargin ) select a.CID ,DateID ,PositionPnL_old ,PositionPnLStocksReal ,PositionPnLCryptoReal ,CopyPositionPnL ,MenualPositionPnL ,StocksPositionPnL ,Commission ,FullCommission ,MirrorStocksPositionPnL ,CryptoPositionPnL ,ManualCryptoPositionPnL ,CopyCryptoPositionPnL ,CopyFundPnL ,current_timestamp() as UpdateDate ,NOP ,Notional ,NOP_Crypto ,Notional_Crypto ,NOP_CFD ,Notional_CFD ,NOP_Crypto_CFD ,Notional_Crypto_CFD , NOP_Stock , Notional_Stock , NOP_Stock_CFD , Notional_Stock_CFD , CommissionByUnits , `FullCommissionByUnits` , FullCommissionByUnitsStocksReal , FullCommissionByUnitsCryptoReal , GuruCopiesPNL , GuruCopiesPNL_Dit , CommissionByUnitsStocksReal , CommissionByUnitsCryptoReal , FullCommissionByUnitsStocksCFD , FullCommissionByUnitsCryptoCFD ,CommissionByUnitsCrypto_TRS ,CopyCryptoPositionPnL_TRS ,CryptoPositionPnL_TRS ,FullCommissionByUnitsCrypto_TRS ,ManualCryptoPositionPnL_TRS ,NOP_Crypto_TRS ,Notional_Crypto_TRS ,PositionPnL , n.MirrorRealFuturesPositionPnL , n.ManualRealFuturesPositionPnL , n.NOP_FuturesReal , n.Notional_FuturesReal , n.PositionPnLFuturesReal , n.FullCommissionByUnitsFuturesReal , n.CommissionByUnitsFuturesReal , NOP_StocksMargin , PositionPnLStocksMargin from (select CID,CAST(date_format(V_RepDate, 'yyyyMMdd') as int) as DateID ,sum(CalculatedNetProfit) as PositionPnL_old ,sum(case when IsSettled = 1 and InstrumentTypeID in(5,6) AND f.IsFuture = 0 THEN PnLInDollars ELSE 0 END ) as PositionPnLStocksReal ,sum(case when IsSettled = 1 and InstrumentTypeID in(10) AND f.IsFuture = 0 THEN PnLInDollars ELSE 0 END ) as PositionPnLCryptoReal ,sum(case WHEN MirrorID>0 THEN PnLInDollars ELSE 0 END) as CopyPositionPnL ,sum(CASE when MirrorID=0 THEN PnLInDollars ELSE 0 end) as MenualPositionPnL ,sum(case when InstrumentTypeID in(5,6) AND f.IsFuture = 0 then PnLInDollars else 0 end) as StocksPositionPnL ,SUM(CASE WHEN InstrumentTypeID in(5,6) AND f.IsFuture = 0 AND MirrorID>0 THEN PnLInDollars ELSE 0 END) AS MirrorStocksPositionPnL ,sum(Commission) as Commission ,sum(FullCommission) as FullCommission ,SUM(CASE WHEN InstrumentTypeID in(10) AND f.IsFuture = 0 THEN PnLInDollars ELSE 0 end) AS CryptoPositionPnL ,SUM(CASE WHEN InstrumentTypeID in(10) AND f.IsFuture = 0 AND MirrorID=0 THEN PnLInDollars ELSE 0 end) AS ManualCryptoPositionPnL ,SUM(CASE WHEN InstrumentTypeID in(10) AND f.IsFuture = 0 AND MirrorID>0 THEN PnLInDollars ELSE 0 END) AS CopyCryptoPositionPnL ,SUM(CASE WHEN CopyFund = 1 THEN PnLInDollars ELSE 0 END) AS CopyFundPnL ,SUM(CommissionByUnits) as CommissionByUnits ,SUM(FullCommissionByUnits) as FullCommissionByUnits ,sum(case when IsSettled = 1 and InstrumentTypeID in(5,6) AND f.IsFuture = 0 THEN FullCommissionByUnits ELSE 0 END ) as FullCommissionByUnitsStocksReal ,sum(case when IsSettled = 1 and InstrumentTypeID in(10) AND f.IsFuture = 0 THEN FullCommissionByUnits ELSE 0 END ) as FullCommissionByUnitsCryptoReal ,sum(case when ConnectedGuruCopies = 1 and MirrorID>0 THEN PnLInDollars ELSE 0 END ) as GuruCopiesPNL ,sum(case when ConnectedGuruCopies = 0 and MirrorID>0 THEN PnLInDollars ELSE 0 END ) as GuruCopiesPNL_Dit ,sum(case when IsSettled = 1 and InstrumentTypeID in(5,6) AND f.IsFuture = 0 THEN CommissionByUnits ELSE 0 END ) as CommissionByUnitsStocksReal ,sum(case when IsSettled = 1 and InstrumentTypeID in(10) AND f.IsFuture = 0 THEN CommissionByUnits ELSE 0 END ) as CommissionByUnitsCryptoReal ,sum(case when IsSettled = 0 and InstrumentTypeID in(5,6) THEN FullCommissionByUnits ELSE 0 END ) as FullCommissionByUnitsStocksCFD ,sum(case when IsSettled = 0 and InstrumentTypeID in(10) THEN FullCommissionByUnits ELSE 0 END ) as FullCommissionByUnitsCryptoCFD ,sum(case when IsSettled = 1 and InstrumentTypeID in(10) and SettlementTypeID=2 THEN CommissionByUnits ELSE 0 END ) as CommissionByUnitsCrypto_TRS ,sum(case when InstrumentTypeID in(10) AND MirrorID>0 and SettlementTypeID=2 THEN PnLInDollars ELSE 0 END) as CopyCryptoPositionPnL_TRS ,sum(case when InstrumentTypeID in(10) and SettlementTypeID=2 THEN PnLInDollars ELSE 0 end) as CryptoPositionPnL_TRS ,sum(case when IsSettled = 0 /*1*/ and InstrumentTypeID in(10) and SettlementTypeID=2 THEN FullCommissionByUnits ELSE 0 END ) as FullCommissionByUnitsCrypto_TRS ,sum(case when InstrumentTypeID in(10) AND MirrorID=0 and SettlementTypeID=2 THEN PnLInDollars ELSE 0 end) as ManualCryptoPositionPnL_TRS ,sum(PnLInDollars) as PositionPnL ,SUM(CASE WHEN SettlementTypeID = 5 THEN PnLInDollars ELSE 0 END) AS PositionPnLStocksMargin from TEMP_TABLE_UnrealizedPnL upl LEFT JOIN TEMP_TABLE_futures f ON upl.InstrumentID = f.InstrumentID where PnLInDollars is NOT null group by CID)a left join TEMP_TABLE_final_NOP_Notional n on(a.CID=n.CID)
call `dbo`.`LastRowCount`( 'Fact_CustomerUnrealized_PnL12' , V_row_count
...
INSERT INTO dwh_daily_process.migration_tables.Fact_CustomerUnrealized_PnL_UserAPI
	 ...
	 `CID`
           ...
           WHERE DateModified = V_dateid
;
call `dbo`.`LastRowCount`( 'Fact_CustomerUnrealized_PnL_UserAPI13' , V_row_count
