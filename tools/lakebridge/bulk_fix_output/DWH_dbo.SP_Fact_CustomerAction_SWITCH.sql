...
CREATE OR REPLACE PROCEDURE dwh_daily_process.migration_tables.SP_Fact_CustomerAction_SWITCH(
)
LANGUAGE SQL
...
SET V_TS = 1
;
set  V_CurrentDay = (SELECT DateID from dwh_daily_process.migration_tables.Fact_CustomerAction_SWITCH_SINGLE);

-----create constraint for table
--set @SqlStr = 'ALTER TABLE Fact_CustomerAction_SWITCH_SINGLE ADD CONSTRAINT FCASS_value_for_switch CHECK (DateID = ' || CAST(@CurrentDay as varchar) || ' );'
...
Set V_SqlStr = 'ALTER TABLE dwh_daily_process.migration_tables.Fact_CustomerAction SWITCH PARTITION ' || CAST(V_PartToSwitch as STRING) ||
' TO dwh_daily_process.migration_tables.Fact_CustomerAction_SWITCH PARTITION ' || CAST(V_PartToSwitch_SGL as STRING) || ';';
--print @SqlStr   
EXECUTE IMMEDIATE V_SqlStr;

--- 2 .switch new data to partitioned table
Set V_SqlStr = 'ALTER TABLE dwh_daily_process.migration_tables.Fact_CustomerAction_SWITCH_SINGLE SWITCH PARTITION ' || CAST(V_PartToSwitch_SGL as STRING) || '  TO dwh_daily_process.migration_tables.Fact_CustomerAction PARTITION ' || CAST(V_PartToSwitch as STRING) ||' WITH (TRUNCATE_TARGET = ON);';
--print @SqlStr
EXECUTE IMMEDIATE V_SqlStr;
...
TRUNCATE TABLE dwh_daily_process.migration_tables.Fact_CustomerAction_SWITCH
