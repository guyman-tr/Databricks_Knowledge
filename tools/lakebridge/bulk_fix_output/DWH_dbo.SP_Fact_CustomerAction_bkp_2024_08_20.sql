...
CREATE OR REPLACE PROCEDURE dwh_daily_process.migration_tables.SP_Fact_CustomerAction_bkp_2024_08_20(
IN V_dt TIMESTAMP)
LANGUAGE SQL
...
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks) INSERT INTO dwh_daily_process.migration_tables.Ext_FCA_Fact_CustomerAction (RealCID , Occurred , IsReal , ActionTypeID , PlatformTypeID , Amount , PositionID , MirrorID , DateID , TimeID , StatusID ,SessionID) SELECT CID , a.Occurred , 1 AS IsReal , CASE WHEN CreditTypeID = 18 THEN 15 WHEN CreditTypeID = 19 THEN 16 WHEN CreditTypeID = 20 THEN 17 WHEN CreditTypeID = 21 THEN 18 WHEN CreditTypeID = 27 THEN 19 WHEN CreditTypeID = 28 THEN 20 END , 0 AS PlatformTypeID , COALESCE(Payment, 0) AS Amount , COALESCE(a.PositionID, 0) , COALESCE(a.MirrorID, 0) , CAST(date_format(a.Occurred, 'yyyyMMdd') AS INT) , EXTRACT(HOUR from a.Occurred) , 1 ,case when CreditTypeID = 20 THEN b.SessionID else null end as SessionID FROM dwh_daily_process.migration_tables.Ext_FCA_Real_History_Credit_ForFactAction_All a left join dwh_daily_process.migration_tables.Ext_FCA_Mirror_Session b on a.MirrorID = b.MirrorID WHERE CreditTypeID IN (18, 19, 20, 21, 27, 28)
...
PlatformID = b.PlatformID ,
SessionID = b.SessionID;
SELECT * from dwh_daily_process.migration_tables.Ext_FCA_Real_Position  where DLTOpen is not null
;

select cast(count(1) as bigint) from dwh_daily_process.migration_tables.Fact_CustomerAction where DLTOpen is not null
;

select cast(count(1) as bigint) from dwh_daily_process.migration_tables.Dim_Position where DLTOpen is not null
;

SELECT * from dwh_daily_process.migration_tables.Ext_FCA_Real_History_Credit_ForFactAction_All 
where  PositionID in (2729275978,2729438212,2729259673,2729021109,901002851);

SELECT * from dwh_daily_process.migration_tables.Ext_FCA_Fact_CustomerAction  where DLTOpen is not null
;

select object_name(object_id),* from sys.sql_modules 
where definition like '%Ext_FCA_Fact_CustomerAction%'
