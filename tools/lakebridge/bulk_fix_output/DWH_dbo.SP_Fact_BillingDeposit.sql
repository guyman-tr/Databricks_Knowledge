...
CREATE OR REPLACE PROCEDURE dwh_daily_process.migration_tables.SP_Fact_BillingDeposit(
IN V_date TIMESTAMP)
LANGUAGE SQL
...
WHERE fbd.ModificationDateID=V_dateID;
...
INNER JOIN dwh_daily_process.migration_tables.Dim_CountryBin cb ON CAST ( fbw.BinCodeAsString AS INT ) = cb.BinCode
)
ON ModificationDateID = V_dateID AND 
