...
CREATE OR REPLACE PROCEDURE dwh_daily_process.migration_tables.SP_Fact_CustomerAction(
IN V_dt TIMESTAMP)
LANGUAGE SQL
...
----------     ----------   ------------------------------------
 ...
 2024-09-22     Ofir A         add IsBuy to Fact_CustomerAction 
 2025-07-30     Guy M	       add CreditID to the final schema
 2025-12-21     Guy M	       add CompensationReasonID on position opens (for airdrop identification reasons)

*********************************************************************************************/
 --   exec [DWH_dbo].[SP_Fact_CustomerAction] '2023-09-17'
    ---declare @dt datetime = cast(getdate()-1 as date)
	--- declare @dt datetime = '20251220'
...
(RealCID
										   ...
										   , IsBuy
										   , CreditID
										   , CompensationReasonID -- guym 2025-12-21
...
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks) INSERT INTO dwh_daily_process.migration_tables.Ext_FCA_Fact_CustomerAction (RealCID , Occurred , IsReal , ActionTypeID , PlatformTypeID , Amount , PositionID , MirrorID , DateID , TimeID , StatusID ,SessionID , Description , CreditID ) SELECT CID , a.Occurred , 1 AS IsReal , CASE WHEN CreditTypeID = 18 THEN 15 WHEN CreditTypeID = 19 THEN 16 WHEN CreditTypeID = 20 THEN 17 WHEN CreditTypeID = 21 THEN 18 WHEN CreditTypeID = 27 THEN 19 WHEN CreditTypeID = 28 THEN 20 END , 0 AS PlatformTypeID , COALESCE(Payment, 0) AS Amount , COALESCE(a.PositionID, 0) , COALESCE(a.MirrorID, 0) , CAST(date_format(a.Occurred, 'yyyyMMdd') AS INT) , EXTRACT(HOUR from a.Occurred) , 1 ,case when CreditTypeID = 20 THEN b.SessionID else null end as SessionID , a.Description , a.CreditID FROM dwh_daily_process.migration_tables.Ext_FCA_Real_History_Credit_ForFactAction_All a left join dwh_daily_process.migration_tables.Ext_FCA_Mirror_Session b on a.MirrorID = b.MirrorID WHERE CreditTypeID IN (18, 19, 20, 21, 27, 28)
...
SELECT * 
from dwh_daily_process.migration_tables.Ext_FCA_Fact_CustomerAction a
INNER JOIN TEMP_TABLE_UpdateSession b -- select * from #UpdateSession
 on a.RealCID = b.RealCID AND a.DepositID = b.DepositID
)
ON a.ActionTypeID in ( 7 ) and ( a.SessionID is null or a.SessionID = 0 or a.SessionID = - 1 or a.PlatformID is null ) -- run script to update PositionID ActionTypeID in (1,2)

WHEN MATCHED THEN UPDATE SET
PlatformID = b.PlatformID ,
SessionID = b.SessionID;
    MERGE INTO dwh_daily_process.migration_tables.Ext_FCA_Fact_CustomerAction A_TGT -- select * from [DWH_dbo].Ext_FCA_Fact_CustomerAction where Occurred between '20240315' and '20240316'
...
PlatformID = b.PlatformID ,
SessionID = b.SessionID;
DROP VIEW IF EXISTS TEMP_TABLE_openCompReason ;
CREATE OR REPLACE TEMPORARY VIEW TEMP_TABLE_openCompReason
        
    as
	SELECT CID, PositionID, CompensationReasonID
	FROM dwh_daily_process.migration_tables.Ext_FCA_Real_History_Credit_ForFactAction_All
	WHERE CreditTypeID = 3
	AND CompensationReasonID IS NOT NULL;

	MERGE INTO dwh_daily_process.migration_tables.Ext_FCA_Fact_CustomerAction t1_TGT
USING (
SELECT * 
FROM dwh_daily_process.migration_tables.Ext_FCA_Fact_CustomerAction t1
INNER JOIN TEMP_TABLE_openCompReason t2 ON t1.RealCID = t2.CID AND t1.PositionID = t2.PositionID

QUALIFY ROW_NUMBER() OVER (PARTITION BY t1.RealCID ORDER BY 1) = 1
)
ON t1.RealCID = t1_TGT.RealCID
WHEN MATCHED THEN UPDATE SET
CompensationReasonID = t2.CompensationReasonID;
