...
CREATE OR REPLACE PROCEDURE dwh_daily_process.migration_tables.SP_Fact_BillingDeposit_DL_To_Synapse(
IN V_dt TIMESTAMP)
LANGUAGE SQL
...
CREATE OR REPLACE TEMPORARY VIEW TEMP_TABLE_Fact_BillingDepositAction  
	...
	and DateID >= CAST(date_format(DATEADD(DAY, -14, V_Yesterday), 'yyyyMMdd') AS int);
	
	
call dwh_daily_process.migration_tables.SP_Fact_BillingDeposit(V_Yesterday);
