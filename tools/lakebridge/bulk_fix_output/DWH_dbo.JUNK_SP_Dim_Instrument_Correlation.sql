...
CREATE OR REPLACE PROCEDURE dwh_daily_process.migration_tables.JUNK_SP_Dim_Instrument_Correlation(
...
----------     ----------   ------------------------------------
2025-01-26		Eitan Lipo	fix bug with "AND  STDEVP(PriceChange_b)>0"
2025-01-26		Eitan Lipo	add last line - call to procedure [DWH_dbo].[SP_Dim_Instrument_Correlation_Half_Records]
...
SET V_row_count = 0;
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks) delete from dwh_daily_process.migration_tables.Dim_Instrument_Correlation where DateID = CAST(date_format(V_auxdate, 'yyyyMMdd') AS int)
...
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
insert into dwh_daily_process.migration_tables.Dim_Instrument_Correlation
...
select a.DateFrom,a.InstrumentID as InstrumentID_a,b.InstrumentID as InstrumentID_b ,a.PriceChange as PriceChange_a,b.PriceChange as PriceChange_b
 from TEMP_TABLE_data a join TEMP_TABLE_data b
on(a.DateFrom=b.DateFrom)
) k
...
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks) call dwh_daily_process.migration_tables.SP_Dim_Instrument_Correlation_Half_Records( V_auxdate));
END;
