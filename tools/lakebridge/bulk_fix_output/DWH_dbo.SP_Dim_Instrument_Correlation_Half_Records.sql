...
CREATE OR REPLACE PROCEDURE dwh_daily_process.migration_tables.SP_Dim_Instrument_Correlation_Half_Records(
...
SET V_row_count = 0;
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks) delete from dwh_daily_process.migration_tables.Dim_Instrument_Correlation_Half_Records where DateID = CAST(date_format(V_auxdate, 'yyyyMMdd') AS int)
...
-- [stub] auxiliary SP call elided (helper not deployed / not needed in Databricks)
insert into dwh_daily_process.migration_tables.Dim_Instrument_Correlation_Half_Records
