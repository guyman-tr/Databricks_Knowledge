...
DDR dashboard-style aggregates **year-to-date through `@edate`** for the calendar year containing `@edate`, rolled up by periodic snapshot dimensions at `@edate`.

**Time logic:** `BI_DB_DDR_Customer_Periodic_Status` uses `Date = @edate` with YTD flags in `*_ThisYear` columns. `BI_DB_V_DDR_MIMO`, `BI_DB_V_DDR_Revenue_Breakdown`, `BI_DB_V_DDR_Non_Revenue_Actions`, and `BI_DB_V_DDR_PnL` are summed per customer over **`Date` from first day of the calendar year through `@edate`** (`DATEADD(yy, DATEDIFF(yy, 0, @edate), 0)` … `@edate`). `BI_DB_V_DDR_AUM` is **`Date = @edate` only**. `*_FTDA` uses `MAX` of daily FTDA over that YTD window, then `SUM(CASE WHEN …)` with `*_ThisYear` first-deposit flags.
...
| 5 | IsValidCustomer | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.IsValidCustomer_ThisYear | MAX(IsValidCustomer_ThisYear) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 6 | MifidCategorizationID | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.MifidCategorizationID_ThisYear | MAX(MifidCategorizationID_ThisYear) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 7 | PlayerLevelID | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.PlayerLevelID_ThisYear | MAX(PlayerLevelID_ThisYear) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 8 | CountryID | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.CountryID_ThisYear | MAX(CountryID_ThisYear) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 9 | MarketingRegion | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.MarketingRegion_ThisYear | MAX(MarketingRegion_ThisYear) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 10 | IsFunded | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.IsFunded_ThisYear | SUM(IsFunded_ThisYear) in PERIODIC where `Date = @edate`; outer SUM over customers per output grain | T2 |
| 11 | FirstTimeFunded | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.FirstTimeFunded_ThisYear | SUM(FirstTimeFunded_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 12 | ActiveTraded | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.ActiveTraded_ThisYear | SUM(ActiveTraded_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 13 | Portfolio_Only | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.Portfolio_Only_ThisYear | SUM(Portfolio_Only_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 14 | BalanceOnlyAccount | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.BalanceOnlyAccount_ThisYear | SUM(BalanceOnlyAccount_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 15 | TPFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.TPFirstDeposited_ThisYear | SUM(TPFirstDeposited_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 16 | IBANFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.IBANFirstDeposited_ThisYear | SUM(IBANFirstDeposited_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 17 | TPExternalFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.TPExternalFirstDeposited_ThisYear | SUM(TPExternalFirstDeposited_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 18 | GlobalFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalFirstDeposited_ThisYear | SUM(GlobalFirstDeposited_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 19 | GlobalDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalDeposited_ThisYear | SUM(GlobalDeposited_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 20 | GlobalRedeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalRedeposited_ThisYear | SUM(GlobalRedeposited_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 21 | GlobalCashedOut | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalCashedOut_ThisYear | SUM(GlobalCashedOut_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 22 | Redeemed | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.Redeemed_ThisYear | SUM(Redeemed_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 23 | DepositedTP | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.DepositedTP_ThisYear | SUM(DepositedTP_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 24 | DepositedIBAN | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.DepositedIBAN_ThisYear | SUM(DepositedIBAN_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 25 | ReDepositedTP | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.ReDepositedTP_ThisYear | SUM(ReDepositedTP_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 26 | ReDepositedIBAN | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.ReDepositedIBAN_ThisYear | SUM(ReDepositedIBAN_ThisYear) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 27 | TP_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN TPFirstDeposited_ThisYear = 1 THEN ds.TP_FTDA ELSE 0 END)`; `ds.TP_FTDA` is `MAX(TP_FTDA)` over **YTD through @edate** | T2 |
| 28 | IBAN_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN IBANFirstDeposited_ThisYear = 1 THEN ds.IBAN_FTDA ELSE 0 END)`; `ds.IBAN_FTDA` is `MAX(IBAN_FTDA)` over **YTD through @edate** | T2 |
| 29 | TP_External_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN TPExternalFirstDeposited_ThisYear = 1 THEN ds.TP_External_FTDA ELSE 0 END)`; `ds.TP_External_FTDA` is `MAX(TP_External_FTDA)` over **YTD through @edate** | T2 |
| 30 | Global_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN GlobalFirstDeposited_ThisYear = 1 THEN ds.Global_FTDA ELSE 0 END)`; `ds.Global_FTDA` is `MAX(Global_FTDA)` over **YTD through @edate** | T2 |
| 31 | ExternalDepositsTPAmount | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalDepositsTPAmount | SUM(ExternalDepositsTPAmount) per customer over MIMO rows with `Date` **YTD through @edate**; outer SUM | T2 |
...
| 73 | ExternalCashedOutTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalCashedOutTP | Outer `SUM` of per-customer `MAX(ExternalCashedOutTP)` from MIMO CTE (**YTD through @edate**) | T2 |
| 74 | InternalCashedOutTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.InternalCashedOutTP | Outer `SUM` of per-customer `MAX(InternalCashedOutTP)` from MIMO CTE (**YTD through @edate**) | T2 |
| 75 | ExternalCashedOutIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalCashedOutIBAN | Outer `SUM` of per-customer `MAX(ExternalCashedOutIBAN)` from MIMO CTE (**YTD through @edate**) | T2 |
| 76 | InternalCashedOutIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO.InternalCashedOutIBAN | Outer `SUM` of per-customer `MAX(InternalCashedOutIBAN)` from MIMO CTE (**YTD through @edate**) | T2 |
| 77 | TradersOpenedFromIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO | Outer `SUM` of per-customer `MAX(CASE WHEN TradeOpenFromIBANCount > 0 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 78 | TradersClosedIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO | Outer `SUM` of per-customer `MAX(CASE WHEN TradeCloseToIBANAmount > 0 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 79 | InternalDepositedTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.InternalDepositedTP | Outer `SUM` of per-customer `MAX(InternalDepositedTP)` from MIMO CTE (**YTD through @edate**) | T2 |
| 80 | ExternalDepositedTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalDepositedTP | Outer `SUM` of per-customer `MAX(ExternalDepositedTP)` from MIMO CTE (**YTD through @edate**) | T2 |
| 81 | CashedOutTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.CashedOutTP | Outer `SUM` of per-customer `MAX(CashedOutTP)` from MIMO CTE (**YTD through @edate**) | T2 |
...
| 150 | RealizedEquityTP | BI_DB_dbo.BI_DB_V_DDR_AUM.RealizedEquityTP | SUM(RealizedEquityTP) where AUM rows are **`Date = @edate` only** (as-of snapshot), not summed across days | T2 |
...
| 216 | PublishCommentCount | BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions | count(DISTINCT CASE when PublishCommentCount > 0 THEN RealCID END) | T2 |
| 217 | PublishLikeCount | BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions | count(DISTINCT CASE when PublishLikeCount > 0 THEN RealCID END) | T2 |
| 218 | PublishPostCount | BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions | count(DISTINCT CASE when PublishPostCount > 0 THEN RealCID END) | T2 |
| 219 | PnLAdjustment | BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions.PnLAdjustment | SUM(PnLAdjustment) | T2 |
| 220 | NewCopyUsers | BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions | count(DISTINCT CASE WHEN NewCopyCount > 0 THEN RealCID END ) | T2 |
| 221 | DepositorsLoggedIn | BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions | count(DISTINCT CASE WHEN DepositorsLoggedIn > 0 THEN RealCID END ) | T2 |
| 222 | BonusComp | BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions.BonusComp | SUM(BonusComp) | T2 |
| 223 | RevenueGenerators | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | Outer `SUM` of per-customer `MAX(CASE WHEN TotalRevenue <> 0 THEN 1 ELSE 0 END)` over revenue rows **YTD through @edate** | T2 |
| 224 | ActiveOpened | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpened = 1 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 225 | ActiveOpenedRealStocks | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedRealStocks | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedRealStocks = 1 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 226 | ActiveOpenedCFDStocks | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFDStocks | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFDStocks = 1 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 227 | ActiveOpenedRealETF | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedRealETF | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedRealETF = 1 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 228 | ActiveOpenedCFDETF | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFDETF | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFDETF = 1 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 229 | ActiveOpenedRealCrypto | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedRealCrypto | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedRealCrypto = 1 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 230 | ActiveOpenedCFDCrypto | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFDCrypto | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFDCrypto = 1 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 231 | ActiveOpenedReal | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedReal | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedReal = 1 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 232 | ActiveOpenedCFD | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFD | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFD = 1 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 233 | ActiveOpenedCopy | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCopy | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCopy = 1 THEN 1 ELSE 0 END)` over **YTD through @edate** | T2 |
| 234 | PnLAdjusted | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions | `SUM(ISNULL(dpnl.DailyTotalPnL,0) + ISNULL(dnrev.PnLAdjustment,0))`; PnL summed **YTD through @edate** | T2 |
| 235 | ZeroPnLAdjusted | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions, BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | `SUM(ISNULL(DailyTotalPnL,0) + ISNULL(PnLAdjustment,0) + ISNULL(Commission,0))` over same YTD windows | T2 |
| 236 | ZeroPnLCopy | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | `SUM(ISNULL(DailyPnLCopy,0) + ISNULL(CommissionCopy,0))` over **YTD through @edate** | T2 |
| 237 | ZeroPnLStocks | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | `SUM(ISNULL(DailyPnLStocks,0) + ISNULL(CommissionStocks,0))` over **YTD through @edate** | T2 |
| 238 | GlobalCashoutsAdjusted | BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions, BI_DB_dbo.BI_DB_V_DDR_MIMO | `SUM(ISNULL(GlobalWithdraw_ExclRedeem,0) - ISNULL(CompensationToAffiliateWithCashoutAmount,0) + ISNULL(CompensationPIWithCashoutAmount,0))` with MIMO/non-rev over **YTD through @edate** | T2 |
...
| 2025-05-05 | Guy M | for some reason the ticketfee breakdown was commented out - uncommented |
