# Object lineage — BI_DB_dbo.Function_Search_Functions
...
| Object | Schema | Notes |
|--------|--------|-------|
| objects | sys | Referenced in function body |
...
- **ALTER**: Companion `Function_Search_Functions.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
