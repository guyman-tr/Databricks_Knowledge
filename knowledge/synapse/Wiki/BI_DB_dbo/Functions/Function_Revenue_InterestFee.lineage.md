# Object lineage — BI_DB_dbo.Function_Revenue_InterestFee
...
| Object | Schema | Notes |
|--------|--------|-------|
| BI_DB_Daily_CreditLine | BI_DB_dbo | Referenced in function body |
...
- **ALTER**: Companion `Function_Revenue_InterestFee.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
