# Object lineage — BI_DB_dbo.Function_MIMO_First_Deposit_All_Platforms
...
| FiatTransactions | eMoney_dbo | Referenced in function body |
...
- **ALTER**: Companion `Function_MIMO_First_Deposit_All_Platforms.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
