# Object lineage — BI_DB_dbo.Function_Revenue_SpotAdjustFee
...
- **ALTER**: Companion `Function_Revenue_SpotAdjustFee.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
