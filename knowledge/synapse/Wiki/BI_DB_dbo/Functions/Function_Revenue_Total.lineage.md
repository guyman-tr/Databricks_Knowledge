# Object lineage — BI_DB_dbo.Function_Revenue_Total
...
| Object | Schema | Notes |
|--------|--------|-------|
| BI_DB_DDR_Fact_Revenue_Generating_Actions | BI_DB_dbo | Referenced in function body |
| Dim_Revenue_Metrics | BI_DB_dbo | Referenced in function body |
| Function_Revenue_StakingFee | BI_DB_dbo | Referenced in function body |
...
- **ALTER**: Companion `Function_Revenue_Total.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
