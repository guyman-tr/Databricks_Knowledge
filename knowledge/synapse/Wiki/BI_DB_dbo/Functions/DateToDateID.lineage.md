# Object lineage — BI_DB_dbo.DateToDateID
...
| Object | Schema | Notes |
|--------|--------|-------|
| *(none — expression only)* | — | Referenced in function body |
...
- **ALTER**: Companion `DateToDateID.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
