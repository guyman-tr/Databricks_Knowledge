# Object lineage — BI_DB_dbo.Function_Revenue_DormantFee
...
- **ALTER**: Companion `Function_Revenue_DormantFee.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
