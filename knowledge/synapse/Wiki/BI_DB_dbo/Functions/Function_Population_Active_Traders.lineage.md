# Object lineage — BI_DB_dbo.Function_Population_Active_Traders
...
- **ALTER**: Companion `Function_Population_Active_Traders.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
