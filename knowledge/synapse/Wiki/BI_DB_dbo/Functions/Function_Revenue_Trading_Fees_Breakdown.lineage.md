# Object lineage — BI_DB_dbo.Function_Revenue_Trading_Fees_Breakdown
...
| Dim_CompensationReason | DWH_dbo | Referenced in function body |
...
- **ALTER**: Companion `Function_Revenue_Trading_Fees_Breakdown.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
