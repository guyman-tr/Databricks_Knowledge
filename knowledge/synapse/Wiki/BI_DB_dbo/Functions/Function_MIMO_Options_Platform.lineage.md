# Object lineage — BI_DB_dbo.Function_MIMO_Options_Platform
...
| Object | Schema | Notes |
|--------|--------|-------|
| External_Sodreconciliation_apex_EXT869_CashActivity | BI_DB_dbo | Referenced in function body |
...
- **ALTER**: Companion `Function_MIMO_Options_Platform.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
