# Object lineage — BI_DB_dbo.Function_DDR_Aggregation_ThisMonth
...
- **ALTER**: Companion `Function_DDR_Aggregation_ThisMonth.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
