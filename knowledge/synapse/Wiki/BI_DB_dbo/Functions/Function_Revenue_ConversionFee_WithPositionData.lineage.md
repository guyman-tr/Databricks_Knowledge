# Object lineage — BI_DB_dbo.Function_Revenue_ConversionFee_WithPositionData
...
| Object | Schema | Notes |
|--------|--------|-------|
| BI_DB_DepositWithdrawFee | BI_DB_dbo | Referenced in function body |
| Fact_SnapshotCustomer | DWH_dbo | Referenced in function body |
| Dim_Range | DWH_dbo | Referenced in function body |
| Fact_BillingDeposit | DWH_dbo | Referenced in function body |
| Fact_BillingWithdraw | DWH_dbo | Referenced in function body |
...
- **ALTER**: Companion `Function_Revenue_ConversionFee_WithPositionData.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
