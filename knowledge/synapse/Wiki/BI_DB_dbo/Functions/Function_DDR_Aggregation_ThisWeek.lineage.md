# Object lineage — BI_DB_dbo.Function_DDR_Aggregation_ThisWeek
...
- **ALTER**: Companion `Function_DDR_Aggregation_ThisWeek.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
