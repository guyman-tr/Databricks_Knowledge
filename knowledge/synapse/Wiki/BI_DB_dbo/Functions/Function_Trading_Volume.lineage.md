# Object lineage — BI_DB_dbo.Function_Trading_Volume
...
- **ALTER**: Companion `Function_Trading_Volume.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
