# Object lineage — BI_DB_dbo.Function_Population_First_Trading_Action
...
- **ALTER**: Companion `Function_Population_First_Trading_Action.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
