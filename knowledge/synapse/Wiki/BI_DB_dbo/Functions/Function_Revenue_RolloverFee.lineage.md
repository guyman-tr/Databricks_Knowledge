# Object lineage — BI_DB_dbo.Function_Revenue_RolloverFee
...
- **ALTER**: Companion `Function_Revenue_RolloverFee.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
