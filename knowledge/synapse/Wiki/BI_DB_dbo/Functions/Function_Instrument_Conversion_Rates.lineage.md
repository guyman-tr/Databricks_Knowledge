# Object lineage — BI_DB_dbo.Function_Instrument_Conversion_Rates
...
- **ALTER**: Companion `Function_Instrument_Conversion_Rates.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
