# Object lineage — BI_DB_dbo.Function_PnL_Single_Day
...
| BI_DB_PositionPnL | BI_DB_dbo | Referenced in function body |
...
- **ALTER**: Companion `Function_PnL_Single_Day.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
