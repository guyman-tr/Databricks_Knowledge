...
| 2 | FirstActionType | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.FirstActionType_ThisMonth | MAX(FirstActionType_ThisMonth) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 3 | RegulationID | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.RegulationID_ThisMonth | MAX(RegulationID_ThisMonth) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 4 | IsCreditReportValidCB | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.IsCreditReportValidCB_ThisMonth | MAX(IsCreditReportValidCB_ThisMonth) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 5 | IsValidCustomer | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.IsValidCustomer_ThisMonth | MAX(IsValidCustomer_ThisMonth) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 6 | MifidCategorizationID | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.MifidCategorizationID_ThisMonth | MAX(MifidCategorizationID_ThisMonth) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 7 | PlayerLevelID | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.PlayerLevelID_ThisMonth | MAX(PlayerLevelID_ThisMonth) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 8 | CountryID | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.CountryID_ThisMonth | MAX(CountryID_ThisMonth) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 9 | MarketingRegion | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.MarketingRegion_ThisMonth | MAX(MarketingRegion_ThisMonth) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 10 | IsFunded | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.IsFunded_ThisMonth | SUM(IsFunded_ThisMonth): PERIODIC CTE sums flags where `Date = @edate`; outer query SUM over customers per output grain | T2 |
| 11 | FirstTimeFunded | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.FirstTimeFunded_ThisMonth | SUM(FirstTimeFunded_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 12 | ActiveTraded | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.ActiveTraded_ThisMonth | SUM(ActiveTraded_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 13 | Portfolio_Only | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.Portfolio_Only_ThisMonth | SUM(Portfolio_Only_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 14 | BalanceOnlyAccount | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.BalanceOnlyAccount_ThisMonth | SUM(BalanceOnlyAccount_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 15 | TPFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.TPFirstDeposited_ThisMonth | SUM(TPFirstDeposited_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 16 | IBANFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.IBANFirstDeposited_ThisMonth | SUM(IBANFirstDeposited_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 17 | TPExternalFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.TPExternalFirstDeposited_ThisMonth | SUM(TPExternalFirstDeposited_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 18 | GlobalFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalFirstDeposited_ThisMonth | SUM(GlobalFirstDeposited_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 19 | GlobalDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalDeposited_ThisMonth | SUM(GlobalDeposited_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 20 | GlobalRedeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalRedeposited_ThisMonth | SUM(GlobalRedeposited_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 21 | GlobalCashedOut | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalCashedOut_ThisMonth | SUM(GlobalCashedOut_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 22 | Redeemed | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.Redeemed_ThisMonth | SUM(Redeemed_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 23 | DepositedTP | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.DepositedTP_ThisMonth | SUM(DepositedTP_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 24 | DepositedIBAN | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.DepositedIBAN_ThisMonth | SUM(DepositedIBAN_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 25 | ReDepositedTP | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.ReDepositedTP_ThisMonth | SUM(ReDepositedTP_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 26 | ReDepositedIBAN | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.ReDepositedIBAN_ThisMonth | SUM(ReDepositedIBAN_ThisMonth) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 27 | TP_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN TPFirstDeposited_ThisMonth = 1 THEN ds.TP_FTDA ELSE 0 END)`; `ds.TP_FTDA` is `MAX(TP_FTDA)` from daily status for that customer over **month start → @edate** | T2 |
| 28 | IBAN_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN IBANFirstDeposited_ThisMonth = 1 THEN ds.IBAN_FTDA ELSE 0 END)`; `ds.IBAN_FTDA` is `MAX(IBAN_FTDA)` over **month start → @edate** | T2 |
| 29 | TP_External_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN TPExternalFirstDeposited_ThisMonth = 1 THEN ds.TP_External_FTDA ELSE 0 END)`; `ds.TP_External_FTDA` is `MAX(TP_External_FTDA)` over **month start → @edate** | T2 |
| 30 | Global_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN GlobalFirstDeposited_ThisMonth = 1 THEN ds.Global_FTDA ELSE 0 END)`; `ds.Global_FTDA` is `MAX(Global_FTDA)` over **month start → @edate** | T2 |
| 31 | ExternalDepositsTPAmount | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalDepositsTPAmount | SUM(ExternalDepositsTPAmount) per customer over MIMO rows with `Date` from **month start through @edate**; outer SUM across customers in group | T2 |
...
| 73 | ExternalCashedOutTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalCashedOutTP | SUM(ExternalCashedOutTP) | T2 |
| 74 | InternalCashedOutTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.InternalCashedOutTP | SUM(InternalCashedOutTP) | T2 |
| 75 | ExternalCashedOutIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalCashedOutIBAN | SUM(ExternalCashedOutIBAN) | T2 |
| 76 | InternalCashedOutIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO.InternalCashedOutIBAN | SUM(InternalCashedOutIBAN) | T2 |
| 77 | TradersOpenedFromIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO.TradersOpenedFromIBAN | SUM(TradersOpenedFromIBAN) | T2 |
| 78 | TradersClosedIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO.TradersClosedIBAN | SUM(TradersClosedIBAN) | T2 |
| 79 | InternalDepositedTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.InternalDepositedTP | SUM(InternalDepositedTP) | T2 |
| 80 | ExternalDepositedTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalDepositedTP | SUM(ExternalDepositedTP) | T2 |
| 81 | CashedOutTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.CashedOutTP | SUM(CashedOutTP) | T2 |
...
| 223 | RevenueGenerators | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | Outer `SUM` of per-customer `MAX(CASE WHEN TotalRevenue <> 0 THEN 1 ELSE 0 END)` over revenue rows **month start → @edate** | T2 |
| 224 | ActiveOpened | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpened = 1 THEN 1 ELSE 0 END)` over **month start → @edate** | T2 |
| 225 | ActiveOpenedRealStocks | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedRealStocks | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedRealStocks = 1 THEN 1 ELSE 0 END)` over **month start → @edate** | T2 |
| 226 | ActiveOpenedCFDStocks | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFDStocks | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFDStocks = 1 THEN 1 ELSE 0 END)` over **month start → @edate** | T2 |
| 227 | ActiveOpenedRealETF | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedRealETF | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedRealETF = 1 THEN 1 ELSE 0 END)` over **month start → @edate** | T2 |
| 228 | ActiveOpenedCFDETF | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFDETF | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFDETF = 1 THEN 1 ELSE 0 END)` over **month start → @edate** | T2 |
| 229 | ActiveOpenedRealCrypto | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedRealCrypto | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedRealCrypto = 1 THEN 1 ELSE 0 END)` over **month start → @edate** | T2 |
| 230 | ActiveOpenedCFDCrypto | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFDCrypto | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFDCrypto = 1 THEN 1 ELSE 0 END)` over **month start → @edate** | T2 |
| 231 | ActiveOpenedReal | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedReal | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedReal = 1 THEN 1 ELSE 0 END)` over **month start → @edate** | T2 |
| 232 | ActiveOpenedCFD | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFD | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFD = 1 THEN 1 ELSE 0 END)` over **month start → @edate** | T2 |
| 233 | ActiveOpenedCopy | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCopy | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCopy = 1 THEN 1 ELSE 0 END)` over **month start → @edate** | T2 |
| 234 | PnLAdjusted | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions | `SUM(ISNULL(dpnl.DailyTotalPnL,0) + ISNULL(dnrev.PnLAdjustment,0))` joined on RealCID; daily PnL summed **month start → @edate** | T2 |
| 235 | ZeroPnLAdjusted | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions, BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | `SUM(ISNULL(DailyTotalPnL,0) + ISNULL(PnLAdjustment,0) + ISNULL(Commission,0))` over same month-to-date windows per source view | T2 |
| 236 | ZeroPnLCopy | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | `SUM(ISNULL(DailyPnLCopy,0) + ISNULL(CommissionCopy,0))` over **month start → @edate** | T2 |
| 237 | ZeroPnLStocks | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | `SUM(ISNULL(DailyPnLStocks,0) + ISNULL(CommissionStocks,0))` over **month start → @edate** | T2 |
| 238 | GlobalCashoutsAdjusted | BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions, BI_DB_dbo.BI_DB_V_DDR_MIMO | `SUM(ISNULL(GlobalWithdraw_ExclRedeem,0) - ISNULL(CompensationToAffiliateWithCashoutAmount,0) + ISNULL(CompensationPIWithCashoutAmount,0))` with MIMO/non-rev sums over **month start → @edate** | T2 |
