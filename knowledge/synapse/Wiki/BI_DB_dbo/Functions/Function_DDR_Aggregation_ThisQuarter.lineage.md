# Object lineage — BI_DB_dbo.Function_DDR_Aggregation_ThisQuarter
...
| BI_DB_DDR_Customer_Daily_Status | BI_DB_dbo | Referenced in function body |
...
- **ALTER**: Companion `Function_DDR_Aggregation_ThisQuarter.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
