# Object lineage — BI_DB_dbo.Function_Revenue_StakingFee
...
| Dealing_Staking_Results | Dealing_dbo | Referenced in function body |
| Dim_Range | DWH_dbo | Referenced in function body |
...
- **ALTER**: Companion `Function_Revenue_StakingFee.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
