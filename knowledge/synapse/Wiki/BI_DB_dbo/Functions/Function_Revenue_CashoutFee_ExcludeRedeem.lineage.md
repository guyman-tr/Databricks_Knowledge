# Object lineage — BI_DB_dbo.Function_Revenue_CashoutFee_ExcludeRedeem
...
- **ALTER**: Companion `Function_Revenue_CashoutFee_ExcludeRedeem.alter.sql` is a **comment-only stub** when UC Target is `_Not_Migrated` — `deploy-alter-dwh` must skip executable statements for these files.
