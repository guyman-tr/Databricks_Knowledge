...
| 2 | FirstActionType | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.FirstActionType_ThisWeek | MAX(FirstActionType_ThisWeek) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 3 | RegulationID | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.RegulationID_ThisWeek | MAX(RegulationID_ThisWeek) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 4 | IsCreditReportValidCB | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.IsCreditReportValidCB_ThisWeek | MAX(IsCreditReportValidCB_ThisWeek) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 5 | IsValidCustomer | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.IsValidCustomer_ThisWeek | MAX(IsValidCustomer_ThisWeek) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 6 | MifidCategorizationID | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.MifidCategorizationID_ThisWeek | MAX(MifidCategorizationID_ThisWeek) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 7 | PlayerLevelID | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.PlayerLevelID_ThisWeek | MAX(PlayerLevelID_ThisWeek) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 8 | CountryID | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.CountryID_ThisWeek | MAX(CountryID_ThisWeek) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 9 | MarketingRegion | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.MarketingRegion_ThisWeek | MAX(MarketingRegion_ThisWeek) in PERIODIC CTE; output GROUP BY dimension | T2 |
| 10 | IsFunded | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.IsFunded_ThisWeek | SUM(IsFunded_ThisWeek) in PERIODIC where `Date = @edate`; outer SUM over customers per output grain | T2 |
| 11 | FirstTimeFunded | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.FirstTimeFunded_ThisWeek | SUM(FirstTimeFunded_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 12 | ActiveTraded | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.ActiveTraded_ThisWeek | SUM(ActiveTraded_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 13 | Portfolio_Only | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.Portfolio_Only_ThisWeek | SUM(Portfolio_Only_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 14 | BalanceOnlyAccount | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.BalanceOnlyAccount_ThisWeek | SUM(BalanceOnlyAccount_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 15 | TPFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.TPFirstDeposited_ThisWeek | SUM(TPFirstDeposited_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 16 | IBANFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.IBANFirstDeposited_ThisWeek | SUM(IBANFirstDeposited_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 17 | TPExternalFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.TPExternalFirstDeposited_ThisWeek | SUM(TPExternalFirstDeposited_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 18 | GlobalFirstDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalFirstDeposited_ThisWeek | SUM(GlobalFirstDeposited_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 19 | GlobalDeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalDeposited_ThisWeek | SUM(GlobalDeposited_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 20 | GlobalRedeposited | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalRedeposited_ThisWeek | SUM(GlobalRedeposited_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 21 | GlobalCashedOut | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.GlobalCashedOut_ThisWeek | SUM(GlobalCashedOut_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 22 | Redeemed | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.Redeemed_ThisWeek | SUM(Redeemed_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 23 | DepositedTP | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.DepositedTP_ThisWeek | SUM(DepositedTP_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 24 | DepositedIBAN | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.DepositedIBAN_ThisWeek | SUM(DepositedIBAN_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 25 | ReDepositedTP | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.ReDepositedTP_ThisWeek | SUM(ReDepositedTP_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 26 | ReDepositedIBAN | BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status.ReDepositedIBAN_ThisWeek | SUM(ReDepositedIBAN_ThisWeek) over `Date = @edate` periodic rows, then outer SUM | T2 |
| 27 | TP_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN TPFirstDeposited_ThisWeek = 1 THEN ds.TP_FTDA ELSE 0 END)`; `ds.TP_FTDA` is `MAX(TP_FTDA)` over **week-to-date through @edate** | T2 |
| 28 | IBAN_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN IBANFirstDeposited_ThisWeek = 1 THEN ds.IBAN_FTDA ELSE 0 END)`; `ds.IBAN_FTDA` is `MAX(IBAN_FTDA)` over **week-to-date through @edate** | T2 |
| 29 | TP_External_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN TPExternalFirstDeposited_ThisWeek = 1 THEN ds.TP_External_FTDA ELSE 0 END)`; `ds.TP_External_FTDA` is `MAX(TP_External_FTDA)` over **week-to-date through @edate** | T2 |
| 30 | Global_FTDA | BI_DB_dbo.BI_DB_DDR_Customer_Daily_Status, BI_DB_dbo.BI_DB_DDR_Customer_Periodic_Status | `SUM(CASE WHEN GlobalFirstDeposited_ThisWeek = 1 THEN ds.Global_FTDA ELSE 0 END)`; `ds.Global_FTDA` is `MAX(Global_FTDA)` over **week-to-date through @edate** | T2 |
| 31 | ExternalDepositsTPAmount | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalDepositsTPAmount | SUM(ExternalDepositsTPAmount) per customer over MIMO rows with `Date` **week-to-date through @edate**; outer SUM | T2 |
...
| 73 | ExternalCashedOutTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalCashedOutTP | Outer `SUM` of per-customer `MAX(ExternalCashedOutTP)` from MIMO CTE (**week-to-date through @edate**) | T2 |
| 74 | InternalCashedOutTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.InternalCashedOutTP | Outer `SUM` of per-customer `MAX(InternalCashedOutTP)` from MIMO CTE (**week-to-date through @edate**) | T2 |
| 75 | ExternalCashedOutIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalCashedOutIBAN | Outer `SUM` of per-customer `MAX(ExternalCashedOutIBAN)` from MIMO CTE (**week-to-date through @edate**) | T2 |
| 76 | InternalCashedOutIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO.InternalCashedOutIBAN | Outer `SUM` of per-customer `MAX(InternalCashedOutIBAN)` from MIMO CTE (**week-to-date through @edate**) | T2 |
| 77 | TradersOpenedFromIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO | Outer `SUM` of per-customer `MAX(CASE WHEN TradeOpenFromIBANCount > 0 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 78 | TradersClosedIBAN | BI_DB_dbo.BI_DB_V_DDR_MIMO | Outer `SUM` of per-customer `MAX(CASE WHEN TradeCloseToIBANAmount > 0 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 79 | InternalDepositedTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.InternalDepositedTP | Outer `SUM` of per-customer `MAX(InternalDepositedTP)` from MIMO CTE (**week-to-date through @edate**) | T2 |
| 80 | ExternalDepositedTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.ExternalDepositedTP | Outer `SUM` of per-customer `MAX(ExternalDepositedTP)` from MIMO CTE (**week-to-date through @edate**) | T2 |
| 81 | CashedOutTP | BI_DB_dbo.BI_DB_V_DDR_MIMO.CashedOutTP | Outer `SUM` of per-customer `MAX(CashedOutTP)` from MIMO CTE (**week-to-date through @edate**) | T2 |
...
| 223 | RevenueGenerators | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | Outer `SUM` of per-customer `MAX(CASE WHEN TotalRevenue <> 0 THEN 1 ELSE 0 END)` over revenue rows **week-to-date through @edate** | T2 |
| 224 | ActiveOpened | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpened = 1 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 225 | ActiveOpenedRealStocks | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedRealStocks | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedRealStocks = 1 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 226 | ActiveOpenedCFDStocks | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFDStocks | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFDStocks = 1 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 227 | ActiveOpenedRealETF | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedRealETF | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedRealETF = 1 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 228 | ActiveOpenedCFDETF | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFDETF | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFDETF = 1 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 229 | ActiveOpenedRealCrypto | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedRealCrypto | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedRealCrypto = 1 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 230 | ActiveOpenedCFDCrypto | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFDCrypto | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFDCrypto = 1 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 231 | ActiveOpenedReal | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedReal | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedReal = 1 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 232 | ActiveOpenedCFD | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCFD | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCFD = 1 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 233 | ActiveOpenedCopy | BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown.ActiveOpenedCopy | Outer `SUM` of per-customer `MAX(CASE WHEN ActiveOpenedCopy = 1 THEN 1 ELSE 0 END)` over **week-to-date through @edate** | T2 |
| 234 | PnLAdjusted | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions | `SUM(ISNULL(dpnl.DailyTotalPnL,0) + ISNULL(dnrev.PnLAdjustment,0))`; PnL summed **week-to-date through @edate** | T2 |
| 235 | ZeroPnLAdjusted | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions, BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | `SUM(ISNULL(DailyTotalPnL,0) + ISNULL(PnLAdjustment,0) + ISNULL(Commission,0))` over same week-to-date windows | T2 |
| 236 | ZeroPnLCopy | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | `SUM(ISNULL(DailyPnLCopy,0) + ISNULL(CommissionCopy,0))` over **week-to-date through @edate** | T2 |
| 237 | ZeroPnLStocks | BI_DB_dbo.BI_DB_V_DDR_PnL, BI_DB_dbo.BI_DB_V_DDR_Revenue_Breakdown | `SUM(ISNULL(DailyPnLStocks,0) + ISNULL(CommissionStocks,0))` over **week-to-date through @edate** | T2 |
| 238 | GlobalCashoutsAdjusted | BI_DB_dbo.BI_DB_V_DDR_Non_Revenue_Actions, BI_DB_dbo.BI_DB_V_DDR_MIMO | `SUM(ISNULL(GlobalWithdraw_ExclRedeem,0) - ISNULL(CompensationToAffiliateWithCashoutAmount,0) + ISNULL(CompensationPIWithCashoutAmount,0))` with MIMO/non-rev over **week-to-date through @edate** | T2 |
