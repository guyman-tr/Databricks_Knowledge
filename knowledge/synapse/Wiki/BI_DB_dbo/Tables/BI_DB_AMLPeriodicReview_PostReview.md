...
| 2 | FirstDepositDate | date | YES | Date of first deposit. DEFAULT='19000101'. Sourced from Dim_Customer.FirstDepositDate (CustomerFinanceDB FTD pipeline). Used as the anchor for GROUP A 3-year review scheduling. (Tier 2 — BI_DB_dbo.BI_DB_AMLPeriodicReview wiki → DWH_dbo.Dim_Customer) |
...
| 14 | EvMatchStatusName | varchar(max) | YES | Electronic verification (EV) match status from Dim_EvMatchStatus. Values: None (31%), Verified (57%), NotVerified (8%), PartiallyVerified (4%). EV-verified customers skip document expiry checks. (Tier 2 — BI_DB_dbo.BI_DB_AMLPeriodicReview wiki via Dim_EvMatchStatus) |
