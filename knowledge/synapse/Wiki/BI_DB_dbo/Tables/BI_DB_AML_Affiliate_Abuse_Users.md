...
| 5 | FirstDepositDate | datetime | YES | Date and time of the customer's first deposit. Sentinel value 1900-01-01 00:00:00 indicates no deposit has been made. Passthrough from Dim_Customer. (Tier 1 — DWH_dbo.Dim_Customer wiki) |
...
| 9 | FirstDepositAmount | float | YES | Amount of the customer's first deposit. 0 or NULL if no deposit. Passthrough from Dim_Customer. (Tier 1 — DWH_dbo.Dim_Customer wiki) |
...
| 11 | IsValidCustomer | bit | YES | 1 if customer passes basic validity checks (not internal/test/blocked at registration). Passthrough from Dim_Customer. (Tier 1 — DWH_dbo.Dim_Customer wiki) |
| 12 | IsDepositor | bit | YES | 1 if customer has made at least one approved deposit. 0 for non-depositors. Passthrough from Dim_Customer. (Tier 1 — DWH_dbo.Dim_Customer wiki) |
