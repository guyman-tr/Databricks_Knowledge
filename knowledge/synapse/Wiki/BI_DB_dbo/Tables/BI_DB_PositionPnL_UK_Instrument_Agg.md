...
| 2 | InstrumentID | int | NO | Traded instrument. GROUP BY key. Only stocks/ETFs (InstrumentTypeID 5,6). FK to Dim_Instrument. Passthrough from BI_DB_PositionPnL. (Tier 1 — BI_DB_PositionPnL) |
...
| 9 | IsBuy | int | YES | Long (1) vs short (0). GROUP BY key. Always 1 in practice (custody = BUY-only). (Tier 1 — BI_DB_PositionPnL) |
...
| 13 | HedgeServerID | int | YES | Hedge server for the position group. GROUP BY key. (Tier 1 — BI_DB_PositionPnL) |
