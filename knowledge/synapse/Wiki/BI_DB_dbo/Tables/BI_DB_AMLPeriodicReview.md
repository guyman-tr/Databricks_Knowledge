...
| 2 | FirstDepositDate | date | YES | Date of first deposit. DEFAULT='19000101'. Updated from CustomerFinanceDB FTD data with FTDRecoveryDate logic. Used as the anchor date for GROUP A 3-year review scheduling. (Tier 2 — DWH_dbo.Dim_Customer wiki) |
...
| 16 | EvMatchStatusName | nvarchar(max) | YES | Electronic verification match status name from Dim_EvMatchStatus (EvMatchStatus). Decision from automated identity verification vendors (Onfido, Au10tix). Sample: Verified, NotVerified, NULL. (Tier 2 — DWH_dbo.Dim_Customer wiki via Dim_EvMatchStatus) |
