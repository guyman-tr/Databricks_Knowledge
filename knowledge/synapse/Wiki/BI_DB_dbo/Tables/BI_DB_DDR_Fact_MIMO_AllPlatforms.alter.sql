...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_fact_mimo_allplatforms ALTER COLUMN IsPlatformFTD COMMENT 'Platform-level first-time deposit flag. `ISNULL(f.IsPlatformFTD, 0)`. Renamed from `IsFTD` in sub-platform tables. 1 = first deposit on this specific platform. Updated by FTD recovery logic for DateID >= 20250901. (Tier 2 - SP_DDR_Fact_Fact_MIMO_AllPlatforms)';
...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_fact_mimo_allplatforms ALTER COLUMN MIMOPlatform COMMENT 'Platform discriminator. Literal: `''TradingPlatform''`, `''eMoney''`, `''Options''`, `''MoneyFarm''`. Set by SP during UNION/INSERT logic. (Tier 2 - SP_DDR_Fact_Fact_MIMO_AllPlatforms)';
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_fact_mimo_allplatforms ALTER COLUMN IsGlobalFTD COMMENT 'Cross-platform first-time deposit flag. `CASE WHEN f.RealCID IS NOT NULL THEN 1 ELSE 0 END` from LEFT JOIN to `Function_MIMO_First_Deposit_All_Platforms`; `ISNULL(...,0)`. Updated by FTD recovery UPDATEs from `Dim_Customer` for DateID >= 20250901. (Tier 2 - SP_DDR_Fact_Fact_MIMO_AllPlatforms)';
...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_fact_mimo_allplatforms ALTER COLUMN IsIBANQuickTransfer COMMENT 'eMoney internal transfer flag (MoveMoneyReasonID = 6). `ISNULL(f.IsIBANQuickTransfer, 0)`. 1 = eMoney "Internal Transfer" feature (distinct from TP internal transfers). Always 0 for Options/MoneyFarm. (Tier 2 - SP_DDR_Fact_Fact_MIMO_AllPlatforms)';
...
-- == LAST EXECUTION ==
-- Timestamp: 2026-03-30 16:04:20 UTC
