...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_emoney_dbo_emoney_dim_account ALTER COLUMN `PlayerStatusID` COMMENT 'Compliance and trading account status. FK to Dictionary.PlayerStatus. 1=Normal; other values indicate restricted, closed, banned, or special states. Default=0. (Tier 1 - Customer.CustomerStatic)';
...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_emoney_dbo_emoney_dim_account ALTER COLUMN `IsValidCustomer` COMMENT 'DWH-computed: 1 when not Internal (PlayerLevelID != 4), not label 30/26, and not CountryID=250. Used in reporting to filter out non-standard customers. Passthrough from Dim_Customer. (Tier 2 - SP_Dim_Customer)';
