# eMoney_dbo.eMoney_Dim_Transaction — Production Lineage Map
...
| **Production Tables** | dbo.FiatTransactions (core), dbo.FiatTransactionsStatuses (status events), dbo.TransactionsProvidersMapping (provider IDs), dbo.FiatCards (card FK), dbo.FiatAccount (account FK), dbo.FiatCurrencyBalances (balance FK), dbo.FiatBankAccount (external bank account FK) |
...
| **ETL SP** | SP_eMoney_DimFact_Transaction (11-step pipeline, Steps 1–9 shared with eMoney_Fact_Transaction_Status, Step 10 = Dim INSERT with WHERE RNDesc=1) |
| **Filter** | WHERE RNDesc=1 (latest status event per transaction = current state snapshot) |
...
Step 11: DELETE + INSERT eMoney_Fact_Transaction_Status FROM #leveled_txs (all statuses)
```
...
| 47 | IsTxSettled | Computed | — | CASE WHEN TxStatusID=2 (Settled) THEN 1 ELSE 0 [KEY DIFFERENCE from Fact_Transaction_Status] |
| 48 | TxStatusID | dbo.FiatTransactionsStatuses | TransactionStatusId | Latest status event (RNDesc=1 by TransactionOccured DESC) |
...
| 51 | AuthorizationTypeID | dbo.FiatTransactionsStatuses | AuthorizationType | Latest status event (RNDesc=1) |
...
| 55 | HolderCurrencyISO | dbo.FiatTransactionsStatuses | HolderCurrency | Latest status event (RNDesc=1) |
...
| 57 | HolderAmount | dbo.FiatTransactionsStatuses | HolderAmount | Latest status event (RNDesc=1) |
| 58 | LocalCurrencyISO | dbo.FiatTransactionsStatuses | TransactionCurrency | Latest status event (RNDesc=1) |
| 59 | LocalCurrencyDesc | eMoney_Currency_Mapping_ISO | CurrencyName | JOIN on LocalCurrencyISO |
| 60 | LocalAmount | dbo.FiatTransactionsStatuses | TransactionAmount | Latest status event (RNDesc=1) |
| 61 | USDAmountApprox | Computed | dbo.FiatTransactionsStatuses.HolderAmount × Fact_CurrencyPriceWithSplit mid-rate | ROUND(HolderAmount × (Ask+Bid)/2, 2) or inverse |
| 62 | USDRateApprox | Computed | DWH_dbo.Fact_CurrencyPriceWithSplit | ROUND((Ask+Bid)/2, 2) at TxLocalDate |
| 63 | AccumulatedAmount | dbo.FiatTransactionsStatuses | AccumulatedAmount | Latest status event (RNDesc=1) |
| 64 | AccumulatedUSDAmountApprox | Computed | AccumulatedAmount × mid-rate | Same logic as USDAmountApprox |
| 65 | TxStatusModificationTime | dbo.FiatTransactionsStatuses | TransactionOccured | Latest status event (RNDesc=1) |
...
| Note | FiatTransactions and FiatTransactionsStatuses have no FiatDwhDB wikis — all their direct columns are Tier 2 |
