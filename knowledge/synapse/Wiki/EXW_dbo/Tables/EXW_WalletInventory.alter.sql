...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_walletinventory ALTER COLUMN `LastWalletPoolStatus` COMMENT 'The lifecycle status: 1=Pending, 2=Verified, 3=Failed, 4=FundingInitiated, 5=FundingSent, 6=FundingVerified, 7=FundingFailed, 10=Timeout [UNVERIFIED: codepoint not in EXW_Dictionary.WalletPoolStatuses], 11=VerifiedForAssign [UNVERIFIED: codepoint not in EXW_Dictionary.WalletPoolStatuses]. FK to Dictionary.WalletPoolStatuses. DWH note: derived as the latest status event per wallet via ROW_NUMBER() OVER (PARTITION BY WalletPoolId ORDER BY Occurred DESC) in SP. (Tier 1 - WalletDB.Wallet.WalletPoolStatuses)';
