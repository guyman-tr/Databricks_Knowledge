...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Allocated Total` COMMENT 'Cumulative count of all wallets of this crypto/status that have been allocated (assigned to any GCID, omnibus or user) up to and including [Date for Report]. Formula: SUM(CASE WHEN Allocated < @EndDate THEN 1 ELSE 0). (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Funded Free` COMMENT 'Count of FundingVerified wallets that are unoccupied and ready for promotion. Formula: SUM(CASE WHEN WalletStatus=''FundingVerified'' AND Occupied=0 AND IsPromotionReady=1 THEN 1 ELSE 0). Non-zero only for WalletStatus=''FundingVerified'' rows. (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Funded Occupied` COMMENT 'Count of FundingVerified wallets that are actively holding customer funds. Formula: SUM(CASE WHEN WalletStatus=''FundingVerified'' AND Occupied=1 AND IsPromotionReady=1 THEN 1 ELSE 0). Non-zero only for WalletStatus=''FundingVerified'' rows. (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Total AllocatedOmnibuses` COMMENT 'Count of wallets allocated to omnibus/system accounts (GCID <= 0). Subset of [Allocated Total] for system-held wallets. Formula: SUM(CASE WHEN Allocated < @EndDate AND GCID <= 0 THEN 1 ELSE 0). (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Total AllocatedToUsers` COMMENT 'Count of wallets allocated to real customer accounts (GCID > 0). Subset of [Allocated Total] for customer-held wallets. Formula: SUM(CASE WHEN Allocated < @EndDate AND GCID > 0 THEN 1 ELSE 0). Should equal [Allocated Total] minus [Total AllocatedOmnibuses]. (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Total Created` COMMENT 'Cumulative count of all wallet addresses ever created for this crypto/status group up to @EndDate. Formula: COUNT(DISTINCT WalletID) WHERE Created < @EndDate. Includes both allocated and available wallets. (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Allocated Daily` COMMENT 'Count of wallets allocated on exactly [Date for Report] (Allocated = @d). Point-in-time daily allocation velocity - how many new assignments happened that day. (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Created Daily` COMMENT 'Count of wallets provisioned on exactly [Date for Report] (CreatedDateID = YYYYMMDD(@d)). Point-in-time daily creation velocity - how many new address records were generated that day. (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Allocated 7 days` COMMENT 'Count of wallets allocated within the 7-day window ending on [Date for Report] (Allocated BETWEEN @d-6 AND @d). Rolling 7-day allocation velocity. (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Allocated 30 days` COMMENT 'Count of wallets allocated within the 30-day window ending on [Date for Report] (Allocated BETWEEN @d-29 AND @d). Rolling 30-day allocation velocity. (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Created 7 days` COMMENT 'Count of wallets created within the 7-day window ending on [Date for Report] (Created BETWEEN @d-6 AND @d). Rolling 7-day provisioning velocity. (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Created 30 days` COMMENT 'Count of wallets created within the 30-day window ending on [Date for Report] (Created BETWEEN @d-29 AND @d). Rolling 30-day provisioning velocity. (Tier 2 - SP_EXW_Inventory_Snapshot_History)';
...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Allocated Total` SET TAGS ('pii' = 'none');
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Funded Free` SET TAGS ('pii' = 'none');
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Funded Occupied` SET TAGS ('pii' = 'none');
...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Date for Report` SET TAGS ('pii' = 'none');
...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Total AllocatedOmnibuses` SET TAGS ('pii' = 'none');
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Total AllocatedToUsers` SET TAGS ('pii' = 'none');
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Total Created` SET TAGS ('pii' = 'none');
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Allocated Daily` SET TAGS ('pii' = 'none');
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Created Daily` SET TAGS ('pii' = 'none');
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Allocated 7 days` SET TAGS ('pii' = 'none');
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Allocated 30 days` SET TAGS ('pii' = 'none');
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Created 7 days` SET TAGS ('pii' = 'none');
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_exw_dbo_exw_inventory_snapshot_history ALTER COLUMN `Created 30 days` SET TAGS ('pii' = 'none');

-- == LAST EXECUTION ==
-- Timestamp: 2026-05-03 06:34:59 UTC
-- Batch deploy resume: EXW_dbo deploy batch 1
-- Statements: 12/38 succeeded
-- Error: [RequestId=c9a05319-6a78-4014-8c5e-6f505b0b0111 ErrorClass=RESOURCE_DOES_NOT_EXIST] Column `Created 30 days` does not exist.
-- ====================
