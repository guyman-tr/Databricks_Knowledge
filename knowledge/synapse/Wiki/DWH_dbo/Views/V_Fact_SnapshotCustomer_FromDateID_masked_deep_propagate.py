"""Deep lineage propagation for DWH_dbo.V_Fact_SnapshotCustomer_FromDateID (masked UC target)."""
import sys, os
sys.stdout.reconfigure(line_buffering=True)
sys.path.insert(0, os.path.normpath(os.path.join(os.path.dirname(__file__), "../..")))
import _deep_propagate_lib as lib

SOURCE_UC = "main.dwh.gold_sql_dp_prod_we_dwh_dbo_v_fact_snapshotcustomer_fromdateid_masked"
SOURCE_SYNAPSE = "DWH_dbo.V_Fact_SnapshotCustomer_FromDateID"
OBJECT_DIR = os.path.dirname(os.path.abspath(__file__))
OBJECT_NAME = "V_Fact_SnapshotCustomer_FromDateID_masked"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description=f"Deep propagation: {SOURCE_SYNAPSE} (masked)")
    ...
    if args.command in ("discover", "both"):
        print(f"\n=== DISCOVER: {SOURCE_SYNAPSE} (masked) ===")
