"""Deep lineage propagation for DWH_dbo.V_Fact_SnapshotCustomer_FromDateID (pii_data UC target)."""
import sys, os
sys.stdout.reconfigure(line_buffering=True)
sys.path.insert(0, os.path.normpath(os.path.join(os.path.dirname(__file__), "../..")))
import _deep_propagate_lib as lib

SOURCE_UC = "main.pii_data.gold_sql_dp_prod_we_dwh_dbo_v_fact_snapshotcustomer_fromdateid"
SOURCE_SYNAPSE = "DWH_dbo.V_Fact_SnapshotCustomer_FromDateID"
OBJECT_DIR = os.path.dirname(os.path.abspath(__file__))
OBJECT_NAME = "V_Fact_SnapshotCustomer_FromDateID_piidata"
...
# Read descriptions from the shared view alter.sql (contains all FSC columns for both targets)
ALTER_SQL_PATH  = os.path.join(OBJECT_DIR, "V_Fact_SnapshotCustomer_FromDateID.alter.sql")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description=f"Deep propagation: {SOURCE_SYNAPSE} (pii_data)")
    ...
    if args.command in ("discover", "both"):
        print(f"\n=== DISCOVER: {SOURCE_SYNAPSE} (pii_data) ===")
    ...
    if args.command in ("execute", "both"):
        print(f"\n=== EXECUTE: {SOURCE_SYNAPSE} (pii_data) ===")
