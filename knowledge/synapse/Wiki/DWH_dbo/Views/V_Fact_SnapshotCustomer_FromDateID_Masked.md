...
| 40 | IsCreditReportValidCB | int | YES | Passthrough from Fact_SnapshotCustomer.IsCreditReportValidCB in masked UC export (same semantics as base table wiki). Financial-customer flag for Client_Balance reports (CB = Client_Balance, NOT CreditBureau). ETL-computed. See §2.3. (Tier 2 - SP_Fact_SnapshotCustomer) |
