...
|---|--------|------|----------|--------|-------------|
| 1 | Gcid | int | YES | STS_User_Operations_Data_History.Gcid | Global Customer ID — unique cross-platform identifier linking Real and Demo accounts for the same person. Distribution key. (Tier 2 — via STS_User_Operations_Data_History) |
| 2 | RealCid | int | YES | STS_User_Operations_Data_History.RealCid | Real-money account Customer ID. NULL when the session is Demo-only. (Tier 2 — via STS_User_Operations_Data_History) |
| 3 | DemoCid | int | YES | STS_User_Operations_Data_History.DemoCid | Virtual/demo account Customer ID. NULL when the session is Real-only. (Tier 2 — via STS_User_Operations_Data_History) |
| 4 | ApplicationIdentifier | nvarchar(100) | YES | STS_User_Operations_Data_History.ApplicationIdentifier | Client application that initiated the session. Known values: `retoro` (web/generic), `retoroios` (iOS app), `retoroandroid` (Android app). (Tier 2 — via STS_User_Operations_Data_History) |
| 5 | ApplicationVersion | nvarchar(20) | YES | STS_User_Operations_Data_History.ApplicationVersion | Build version of the client application, e.g. `340.0.10`, `355.0.1`. (Tier 2 — via STS_User_Operations_Data_History) |
| 6 | ClientIp | varchar(20) | YES | STS_User_Operations_Data_History.ClientIp | IPv4 address of the client at the time of the session event. (Tier 2 — via STS_User_Operations_Data_History) |
| 7 | ClientName | nvarchar(100) | YES | STS_User_Operations_Data_History.ClientName | Server-side service name that processed the authentication request. Consistently `STS.WebAPI` across all observed data. (Tier 2 — via STS_User_Operations_Data_History) |
| 8 | CreatedAt | datetime | YES | STS_User_Operations_Data_History.CreatedAt | Timestamp when the authentication/session event occurred in the STS service. This is the business event time (not the ETL load time). (Tier 2 — via STS_User_Operations_Data_History) |
| 9 | UserAgent | nvarchar(512) | YES | STS_User_Operations_Data_History.UserAgent | Full HTTP User-Agent string from the client browser or mobile WebView. Contains OS, browser, and app metadata. May be NULL for some mobile token exchanges. (Tier 2 — via STS_User_Operations_Data_History) |
| 10 | AccessTokenHashed | nvarchar(256) | YES | STS_User_Operations_Data_History.AccessTokenHashed | Hashed authentication access token for security audit trail. Not reversible. Sparsely populated. (Tier 2 — via STS_User_Operations_Data_History) |
| 11 | ClientDeviceId | nvarchar(max) | YES | CAST(STS_User_Operations_Data_History.ClientDeviceId AS nvarchar(max)) | UUID-format device identifier (e.g. `3c24d4e9-8ef0-405f-...`). Populated primarily for mobile app sessions; typically NULL or empty for web. (Tier 2 — via STS_User_Operations_Data_History) |
| 12 | ParentSessionId | bigint | YES | STS_User_Operations_Data_History.ParentSessionId | Session ID of the parent session for linked/chained sessions. Value `0` indicates a root session (no parent). Enables session chain tracing. (Tier 2 — via STS_User_Operations_Data_History) |
| 13 | AccountTypeName | varchar(100) | YES | STS_User_Operations_Data_History.AccountTypeName | Account context for the session: `Real` (live trading) or `Demo` (virtual portfolio). (Tier 2 — via STS_User_Operations_Data_History) |
| 14 | LoginTypeName | varchar(100) | YES | STS_User_Operations_Data_History.LoginTypeName | Type of authentication event. Known values: `Login` (new session), `Authenticate` (credential re-validation), `TokenExchange` (token refresh), `Logout` (session end). (Tier 2 — via STS_User_Operations_Data_History) |
| 15 | SessionId | bigint | YES | STS_User_Operations_Data_History.SessionId | Unique session identifier assigned by the STS service. Monotonically increasing. (Tier 2 — via STS_User_Operations_Data_History) |
| 16 | GatewayAppId | int | YES | STS_User_Operations_Data_History.GatewayAppId | Identifier of the API gateway application that routed the request. Commonly `1` or `2`. NULL for some Logout events. (Tier 2 — via STS_User_Operations_Data_History) |
| 17 | DateID | int | YES | STS_User_Operations_Data_History.DateID | Date partition key in YYYYMMDD integer format (e.g. `20210901`). Computed in ETL from the `@Yesterday` parameter: `CONVERT(INT, CONVERT(VARCHAR, DATEADD(DAY, DATEDIFF(DAY, 0, @Yesterday), 0), 112))`. Clustered index and partition column. (Tier 2 — via STS_User_Operations_Data_History) |
| 18 | UpdateDate | datetime | YES | STS_User_Operations_Data_History.UpdateDate | Timestamp when this row was loaded into the DWH, set to `GETDATE()` during ETL execution. Not the business event time. (Tier 2 — via STS_User_Operations_Data_History) |
| 19 | ProxyType | nvarchar(max) | YES | STS_User_Operations_Data_History.ProxyType | Type of proxy detected for the client IP connection (e.g. VPN, TOR). Sparsely populated — NULL in most observed rows. Added after initial table creation. (Tier 3 — via STS_User_Operations_Data_History) |
| 20 | CountryISOCode | nvarchar(max) | YES | STS_User_Operations_Data_History.CountryISOCode | ISO country code resolved from the ClientIp address. Sparsely populated — NULL in most observed rows. Added after initial table creation. (Tier 3 — via STS_User_Operations_Data_History) |
| 21 | AdditionalData | nvarchar(max) | YES | STS_User_Operations_Data_History.AdditionalData | Extensible JSON or free-text field for additional session metadata. Sparsely populated. (Tier 3 — via STS_User_Operations_Data_History) |
