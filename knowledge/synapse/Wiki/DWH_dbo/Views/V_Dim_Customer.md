...
| 3 | DemoCID | int | Dim_Customer.DemoCID | Demo account CID associated with this customer. From `UserApiDB_Customer_CustomerIdentification`. (Tier 2 — via Dim_Customer) |
...
| 14 | SubChannelID | int | Dim_Customer.SubChannelID | Sub-channel ID. Populated post-load from SubChannel unify code via AffiliateID mapping. DEFAULT=0. (Tier 2 — via Dim_Customer) |
...
| 22 | AccountExpirationDate | varchar(50) | Dim_Customer.AccountExpirationDate | Expiration date for demo or time-limited accounts. NULL for standard real-money accounts (Tier 1 - Dim_Customer.md) |
| 23 | SocialConnectID | int | Dim_Customer.SocialConnectID | Social media connection type. DEFAULT=0. (Tier 2 — via Dim_Customer) |
...
| 25 | DocsOK | varchar(10) | Dim_Customer.DocsOK | Whether required documents are verified. (Tier 2 — via Dim_Customer) |
...
| 27 | Bankruptcy | varchar(10) | Dim_Customer.Bankruptcy | Bankruptcy flag. (Tier 2 — via Dim_Customer) |
...
| 31 | RegisteredDemo | varchar(50) | Dim_Customer.RegisteredDemo | Demo account registration date. Source unclear — may be populated separately. (Tier 2 — via Dim_Customer) |
...
| 38 | PremiumAccount | varchar(10) | Dim_Customer.PremiumAccount | Whether this is a premium account. (Tier 2 — via Dim_Customer) |
| 39 | Evangelist | varchar(10) | Dim_Customer.Evangelist | Whether this customer is an evangelist/ambassador. (Tier 2 — via Dim_Customer) |
...
| 41 | NumOfGurus | int | Dim_Customer.NumOfGurus | Number of Popular Investors this customer is copying. (Tier 2 — via Dim_Customer) |
| 42 | NumOfCopiers | int | Dim_Customer.NumOfCopiers | Number of customers copying this customer's trades. (Tier 2 — via Dim_Customer) |
| 43 | NumOfRAF | int | Dim_Customer.NumOfRAF | Number of successful Refer-A-Friend referrals. (Tier 2 — via Dim_Customer) |
...
| 49 | HasAvatar | varchar(10) | Dim_Customer.HasAvatar | Whether customer has uploaded a custom avatar. Updated post-load from Avatars staging (excludes default/avatoros images). (Tier 2 — via Dim_Customer) |
| 50 | AvatarUploadDate | varchar(50) | Dim_Customer.AvatarUploadDate | When the avatar was uploaded. (Tier 2 — via Dim_Customer) |
| 51 | UpdateDate | varchar(50) | Dim_Customer.UpdateDate | ETL load/update timestamp (GETDATE()). (Tier 2 — via Dim_Customer) |
| 52 | IsDepositor | varchar(1) | Dim_Customer.IsDepositor | Whether the customer has ever deposited. DEFAULT=0. Updated post-load from FTD data. (Tier 2 — via Dim_Customer) |
| 53 | FirstDepositDate | varchar(50) | Dim_Customer.FirstDepositDate | Date of first deposit. DEFAULT='19000101'. Updated from CustomerFinanceDB FTD data with FTDRecoveryDate logic. (Tier 2 — via Dim_Customer) |
...
| 67 | FirstDepositAmount | decimal(19,4) | Dim_Customer.FirstDepositAmount | Amount of first deposit (in USD). Updated from FTDAmountInUsd. (Tier 2 — via Dim_Customer) |
...
| 69 | MifidCategorizationID | int | Dim_Customer.MifidCategorizationID | MiFID II investor classification. FK to Dictionary.MifidCategorization. 1=Retail (97.3%), 4=Eligible Counterparty (2.6%), 5=Professional (0.03%). Default=1 (Tier 1 - Dim_Customer.md) |
...
| 71 | IsValidCustomer | int | Dim_Customer.IsValidCustomer | DWH-computed: 1 when not Popular Investor (PlayerLevelID≠4), not label 30/26, and not CountryID=250. Used in reporting to filter out non-standard customers. (Tier 2 — via Dim_Customer) |
...
| 74 | ScreeningStatusID | int | Dim_Customer.ScreeningStatusID | Compliance screening status. Updated from ScreeningService. (Tier 2 — via Dim_Customer) |
...
| 76 | WorldCheckResultsUpdated | varchar(50) | Dim_Customer.WorldCheckResultsUpdated | When World-Check results were last updated. Preserved from previous row. (Tier 2 — via Dim_Customer) |
...
| 78 | IsAddressProof | int | Dim_Customer.IsAddressProof | Whether address proof document is on file (1/0). Updated from BackOffice.CustomerDocument. (Tier 2 — via Dim_Customer) |
| 79 | IsAddressProofExpiryDate | varchar(50) | Dim_Customer.IsAddressProofExpiryDate | Expiry date of address proof document. (Tier 2 — via Dim_Customer) |
| 80 | IsIDProof | int | Dim_Customer.IsIDProof | Whether ID proof document is on file (1/0). (Tier 2 — via Dim_Customer) |
| 81 | IsIDProofExpiryDate | varchar(50) | Dim_Customer.IsIDProofExpiryDate | Expiry date of ID proof document. (Tier 2 — via Dim_Customer) |
...
| 85 | IsCreditReportValidCB | int | Dim_Customer.IsCreditReportValidCB | DWH-computed: similar to IsValidCustomer but with additional AccountTypeID≠2 exclusion and specific CID exceptions for CountryID=250. (Tier 2 — via Dim_Customer) |
| 86 | CashoutFeeGroupID | int | Dim_Customer.CashoutFeeGroupID | Determines which withdrawal fee schedule applies to this customer. FK to Dictionary.CashoutFeeGroup. 1 = Default (standard fees), 2 = Exempt (zero fees), 3 = Discount (reduced fees). Default at registration is 1. |
