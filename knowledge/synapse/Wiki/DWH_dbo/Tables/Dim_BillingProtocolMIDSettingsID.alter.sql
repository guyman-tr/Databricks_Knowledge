...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_dwh_dbo_dim_billingprotocolmidsettingsid ALTER COLUMN Value COMMENT 'The protocol identifier string (MID, merchant ID, API key, etc.) passed to the payment processor for routing. SENSITIVE -- contains payment gateway credentials. Examples: merchant ID numbers, API endpoint identifiers. (Tier 1 - upstream wiki, Billing.ProtocolMIDSettings)';
...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_dwh_dbo_dim_billingprotocolmidsettingsid ALTER COLUMN MerchantAccountID COMMENT 'Optional link to a specific merchant account configuration in Billing.MerchantAccountValues. When set (~25% of rows), enables finer-grained routing to a specific acquiring account within a depot. NULL when not applicable. (Tier 1 - upstream wiki, Billing.ProtocolMIDSettings)';
