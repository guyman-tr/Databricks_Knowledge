...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_fact_customeraction ALTER COLUMN PositionID COMMENT 'Position identifier from the source trading system. NOT a primary key of this table - defaults to 0 for non-position events, and the same PositionID appears in both open and close rows.';
...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_fact_customeraction ALTER COLUMN MoveMoneyReasonID COMMENT 'NULL in all archive branches; natively populated only in History.ActiveCredit. Do not join from this view. (Tier 1 - History.Credit.md)';
