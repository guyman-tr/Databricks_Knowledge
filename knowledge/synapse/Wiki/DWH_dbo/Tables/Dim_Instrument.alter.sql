...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_instrument ALTER COLUMN Name COMMENT 'Computed: TDCUR_BUY.Abbreviation + ''/'' + TDCUR_SEL.Abbreviation. Display name for UI (e.g., EUR/USD, AAPL/USD). (Tier 1 - Trade.GetInstrument)';
...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_instrument ALTER COLUMN BuyCurrencyID COMMENT 'FK to Dictionary.Currency. Buy-side asset. For forex: base currency; for stocks: asset itself (BuyCurrencyID=InstrumentID). Inherited from Trade.Instrument. (Tier 1 - Trade.GetInstrument)';
...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_instrument ALTER COLUMN CUSIP COMMENT 'Alias for InstrumentMetaData.Cusip. Committee on Uniform Securities Identification Procedures code for US/Canada securities. NULL for forex, crypto, many non-US instruments. (Tier 1 - Trade.InstrumentCusip)';
...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_instrument ALTER COLUMN SettlementTime COMMENT 'Time of day for settlement. DWH note: reformatted from Trade.FuturesMetaData.SettlementTime via FORMAT(DATEPART(HOUR)*100 + DATEPART(MINUTE), ''00:00''). (Tier 1 - Trade.FuturesMetaData)';
