...
| 4 | CountryCode | varchar(50) | YES | ISO 3166-1 alpha-2 country code from IP2Location (e.g., "AU", "JP"). Source column: `ISNULL(country_code, 'NA')` - NULL guard prevents Namibia rows from being silently dropped. Used to resolve CountryID via JOIN to Dim_Country.Abbreviation. (Tier 2 - SP_Dictionaries_Country_DL_To_Synapse) |
