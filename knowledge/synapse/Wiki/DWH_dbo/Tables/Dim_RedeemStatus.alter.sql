...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_redeemstatus ALTER COLUMN DisplayName COMMENT 'User-facing display label. More readable than the internal Name. Shown in copy-trading UI and notifications (Tier 1 - Dictionary.RedeemStatus)';
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_redeemstatus ALTER COLUMN IsCancelable COMMENT 'Whether the user can cancel the redeem request at this stage. 1=cancelable, 0=committed. Per upstream wiki, only Pending (ID 1) is cancelable; InProcess, Completed, Failed, and CompletedPartially are all IsCancelable=0. The False list must include upstream IDs 2, 3, 5, 6 at minimum, not just IDs 6, 7, 8, 20.';
...
-- == LAST EXECUTION ==
-- Timestamp: 2026-03-30 11:26:21 UTC
