...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_playerstatus ALTER COLUMN CanDeposit COMMENT 'Whether the user can add funds to their account. False for full-block statuses (IsBlocked=1), close-only/pending statuses (9, 13, 15), status 10 (Deposit Blocked), and status 11 (Social Index).';
...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_playerstatus ALTER COLUMN CanLogin COMMENT 'Whether the user can authenticate and access the platform. When false, login attempts are rejected at the gate. Checked by History.LogIn and History.LogInIB procedures (Tier 1 - Dictionary.PlayerStatus)';
...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_playerstatus ALTER COLUMN DWHPlayerStatusID COMMENT 'DWH surrogate key -- always equals PlayerStatusID (redundant copy). Set by ETL: SELECT [PlayerStatusID] AS [DWHPlayerStatusID]. 0 for the ID=0 sentinel.';
