...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_affiliate_masked ALTER COLUMN ContractType COMMENT 'Computed affiliate payment model: 0=N/A, 2=CPA, 3=Rev, 4=Hyb, 6=eCost, 7=ZeroCost, 8=CPL. Derived from ContractName via CASE expression. (Tier 2 - SP_Dim_Affiliate)';
...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_affiliate_masked ALTER COLUMN FTDeFirstDate COMMENT 'Date of the affiliate''s first referred FTDe (Valid first-time deposit). FTDe uses the same deposit source as FTD (CreditTypeID=1, IsFirstDeposit=1) but adds a Valid=1 filter. Sourced from Ext_Dim_Affiliate_FTDe.';
...
ALTER TABLE main.pii_data.gold_sql_dp_prod_we_dwh_dbo_dim_affiliate ALTER COLUMN ContractType COMMENT 'Computed affiliate payment model: 0=N/A, 2=CPA, 3=Rev, 4=Hyb, 6=eCost, 7=ZeroCost, 8=CPL. Derived from ContractName via CASE expression. (Tier 2 - SP_Dim_Affiliate)';
...
ALTER TABLE main.pii_data.gold_sql_dp_prod_we_dwh_dbo_dim_affiliate ALTER COLUMN FTDeFirstDate COMMENT 'Date of the affiliate''s first referred FTDe (Valid first-time deposit). FTDe uses the same deposit source as FTD (CreditTypeID=1, IsFirstDeposit=1) but adds a Valid=1 filter. Sourced from Ext_Dim_Affiliate_FTDe.';
...
-- == LAST EXECUTION ==
-- Timestamp: 2026-03-30 11:38:15 UTC
-- Batch deploy resume: DWH_dbo deploy batch 1
-- Statements: 228/228 succeeded
-- ====================
