...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_fact_customerunrealized_pnl ALTER COLUMN Notional COMMENT 'Total absolute USD exposure across all positions. Computed as SUM(ABS(per-position signed USD exposure)) grouped by CID - ABS is applied per position, not per instrument. Always >= 0.';
