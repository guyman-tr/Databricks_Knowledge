...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_fact_billingredeem ALTER COLUMN ModificationDateID COMMENT 'Integer date key in YYYYMMDD format derived from LastModificationDate via `CONVERT(INT, CONVERT(VARCHAR, DATEADD(DAY, DATEDIFF(DAY, 0, LastModificationDate), 0), 112))`. The ETL rolling 7-day delete window operates on this column.';
