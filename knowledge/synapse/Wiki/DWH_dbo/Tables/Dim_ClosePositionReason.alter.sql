...
-- ---- Column Comments ----
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_closepositionreason ALTER COLUMN Tier 1 COMMENT 'Verbatim from upstream production wiki';
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_closepositionreason ALTER COLUMN Tier 2 COMMENT 'Confirmed from Synapse ETL SP code';
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_closepositionreason ALTER COLUMN ClosePositionReasonID COMMENT 'Primary key. DWH rename of production `ID`. Values 0-26. 0=Customer (manual), 1=Stop Loss, 2=End of Week, 3=SL via trade server, 4=Return to Market, 5=Take Profit, 6=TP via trade server, 7=Contract Rollover, 8=BackOffice, 9=Hierarchical Close, 10=Recovery, 11=Join Demo Challenge, 12=Close All, 13=Copy Stop Loss, 14=Mirror manual, 15=Manual Liquidation, 16=BSL, 17=Manual Unregister, 18=BO Unregister, 19=Redeem, 20=Operational adjustment, 21=Orphaned, 22=Transferred Out, 23=Alignment, 24=Delist, 25=Close by rate, 26=Expiry. Stored permanently with every closed position. (Tier 1 - upstream wiki, Dictionary.ClosePositionActionType)';
...
-- ---- Column PII Tags ----
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_closepositionreason ALTER COLUMN Tier 1 SET TAGS ('pii' = 'none');
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_closepositionreason ALTER COLUMN Tier 2 SET TAGS ('pii' = 'none');
