...
ALTER TABLE main.bi_db.gold_sql_dp_prod_we_dwh_dbo_fact_cashout_state ALTER COLUMN ModificationDateID COMMENT 'Integer date in YYYYMMDD format derived from `CONVERT(INT, ModificationDate)`. ETL key for the daily DELETE+INSERT pattern. Use for date-range queries and partitioning. (Tier 2 - SP_Fact_Cashout_State)';
