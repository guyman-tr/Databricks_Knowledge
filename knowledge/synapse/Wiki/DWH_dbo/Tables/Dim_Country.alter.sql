...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_country ALTER COLUMN IsHighRiskCountry COMMENT 'AML/compliance risk flag. 0=standard risk, 1=high risk. Recomputed by SP from RiskGroupID: CASE WHEN RiskGroupID IN (0,4) THEN 0 ELSE 1 END - countries with RiskGroupID 1, 2, or 3 are flagged high-risk (14 countries per upstream Dictionary.Country). Triggers enhanced due diligence.';
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_country ALTER COLUMN Region COMMENT 'Marketing region label for this country. Loaded from Dictionary.MarketingRegion.Name via JOIN on MarketingRegionID. NOT the geographic region from Dictionary.Region. Up to 21 distinct values (e.g., "ROW", "Africa", "French", "Arabic"). Used for marketing campaign grouping.';
