...
-- ---- Column Comments ----
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_movemoneyreason ALTER COLUMN MoveMoneyReasonID COMMENT 'Internal money movement reason identifier. DWH values: 1=Adjustment, 2=Bonus Abuser, 3=Staking, 4=Airdrop (DWH-only label). Production has additional IDs 5=InternalTransfer Trade [UNVERIFIED: codepoint not in Dim_MoveMoneyReason], 6=InternalTransfer [UNVERIFIED: codepoint not in Dim_MoveMoneyReason], 7=Not In Use [UNVERIFIED: codepoint not in Dim_MoveMoneyReason], 8=Recurring Deposit [UNVERIFIED: codepoint not in Dim_MoveMoneyReason], 9=Recurring Investment [UNVERIFIED: codepoint not in Dim_MoveMoneyReason] - all absent from DWH. ID=5 is critical: used in SP_Fact_CustomerAction to derive ActionTypeID 44 (internal deposit) and 45 (internal withdrawal). (Tier 1 - upstream wiki, Dictionary.MoveMoneyReason)';
