...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_cashoutstatus ALTER COLUMN Name COMMENT 'Human-readable status label (e.g. Pending, InProcess, Processed, Canceled, Rejected, etc.). All 17 production values are loaded from staging; the DWH is not limited to 5 values. UNIQUE constraint exists at production level but is not enforced in the DWH DDL.';
