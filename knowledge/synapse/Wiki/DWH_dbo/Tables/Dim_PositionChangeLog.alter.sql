...
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_positionchangelog ALTER COLUMN PreviousIsSettled COMMENT 'Settlement status before this change (0=not settled, 1=settled). Cast from bit staging column. NULL if this event didn''t involve settlement change. (Tier 2 - SP_Dim_PositionChangeLog_DL_To_Synapse)';
ALTER TABLE main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_positionchangelog ALTER COLUMN IsSettled COMMENT 'Settlement status after this change. Cast from bit staging column. Used by Dim_Position ETL to backfill IsSettled corrections. NULL if no settlement change. (Tier 2 - SP_Dim_PositionChangeLog_DL_To_Synapse)';
