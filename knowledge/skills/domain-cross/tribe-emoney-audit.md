---
id: tribe-emoney-audit
name: "Tribe / FiatDwh -> eMoney Audit Trail"
...
**Side classification:** Compliance / regulator-facing audit trail. This cross-domain skill does NOT own AML risk classification or AML alert interpretation — that's [`../domain-compliance-and-aml/`](../domain-compliance-and-aml/SKILL.md). The Treezor envelope INTERPRETATION rules (what does `RiskActionType=7` actually mean for SOC2) are still doc-debt; the production source is Compliance Eng + the Treezor partnership documentation.
...
- **AML risk classification + alert routing on the customer behind the envelope** → [`../domain-compliance-and-aml/`](../domain-compliance-and-aml/SKILL.md). **KYC sanctions/PEP screening DECISION** → B `compliance-customer-snapshot-and-club` (planned v1.5). **FCA SAR / MAS regulator submission** → future spec 013-regulatory-reporting.
...
Out of scope: eMoney customer transaction volumes / FX / IBAN inflows (`emoney-accounts-and-cards`); AML risk classification + AML alerts on the customer ([`../domain-compliance-and-aml/`](../domain-compliance-and-aml/SKILL.md)); KYC sanctions/PEP identity-side screening (B compliance-customer-snapshot-and-club planned v1.5); regulator submission output (future spec 013); crypto-to-fiat forensics (`crypto-to-fiat`); fiat provider settlement recon for TP deposits (`provider-reconciliation`); single-dispute chargeback chain (`refund-chargeback-chain`).
...
10. **Tier 2 — Compliance owns the interpretation, not Payments.** This skill provides the join keys and the "which table" map. For the AML side of the interpretation (does the envelope correspond to a fired AML alert? what was the risk classification at the time?) load [`../domain-compliance-and-aml/`](../domain-compliance-and-aml/SKILL.md). For the Treezor-specific SOC2 lookup (what does `RiskActionType=7` actually mean) the production source is still Compliance Eng + the Treezor partnership doc — no replacement yet.
11. **Tier 3 — Two halves of "Tribe / FiatDwh" get conflated in conversation.** **Tribe** (`eMoney_Tribe.*` / `emoney.bronze_fiatdwhdb_tribe_*`) is the raw XML audit log — forensic input only, not analytical fact. **FiatDwhDB** (`FiatDwhDB.dbo.*` / `emoney.bronze_fiatdwhdb_dbo_*` + a few in `bi_db.bronze_fiatdwhdb_*`) is Treezor's operational fiat tables — the provider-side source of truth for reconciliation. An AML/Compliance audit usually needs BOTH halves: FiatDwhDB tells you what Treezor's books say, Tribe XML tells you why and who. For the linked customer's AML risk classification + alert state at the time of the envelope, cross-reference into [`../domain-compliance-and-aml/`](../domain-compliance-and-aml/SKILL.md) (`aml-risk-scoring` + `aml-alert-routing`).
...
| AML risk classification + alert routing on the customer | [`../domain-compliance-and-aml/`](../domain-compliance-and-aml/SKILL.md) |
| KYC sanctions/PEP identity-side screening DECISION | B `compliance-customer-snapshot-and-club` (planned v1.5) |
| FCA SAR / MAS regulator submission output | future spec 013-regulatory-reporting |
| Crypto came in → converted to fiat on IBAN forensics | `crypto-to-fiat` |
...
- Intersecting skills: `domain-payments/emoney-accounts-and-cards` (supplies join keys via `eMoney_Dim_Account` / `eMoney_Dim_Transaction`), [`../domain-compliance-and-aml/`](../domain-compliance-and-aml/SKILL.md) (owns AML risk classification + alert interpretation for the customer behind the envelope), `domain-cross/crypto-to-fiat`, `domain-cross/provider-reconciliation`, `domain-cross/refund-chargeback-chain`.
