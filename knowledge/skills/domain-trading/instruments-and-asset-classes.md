...
| **IsSQF** | 1 = Special Qualifying Flag (Trade.InstrumentGroups GroupID = 59). Enriched view only. **8 instruments total** (4 indices + 4 crypto, all also `IsFuture=1`). UK regulatory designation. | Rare. UK regulatory analyses. |
...
| `IsSQF` | `1 if InstrumentID ∈ (SELECT InstrumentID FROM Trade.InstrumentGroups WHERE GroupID = 59) else 0`. 8 flagged (4 indices + 4 crypto, all futures). UK Special Qualifying instruments. |
...
This sub-skill (v2) was rebuilt 2026-05-11 per `/speckit.skill` Phase 2.5 (classify-then-reach content sourcing). It deeply incorporates the DE workspace-root skill `instruments` (`/Workspace/.assistant/skills/instruments/SKILL.md`, v2 from 2026-05-07) and adds seven knowledge sources surfaced during the rebuild — the Dim_Instrument wiki + lineage + review-needed files, the BI_DB `Function_Instrument_Snapshot_Enriched` wiki, the OLTP `Trade.Instrument` wiki, live UC schema for both the base dim and the enriched view, and 7 live UC distinct-value verifications. **Key v2 additions:** the `Tradable` vs `Tradeable` distinction (Warning #3, 3,005-row delta), UC comment health caveats on the enriched view (Warning #4), the `IsTicketFeePercentInstrument` content gap (Warning #5), `Symbol` non-uniqueness vs `SymbolFull` (Warning #7), `DollarRatio` (Warning #12), `IsMajor`/`IsMajorID` and corrected `Is_245_Instrument` semantics, the Broker-Dealer framing (`ProviderID` as broker→dealer bridge, Pattern 5), live catalogue refresh (+602 instruments since wiki snapshot), and extra Ethereum-adjacent traps (`ETC` / `ENS` / `ETHFI` / `ETHA`-cross-pair vs `ETHA.US` ETF).
