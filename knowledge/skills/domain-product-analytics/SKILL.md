...
| Customer master attributes (regulation, country, club tier) for per-CID rollup of Mixpanel/ABtoro data | `domain-customer-and-identity/SKILL.md` + `cross-cutting/valid-users-filter-contract.md` |
...
- **Federation references.** `domain-customer-and-identity/customer-models-and-segmentation.md` (consumes Mixpanel-derived cluster features); `cross-cutting/valid-users-filter-contract.md` (per-CID rollup contract).
