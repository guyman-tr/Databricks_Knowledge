---
id: aml-alert-routing
name: "AML Alert Routing"
...
required_tables:
  ...
  - main.pii_data_stg.aml_snapshotcustomer_enriched_v
external_references:
...
8. **Tier 2 — `pii_data.aml_snapshotcustomer_enriched_v` (54c) joins the periodic-review back to customer state.** PII-enriched view; analyst entry point for "show me customer X's AML state across classification + alerts + periodic-review-due". Both `pii_data` (prod) and `pii_data_stg` (staging) variants exist — use `pii_data.*` for canonical reads.
