SELECT fsc.RealCID
	  ...
	  ,atnm.Restriction_Status_Reason
          ,atnm.Restriction_Status_Subreason
...
(
	SELECT curr.GCID, 
		   drs.Name 'Restriction_Status', 
		   curr.BeginTime,
		   curr.EndTime,
		   curr.RestrictionStatusReasonID,
		   rsr.Name 'Restriction_Status_Reason',
		   RSS.Name AS 'Restriction_Status_Subreason'
	FROM BI_DB_dbo.External_ComplianceStateDB_Compliance_UserTradingData curr
	INNER JOIN [BI_DB_dbo].[External_ComplianceStateDB_Dictionary_RestrictionStatus] drs ON drs.RestrictionStatusID = curr.CFDRestrictionStatusID
	LEFT JOIN [BI_DB_dbo].[External_ComplianceStateDB_Dictionary_RestrictionStatusReason] rsr ON curr.RestrictionStatusReasonID = rsr.RestrictionStatusReasonID
	LEFT JOIN [BI_DB_dbo].[External_ComplianceStateDB_Compliance_CustomerRestrictions] CR	ON curr.GCID=CR.GCID
																				AND CR.RestrictionStatusReasonID=12
	LEFT JOIN [BI_DB_dbo].[External_ComplianceStateDB_Dictionary_RestrictionStatusSubreason] RSS on RSS.RestrictionStatusSubreasonID=CR.RestrictionStatusSubreasonID
	WHERE curr.RestrictionStatusReasonID=12
	...
	SELECT CR.GCID,
			...
			'N/A' AS 'Restriction_Status_Subreason'
	FROM [BI_DB_dbo].External_ComplianceStateDB_Compliance_CustomerRestrictions CR
