...
sum(dp.Amount+dp.NetProfit)'Value',
sum(dp.Amount+dp.NetProfit)/sum(dp.AmountInUnitsDecimal) AS 'Price Per Unit',
sum(dp.AmountInUnitsDecimal) AS 'Units to be transfered',
sum(FLOOR(dp.AmountInUnitsDecimal)*dp.EndForexRate/dp.LastOpConversionRate) AS 'Value of units to be transfered'
