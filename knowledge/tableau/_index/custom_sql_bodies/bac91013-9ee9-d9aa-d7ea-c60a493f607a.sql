...
select f.FromDate
      ...
      ,case when e.CardID is NULL then 'No'
            when e.CardID is not NULL and e.CardCreateDate >= f.period_start_date then 'Yes'
            else 'No' end as Has_eMoney_Card
      ,e.CardStatus
from final f
left join lateral 
(select user_id
...
LEFT JOIN 
  main.bi_db.gold_sql_dp_prod_we_emoney_dbo_emoney_dim_account e on e.CID = dc.RealCID
where  
