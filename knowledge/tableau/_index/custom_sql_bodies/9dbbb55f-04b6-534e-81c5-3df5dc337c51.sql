SELECT
	...
	, bdcbcln.Regulation
	, CASE WHEN bdcbcln.TransferDirection = 1 THEN Regulation ELSE bdcbcln.ToRegulation END AS ToRegulation
	, bdcbcln.DidCBValidTransfer
	, bdcbcln.TransferDirection
	, bdcbcln.DidRegulationTransfer
        , COUNT(CID) OVER (PARTITION BY CID, bdcbcln.DateID, bdcbcln.Regulation ORDER BY DateID) AS CountRows
...
CAST(CONVERT(VARCHAR(10), CAST(<[Parameters].[Parameter 4]> as DATE), 112) AS INT)
AND
CAST(CONVERT(VARCHAR(10), CAST(<[Parameters].[Parameter 5]> as DATE), 112) AS INT)
