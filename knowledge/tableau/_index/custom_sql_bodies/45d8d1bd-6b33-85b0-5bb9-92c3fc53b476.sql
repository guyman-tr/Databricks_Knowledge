...
Left Join
	 (select distinct t.GCID, t.[End of Week]
	 , CASE 
	WHEN 	DATEDIFF(Month,LAG (t.[End of Week],1,Null)  OVER ( partition by t.GCID ORDER BY t.[End of Week]), t.[End of Week])>=6
	then '1' ELSE '0' end 'Reactivated'
		 from  (select distinct DATEADD(DAY, 7 - DATEPART(WEEKDAY, b.TranDate), b.TranDate )'End of Week'
		 , b.GCID from EXW_dbo.EXW_FactTransactions b where b.SenderAddress<>'0x5be786ad38f5846f605a8003550074cdfd4899a1') t 
	 where t.GCID>0 )ta 
	 on  ta.GCID = fb.GCID and ta.[End of Week] =fb.BalanceDate
 Where 1=1
 AND fb.ComplianceClosureEvent=0 
 AND fb.AMLClosureEvent =0
--and dd.FullDate >=  '2019-12-31' 
--and dd.FullDate <=  '2020-01-31'
			
and dd.FullDate >=  <[Parameters].[Parameter 4]>
and dd.FullDate <=  <[Parameters].[Parameter 5]>
AND  dd.DayNumberOfWeek_Sun_Start=7 --only saturdays
and fb.GCID>0
