...
select TransferDirection
	 ...
	 , DateID
	 , OpeningBalance 
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') then ISNULL(OpeningBalance,0) - ISNULL(RealStocksOpeningBalance,0) ELSE ISNULL(OpeningBalance,0) END AS OpeningBalanceAdjusted
	 , Deposits
	 , CompensationDeposit
	 , Bonus
	 , Compensation
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') then ISNULL(Compensation,0) - ISNULL(CompensationsApexUSStocks,0) ELSE ISNULL(Compensation,0) END AS CompensationAdjusted
	 , CompensationPI
	 ...
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') then ISNULL(ClientBalanceRealizedPnL,0) - ISNULL(ClientBalanceRealizedPnLRealStocks,0) ELSE ISNULL(ClientBalanceRealizedPnL,0) END AS ClientBalanceRealizedPnLAdjusted
	 , ClientBalanceRealizedPnLCFD
	 , ClientBalanceRealizedPnLRealStocks
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') then 0  ELSE ISNULL(ClientBalanceRealizedPnLRealStocks,0) END AS ClientBalanceRealizedPnLRealStocksAdjusted
	 , ClientBalanceRealizedPnLRealCrypto
	 , TransferCoins
	 , TransferCoinFees
	 , ClosingBalance
	 ,  CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') then ISNULL(ClosingBalance,0) - ISNULL(RealStocksClosingBalance,0) ELSE ISNULL(ClosingBalance,0) END AS ClosingBalanceAdjusted
	 , realizedEquity
	 ...
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE RealStocksOpeningBalance END AS RealStocksOpeningBalanceAdjusted
	 , RealStocksClosingBalance
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE RealStocksClosingBalance END AS RealStocksClosingBalanceAdjusted
	 , ClientBalanceFullCommission
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') AND Regulation <> FromRegulation then ISNULL(ClientBalanceFullCommission,0) - ISNULL(ClientBalanceFullCommissionRealStocks,0) ELSE ISNULL(ClientBalanceFullCommission,0) END AS ClientBalanceFullCommissionAdjusted
	 , ClientBalanceCommission
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') AND Regulation <> FromRegulation then ISNULL(ClientBalanceCommission,0) - ISNULL(ClientBalanceCommissionRealStocks,0) ELSE ISNULL(ClientBalanceCommission,0) END AS ClientBalanceCommissionAdjusted
	 ...
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE ClientBalanceFullCommissionRealStocks end  AS ClientBalanceFullCommissionRealStocksAdjusted
	 , ClientBalanceCommissionRealStocks
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE ClientBalanceCommissionRealStocks END AS ClientBalanceCommissionRealStocksAdjusted
	 , DividendsPaid
	 , SDRT
	 , TicketFee
	 , TicketFeeByPercent
	 ...
	 , NOPCryptoCFD
	 , NOPStocks
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE NOPStocks END AS NOPStocksAdjusted
	 , NOPStocksCFD
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE NOPStocksCFD END AS NOPStocksCFDAdjusted
	 , TotalRealCryptoLoan
	 , TotalRealCrypto
	 , TotalRealStocks
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE TotalRealStocks END AS TotalRealStocksAdjusted
	 , PositionPNLCryptoReal
	 , PositionPNLStocksReal
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE PositionPNLStocksReal END AS PositionPNLStocksRealAdjusted
	 , PositionPNL
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN ISNULL(PositionPNL,0) - ISNULL(PositionPNLStocksReal,0) ELSE ISNULL(PositionPNL,0) END AS PositionPNLAdjusted
	 , AvailableCash
	 , CashInCopy
	 , NOP
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN ISNULL(NOP,0) - ISNULL(NOPStocks,0) ELSE ISNULL(NOP,0) END AS NOPAdjusted
	 , PositionAmount
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN ISNULL(PositionAmount,0) - ISNULL(TotalStocksPositionAmount,0) ELSE ISNULL(PositionAmount,0) END AS PositionAmountAdjusted
	 , StockOrders
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE StockOrders END AS StockOrdersAdjusted
	 , actualNWA
	 , UsedBonus
	 , UnrealizedCommissionChange
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN ISNULL(UnrealizedCommissionChange,0) - ISNULL(UnrealizedCommissionChangeRealStocks,0) ELSE ISNULL(UnrealizedCommissionChange,0) END AS UnrealizedCommissionChangeAdjusted
	 , UnrealizedFullCommissionChange
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN ISNULL(UnrealizedFullCommissionChange,0) - ISNULL(UnrealizedFullCommissionChangeRealStocks,0) ELSE ISNULL(UnrealizedFullCommissionChange,0) END AS UnrealizedFullCommissionChangeAdjusted
	 , UnrealizedPnLChange
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN ISNULL(UnrealizedPnLChange,0) - ISNULL(UnrealizedPnLChangeStocksReal,0) ELSE ISNULL(UnrealizedPnLChange,0) END AS UnrealizedPnLChangeAdjusted
	 , UnrealizedPnLChangeCFD
	 , UnrealizedPnLChangeCryptoReal
	 , UnrealizedPnLChangeStocksReal
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE UnrealizedPnLChangeStocksReal END AS UnrealizedPnLChangeStocksRealAdjusted
	 , UnrealizedFullCommissionChangeRealStocks
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE UnrealizedFullCommissionChangeRealStocks END AS UnrealizedFullCommissionChangeRealStocksAdjusted
	 , TotalNetTransfers
	 ...
	 , PositionPnLCrypto
	 , PositionPnLStocks 
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE PositionPnLStocks END AS PositionPnLStocksAdjusted
	 , TotalCryptoPositionAmount
	 , TotalStocksPositionAmount
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE TotalStocksPositionAmount END AS TotalStocksPositionAmountAdjusted
	 , IsGermanBaFin
	 , IsValidCustomer
	 , Date
	 , YearMonth
     ...
     , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE UnrealizedCommissionChangeRealStocks END AS UnrealizedCommissionChangeRealStocksAdjusted
     , TotalRealStocksEquityChange
     , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE TotalRealStocksEquityChange END AS TotalRealStocksEquityChangeAdjusted
	 ...
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFSFINRA') THEN 0 ELSE TotalRealStocks + PositionPNLStocksReal end as EquityRealStocksAdjusted
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN -1 * (ISNULL(TotalRealStocksEquityChange,0) - ISNULL(UnrealizedPnLChangeStocksReal,0) - ISNULL(ClientBalanceRealizedPnLRealStocks,0)) ELSE 0 END as RealStockInvestedAmountChangeAdjusted
	 , CASE WHEN Regulation in ( 'FinCEN+FINRA','NYDFS+FINRA') THEN 0 ELSE [RealStocksClosingBalance] END as RealStocksAdjusted
	 , CashoutRollback
	 , ReverseDeposit
	 , DepositConversionFee
	 , WithdrawConversionFee
