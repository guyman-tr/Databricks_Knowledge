...
case when (coalesce(nd.net_Deposits,0) + coalesce(cc.Credit_30_04_24,0 ) >=10000
          ...
          or ( pp.GCID in  (
                               ...
                               from bi_db.bronze_fivetran_google_sheets_manually_approved_tactical_edge
                                where TO_DATE(TO_TIMESTAMP(date, 'dd/MM/yyyy') ,'MM/dd/yyyy') <= cast(<[Parameters].[Parameter 1]> as date)
                            )
                 and  pp.Net_MI>=10000
             )
...
from 
(
  select cast(mrr.ModificationDate as date ) date 
  ,mrr.MirrorID 
  ,mrr.ParentCID
  ,mrr.CID
  ,dc.GCID
  ...
  group by mrr.MirrorID, cast(mrr.ModificationDate as date ), mrr.ParentCID, mrr.CID, dc.GCID
...
left join 
 (
  ...
  and a.etr_ymd = cast(<[Parameters].[Parameter 1]> as date) 
  and b.etr_ymd = date_add(day,1,<[Parameters].[Parameter 1]>)
  and cast(a.UpdateDate as date )= cast(b.UpdateDate as date) 
  JOIN main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_customer_masked dc 
  on a.CID = dc.RealCID and dc.IsValidCustomer = 1
  where b.ParentCID in (37890645)and b.CloseDateID = 0 and b.OpenDateID >= 20240101
  group by a.etr_ymd
  ,a.CID
  , a.MirrorID
  , b.Amount
...
,ec.IsEligible
,g.Name,ec.Agreed_Typeform,net_Deposits,Net_MI,Credit_30_04_24