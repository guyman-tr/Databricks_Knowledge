select hh.*, COALESCE(SUM(CASE WHEN fca1.ActionTypeID IN (7) THEN fca1.Amount END),0) as Deposit
,COALESCE(SUM(CASE WHEN fca1.ActionTypeID IN (8) THEN (-1*fca1.Amount) END),0) AS CashOut
from (
select vl.CID
    ,dc.GCID
    ,vl.Credit  Credit_201024     
...
,dc1.MarketingRegionManualName Region
    ...
    and  TO_DATE(TO_TIMESTAMP(a.Message, 'M/d/yyyy h:mm:ss a') ,'MM/dd/yyyy')<='2024-11-30'
     left join  ( select distinct CID 
      FROM main.bi_output.bi_output_customer_investment_capital_guarantee_capital_guarantee_q42024_global mm) hh
    on hh.CID=dc.RealCID
