...
FROM BI_DB_dbo.Function_Revenue_TicketFeeByPercent(
    CAST(FORMAT(DATEADD(DAY, 1, EOMONTH(GETDATE(), -2)), 'yyyyMMdd') AS INT),
    CAST(FORMAT(EOMONTH(GETDATE(), -1), 'yyyyMMdd') AS INT),
    0
) AS frtfbp
