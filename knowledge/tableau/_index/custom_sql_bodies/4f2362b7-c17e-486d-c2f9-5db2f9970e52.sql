...
tp_mimo as (
	SELECT 
		...
		WHERE fca.ActionTypeID in (7,8) -- 7 - Deposit, 8 - withdrawal
		    ...
		    AND fca.DateID BETWEEN  date_format(add_months(date_trunc('year', current_date()), -48), 'yyyyMMdd')
...
apex_ftda_base AS (
	...
	HAVING CAST(MIN(ProcessDate) AS DATE) BETWEEN add_months(date_trunc('year', current_date()), -48)
           AND current_date()
),
...
future_per_account AS (
    ...
    WHERE 
		...
		AND p.ProcessDate BETWEEN add_months(date_trunc('year', current_date()), -48) AND
              current_date()
...
depositors_agg AS (
  ...
  FROM apex_ftda_prep
  WHERE ProcessDate BETWEEN add_months(date_trunc('year', current_date()), -48) AND
              current_date()
...
ops_with AS (
	...
	WHERE OfficeCode in ('4GS','5GU') 
		...
		AND ProcessDate BETWEEN add_months(date_trunc('year', current_date()), -48) AND current_date()
	GROUP BY last_day(ProcessDate)
)
...
(tm.TP_WithdrawalsSum + coalesce(ow.OptionsWithdrawalsSum,0)) / (tm.TP_TotalDepositsSum + coalesce(ad.ApexDepositsSum,0)) AS `Cashout Ratio %`
FROM tp_mimo tm	 
