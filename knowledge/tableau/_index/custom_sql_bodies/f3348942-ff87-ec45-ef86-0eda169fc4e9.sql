...
ncs.CaseSkills__c,
        ncs.Club_Level_on_Creation__c AS ClubTier,
        ncs.Status AS TicketStatus,
        ncs.Type__c AS ActionType,
        ncs.Type AS Type,
        ncs.Sub_Type__c AS SubType,
        ncs.Sub_Type_2__c AS SubType2,
        ncs.Priority AS Priority,
        ncs.Product__c AS Product,
        ncs.SLA_Score__c AS SLAScore,
        ncs.Number_of_touches__c AS NumberOfTouches,
        ncs.Regulation__c AS RegulationAtOpen,
        CONCAT(us.FirstName, ' ', us.LastName) AS FullName,
        us.Department AS Department,
        ncs.Owner_role_name__c AS UserRole,
        us.SubRole AS SubRole,
        csat.cSATLast AS LastCsat,
        us.Team AS Team,
        ncs.Owner_role_name__c AS TicketRole,
        ncs.Owner_Sub_Role__c AS TicketSubRole,
hw.HasWallet,
...
left join (select RealCID,HasWallet from main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_customer_masked ) hw on hw.RealCID=ncs.CID__c
