SELECT 
    case  
        when CaseSkills like '%Verification%' 
            then 'Verification' 
        when CaseSkills like '%Screening%'
            then 'Screening'
        end as Type
...
where
     ...
     (CaseSkills like '%Verification%' or CaseSkills like '%Screening%')
...
group by
    ...
    ,case when c.Status in ('In Routing') then 'Backlog' else 'Resolved' end
    ,dc.VerificationLevelID
    ,case 
	    when Year(dc.FirstDepositDate) = 1900 then 'No'
	    else 'Yes'
