...
from  DWH.[dbo].[Fact_BillingRedeem] red
join DWH.dbo.Dim_Customer dc on dc.RealCID=red.CID
JOIN DWH.dbo.Dim_Country country ON country.CountryID=dc.CountryID
JOin DWH.dbo.Dim_Regulation dr on dr.ID=dc.RegulationID
JOin DWH.dbo.Dim_Regulation dr1 on dr1.ID=dc.DesignatedRegulationID
where red.RedeemStatusID =8 --TransactionDone
and CAST(red.RequestDate AS DATE)>='20210101'