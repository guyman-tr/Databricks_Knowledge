...
UNION ALL 
SELECT 'Sell' AS Action, fca.DateID, fca.IsSettled, case when IsSettled = 1 then 'Real' else 'CFD' end as Real_CFD, di.InstrumentType,  di.IsFuture, dc.Name AS Country, dr1.Name AS Regulation, sum(fca.Amount ) AS AmountUSD
FROM DWH_dbo.Fact_CustomerAction fca
	JOIN DWH_dbo.Fact_SnapshotCustomer fsc
		ON fca.RealCID = fsc.RealCID
	JOIN DWH_dbo.Dim_Range dr
