...
tstatus.[Status] AS [TicketStatus],
...
left JOIN [BI_DB].[dbo].[BI_DB_SF_Users] us WITH (NOLOCK) ON ncs.CaseOwner = us.FullName
LEFT JOIN BI_DB.dbo.BI_DB_SF_STG_M_Case tstatus ON tstatus.CaseNumber=ncs.CaseNumber

WHERE
ncs.CreatedDate>= DATEADD(MONTH, -4, DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0))	
) A
WHERE RN=1