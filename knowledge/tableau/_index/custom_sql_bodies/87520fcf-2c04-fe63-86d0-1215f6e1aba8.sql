...
,        count(distinct case when cc.VerificationLevelID=3 AND cc.EvMatchStatus=2 then cc.RealCID end) as NoLevel3,ps.Name as PlayerStatus
...
LEFT JOIN main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_playerstatus ps on ps.PlayerStatusID =cc.PlayerStatusID
...
where 
cast(TransactionDate as date)>='2025-01-01'
...
CAST (cr.TransactionDate AS date),
 dr.Name,cast(cc.RegisteredReal as Date) ,ps.Name
Order by
        dc.Name asc,
      CAST (cr.TransactionDate AS date)