...
aum_raw AS (
  ...
  WHERE (Product IS NOT NULL AND trim(Product) <> '')
     OR (Portfolio_Model_Id IS NOT NULL AND trim(cast(Portfolio_Model_Id as string)) <> '')   -- ADDED (so model id-only rows can be used)
),
...
/* Resolve Portfolio_Model_Id from Silver: latest non-blank per GCID × PortfolioID_SILVER */
aum_model_latest_nonblank AS (   -- ADDED
  SELECT
    GCID,
    PortfolioID_SILVER,
    MAX(etr_ymd) AS latest_model_ymd
  FROM aum_raw
...
aum_model_latest_pick AS (       -- ADDED
  SELECT
    r.GCID,
    r.PortfolioID_SILVER,
    r.Portfolio_Model_Id_SILVER_NORM AS Portfolio_Model_Id_SILVER_LATEST_NONNULL
  FROM aum_raw r
  JOIN aum_model_latest_nonblank m
    ON  r.GCID = m.GCID
    AND r.PortfolioID_SILVER = m.PortfolioID_SILVER
    AND r.etr_ymd = m.latest_model_ymd
),
...
user_first_deposit_with_product AS (
  ...
  LEFT JOIN aum_model_latest_pick mlp                                          -- ADDED
    ON mlp.GCID               = pdu.GCID
   AND mlp.PortfolioID_SILVER = pdu.PortfolioID
),

-- Final aggregation with your existing joins
Final AS (
  SELECT
    dc.GCID,
    ...
    CASE
      ...
      -- ADDED: Cash ISA V2 split (specific rule first)
      WHEN u.ProductID = 'isa-cash'
       AND lower(coalesce(u.Portfolio_Model_Id_Resolved, '')) = 'etoro-gb-r0-c1-cash-saving-account-v2'
        THEN 'Cash ISA - V2'

      WHEN u.ProductID = 'isa-cash'             THEN 'Cash ISA'
...
SELECT
  ...
  f.Product_label,
  f.Event_FTD_Date,
  SUM(f.GBP_FTDA)        AS GBP_FTDA,
  SUM(f.USD_FTDA)        AS USD_FTDA,
  COUNT(DISTINCT f.GCID) AS FTD_Count
FROM Final f
GROUP BY
  f.FTD_date,
  f.FTD_Platform,
