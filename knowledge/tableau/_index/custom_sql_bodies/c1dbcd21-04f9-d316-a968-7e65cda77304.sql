select max_risk_scenario
,case when max_risk_scenario>=50E6 and max_risk_scenario<=200E6 then 1 else 0 end as ind_alert
from
(
select max(risk_scenario) as max_risk_scenario 
from
(

select scenario, scenario_market, sum(risk) risk_scenario
...
group by scenario, scenario_market
)alert_final
)alert_final1