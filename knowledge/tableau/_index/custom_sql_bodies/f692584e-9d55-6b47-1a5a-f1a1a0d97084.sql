select Percentile, group_name,sum (NOP_UNHedged_exposure) as NOP_UNHedged_exposure
from
(
select base.*
...
from
(
select InstrumentID, Percentile,coalesce (current_NOP,0)-coalesce(NOP_Hedged,0 ) NOP_UnHedged
 from risk.risk_output_rm_tables_var_instruments_hs_25
)base
)final
group by Percentile, group_name