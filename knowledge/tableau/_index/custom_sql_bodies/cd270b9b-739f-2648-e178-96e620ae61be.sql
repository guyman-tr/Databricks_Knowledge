SELECT efrbn.GCID
	...
	--  LEFT JOIN  EXW_dbo.EXW_DimUser_Enriched edue ON efrbn.GCID = edue.GCID
	 --- LEFT JOIN EXW_dbo.EXW_WalletRegulation  wr ON efrbn.GCID = wr.GCID 
--AND BalanceDate  >=  <[Parameters].[Parameter 4]>
--AND BalanceDate  <=  <[Parameters].[Parameter 5]>
 AND BalanceDateID >= CAST(FORMAT(CAST(<[Parameters].[Parameter 4]> AS DATE),'yyyyMMdd') as INT)
 AND BalanceDateID <= CAST(FORMAT(CAST(<[Parameters].[Parameter 5]> AS DATE),'yyyyMMdd') as INT)
