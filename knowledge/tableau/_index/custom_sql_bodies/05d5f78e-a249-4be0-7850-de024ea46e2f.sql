...
SELECT Report_time_history as report_time, cvar as cvar99, Percentile
, year(Report_time_history)*1e8 + month(Report_time_history)*1e6 + day(Report_time_history)*1e4 + hour(Report_time_history)*1e2 + minute(Report_time_history) as report_time_id
FROM risk.risk_output_rm_tables_history_var_cvar 
where 
 cvar<100E6
...
select base1.*
,case when dif_difussion_lp>=2 or dif_diffusion_now>=6 or dif_lp_now>=6 then 0 else 1 end  as ind_continue
from
(
select base.*
