SELECT a.TransferDirection
	 ...
	 , a.UnrealizedFullCommissionChangeRealCrypto
	 , a.TotalNetTransfers
     ...
     , a.UnrealizedCommissionChangeRealCrypto
	 ...
	 , WithdrawConversionFee
        , IsDLTUser
		, DidDLTTransfer
	,TotalRealCryptoEquityChange
	, NetTransfersUnrealizedPnLCryptoReal
	, NetTransfersLiabilityCryptoReal
-- INTO #temp
FROM 
(
select TransferDirection
	 ...
	 , UnrealizedFullCommissionChangeRealCrypto
	 ...
	 , UnrealizedCommissionChangeRealCrypto
	 ...
	 , ISNULL(PositionAmount,0) + ISNULL(PositionPNL,0) - ISNULL(TotalRealStocks,0) + ISNULL(PositionPNLStocksReal,0) - ISNULL(TotalRealCrypto,0) + ISNULL(PositionPNLCryptoReal,0) AS EquityCFD
	...
	, NetTransfersLiabilityCryptoReal
        , IsDLTUser
		, DidDLTTransfer
from BI_DB_dbo.BI_DB_Client_Balance_Aggregate_Level_New_DLTSimulation_With_Transfer  with (nolock)
