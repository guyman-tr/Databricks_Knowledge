...
, etoro_state_regulation as (
    ...
    JOIN main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_regulation r 
            ON r.ID = fsc.RegulationID AND r.ID IN (7,8) 
...
, inc_reg12_states as (
    SELECT 
        RealCID
    FROM etoro_state_regulation
    where eToroFunnel_StateShortName in ('NY', 'NV', 'HI', 'PR', 'VI')
    group by RealCID 
) 
...
, cid_state_regulation as (
    ...
    join inc_reg12_states ir on ir.RealCID=sr.RealCID
)
...
SELECT c.RealCID,  
    ...
    p.PlayerStatus
FROM pop p
left join cid_state_regulation c
    on c.RealCID=p.RealCID
join pop_firsts pf 
    on c.RealCID=pf.RealCID
