SELECT
    ...
    mda.Club,
    CASE 
        WHEN LEFT(mda.BankAccountIBAN, 2) = 'GB' THEN 'UK'
        ELSE 'EU'
    END AS IBAN_Region,
  ...
  SUM(CASE WHEN fca.ActionTypeID = 44 AND fca.Occurred >= DATEADD(DAY, -30, CAST(GETDATE() AS DATE)) THEN ISNULL(f.PIPsCalculation, 0) ELSE 0 END) AS Open_Fees_Last_30_Days,
  SUM(CASE WHEN fca.ActionTypeID = 45 AND fca.Occurred >= DATEADD(DAY, -30, CAST(GETDATE() AS DATE)) THEN ISNULL(wi.PIPsCalculation, 0) ELSE 0 END) AS Close_Fees_Last_30_Days,
  SUM(CASE WHEN fca.ActionTypeID = 44 AND fca.Occurred >= DATEADD(DAY, -365, CAST(GETDATE() AS DATE)) THEN ISNULL(f.PIPsCalculation, 0) ELSE 0 END) AS Open_Fees_Last_365_Days,
  SUM(CASE WHEN fca.ActionTypeID = 45 AND fca.Occurred >= DATEADD(DAY, -365, CAST(GETDATE() AS DATE)) THEN ISNULL(wi.PIPsCalculation, 0) ELSE 0 END) AS Close_Fees_Last_365_Days,
