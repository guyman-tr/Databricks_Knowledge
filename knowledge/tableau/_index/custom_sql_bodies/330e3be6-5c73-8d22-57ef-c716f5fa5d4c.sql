...
WHERE CampaignNumber IN ('6439','5729','7384') AND bdsr.SendDateID>=20220202 AND Delivered<>0 ) cd

LEFT JOIN ETL_FiatAccount efa ---accounts that didnt created account
ON cd.CampaignNumber='5729' AND efa.Date>=cd.SendDateID AND cd.GCID=efa.Gcid
...
WHERE cs.DateID>='2020202') efc WHERE efc.RN=1 AND efc.Name='Activated'
) card_activation---accounts that didnt activated
ON cd.CampaignNumber='6439' AND card_activation.EventTimeStamp>=cd.SendDateID
                                                                  AND cd.GCID=card_activation.GCID

LEFT JOIN (
SELECT DISTINCT mft.GCID,mft.DateID
FROM eMoney_Fact_Transactions mft
WHERE mft.DateID>='2020202' AND mft.TransactionType IN ('CardPayment', 'Contactless','OnlinePayment','CashWithdrawal')
)AS did_trnsaction
ON cd.CampaignNumber='7384' AND CONVERT(date, convert(varchar(10), did_trnsaction.DateID))
                                 >=cd.SendDateID
                                                                  AND cd.GCID=did_trnsaction.GCID
