SELECT DISTINCT
   ...
   ,CASE WHEN du.IsTestAccount = 1 THEN 'TestUser'
		 ...
		 ELSE 'RealUser'
	END AS UserType
	,esm.reason  AS SimplexReason
	,esm.stage_drop  AS SimplexStageDrop
	,sp.ShortName AS StateCode
	,du.UserRegion_State State
    ,du.Regulation
	,esm.bin_country
	,esm.bank_name
	
   FROM [EXW_dbo].[EXW_PaymentReconciliation] re
LEFT JOIN 	EXW_dbo.EXW_SimplexMapping esm		ON esm.long_id = re.ProviderPaymentID
LEFT JOIN EXW_dbo.EXW_FactTransactions ft		ON re.SentTransactionID = ft.TranID and ActionTypeID = 1
	--left join DWH_dbo.Dim_Customer dc     on re.RealCID = dc.RealCID
LEFT JOIN EXW_dbo.EXW_InternalWallet iw		ON ft.SenderAddress = iw.Address
	 JOIN EXW_dbo.EXW_DimUser du		ON re.GCID = du.GCID
...
--	join DWH_dbo.Dim_Regulation dr  	    on dc.RegulationID = dr.DWHRegulationID
LEFT JOIN DWH_dbo.[Dim_State_and_Province] sp 		ON du.UserRegionID = sp.RegionByIP_ID
LEFT JOIN EXW_dbo.EXW_SimplexChargebacks esc	ON re.ProviderPaymentID = esc.Payment_ID
