WITH base_triggers AS (
    -- 1. Gather all 'Monthly active open' Triggers (Rolling 12 Months)
    SELECT 
        t.Id AS TriggerId,
        t.Account__c,
    ...
    WHERE t.name = 'Monthly active open'
...
SELECT 
    ...
    -- [Success Metric] Did they open a position within 30 days?
    CASE
        WHEN EXISTS (
            SELECT 1
            ...
            WHERE np.CID = t.CID
              ...
              AND np.PositionOpenDate <= DATEADD(day, 30, t.Trigger_Created_Date__c) -- 30 DAY WINDOW
              AND np.Amount > 0
        ) THEN 1 
        ELSE 0 
    END AS IsSuccessful,

    -- [Contextual Data] How many positions were opened in those 30 days?
    COALESCE((
        SELECT COUNT(1)
        FROM new_positions np
        WHERE np.CID = t.CID
          AND np.PositionOpenDate > t.Trigger_Created_Date__c
          AND np.PositionOpenDate <= DATEADD(day, 30, t.Trigger_Created_Date__c)
    ), 0) AS Count_New_Positions_30Days,
    ...
    COALESCE((
        ...
        WHERE np.CID = t.CID
          AND np.PositionOpenDate > t.Trigger_Created_Date__c
          AND np.PositionOpenDate <= DATEADD(day, 30, t.Trigger_Created_Date__c)
    ), 0) AS TotalAmountInvested_30Days
