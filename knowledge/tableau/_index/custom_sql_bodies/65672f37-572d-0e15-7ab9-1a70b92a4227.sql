...
FROM (
    ...
    WHERE dc.RegulationID IN (6,7,8,12)
      AND dc.IsDepositor = 1
      AND dc.FirstDepositDate >= DATEADD(MONTH, -13, CAST(GETDATE() AS date))
    ...
    FROM DWH_dbo.Dim_Customer dc
    WHERE dc.RegulationID IN (6,7,8,12)
      AND dc.RegisteredReal >= DATEADD(MONTH, -13, CAST(GETDATE() AS date))
) t
GROUP BY eomonth(t.[Date])