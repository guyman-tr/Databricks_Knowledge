...
dm.ParentUserName,
cc.Name CopyType_Desc,
case when (a.CopyType in (1,4) and dm.MirrorID is not null) or (a.OrderID is not null and  dp.OrderID is not null and a.CopyType not in (1,4) and dp.CloseDateID=0 ) then 0 else 1  end as IsPositionClosed_Ind,
cast(dp.CloseOccurred as date)CloseOccurred ,
dp.CloseDateID
...
left join main.experience.bronze_recurringinvestment_dictionary_copytype  cc on cc.ID=a.CopyType 
left join  main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_mirror dm on /*dm.ParentCID=vd.CopyParentCID and*/ a.MirrorID=dm.MirrorID and dm.CloseDateID=0
left join main.dwh.dim_position dp on dp.OrderID=a.OrderID --and dp.CloseDateID=0
