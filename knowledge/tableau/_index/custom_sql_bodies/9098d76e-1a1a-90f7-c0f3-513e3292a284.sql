SELECT
    ...
    , COUNT(CASE
        WHEN dc.RegisteredReal >= DATEADD(DAY, 1, EOMONTH(GETDATE(), -7))
         AND dc.RegisteredReal <  DATEADD(DAY, 1, EOMONTH(GETDATE(), -1))
        THEN dc.RealCID
      END) AS Registered
    , COUNT(CASE
        WHEN dc.FirstDepositDate >= DATEADD(DAY, 1, EOMONTH(GETDATE(), -7))
...
WHERE dc.RegisteredReal >= DATEADD(DAY, 1, EOMONTH(GETDATE(), -7))
  AND dc.RegisteredReal <  DATEADD(DAY, 1, EOMONTH(GETDATE(), -1))
