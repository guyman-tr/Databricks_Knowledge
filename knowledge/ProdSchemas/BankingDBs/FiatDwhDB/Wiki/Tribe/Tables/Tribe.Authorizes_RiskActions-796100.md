...
> Child table storing risk action details from Tribe authorization records.
...
Stores risk action records triggered during card authorization processing from Tribe. Parent: Tribe.Authorizes-837045. Same pattern as AccountsActivities_RiskActions but for authorization events.
...
```
Tribe.Authorizes_RiskActions-796100 (table)
...
```sql
SELECT TOP 10 * FROM Tribe.[Authorizes_RiskActions-796100] WITH (NOLOCK) ORDER BY Created DESC;
```

### 8.2 Join with parent
```sql
SELECT p.[@FileName], c.* FROM Tribe.[Authorizes-837045] p WITH (NOLOCK)
JOIN Tribe.[Authorizes_RiskActions-796100] c WITH (NOLOCK) ON c.[@Authorizes@Id-837045] = p.[@Id] ORDER BY c.Created DESC;
```

### 8.3 Count
```sql
SELECT COUNT(*) FROM Tribe.[Authorizes_RiskActions-796100] WITH (NOLOCK);
```
...
*Object: Tribe.Authorizes_RiskActions-796100 | Type: Table | Source: FiatDwhDB/Tribe/Tables/Tribe.Authorizes_RiskActions-796100.sql*
