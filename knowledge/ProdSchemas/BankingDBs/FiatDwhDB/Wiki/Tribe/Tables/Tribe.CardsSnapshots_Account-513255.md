...
> Child table storing individual account details from Tribe card snapshot files (singular account per card snapshot).
...
Stores individual account records from Tribe card snapshots. Parent: Tribe.CardsSnapshots-890718. Contains raw account details associated with each card in the snapshot.
...
```
Tribe.CardsSnapshots_Account-513255 (table)
...
```sql
SELECT TOP 10 * FROM Tribe.[CardsSnapshots_Account-513255] WITH (NOLOCK) ORDER BY Created DESC;
```

### 8.2 Join with parent
```sql
SELECT TOP 5 p.[@FileName], c.* FROM Tribe.[CardsSnapshots-890718] p WITH (NOLOCK)
JOIN Tribe.[CardsSnapshots_Account-513255] c WITH (NOLOCK) ON c.[@CardsSnapshots@Id-890718] = p.[@Id] ORDER BY c.Created DESC;
```

### 8.3 Count
```sql
SELECT COUNT(*) FROM Tribe.[CardsSnapshots_Account-513255] WITH (NOLOCK);
```
...
*Object: Tribe.CardsSnapshots_Account-513255 | Type: Table | Source: FiatDwhDB/Tribe/Tables/Tribe.CardsSnapshots_Account-513255.sql*
