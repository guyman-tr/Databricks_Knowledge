# Tribe.Cards_Account-965632

> Child table storing account details associated with cards from Tribe card data files (singular form - individual account per card record).
...
Cards_Account-965632 stores account detail records associated with cards from Tribe. Each row links a card to its account attributes (account ID, holder, status, program). This is the singular form. Parent: Tribe.Cards-432613.
...
```
Tribe.Cards_Account-965632 (table)
...
```sql
SELECT TOP 10 * FROM Tribe.[Cards_Account-965632] WITH (NOLOCK) ORDER BY Created DESC;
```

### 8.2 Join with parent
```sql
SELECT TOP 5 p.[@FileName], c.* FROM Tribe.[Cards-432613] p WITH (NOLOCK)
JOIN Tribe.[Cards_Account-965632] c WITH (NOLOCK) ON c.[@Cards@Id-432613] = p.[@Id] ORDER BY c.Created DESC;
```

### 8.3 Count
```sql
SELECT COUNT(*) FROM Tribe.[Cards_Account-965632] WITH (NOLOCK);
```
...
*Object: Tribe.Cards_Account-965632 | Type: Table | Source: FiatDwhDB/Tribe/Tables/Tribe.Cards_Account-965632.sql*
