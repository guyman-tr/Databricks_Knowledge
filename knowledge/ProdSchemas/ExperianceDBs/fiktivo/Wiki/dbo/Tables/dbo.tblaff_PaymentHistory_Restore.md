# dbo.tblaff_PaymentHistory_Restore

> Developer backup/snapshot of tblaff_PaymentHistory, created as a restore point for payment data recovery. No indexes, no FKs, no triggers - exists purely as a point-in-time data safety copy.
...
dbo.tblaff_PaymentHistory_Restore is a developer backup copy of dbo.tblaff_PaymentHistory, created as a restore point for payment data recovery. The "_Restore" suffix indicates this table was retained as a safety net to allow the payment data to be recovered to a known good state if a subsequent operation caused data loss or corruption. These tables should be reviewed for potential cleanup.
...
Developer backup - data represents a restore point snapshot of the payment history data.
...
```sql
SELECT COUNT(*) AS RowCount FROM dbo.tblaff_PaymentHistory_Restore WITH (NOLOCK)
```
...
UNION ALL
SELECT 'Backup', COUNT(*) FROM dbo.tblaff_PaymentHistory_Restore WITH (NOLOCK)
```

### 8.3 Sample backup data
```sql
SELECT TOP 5 * FROM dbo.tblaff_PaymentHistory_Restore WITH (NOLOCK)
```
...
*Object: dbo.tblaff_PaymentHistory_Restore | Type: Table (backup) | Source: fiktivo/dbo/Tables/dbo.tblaff_PaymentHistory_Restore.sql*
