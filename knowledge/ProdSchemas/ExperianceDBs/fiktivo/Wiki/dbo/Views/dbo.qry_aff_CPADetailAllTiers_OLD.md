# dbo.qry_aff_CPADetailAllTiers_OLD

> Pivoted all-tiers detail view for CPA events, joining the base event table with all 5 tier-filter views to show complete multi-tier commission breakdown per event in a single row.
...
dbo.qry_aff_CPADetailAllTiers_OLD is the comprehensive detail view for CPA commissions, pivoting all 5 tier levels into a single row per event. It LEFT OUTER JOINs the base event table (tblaff_CPA) with each of the 5 tier-filter views (qry_aff_Tier1-5CPACommissions) and tblaff_Country for the country name. This produces a denormalized row showing the event details alongside each tier's affiliate, commission amount, paid status, and sub-affiliate tag.

Used by the affiliate admin interface for detailed commission reports where users need to see all tier breakdowns for each event on a single line. This is a legacy version retained for backward compatibility. See dbo.qry_aff_CPADetailAllTiers for the current implementation.
...
**Columns/Parameters Involved**: DepositID, Tier1-5 AffiliateID/Commission/Paid/SubAffiliateID
...
|---|---------|------|----------|---------|------------|-------------|
| 1 | DepositID | int | NO | - | VERIFIED | Event ID from base table. |
...
| 6 | AffiliateDepositAccepted | bit/int | YES | - | VERIFIED | Whether the affiliate accepted this event for commission. |
...
|---------|---------------|-------------------|-------------|
| (event cols) | dbo.tblaff_CPA | Base table | Event details |
| (tier 1 cols) | dbo.qry_aff_Tier1CPACommissions | LEFT OUTER JOIN | Tier 1 commission data |
| (tier 2 cols) | dbo.qry_aff_Tier2CPACommissions | LEFT OUTER JOIN | Tier 2 |
| (tier 3 cols) | dbo.qry_aff_Tier3CPACommissions | LEFT OUTER JOIN | Tier 3 |
| (tier 4 cols) | dbo.qry_aff_Tier4CPACommissions | LEFT OUTER JOIN | Tier 4 |
| (tier 5 cols) | dbo.qry_aff_Tier5CPACommissions | LEFT OUTER JOIN | Tier 5 |
...
```
dbo.qry_aff_CPADetailAllTiers_OLD (view)
  +-- dbo.tblaff_CPA (table)
  +-- dbo.qry_aff_Tier1CPACommissions (view)
  +-- dbo.qry_aff_Tier2CPACommissions (view)
  +-- dbo.qry_aff_Tier3CPACommissions (view)
  +-- dbo.qry_aff_Tier4CPACommissions (view)
  +-- dbo.qry_aff_Tier5CPACommissions (view)
...
|--------|------|----------|
| dbo.tblaff_CPA | Table | Base event data |
| dbo.qry_aff_Tier1-5CPACommissions | Views | Tier commission data (LEFT OUTER JOIN) |
...
```sql
SELECT * FROM dbo.qry_aff_CPADetailAllTiers_OLD WITH (NOLOCK)
WHERE DepositID = @EventID
```

### 8.2 Unpaid events with tier 1 commission
```sql
SELECT DepositID, Tier1AffiliateID, Tier1Commission, ORDER_DATE
FROM dbo.qry_aff_CPADetailAllTiers_OLD WITH (NOLOCK)
...
```sql
SELECT TOP 10 DepositID, ORDER_DATE,
...
*Object: dbo.qry_aff_CPADetailAllTiers_OLD | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_CPADetailAllTiers_OLD.sql*
