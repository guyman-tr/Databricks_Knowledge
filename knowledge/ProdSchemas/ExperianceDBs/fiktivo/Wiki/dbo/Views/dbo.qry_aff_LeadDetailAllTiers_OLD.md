# dbo.qry_aff_LeadDetailAllTiers_OLD

> Pivoted all-tiers detail view for Lead events, joining the base event table with all 5 tier-filter views to show complete multi-tier commission breakdown per event in a single row.
...
dbo.qry_aff_LeadDetailAllTiers_OLD is the comprehensive detail view for Lead commissions, pivoting all 5 tier levels into a single row per event. It LEFT OUTER JOINs the base event table (tblaff_Leads) with each of the 5 tier-filter views (qry_aff_Tier1-5LeadCommissions) and tblaff_Country for the country name. This produces a denormalized row showing the event details alongside each tier's affiliate, commission amount, paid status, and sub-affiliate tag.

Used by the affiliate admin interface for detailed commission reports where users need to see all tier breakdowns for each event on a single line. This is a legacy version retained for backward compatibility. See dbo.qry_aff_LeadDetailAllTiers for the current implementation.
...
**Columns/Parameters Involved**: LeadID, Tier1-5 AffiliateID/Commission/Paid/SubAffiliateID
...
|---|---------|------|----------|---------|------------|-------------|
| 1 | LeadID | int | NO | - | VERIFIED | Event ID from base table. |
...
|---------|---------------|-------------------|-------------|
| (event cols) | dbo.tblaff_Leads | Base table | Event details |
| (tier 1 cols) | dbo.qry_aff_Tier1LeadCommissions | LEFT OUTER JOIN | Tier 1 commission data |
| (tier 2 cols) | dbo.qry_aff_Tier2LeadCommissions | LEFT OUTER JOIN | Tier 2 |
| (tier 3 cols) | dbo.qry_aff_Tier3LeadCommissions | LEFT OUTER JOIN | Tier 3 |
| (tier 4 cols) | dbo.qry_aff_Tier4LeadCommissions | LEFT OUTER JOIN | Tier 4 |
| (tier 5 cols) | dbo.qry_aff_Tier5LeadCommissions | LEFT OUTER JOIN | Tier 5 |
...
```
dbo.qry_aff_LeadDetailAllTiers_OLD (view)
  +-- dbo.tblaff_Leads (table)
  +-- dbo.qry_aff_Tier1LeadCommissions (view)
  +-- dbo.qry_aff_Tier2LeadCommissions (view)
  +-- dbo.qry_aff_Tier3LeadCommissions (view)
  +-- dbo.qry_aff_Tier4LeadCommissions (view)
  +-- dbo.qry_aff_Tier5LeadCommissions (view)
...
|--------|------|----------|
| dbo.tblaff_Leads | Table | Base event data |
| dbo.qry_aff_Tier1-5LeadCommissions | Views | Tier commission data (LEFT OUTER JOIN) |
...
```sql
SELECT * FROM dbo.qry_aff_LeadDetailAllTiers_OLD WITH (NOLOCK)
WHERE LeadID = @EventID
```

### 8.2 Unpaid events with tier 1 commission
```sql
SELECT LeadID, Tier1AffiliateID, Tier1Commission, ORDER_DATE
FROM dbo.qry_aff_LeadDetailAllTiers_OLD WITH (NOLOCK)
...
```sql
SELECT TOP 10 LeadID, ORDER_DATE,
...
*Object: dbo.qry_aff_LeadDetailAllTiers_OLD | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_LeadDetailAllTiers_OLD.sql*
