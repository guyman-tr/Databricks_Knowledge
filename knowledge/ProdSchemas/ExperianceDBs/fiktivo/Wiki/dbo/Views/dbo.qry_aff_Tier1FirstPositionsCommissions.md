# dbo.qry_aff_Tier1FirstPositionsCommissions

> Tier 1 filter view over tblaff_FirstPositions_Commissions, extracting firstpositions commissions for tier 1 (direct/referring) affiliates with aliased column names for cross-tier joining.
...
Filtered subset of tblaff_FirstPositions_Commissions where Tier = 1.
...
|---------|---------------|-------------------|-------------|
| (all) | dbo.tblaff_FirstPositions_Commissions | Base table | Filtered by Tier = 1 |

### 5.2 Referenced By (other objects point to this)

| Source Object | Source Element | Relationship Type | Description |
|--------------|---------------|-------------------|-------------|
| dbo.qry_aff_FirstPositionsDetailAllTiers | LEFT OUTER JOIN | View | All-tiers pivot joins this for tier 1 data |
...
|--------|------|----------|
| dbo.tblaff_FirstPositions_Commissions | Table | Base table (WHERE Tier = 1) |

### 6.2 Objects That Depend On This

| Object | Type | How Used |
|--------|------|----------|
| dbo.qry_aff_FirstPositionsDetailAllTiers | View | LEFT OUTER JOIN for tier 1 pivot |
...
### 8.1 Tier 1 firstpositions commissions for an affiliate
```sql
SELECT FirstPositionID, Tier1Commission, Tier1Paid
FROM dbo.qry_aff_Tier1FirstPositionsCommissions WITH (NOLOCK)
WHERE Tier1AffiliateID = @AffiliateID
```

### 8.2 Total unpaid tier 1 firstpositions commissions
```sql
...
```sql
SELECT COUNT(*) AS TierRecords FROM dbo.qry_aff_Tier1FirstPositionsCommissions WITH (NOLOCK)
```
...
*Object: dbo.qry_aff_Tier1FirstPositionsCommissions | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_Tier1FirstPositionsCommissions.sql*
