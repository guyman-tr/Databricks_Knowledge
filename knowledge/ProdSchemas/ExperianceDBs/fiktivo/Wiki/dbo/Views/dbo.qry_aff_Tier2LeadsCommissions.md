# dbo.qry_aff_Tier2LeadsCommissions

> Tier 2 filter view over tblaff_Leads_Commissions, extracting leads commissions for tier 2 (parent of direct) affiliates with aliased column names for cross-tier joining.
...
Filtered subset of tblaff_Leads_Commissions where Tier = 2.
...
|---------|---------------|-------------------|-------------|
| (all) | dbo.tblaff_Leads_Commissions | Base table | Filtered by Tier = 2 |

### 5.2 Referenced By (other objects point to this)

| Source Object | Source Element | Relationship Type | Description |
|--------------|---------------|-------------------|-------------|
| dbo.qry_aff_LeadDetailAllTiers | LEFT OUTER JOIN | View | All-tiers pivot joins this for tier 2 data |
...
```
dbo.qry_aff_Tier2LeadsCommissions (view)
...
|--------|------|----------|
| dbo.tblaff_Leads_Commissions | Table | Base table (WHERE Tier = 2) |

### 6.2 Objects That Depend On This

| Object | Type | How Used |
|--------|------|----------|
| dbo.qry_aff_LeadDetailAllTiers | View | LEFT OUTER JOIN for tier 2 pivot |
...
### 8.1 Tier 2 leads commissions for an affiliate
```sql
SELECT LeadID, Tier2Commission, Tier2Paid
FROM dbo.qry_aff_Tier2LeadsCommissions WITH (NOLOCK)
WHERE Tier2AffiliateID = @AffiliateID
```

### 8.2 Total unpaid tier 2 leads commissions
```sql
...
```sql
SELECT COUNT(*) AS TierRecords FROM dbo.qry_aff_Tier2LeadsCommissions WITH (NOLOCK)
```
...
*Object: dbo.qry_aff_Tier2LeadsCommissions | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_Tier2LeadsCommissions.sql*
