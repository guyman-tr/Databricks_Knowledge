# dbo.qry_aff_Tier4LeadsCommissions

> Tier 4 filter view over tblaff_Leads_Commissions, extracting leads commissions for tier 4 (great-grandparent) affiliates with aliased column names for cross-tier joining.
...
Filtered subset of tblaff_Leads_Commissions where Tier = 4.
...
|---------|---------------|-------------------|-------------|
| (all) | dbo.tblaff_Leads_Commissions | Base table | Filtered by Tier = 4 |

### 5.2 Referenced By (other objects point to this)

| Source Object | Source Element | Relationship Type | Description |
|--------------|---------------|-------------------|-------------|
| dbo.qry_aff_LeadDetailAllTiers | LEFT OUTER JOIN | View | All-tiers pivot joins this for tier 4 data |
...
```
dbo.qry_aff_Tier4LeadsCommissions (view)
...
|--------|------|----------|
| dbo.tblaff_Leads_Commissions | Table | Base table (WHERE Tier = 4) |

### 6.2 Objects That Depend On This

| Object | Type | How Used |
|--------|------|----------|
| dbo.qry_aff_LeadDetailAllTiers | View | LEFT OUTER JOIN for tier 4 pivot |
...
### 8.1 Tier 4 leads commissions for an affiliate
```sql
SELECT LeadID, Tier4Commission, Tier4Paid
FROM dbo.qry_aff_Tier4LeadsCommissions WITH (NOLOCK)
WHERE Tier4AffiliateID = @AffiliateID
```

### 8.2 Total unpaid tier 4 leads commissions
```sql
...
```sql
SELECT COUNT(*) AS TierRecords FROM dbo.qry_aff_Tier4LeadsCommissions WITH (NOLOCK)
```
...
*Object: dbo.qry_aff_Tier4LeadsCommissions | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_Tier4LeadsCommissions.sql*
