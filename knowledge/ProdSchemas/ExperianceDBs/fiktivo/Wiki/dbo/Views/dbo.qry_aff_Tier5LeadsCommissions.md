# dbo.qry_aff_Tier5LeadsCommissions

> Tier 5 filter view over tblaff_Leads_Commissions, extracting leads commissions for tier 5 (great-great-grandparent) affiliates with aliased column names for cross-tier joining.
...
Filtered subset of tblaff_Leads_Commissions where Tier = 5.
...
|---------|---------------|-------------------|-------------|
| (all) | dbo.tblaff_Leads_Commissions | Base table | Filtered by Tier = 5 |

### 5.2 Referenced By (other objects point to this)

| Source Object | Source Element | Relationship Type | Description |
|--------------|---------------|-------------------|-------------|
| dbo.qry_aff_LeadDetailAllTiers | LEFT OUTER JOIN | View | All-tiers pivot joins this for tier 5 data |
...
```
dbo.qry_aff_Tier5LeadsCommissions (view)
...
|--------|------|----------|
| dbo.tblaff_Leads_Commissions | Table | Base table (WHERE Tier = 5) |

### 6.2 Objects That Depend On This

| Object | Type | How Used |
|--------|------|----------|
| dbo.qry_aff_LeadDetailAllTiers | View | LEFT OUTER JOIN for tier 5 pivot |
...
### 8.1 Tier 5 leads commissions for an affiliate
```sql
SELECT LeadID, Tier5Commission, Tier5Paid
FROM dbo.qry_aff_Tier5LeadsCommissions WITH (NOLOCK)
WHERE Tier5AffiliateID = @AffiliateID
```

### 8.2 Total unpaid tier 5 leads commissions
```sql
...
```sql
SELECT COUNT(*) AS TierRecords FROM dbo.qry_aff_Tier5LeadsCommissions WITH (NOLOCK)
```
...
*Object: dbo.qry_aff_Tier5LeadsCommissions | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_Tier5LeadsCommissions.sql*
