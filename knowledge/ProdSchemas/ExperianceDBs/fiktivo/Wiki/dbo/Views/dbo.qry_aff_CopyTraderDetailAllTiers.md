# dbo.qry_aff_CopyTraderDetailAllTiers

> Pivoted all-tiers detail view for CopyTrader events, joining the base event table with all 5 tier-filter views to show complete multi-tier commission breakdown per event in a single row.

| Property | Value |
|----------|-------|
| **Schema** | dbo |
| **Object Type** | View |
| **Key Identifier** | Base table: dbo.tblaff_CopyTraders |
| **Partition** | N/A |
| **Indexes** | N/A |
...
dbo.qry_aff_CopyTraderDetailAllTiers is the comprehensive detail view for CopyTrader commissions, pivoting all 5 tier levels into a single row per event. It LEFT OUTER JOINs the base event table (tblaff_CopyTraders) with each of the 5 tier-filter views (qry_aff_Tier1-5CopyTradersCommissions) and tblaff_Country for the country name. This produces a denormalized row showing the event details alongside each tier's affiliate, commission amount, paid status, and sub-affiliate tag.

Note: The tier views use the plural form "CopyTraders" (qry_aff_Tier1-5CopyTradersCommissions) while this AllTiers view uses the singular "CopyTrader" in its name.
...
**Columns/Parameters Involved**: CopyTraderID, Tier1-5 AffiliateID/Commission/Paid/SubAffiliateID
...
|---|---------|------|----------|---------|------------|-------------|
| 1 | CopyTraderID | int | NO | - | VERIFIED | Event ID from base table. |
...
| 6 | AffiliateCopyTraderAccepted | bit/int | YES | - | VERIFIED | Whether the affiliate accepted this event for commission. |
...
|---------|---------------|-------------------|-------------|
| (event cols) | dbo.tblaff_CopyTraders | Base table | Event details |
| (tier 1 cols) | dbo.qry_aff_Tier1CopyTradersCommissions | LEFT OUTER JOIN | Tier 1 commission data |
| (tier 2 cols) | dbo.qry_aff_Tier2CopyTradersCommissions | LEFT OUTER JOIN | Tier 2 |
| (tier 3 cols) | dbo.qry_aff_Tier3CopyTradersCommissions | LEFT OUTER JOIN | Tier 3 |
| (tier 4 cols) | dbo.qry_aff_Tier4CopyTradersCommissions | LEFT OUTER JOIN | Tier 4 |
| (tier 5 cols) | dbo.qry_aff_Tier5CopyTradersCommissions | LEFT OUTER JOIN | Tier 5 |
...
```
dbo.qry_aff_CopyTraderDetailAllTiers (view)
  +-- dbo.tblaff_CopyTraders (table)
  +-- dbo.qry_aff_Tier1CopyTradersCommissions (view)
  +-- dbo.qry_aff_Tier2CopyTradersCommissions (view)
  +-- dbo.qry_aff_Tier3CopyTradersCommissions (view)
  +-- dbo.qry_aff_Tier4CopyTradersCommissions (view)
  +-- dbo.qry_aff_Tier5CopyTradersCommissions (view)
...
|--------|------|----------|
| dbo.tblaff_CopyTraders | Table | Base event data |
| dbo.qry_aff_Tier1-5CopyTradersCommissions | Views | Tier commission data (LEFT OUTER JOIN) |
...
```sql
SELECT * FROM dbo.qry_aff_CopyTraderDetailAllTiers WITH (NOLOCK)
WHERE CopyTraderID = @EventID
```

### 8.2 Unpaid events with tier 1 commission
```sql
SELECT CopyTraderID, Tier1AffiliateID, Tier1Commission, ORDER_DATE
FROM dbo.qry_aff_CopyTraderDetailAllTiers WITH (NOLOCK)
...
```sql
SELECT TOP 10 CopyTraderID, ORDER_DATE,
...
*Object: dbo.qry_aff_CopyTraderDetailAllTiers | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_CopyTraderDetailAllTiers.sql*
