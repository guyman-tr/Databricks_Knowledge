# dbo.qry_aff_Tier1SalesCommissions

> Tier 1 filter view over tblaff_Sales_Commissions, extracting sales commissions for tier 1 (direct/referring) affiliates with aliased column names for cross-tier joining.
...
Filtered subset of tblaff_Sales_Commissions where Tier = 1.
...
|--------------|---------------|-------------------|-------------|
| dbo.qry_aff_SalesDetailAllTiers | LEFT OUTER JOIN | View | All-tiers pivot joins this for tier 1 data |
...
|--------|------|----------|
| dbo.tblaff_Sales_Commissions | Table | Base table (WHERE Tier = 1) |

### 6.2 Objects That Depend On This

| Object | Type | How Used |
|--------|------|----------|
| dbo.qry_aff_SalesDetailAllTiers | View | LEFT OUTER JOIN for tier 1 pivot |
...
### 8.1 Tier 1 sales commissions for an affiliate
```sql
SELECT SalesID, Tier1Commission, Tier1Paid
FROM dbo.qry_aff_Tier1SalesCommissions WITH (NOLOCK)
WHERE Tier1AffiliateID = @AffiliateID
```

### 8.2 Total unpaid tier 1 sales commissions
```sql
...
```sql
SELECT COUNT(*) AS TierRecords FROM dbo.qry_aff_Tier1SalesCommissions WITH (NOLOCK)
```
...
*Object: dbo.qry_aff_Tier1SalesCommissions | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_Tier1SalesCommissions.sql*
