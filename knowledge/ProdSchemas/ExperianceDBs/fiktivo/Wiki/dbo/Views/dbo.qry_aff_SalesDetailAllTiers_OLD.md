# dbo.qry_aff_SalesDetailAllTiers_OLD

> Pivoted all-tiers detail view for Sales events, joining the base event table with all 5 tier-filter views to show complete multi-tier commission breakdown per event in a single row.
...
dbo.qry_aff_SalesDetailAllTiers_OLD is the comprehensive detail view for Sales commissions, pivoting all 5 tier levels into a single row per event. It LEFT OUTER JOINs the base event table (tblaff_Sales) with each of the 5 tier-filter views (qry_aff_Tier1-5SalesCommissions) and tblaff_Country for the country name. This produces a denormalized row showing the event details alongside each tier's affiliate, commission amount, paid status, and sub-affiliate tag.

Used by the affiliate admin interface for detailed commission reports where users need to see all tier breakdowns for each event on a single line. This is a legacy version retained for backward compatibility. See dbo.qry_aff_SalesDetailAllTiers for the current implementation.
...
**Columns/Parameters Involved**: SalesID, Tier1-5 AffiliateID/Commission/Paid/SubAffiliateID
...
|---|---------|------|----------|---------|------------|-------------|
| 1 | SalesID | int | NO | - | VERIFIED | Event ID from base table. |
...
| (tier 1 cols) | dbo.qry_aff_Tier1SalesCommissions | LEFT OUTER JOIN | Tier 1 commission data |
| (tier 2 cols) | dbo.qry_aff_Tier2SalesCommissions | LEFT OUTER JOIN | Tier 2 |
| (tier 3 cols) | dbo.qry_aff_Tier3SalesCommissions | LEFT OUTER JOIN | Tier 3 |
| (tier 4 cols) | dbo.qry_aff_Tier4SalesCommissions | LEFT OUTER JOIN | Tier 4 |
| (tier 5 cols) | dbo.qry_aff_Tier5SalesCommissions | LEFT OUTER JOIN | Tier 5 |
...
```
dbo.qry_aff_SalesDetailAllTiers_OLD (view)
  +-- dbo.tblaff_Sales (table)
  +-- dbo.qry_aff_Tier1SalesCommissions (view)
  +-- dbo.qry_aff_Tier2SalesCommissions (view)
  +-- dbo.qry_aff_Tier3SalesCommissions (view)
...
| dbo.qry_aff_Tier1-5SalesCommissions | Views | Tier commission data (LEFT OUTER JOIN) |
...
```sql
SELECT * FROM dbo.qry_aff_SalesDetailAllTiers_OLD WITH (NOLOCK)
...
FROM dbo.qry_aff_SalesDetailAllTiers_OLD WITH (NOLOCK)
...
*Object: dbo.qry_aff_SalesDetailAllTiers_OLD | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_SalesDetailAllTiers_OLD.sql*
