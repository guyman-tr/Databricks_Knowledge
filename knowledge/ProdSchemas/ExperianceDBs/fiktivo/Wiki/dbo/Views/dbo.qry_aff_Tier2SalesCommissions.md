# dbo.qry_aff_Tier2SalesCommissions

> Tier 2 filter view over tblaff_Sales_Commissions, extracting sales commissions for tier 2 (parent of direct) affiliates with aliased column names for cross-tier joining.
...
Filtered subset of tblaff_Sales_Commissions where Tier = 2.
...
|--------------|---------------|-------------------|-------------|
| dbo.qry_aff_SalesDetailAllTiers | LEFT OUTER JOIN | View | All-tiers pivot joins this for tier 2 data |
...
|--------|------|----------|
| dbo.tblaff_Sales_Commissions | Table | Base table (WHERE Tier = 2) |

### 6.2 Objects That Depend On This

| Object | Type | How Used |
|--------|------|----------|
| dbo.qry_aff_SalesDetailAllTiers | View | LEFT OUTER JOIN for tier 2 pivot |
...
### 8.1 Tier 2 sales commissions for an affiliate
```sql
SELECT SalesID, Tier2Commission, Tier2Paid
FROM dbo.qry_aff_Tier2SalesCommissions WITH (NOLOCK)
WHERE Tier2AffiliateID = @AffiliateID
```

### 8.2 Total unpaid tier 2 sales commissions
```sql
...
```sql
SELECT COUNT(*) AS TierRecords FROM dbo.qry_aff_Tier2SalesCommissions WITH (NOLOCK)
```
...
*Object: dbo.qry_aff_Tier2SalesCommissions | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_Tier2SalesCommissions.sql*
