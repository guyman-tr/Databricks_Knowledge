# dbo.qry_aff_ChargebacksDetailAllTiers

> Pivoted all-tiers detail view for Chargebacks events, joining the base event table with all 5 tier-filter views to show complete multi-tier commission breakdown per event in a single row.

| Property | Value |
|----------|-------|
| **Schema** | dbo |
| **Object Type** | View |
| **Key Identifier** | Base table: dbo.tblaff_Chargebacks |
| **Partition** | N/A |
| **Indexes** | N/A |
...
dbo.qry_aff_ChargebacksDetailAllTiers is the comprehensive detail view for Chargebacks commissions, pivoting all 5 tier levels into a single row per event. It LEFT OUTER JOINs the base event table (tblaff_Chargebacks) with each of the 5 tier-filter views (qry_aff_Tier1-5ChargebacksCommissions) and tblaff_Country for the country name. This produces a denormalized row showing the event details alongside each tier's affiliate, commission amount, paid status, and sub-affiliate tag.
...
**Columns/Parameters Involved**: ChargebackID, Tier1-5 AffiliateID/Commission/Paid/SubAffiliateID
...
|---|---------|------|----------|---------|------------|-------------|
| 1 | ChargebackID | int | NO | - | VERIFIED | Event ID from base table. |
...
| 6 | AffiliateChargebackAccepted | bit/int | YES | - | VERIFIED | Whether the affiliate accepted this event for commission. |
...
|---------|---------------|-------------------|-------------|
| (event cols) | dbo.tblaff_Chargebacks | Base table | Event details |
| (tier 1 cols) | dbo.qry_aff_Tier1ChargebacksCommissions | LEFT OUTER JOIN | Tier 1 commission data |
| (tier 2 cols) | dbo.qry_aff_Tier2ChargebacksCommissions | LEFT OUTER JOIN | Tier 2 |
| (tier 3 cols) | dbo.qry_aff_Tier3ChargebacksCommissions | LEFT OUTER JOIN | Tier 3 |
| (tier 4 cols) | dbo.qry_aff_Tier4ChargebacksCommissions | LEFT OUTER JOIN | Tier 4 |
| (tier 5 cols) | dbo.qry_aff_Tier5ChargebacksCommissions | LEFT OUTER JOIN | Tier 5 |
...
```
dbo.qry_aff_ChargebacksDetailAllTiers (view)
  +-- dbo.tblaff_Chargebacks (table)
  +-- dbo.qry_aff_Tier1ChargebacksCommissions (view)
  +-- dbo.qry_aff_Tier2ChargebacksCommissions (view)
  +-- dbo.qry_aff_Tier3ChargebacksCommissions (view)
  +-- dbo.qry_aff_Tier4ChargebacksCommissions (view)
  +-- dbo.qry_aff_Tier5ChargebacksCommissions (view)
...
|--------|------|----------|
| dbo.tblaff_Chargebacks | Table | Base event data |
| dbo.qry_aff_Tier1-5ChargebacksCommissions | Views | Tier commission data (LEFT OUTER JOIN) |
...
```sql
SELECT * FROM dbo.qry_aff_ChargebacksDetailAllTiers WITH (NOLOCK)
WHERE ChargebackID = @EventID
```

### 8.2 Unpaid events with tier 1 commission
```sql
SELECT ChargebackID, Tier1AffiliateID, Tier1Commission, ORDER_DATE
FROM dbo.qry_aff_ChargebacksDetailAllTiers WITH (NOLOCK)
...
```sql
SELECT TOP 10 ChargebackID, ORDER_DATE,
...
*Object: dbo.qry_aff_ChargebacksDetailAllTiers | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_ChargebacksDetailAllTiers.sql*
