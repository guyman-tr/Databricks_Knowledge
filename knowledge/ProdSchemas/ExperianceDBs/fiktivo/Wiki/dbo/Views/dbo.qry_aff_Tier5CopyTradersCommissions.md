# dbo.qry_aff_Tier5CopyTradersCommissions

> Tier 5 filter view over tblaff_CopyTraders_Commissions, extracting copytraders commissions for tier 5 (great-great-grandparent) affiliates with aliased column names for cross-tier joining.
...
Filtered subset of tblaff_CopyTraders_Commissions where Tier = 5.
...
|---------|---------------|-------------------|-------------|
| (all) | dbo.tblaff_CopyTraders_Commissions | Base table | Filtered by Tier = 5 |

### 5.2 Referenced By (other objects point to this)

| Source Object | Source Element | Relationship Type | Description |
|--------------|---------------|-------------------|-------------|
| dbo.qry_aff_CopyTraderDetailAllTiers | LEFT OUTER JOIN | View | All-tiers pivot joins this for tier 5 data |
...
|--------|------|----------|
| dbo.tblaff_CopyTraders_Commissions | Table | Base table (WHERE Tier = 5) |

### 6.2 Objects That Depend On This

| Object | Type | How Used |
|--------|------|----------|
| dbo.qry_aff_CopyTraderDetailAllTiers | View | LEFT OUTER JOIN for tier 5 pivot |
...
### 8.1 Tier 5 copytraders commissions for an affiliate
```sql
SELECT CopyTraderID, Tier5Commission, Tier5Paid
FROM dbo.qry_aff_Tier5CopyTradersCommissions WITH (NOLOCK)
WHERE Tier5AffiliateID = @AffiliateID
```

### 8.2 Total unpaid tier 5 copytraders commissions
```sql
...
```sql
SELECT COUNT(*) AS TierRecords FROM dbo.qry_aff_Tier5CopyTradersCommissions WITH (NOLOCK)
```
...
*Object: dbo.qry_aff_Tier5CopyTradersCommissions | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_Tier5CopyTradersCommissions.sql*
