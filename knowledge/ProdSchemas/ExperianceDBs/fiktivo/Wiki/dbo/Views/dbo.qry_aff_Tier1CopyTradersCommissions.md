# dbo.qry_aff_Tier1CopyTradersCommissions

> Tier 1 filter view over tblaff_CopyTraders_Commissions, extracting copytraders commissions for tier 1 (direct/referring) affiliates with aliased column names for cross-tier joining.
...
Filtered subset of tblaff_CopyTraders_Commissions where Tier = 1.
...
|---------|---------------|-------------------|-------------|
| (all) | dbo.tblaff_CopyTraders_Commissions | Base table | Filtered by Tier = 1 |

### 5.2 Referenced By (other objects point to this)

| Source Object | Source Element | Relationship Type | Description |
|--------------|---------------|-------------------|-------------|
| dbo.qry_aff_CopyTraderDetailAllTiers | LEFT OUTER JOIN | View | All-tiers pivot joins this for tier 1 data |
...
|--------|------|----------|
| dbo.tblaff_CopyTraders_Commissions | Table | Base table (WHERE Tier = 1) |

### 6.2 Objects That Depend On This

| Object | Type | How Used |
|--------|------|----------|
| dbo.qry_aff_CopyTraderDetailAllTiers | View | LEFT OUTER JOIN for tier 1 pivot |
...
### 8.1 Tier 1 copytraders commissions for an affiliate
```sql
SELECT CopyTraderID, Tier1Commission, Tier1Paid
FROM dbo.qry_aff_Tier1CopyTradersCommissions WITH (NOLOCK)
WHERE Tier1AffiliateID = @AffiliateID
```

### 8.2 Total unpaid tier 1 copytraders commissions
```sql
...
```sql
SELECT COUNT(*) AS TierRecords FROM dbo.qry_aff_Tier1CopyTradersCommissions WITH (NOLOCK)
```
...
*Object: dbo.qry_aff_Tier1CopyTradersCommissions | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_Tier1CopyTradersCommissions.sql*
