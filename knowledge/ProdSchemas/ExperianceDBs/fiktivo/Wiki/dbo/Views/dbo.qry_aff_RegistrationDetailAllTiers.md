# dbo.qry_aff_RegistrationDetailAllTiers

> Pivoted all-tiers detail view for Registration events, joining the base event table with all 5 tier-filter views to show complete multi-tier commission breakdown per event in a single row.
...
dbo.qry_aff_RegistrationDetailAllTiers is the comprehensive detail view for Registration commissions, pivoting all 5 tier levels into a single row per event. It LEFT OUTER JOINs the base event table (tblaff_Registrations) with each of the 5 tier-filter views (qry_aff_Tier1-5RegistrationCommissions) and tblaff_Country for the country name. This produces a denormalized row showing the event details alongside each tier's affiliate, commission amount, paid status, and sub-affiliate tag.
...
**Columns/Parameters Involved**: RegistrationID, Tier1-5 AffiliateID/Commission/Paid/SubAffiliateID
...
|---|---------|------|----------|---------|------------|-------------|
| 1 | RegistrationID | int | NO | - | VERIFIED | Event ID from base table. |
...
|---------|---------------|-------------------|-------------|
| (event cols) | dbo.tblaff_Registrations | Base table | Event details |
| (tier 1 cols) | dbo.qry_aff_Tier1RegistrationCommissions | LEFT OUTER JOIN | Tier 1 commission data |
| (tier 2 cols) | dbo.qry_aff_Tier2RegistrationCommissions | LEFT OUTER JOIN | Tier 2 |
| (tier 3 cols) | dbo.qry_aff_Tier3RegistrationCommissions | LEFT OUTER JOIN | Tier 3 |
| (tier 4 cols) | dbo.qry_aff_Tier4RegistrationCommissions | LEFT OUTER JOIN | Tier 4 |
| (tier 5 cols) | dbo.qry_aff_Tier5RegistrationCommissions | LEFT OUTER JOIN | Tier 5 |
...
```
dbo.qry_aff_RegistrationDetailAllTiers (view)
  +-- dbo.tblaff_Registrations (table)
  +-- dbo.qry_aff_Tier1RegistrationCommissions (view)
  +-- dbo.qry_aff_Tier2RegistrationCommissions (view)
  +-- dbo.qry_aff_Tier3RegistrationCommissions (view)
  +-- dbo.qry_aff_Tier4RegistrationCommissions (view)
  +-- dbo.qry_aff_Tier5RegistrationCommissions (view)
...
|--------|------|----------|
| dbo.tblaff_Registrations | Table | Base event data |
| dbo.qry_aff_Tier1-5RegistrationCommissions | Views | Tier commission data (LEFT OUTER JOIN) |
...
```sql
SELECT * FROM dbo.qry_aff_RegistrationDetailAllTiers WITH (NOLOCK)
WHERE RegistrationID = @EventID
```

### 8.2 Unpaid events with tier 1 commission
```sql
SELECT RegistrationID, Tier1AffiliateID, Tier1Commission, ORDER_DATE
FROM dbo.qry_aff_RegistrationDetailAllTiers WITH (NOLOCK)
...
```sql
SELECT TOP 10 RegistrationID, ORDER_DATE,
...
*Object: dbo.qry_aff_RegistrationDetailAllTiers | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_RegistrationDetailAllTiers.sql*
