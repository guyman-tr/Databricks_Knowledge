# dbo.qry_aff_Tier2CopyTradersCommissions

> Tier 2 filter view over tblaff_CopyTraders_Commissions, extracting copytraders commissions for tier 2 (parent of direct) affiliates with aliased column names for cross-tier joining.
...
Filtered subset of tblaff_CopyTraders_Commissions where Tier = 2.
...
|---------|---------------|-------------------|-------------|
| (all) | dbo.tblaff_CopyTraders_Commissions | Base table | Filtered by Tier = 2 |

### 5.2 Referenced By (other objects point to this)

| Source Object | Source Element | Relationship Type | Description |
|--------------|---------------|-------------------|-------------|
| dbo.qry_aff_CopyTraderDetailAllTiers | LEFT OUTER JOIN | View | All-tiers pivot joins this for tier 2 data |
...
|--------|------|----------|
| dbo.tblaff_CopyTraders_Commissions | Table | Base table (WHERE Tier = 2) |

### 6.2 Objects That Depend On This

| Object | Type | How Used |
|--------|------|----------|
| dbo.qry_aff_CopyTraderDetailAllTiers | View | LEFT OUTER JOIN for tier 2 pivot |
...
### 8.1 Tier 2 copytraders commissions for an affiliate
```sql
SELECT CopyTraderID, Tier2Commission, Tier2Paid
FROM dbo.qry_aff_Tier2CopyTradersCommissions WITH (NOLOCK)
WHERE Tier2AffiliateID = @AffiliateID
```

### 8.2 Total unpaid tier 2 copytraders commissions
```sql
...
```sql
SELECT COUNT(*) AS TierRecords FROM dbo.qry_aff_Tier2CopyTradersCommissions WITH (NOLOCK)
```
...
*Object: dbo.qry_aff_Tier2CopyTradersCommissions | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_Tier2CopyTradersCommissions.sql*
