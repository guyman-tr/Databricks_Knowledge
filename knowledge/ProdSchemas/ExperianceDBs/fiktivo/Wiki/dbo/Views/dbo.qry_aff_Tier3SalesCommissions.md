# dbo.qry_aff_Tier3SalesCommissions

> Tier 3 filter view over tblaff_Sales_Commissions, extracting sales commissions for tier 3 (grandparent) affiliates with aliased column names for cross-tier joining.
...
Filtered subset of tblaff_Sales_Commissions where Tier = 3.
...
|--------------|---------------|-------------------|-------------|
| dbo.qry_aff_SalesDetailAllTiers | LEFT OUTER JOIN | View | All-tiers pivot joins this for tier 3 data |
...
|--------|------|----------|
| dbo.tblaff_Sales_Commissions | Table | Base table (WHERE Tier = 3) |

### 6.2 Objects That Depend On This

| Object | Type | How Used |
|--------|------|----------|
| dbo.qry_aff_SalesDetailAllTiers | View | LEFT OUTER JOIN for tier 3 pivot |
...
### 8.1 Tier 3 sales commissions for an affiliate
```sql
SELECT SalesID, Tier3Commission, Tier3Paid
FROM dbo.qry_aff_Tier3SalesCommissions WITH (NOLOCK)
WHERE Tier3AffiliateID = @AffiliateID
```

### 8.2 Total unpaid tier 3 sales commissions
```sql
...
```sql
SELECT COUNT(*) AS TierRecords FROM dbo.qry_aff_Tier3SalesCommissions WITH (NOLOCK)
```
...
*Object: dbo.qry_aff_Tier3SalesCommissions | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_Tier3SalesCommissions.sql*
