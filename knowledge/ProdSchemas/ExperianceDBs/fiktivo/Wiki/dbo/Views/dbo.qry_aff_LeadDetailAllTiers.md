# dbo.qry_aff_LeadDetailAllTiers
...
dbo.qry_aff_LeadDetailAllTiers is the comprehensive detail view for Lead commissions, pivoting all 5 tier levels into a single row per event. It LEFT OUTER JOINs the base event table (tblaff_Leads) with each of the 5 tier-filter views (qry_aff_Tier1-5LeadCommissions) and tblaff_Country for the country name. This produces a denormalized row showing the event details alongside each tier's affiliate, commission amount, paid status, and sub-affiliate tag.
...
```
dbo.qry_aff_LeadDetailAllTiers (view)
...
```sql
SELECT * FROM dbo.qry_aff_LeadDetailAllTiers WITH (NOLOCK)
...
FROM dbo.qry_aff_LeadDetailAllTiers WITH (NOLOCK)
...
*Object: dbo.qry_aff_LeadDetailAllTiers | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_LeadDetailAllTiers.sql*
