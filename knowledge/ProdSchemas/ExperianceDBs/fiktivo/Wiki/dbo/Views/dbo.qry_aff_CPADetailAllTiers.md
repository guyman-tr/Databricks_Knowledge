...
dbo.qry_aff_CPADetailAllTiers is the comprehensive detail view for CPA commissions, pivoting all 5 tier levels into a single row per event. It LEFT OUTER JOINs the base event table (tblaff_CPA) with each of the 5 tier-filter views (qry_aff_Tier1-5CPACommissions) and tblaff_Country for the country name. This produces a denormalized row showing the event details alongside each tier's affiliate, commission amount, paid status, and sub-affiliate tag.
...
```
dbo.qry_aff_CPADetailAllTiers (view)
...
```sql
SELECT * FROM dbo.qry_aff_CPADetailAllTiers WITH (NOLOCK)
...
FROM dbo.qry_aff_CPADetailAllTiers WITH (NOLOCK)
...
*Object: dbo.qry_aff_CPADetailAllTiers | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_CPADetailAllTiers.sql*
