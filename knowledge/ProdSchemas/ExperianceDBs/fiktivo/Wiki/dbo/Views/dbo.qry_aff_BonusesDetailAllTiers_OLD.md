# dbo.qry_aff_BonusesDetailAllTiers_OLD

> Pivoted all-tiers detail view for Bonuses events, joining the base event table with all 5 tier-filter views to show complete multi-tier commission breakdown per event in a single row.

| Property | Value |
|----------|-------|
| **Schema** | dbo |
| **Object Type** | View |
| **Key Identifier** | Base table: dbo.tblaff_Bonuses |
| **Partition** | N/A |
| **Indexes** | N/A |
...
dbo.qry_aff_BonusesDetailAllTiers_OLD is the comprehensive detail view for Bonuses commissions, pivoting all 5 tier levels into a single row per event. It LEFT OUTER JOINs the base event table (tblaff_Bonuses) with each of the 5 tier-filter views (qry_aff_Tier1-5BonusesCommissions) and tblaff_Country for the country name. This produces a denormalized row showing the event details alongside each tier's affiliate, commission amount, paid status, and sub-affiliate tag.

Used by the affiliate admin interface for detailed commission reports where users need to see all tier breakdowns for each event on a single line. This is a legacy version retained for backward compatibility. See dbo.qry_aff_BonusesDetailAllTiers for the current implementation.
...
**Columns/Parameters Involved**: BonusID, Tier1-5 AffiliateID/Commission/Paid/SubAffiliateID
...
|---|---------|------|----------|---------|------------|-------------|
| 1 | BonusID | int | NO | - | VERIFIED | Event ID from base table. |
...
| 6 | AffiliateBonusAccepted | bit/int | YES | - | VERIFIED | Whether the affiliate accepted this event for commission. |
...
|---------|---------------|-------------------|-------------|
| (event cols) | dbo.tblaff_Bonuses | Base table | Event details |
| (tier 1 cols) | dbo.qry_aff_Tier1BonusesCommissions | LEFT OUTER JOIN | Tier 1 commission data |
| (tier 2 cols) | dbo.qry_aff_Tier2BonusesCommissions | LEFT OUTER JOIN | Tier 2 |
| (tier 3 cols) | dbo.qry_aff_Tier3BonusesCommissions | LEFT OUTER JOIN | Tier 3 |
| (tier 4 cols) | dbo.qry_aff_Tier4BonusesCommissions | LEFT OUTER JOIN | Tier 4 |
| (tier 5 cols) | dbo.qry_aff_Tier5BonusesCommissions | LEFT OUTER JOIN | Tier 5 |
...
```
dbo.qry_aff_BonusesDetailAllTiers_OLD (view)
  +-- dbo.tblaff_Bonuses (table)
  +-- dbo.qry_aff_Tier1BonusesCommissions (view)
  +-- dbo.qry_aff_Tier2BonusesCommissions (view)
  +-- dbo.qry_aff_Tier3BonusesCommissions (view)
  +-- dbo.qry_aff_Tier4BonusesCommissions (view)
  +-- dbo.qry_aff_Tier5BonusesCommissions (view)
...
|--------|------|----------|
| dbo.tblaff_Bonuses | Table | Base event data |
| dbo.qry_aff_Tier1-5BonusesCommissions | Views | Tier commission data (LEFT OUTER JOIN) |
...
```sql
SELECT * FROM dbo.qry_aff_BonusesDetailAllTiers_OLD WITH (NOLOCK)
WHERE BonusID = @EventID
```

### 8.2 Unpaid events with tier 1 commission
```sql
SELECT BonusID, Tier1AffiliateID, Tier1Commission, ORDER_DATE
FROM dbo.qry_aff_BonusesDetailAllTiers_OLD WITH (NOLOCK)
...
```sql
SELECT TOP 10 BonusID, ORDER_DATE,
...
*Object: dbo.qry_aff_BonusesDetailAllTiers_OLD | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_BonusesDetailAllTiers_OLD.sql*
