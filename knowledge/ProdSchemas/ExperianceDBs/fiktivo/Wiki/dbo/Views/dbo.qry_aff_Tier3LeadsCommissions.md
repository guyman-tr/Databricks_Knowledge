# dbo.qry_aff_Tier3LeadsCommissions

> Tier 3 filter view over tblaff_Leads_Commissions, extracting leads commissions for tier 3 (grandparent) affiliates with aliased column names for cross-tier joining.
...
Filtered subset of tblaff_Leads_Commissions where Tier = 3.
...
|---------|---------------|-------------------|-------------|
| (all) | dbo.tblaff_Leads_Commissions | Base table | Filtered by Tier = 3 |

### 5.2 Referenced By (other objects point to this)

| Source Object | Source Element | Relationship Type | Description |
|--------------|---------------|-------------------|-------------|
| dbo.qry_aff_LeadDetailAllTiers | LEFT OUTER JOIN | View | All-tiers pivot joins this for tier 3 data |
...
```
dbo.qry_aff_Tier3LeadsCommissions (view)
...
|--------|------|----------|
| dbo.tblaff_Leads_Commissions | Table | Base table (WHERE Tier = 3) |

### 6.2 Objects That Depend On This

| Object | Type | How Used |
|--------|------|----------|
| dbo.qry_aff_LeadDetailAllTiers | View | LEFT OUTER JOIN for tier 3 pivot |
...
### 8.1 Tier 3 leads commissions for an affiliate
```sql
SELECT LeadID, Tier3Commission, Tier3Paid
FROM dbo.qry_aff_Tier3LeadsCommissions WITH (NOLOCK)
WHERE Tier3AffiliateID = @AffiliateID
```

### 8.2 Total unpaid tier 3 leads commissions
```sql
...
```sql
SELECT COUNT(*) AS TierRecords FROM dbo.qry_aff_Tier3LeadsCommissions WITH (NOLOCK)
```
...
*Object: dbo.qry_aff_Tier3LeadsCommissions | Type: View | Source: fiktivo/dbo/Views/dbo.qry_aff_Tier3LeadsCommissions.sql*
