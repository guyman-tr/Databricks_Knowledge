# AffiliateAdmin.AffiliatesGroups_NogaJunk100226

> SSDT-only backup copy of AffiliateAdmin.AffiliatesGroups, created by developer Noga on 2026-02-10 as part of a schema migration. Does not exist in the live database.
...
This table is a backup snapshot of AffiliateAdmin.AffiliatesGroups created during a development maintenance operation. The naming convention `_NogaJunk100226` indicates: developer "Noga", "Junk" (backup), created on 2026-02-10.

This is the second backup of AffiliatesGroups taken 6 days after the first backup (NogaJnk040226), suggesting an iterative migration process. The structure is identical to the 040226 backup, preserving the 6-column layout of AffiliatesGroups before changes were applied.
...
| 2 | AffiliatesGroupsName | nvarchar(50) | NO | - | CODE-BACKED | Display name for the affiliate group. Required field. |
| 3 | AccountManagerName | nvarchar(50) | YES | - | CODE-BACKED | Name of the account manager responsible for this group. Nullable. |
...
```sql
SELECT OBJECT_ID('AffiliateAdmin.AffiliatesGroups_NogaJunk100226') AS ObjectExists;
-- Returns NULL in production
```

### 8.2 List all AffiliatesGroups backup tables chronologically
```sql
SELECT name, create_date
FROM sys.tables WITH (NOLOCK)
WHERE schema_id = SCHEMA_ID('AffiliateAdmin')
  AND name LIKE 'AffiliatesGroups_Noga%'
ORDER BY name;
```

### 8.3 Compare with current AffiliatesGroups structure
```sql
...
*Object: AffiliateAdmin.AffiliatesGroups_NogaJunk100226 | Type: Table | Source: fiktivo/AffiliateAdmin/Tables/AffiliateAdmin.AffiliatesGroups_NogaJunk100226.sql*
