...
> Synonym pointing to [etoro].[Customer].[Customer]. Cross-database pointer.

| Property | Value |
|----------|-------|
| **Schema** | dbo |
| **Object Type** | Synonym |
| **Target** | [etoro].[Customer].[Customer] |
...
dbo.Real_Customer is a database synonym that provides a local alias for [etoro].[Customer].[Customer]. This enables stored procedures and views in UserApiDB to reference external objects without hardcoding the full four-part name, simplifying cross-database queries and enabling environment-specific configuration.
...
|---------|---------------|-------------------|-------------|
| - | [etoro].[Customer].[Customer] | Synonym target | External object this points to |
...
dbo.Real_Customer (synonym)
  +-- [etoro].[Customer].[Customer] (external)
```
...
|--------|------|----------|
| [etoro].[Customer].[Customer] | External | Synonym target |
...
### 8.1 Use the synonym
```sql
SELECT TOP 10 * FROM Real_Customer WITH (NOLOCK)
```

### 8.2 Check synonym target
```sql
SELECT name, base_object_name FROM sys.synonyms WHERE name = 'Real_Customer'
```

### 8.3 Verify synonym resolves
```sql
SELECT OBJECT_ID('dbo.Real_Customer') AS ObjectID
```
...
*Object: dbo.Real_Customer | Type: Synonym | Source: UserApiDB/UserApiDB/dbo/Synonyms/dbo.Real_Customer.sql*
