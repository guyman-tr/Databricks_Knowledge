# dbo.History_Position

> Synonym pointing to [etoro].[History].[Position]. Cross-database pointer.

| Property | Value |
|----------|-------|
| **Schema** | dbo |
| **Object Type** | Synonym |
| **Target** | [etoro].[History].[Position] |
...
|---------|---------------|-------------------|-------------|
| - | [etoro].[History].[Position] | Synonym target | External object this points to |
...
```
dbo.History_Position (synonym)
  +-- [etoro].[History].[Position] (external)
```
...
|--------|------|----------|
| [etoro].[History].[Position] | External | Synonym target |
...
### 8.1 Use the synonym
```sql
SELECT TOP 10 * FROM History_Position WITH (NOLOCK)
```

### 8.2 Check synonym target
```sql
SELECT name, base_object_name FROM sys.synonyms WHERE name = 'History_Position'
```

### 8.3 Verify synonym resolves
```sql
SELECT OBJECT_ID('dbo.History_Position') AS ObjectID
```
...
*Object: dbo.History_Position | Type: Synonym | Source: UserApiDB/UserApiDB/dbo/Synonyms/dbo.History_Position.sql*
