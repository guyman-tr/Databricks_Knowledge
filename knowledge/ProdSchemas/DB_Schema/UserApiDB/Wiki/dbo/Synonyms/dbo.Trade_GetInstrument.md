...
> Synonym pointing to [etoro].[Trade].[GetInstrument]. Cross-database pointer.

| Property | Value |
|----------|-------|
| **Schema** | dbo |
| **Object Type** | Synonym |
| **Target** | [etoro].[Trade].[GetInstrument] |
...
dbo.Trade_GetInstrument is a database synonym that provides a local alias for [etoro].[Trade].[GetInstrument]. This enables stored procedures and views in UserApiDB to reference external objects without hardcoding the full four-part name, simplifying cross-database queries and enabling environment-specific configuration.
...
|---------|---------------|-------------------|-------------|
| - | [etoro].[Trade].[GetInstrument] | Synonym target | External object this points to |
...
dbo.Trade_GetInstrument (synonym)
  +-- [etoro].[Trade].[GetInstrument] (external)
```
...
|--------|------|----------|
| [etoro].[Trade].[GetInstrument] | External | Synonym target |
...
### 8.1 Use the synonym
```sql
SELECT TOP 10 * FROM Trade_GetInstrument WITH (NOLOCK)
```
...
```sql
SELECT OBJECT_ID('dbo.Trade_GetInstrument') AS ObjectID
```
...
*Object: dbo.Trade_GetInstrument | Type: Synonym | Source: UserApiDB/UserApiDB/dbo/Synonyms/dbo.Trade_GetInstrument.sql*
