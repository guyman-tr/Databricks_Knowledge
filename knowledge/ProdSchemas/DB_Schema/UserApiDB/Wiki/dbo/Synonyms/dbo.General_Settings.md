# dbo.General_Settings

> Synonym pointing to [Customer].[Settings]. Cross-database pointer.
...
dbo.General_Settings is a database synonym that provides a local alias for [Customer].[Settings]. This enables stored procedures and views in UserApiDB to reference external objects without hardcoding the full four-part name, simplifying cross-database queries and enabling environment-specific configuration.
...
|---------|---------------|-------------------|-------------|
| - | [Customer].[Settings] | Synonym target | External object this points to |
...
```
dbo.General_Settings (synonym)
  +-- [Customer].[Settings] (external)
```
...
|--------|------|----------|
| [Customer].[Settings] | External | Synonym target |
...
### 8.1 Use the synonym
```sql
SELECT TOP 10 * FROM General_Settings WITH (NOLOCK)
```

### 8.2 Check synonym target
```sql
SELECT name, base_object_name FROM sys.synonyms WHERE name = 'General_Settings'
```

### 8.3 Verify synonym resolves
```sql
SELECT OBJECT_ID('dbo.General_Settings') AS ObjectID
```
...
*Object: dbo.General_Settings | Type: Synonym | Source: UserApiDB/UserApiDB/dbo/Synonyms/dbo.General_Settings.sql*
