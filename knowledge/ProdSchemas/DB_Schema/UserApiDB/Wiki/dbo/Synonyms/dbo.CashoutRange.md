...
> Synonym pointing to [etoro].[Trade].[CashoutRange]. Cross-database pointer.

| Property | Value |
|----------|-------|
| **Schema** | dbo |
| **Object Type** | Synonym |
| **Target** | [etoro].[Trade].[CashoutRange] |
...
dbo.CashoutRange is a database synonym that provides a local alias for [etoro].[Trade].[CashoutRange]. This enables stored procedures and views in UserApiDB to reference external objects without hardcoding the full four-part name, simplifying cross-database queries and enabling environment-specific configuration.
...
|---------|---------------|-------------------|-------------|
| - | [etoro].[Trade].[CashoutRange] | Synonym target | External object this points to |
...
dbo.CashoutRange (synonym)
  +-- [etoro].[Trade].[CashoutRange] (external)
```
...
|--------|------|----------|
| [etoro].[Trade].[CashoutRange] | External | Synonym target |
...
### 8.1 Use the synonym
```sql
SELECT TOP 10 * FROM CashoutRange WITH (NOLOCK)
```

### 8.2 Check synonym target
```sql
SELECT name, base_object_name FROM sys.synonyms WHERE name = 'CashoutRange'
```

### 8.3 Verify synonym resolves
```sql
SELECT OBJECT_ID('dbo.CashoutRange') AS ObjectID
```
...
*Object: dbo.CashoutRange | Type: Synonym | Source: UserApiDB/UserApiDB/dbo/Synonyms/dbo.CashoutRange.sql*
