# Trade.GetRealEditOWMMRecovery

> MM recovery view that finds child positions where CloseOnEndOfWeek (OW) differs from the parent—overnight weekend recovery for copy-trade alignment.
...
Trade.GetRealEditOWMMRecovery identifies **copy-trade child positions whose CloseOnEndOfWeek (OW) setting is out of sync** with the leader's parent position. CloseOnEndOfWeek controls whether a position is automatically closed at the end of the week (typically Friday) to avoid overnight/weekend exposure. When a leader changes this flag, the change should propagate to all copier positions. If propagation fails, the child retains the old value while the parent has the new one—requiring MM recovery to align them.

This view exists to feed the OW (Overnight Weekend) recovery pipeline. It JOINs Trade.GetPositionData (child TGP) to Trade.Position (parent tg2) where TGP.CloseOnEndOfWeek <> tg2.CloseOnEndOfWeek. Only open child positions (IsOpened=1) with a parent (ParentPositionID>0) are considered. The view excludes positions that have already logged an OW failure (FailTypeID=9) in History.MMLog, preventing repeated recovery attempts on known failures. Mirror filter: active mirror or manual (MirrorID=0).

The output schema is identical to Trade.GetPositionData (47 columns). Each row represents a copy-trade position needing an OW edit to match the parent.

---

## 2. Business Logic

### 2.1 OW Mismatch Detection (CloseOnEndOfWeek Differs from Parent)

**What**: JOIN child to parent and compare CloseOnEndOfWeek; exclude already-failed OW positions.

**Columns/Parameters Involved**: `CloseOnEndOfWeek`, `ParentPositionID`, `PositionID`, `MirrorID`, `IsActive`, `FailTypeID`

**Rules**:
- Child (TGP): TGP.IsOpened=1, TGP.ParentPositionID>0
- Parent (tg2): tg2.PositionID = TGP.ParentPositionID AND TGP.CloseOnEndOfWeek <> tg2.CloseOnEndOfWeek
...
| 3 | ParentPositionID | bigint | YES | - | CODE-BACKED | Parent position whose CloseOnEndOfWeek differs. |
...
| 26 | CloseOnEndOfWeek | bit | YES | - | CODE-BACKED | Child's OW setting—differs from parent (mismatch). |
...
| ParentPositionID | Trade.Position | JOIN | Parent position (CloseOnEndOfWeek comparison) |
| MirrorID | Trade.Mirror | LEFT JOIN | Mirror activity filter |
| PositionID | History.MMLog | NOT EXISTS | Exclude OW fail (FailTypeID=9) |
...
```
Trade.GetRealEditOWMMRecovery (view)
...
| Trade.Position | View | Parent position for CloseOnEndOfWeek comparison |
| Trade.Mirror | Table | LEFT JOIN for active-mirror / manual filter |
| History.MMLog | Table | NOT EXISTS to exclude FailTypeID=9 (OW fail) |
...
### 8.1 Count OW recovery items

```sql
SELECT COUNT(*) AS RecoveryCount
FROM   Trade.GetRealEditOWMMRecovery WITH (NOLOCK);
```

### 8.2 OW recovery by customer
...
### 8.3 Full recovery payload for a position
...
*Object: Trade.GetRealEditOWMMRecovery | Type: View | Source: etoro/etoro/Trade/Views/Trade.GetRealEditOWMMRecovery.sql*
