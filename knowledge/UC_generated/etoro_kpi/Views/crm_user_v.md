---
object_fqn: main.etoro_kpi.crm_user_v
object_type: VIEW
...
writer:
  ...
  path: knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_user_v.sql
  source_code_snapshot: knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_user_v.sql
concept_count: 1
formula_count: 21
tier_breakdown:
  tier1_columns: 0
...
> View in `main.etoro_kpi`. 1 business concept(s) in §2; 21 of 21 columns documented from anchored evidence; 0 unverified (see sidecar).

| Property | Value |
|----------|-------|
| **UC Object** | `main.etoro_kpi.crm_user_v` |
| **Type** | VIEW |
| **Format** | n/a |
...
| **Created** | Tue May 12 07:33:01 UTC 2026 |

---

## 1. Business Meaning

`crm_user_v` is a view in `main.etoro_kpi` that composes 1 top-level filter block(s) (settled / status / dedup discriminators).
...
**Rules**:
- `Position__c = '  '`
**Evidence**: `knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_user_v.sql` L48
...
|---|---------|------|----------|-------------|
| 1 | UserId | STRING | YES | Direct passthrough from upstream. Formula: `Id`. (Tier 2 — from `main.crm.silver_crm_user`) |
...
| 10 | TimeZoneSidKeys | STRING | YES | Computed flag (CASE expression in source). Formula: `CASE WHEN ManagerId IN ('0050800000EE0zOAAT', '0050800000GyOLrAAN', '0050800000DArh6AAD') THEN 'Australia/Sydney' ELSE TimeZoneSidKey END AS T…`. (Tier 2 — from `main.crm.silver_crm_user`) |
| 11 | Manager_BO_User_ID | STRING | YES | Direct passthrough from upstream. Formula: `BO_User_ID__c`. (Tier 2 — from `main.crm.silver_crm_user`) |
...
main.crm.silver_crm_user
        │
        ▼
main.etoro_kpi.crm_user_v   ←── this object
```
