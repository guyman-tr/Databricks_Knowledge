---
object_fqn: main.etoro_kpi.crm_case_v
object_type: VIEW
...
writer:
  ...
  path: knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_case_v.sql
  source_code_snapshot: knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_case_v.sql
concept_count: 8
formula_count: 49
tier_breakdown:
  tier1_columns: 0
...
| Property | Value |
|----------|-------|
| **UC Object** | `main.etoro_kpi.crm_case_v` |
| **Type** | VIEW |
| **Format** | n/a |
...
| **Created** | Mon May 04 08:02:05 UTC 2026 |

---

## 1. Business Meaning

`crm_case_v` is a view in `main.etoro_kpi` that composes 5 CASE-based classifier flag(s) computed from upstream IDs, 2 top-level filter block(s) (settled / status / dedup discriminators).
...
**Evidence**: `knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_case_v.sql` etoro_kpi.sql L12-L15
...
**Rules**:
- `Case_Owner_Title__c <> '   '`
**Evidence**: `knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_case_v.sql` etoro_kpi.sql L107-L110
...
**Rules**:
- (no explicit predicates / pattern-only concept)
**Evidence**: `knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_case_v.sql` etoro_kpi.sql L113-L113
...
**Rules**:
- (no explicit predicates / pattern-only concept)
**Evidence**: `knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_case_v.sql` etoro_kpi.sql L114-L114
**Source(s)**: `main.crm.gold_crm_case_tiny`, `main.crm.gold_crm_web_chat_sessions`, `main.crm.gold_crm_bot_eligible_chats`
...
**Rules**:
- (no explicit predicates / pattern-only concept)
**Evidence**: `knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_case_v.sql` etoro_kpi.sql L115-L115
...
- `RN = 1`
- `IsDeEscalation = 0`
**Evidence**: `knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_case_v.sql` L47
...
- `EventType = '             '`
- `NewStatus = '      '`
**Evidence**: `knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_case_v.sql` L56
...
**Rules**:
- (no explicit predicates / pattern-only concept)
**Evidence**: `knowledge/UC_generated/etoro_kpi/_discovery/source_code/crm_case_v.sql` uc_inventory.json
...
|---|---------|------|----------|-------------|
| 1 | CaseID | STRING | YES | Direct passthrough from upstream. Formula: `Case_Id_18__c`. (Tier 2 — from `main.crm.gold_crm_case_tiny`) |
| 1 | CID | STRING | YES | Computed in source (transform kind not classified). Formula: `,CID__c`. (Tier 2 — literal) |
| 2 | CreatedDate | TIMESTAMP | YES | Computed in source (transform kind not classified). Formula: `WHERE CID__c = CID__c AND Category__c = Category__c AND CreatedDate > CreatedDate`. (Tier 2 — from `main.crm.gold_crm_case_tiny`) |
...
| `main.crm.gold_crm_case_tiny` | JOIN/UNION | `(no wiki — see `.review-needed.md`)` |
...
... (3 more upstream(s))
        │
        ▼
main.etoro_kpi.crm_case_v   ←── this object
```
