---
object_fqn: main.etoro_kpi_prep.v_revenue_fullcommission
object_type: VIEW
...
writer:
  ...
  path: knowledge/UC_generated/etoro_kpi_prep/_discovery/source_code/v_revenue_fullcommission.sql
  source_code_snapshot: knowledge/UC_generated/etoro_kpi_prep/_discovery/source_code/v_revenue_fullcommission.sql
concept_count: 2
formula_count: 15
tier_breakdown:
  tier1_columns: 9
...
| **Created** | Sun Apr 12 15:03:06 UTC 2026 |

---

## 1. Business Meaning

`v_revenue_fullcommission` is a view in `main.etoro_kpi_prep` that composes 2 CASE-based classifier flag(s) computed from upstream IDs.
...
**Evidence**: `knowledge/UC_generated/etoro_kpi_prep/_discovery/source_code/v_revenue_fullcommission.sql` etoro_kpi_prep.sql L17-L20
...
**Rules**:
- `MirrorID > 0`
**Evidence**: `knowledge/UC_generated/etoro_kpi_prep/_discovery/source_code/v_revenue_fullcommission.sql` etoro_kpi_prep.sql L21-L28
**Source(s)**: `main.dwh.gold_sql_dp_prod_we_dwh_dbo_fact_customeraction`
...
| 5 | FullCommission | DECIMAL | YES | Gross commission inclusive of hidden spread uplift at open (`/100` ingestion note). (Tier 1 — Trade.PositionTbl) |
| 6 | FullCommissionOnClose | DECIMAL | YES | Gross commission on exit — symmetrical reopen-adjust story to `CommissionOnClose`. (Tier 1 — Trade.PositionTbl) |
| 7 | FullCommissionByUnits | DECIMAL | YES | Prorated full commission for partial close. Same proration formula as CommissionByUnits applied to FullCommission. (Tier 1 — Trade.Position) |
...
| 14 | TotalFullCommission | DECIMAL | YES | Computed in source (transform kind not classified). Formula: `ELSE 0 END AS DECIMAL(38, 6) )`. (Tier 2 — literal) |
