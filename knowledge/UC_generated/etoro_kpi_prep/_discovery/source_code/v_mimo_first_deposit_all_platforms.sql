...
WITH
new_tp AS (
    SELECT fca.RealCID, fca.DepositID, CAST(NULL AS INT) AS IsCryptoToFiat,
...
new_iban AS (
    SELECT a.RealCID, a.TransactionID AS DepositID, a.IsCryptoToFiat, a.IsIBANTrade, a.IsIBANQuickTransfer, a.SourceCugTransactionID
    FROM (
        SELECT mfts.CID AS RealCID, mfts.TxStatusModificationTime, mfts.USDAmountApprox, mfts.TransactionID,
               ...
               CAST(NULL AS INT) AS IsIBANTrade, CAST(NULL AS INT) AS IsIBANQuickTransfer, mfts.SourceCugTransactionID
        ...
        WHERE mfts.MoneyMoveDirection = 'MoneyIn' AND mfts.TxStatusID = 2 AND mfts.TxTypeID IN (7, 14)
    ) a WHERE a.RN = 1
),
c2usd AS (
    SELECT CID, fbd.DepositID FROM main.dwh.gold_sql_dp_prod_we_dwh_dbo_fact_billingdeposit fbd
    WHERE fbd.IsFTD = 1 AND fbd.FundingTypeID = 27
),
dimcust AS (
    SELECT dc.RealCID,
        CASE WHEN dc.FTDPlatformID = 3 THEN ib.DepositID
             WHEN dc.FTDPlatformID = 1 THEN CAST(dc.FTDTransactionID AS BIGINT)
             ELSE NULL
        END AS DepositID,
        dc.FirstDepositDate, dc.FirstDepositAmount,
        CASE WHEN dc.FTDPlatformID = 1 THEN 'TradingPlatform'
...
remove_bad_ftds AS (
    SELECT dc.RealCID FROM main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_customer_masked dc
    WHERE CAST(dc.FirstDepositDate AS DATE) IN (
      ...
      AND dc.RealCID NOT IN (
          SELECT map.RealCID FROM main.bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_fact_mimo_allplatforms map
          WHERE map.MIMOAction = 'Deposit'
          GROUP BY map.RealCID
          HAVING COUNT(map.RealCID) > 1
      )
)
SELECT RealCID, DepositID, FirstDepositDate, FirstDepositAmount, FTDPlatform, FTDPlatformID
FROM dimcust
