...
`v_dim_ftdplatform` is a view in schema `main.etoro_kpi_prep`. It reads from 190 upstream UC object(s); the primary upstream is `main.bi_db.bronze_moneybusdb_dictionary_accounttypes`.
...
Source: `knowledge/UC_generated/etoro_kpi_prep/_discovery/source_code/v_dim_ftdplatform.sql`. Transform patterns present:

### 2.1 case
**Columns**: `Name`
**Source-code reference**: see `knowledge/UC_generated/etoro_kpi_prep/_discovery/source_code/v_dim_ftdplatform.sql` and `.lineage.md` row-level `transform` column.
