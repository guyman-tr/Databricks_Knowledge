---
object_fqn: main.etoro_kpi_prep.v_revenue_dormantfee
object_type: VIEW
...
`v_revenue_dormantfee` is a view in `main.etoro_kpi_prep` that composes a UNION ALL with sign-flipped amount legs (deposit/withdraw composition), 1 JOIN-enriched dimension lookup(s).
...
### 2.1 Sign-flip leg `main` (multiplies `DormantFee` by -1)
...
**Columns Involved**: `DormantFee`
**Rules**:
- `-1 * fca.Amount` (sign-flip on amount)
**Evidence**: `knowledge/UC_generated/etoro_kpi_prep/_discovery/source_code/v_revenue_dormantfee.sql` L12
...
**Evidence**: `knowledge/UC_generated/etoro_kpi_prep/_discovery/source_code/v_revenue_dormantfee.sql` L17
...
| 4 | DormantFee | DECIMAL | YES | Arithmetic combination of upstream columns. Formula: `-1 * Amount`. (Tier 2 — from `main.dwh.gold_sql_dp_prod_we_dwh_dbo_fact_customeraction`) |
| 5 | IsValidCustomer | INT | YES | Direct passthrough from upstream. Formula: `IsValidCustomer`. (Tier 2 — from `main.dwh.gold_sql_dp_prod_we_dwh_dbo_v_fact_snapshotcustomer_fromdateid_masked`) |
...
*Generated: 2026-05-19 | Concepts: 2 | Formulas: 6 | Tiers: 2 T1, 4 T2, 0 T3, 0 T4, 0 T5, 0 TN, 0 U | Elements: 6/6 | Source: view_definition*
