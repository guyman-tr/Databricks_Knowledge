...
| 1 | InstrumentID | INT | YES | The forex trading instrument for this currency pair (e.g., EUR/USD=1, GBP/USD=2, AUD/USD=7). References `Trade.Instrument` implicitly. Used by the exchange rate SP to retrieve current bid/ask rates for the conversion (Tier 1 — inherited from etoro.Billing.ConversionFee). |
