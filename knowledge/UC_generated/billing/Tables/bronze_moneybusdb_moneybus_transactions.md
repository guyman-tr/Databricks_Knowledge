...
| 1 | GCID | LONG | YES | Global Customer ID - identifies the user who owns this transaction. Indexed (IX_Transactions_GCID) for user-level queries. Nullable for system-generated transactions (Tier 2 — inherited from MoneyBusDB.MoneyBus.Transactions). |
...
| 8 | Amount | DECIMAL | YES | Transaction amount in the currency specified by CurrencyID. Pre-calculated by the application. Ranges from small fractional amounts to large sums (Tier 2 — inherited from MoneyBusDB.MoneyBus.Transactions). |
