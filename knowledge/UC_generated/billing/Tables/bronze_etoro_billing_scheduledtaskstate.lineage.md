# Column Lineage: main.billing.bronze_etoro_billing_scheduledtaskstate

| Property | Value |
|----------|-------|
| **UC Object** | `main.billing.bronze_etoro_billing_scheduledtaskstate` |
| **Object Type** | `EXTERNAL` |
| **Source** | (no source code snapshot — JOB-written table or fetch failed) |
| **Generated** | 2026-05-18 |

> No SQL/notebook source was cached for this object. The wiki for this object
> relies on `system.access.column_lineage` data cached under
> `_discovery/column_lineage/bronze_etoro_billing_scheduledtaskstate.json` for upstream resolution.

## Column Lineage

| # | Element | source_object | source_column | transform |
|---|---------|---------------|---------------|-----------|
| 1 | `DepositID` | `—` | `—` | `runtime_lineage` |
| 2 | `TaskID` | `—` | `—` | `runtime_lineage` |
| 3 | `TaskState` | `—` | `—` | `runtime_lineage` |
| 4 | `ReasonID` | `—` | `—` | `runtime_lineage` |
| 5 | `Created` | `—` | `—` | `runtime_lineage` |
