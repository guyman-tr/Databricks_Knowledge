---
object_fqn: main.bi_output.vg_payments_mimo_basedonddrallplatfrommimo_for_genie
object_type: VIEW
...
writer:
  ...
  path: knowledge/UC_generated/bi_output/_discovery/source_code/vg_payments_mimo_basedonddrallplatfrommimo_for_genie.sql
  source_code_snapshot: knowledge/UC_generated/bi_output/_discovery/source_code/vg_payments_mimo_basedonddrallplatfrommimo_for_genie.sql
concept_count: 1
formula_count: 19
tier_breakdown:
  tier1_columns: 0
...
# vg_payments_mimo_basedonddrallplatfrommimo_for_genie

> View in `main.bi_output`. 1 business concept(s) in §2; 19 of 19 columns documented from anchored evidence; 0 unverified (see sidecar).
...
`vg_payments_mimo_basedonddrallplatfrommimo_for_genie` is a view in `main.bi_output` that composes 1 CASE-based classifier flag(s) computed from upstream IDs.
...
- `IsTradeFromIBAN = 0`
- `IsValidCustomer = 1`
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/vg_payments_mimo_basedonddrallplatfrommimo_for_genie.sql` bi_output.sql L48-L90
...
| 1 | TransactionID | INT | YES | Direct passthrough from upstream. Formula: `aa.TransactionID`. (Tier 2 — computed in source) |
| 2 | RealCID | INT | YES | Direct passthrough from upstream. Formula: `aa.RealCID`. (Tier 2 — computed in source) |
| 3 | MarketingRegionManualName | STRING | YES | Direct passthrough from upstream. Formula: `aa.MarketingRegionManualName`. (Tier 2 — computed in source) |
| 4 | Country | STRING | YES | Direct passthrough from upstream. Formula: `Name`. (Tier 2 — from `dwh.gold_sql_dp_prod_we_dwh_dbo_dim_country`) |
| 5 | Club | STRING | YES | Direct passthrough from upstream. Formula: `Name`. (Tier 2 — from `dwh.gold_sql_dp_prod_we_dwh_dbo_dim_playerlevel`) |
| 6 | Regulation | STRING | YES | Direct passthrough from upstream. Formula: `Name`. (Tier 2 — from `dwh.gold_sql_dp_prod_we_dwh_dbo_dim_regulation`) |
| 7 | Date_MIMO | TIMESTAMP | YES | Direct passthrough from upstream. Formula: `Date`. (Tier 2 — from `bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_fact_mimo_allplatforms`) |
| 8 | EOM_MIMO | DATE | YES | Function call computed in source. Formula: `last_day(Date)`. (Tier 2 — from `bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_fact_mimo_allplatforms`) |
| 9 | MIMOPlatform | STRING | YES | Direct passthrough from upstream. Formula: `aa.MIMOPlatform`. (Tier 2 — computed in source) |
| 10 | EOM_FTD | DATE | YES | Function call computed in source. Formula: `last_day(FirstDepositDate)`. (Tier 2 — from `dwh.gold_sql_dp_prod_we_dwh_dbo_dim_country`) |
| 11 | EOM_Reg | DATE | YES | Function call computed in source. Formula: `last_day(RegisteredReal)`. (Tier 2 — from `dwh.gold_sql_dp_prod_we_dwh_dbo_dim_country`) |
| 12 | IsInternalTransfer | INT | YES | Direct passthrough from upstream. Formula: `aa.IsInternalTransfer`. (Tier 2 — computed in source) |
| 13 | IsTradeFromIBAN | INT | YES | Direct passthrough from upstream. Formula: `aa.IsTradeFromIBAN`. (Tier 2 — computed in source) |
| 14 | Currency | STRING | YES | Direct passthrough from upstream. Formula: `Abbreviation`. (Tier 2 — from `dwh.gold_sql_dp_prod_we_dwh_dbo_dim_currency`) |
| 15 | MOP | STRING | YES | Computed flag (CASE expression in source). Formula: `CASE WHEN MIMOPlatform = 'eMoney' AND IsInternalTransfer = 0 AND IsTradeFromIBAN = 0 THEN o.Fundingtype_Txtype_7 …`. (Tier 2 — from `bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_fact_mimo_allplatforms`) |
| 16 | IsFTD | INT | YES | Direct passthrough from upstream. Formula: `IsPlatformFTD`. (Tier 2 — from `bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_fact_mimo_allplatforms`) |
| 17 | AmountUSD_MIMO | DECIMAL | YES | Direct passthrough from upstream. Formula: `aa.AmountUSD`. (Tier 2 — computed in source) |
| 18 | Rank_Amount | INT | NO | Window function over upstream rows. Formula: `row_number() over ( partition by aa.RealCID order by aa.AmountUSD DESC )`. (Tier 2 — literal) |
