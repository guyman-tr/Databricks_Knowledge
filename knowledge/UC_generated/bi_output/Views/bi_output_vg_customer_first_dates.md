...
| 7 | VerificationLevelID | INT | YES | Computed in source (transform kind not classified). Formula: `,GCID ,to_date(RegisteredReal) RegistrationDate ,VerificationLevel1Date ,VerificationLevel2Date ,VerificationLevel3Date ,EmailVerifiedDate ,…`. (Tier 2 — from `main.bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_cidfirstdates_masked`, `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_customer_masked`) |
...
| 45 | AccountStatusID | INT | YES | Computed in source (transform kind not classified). Formula: `,Options_FTD_Date ,Options_FTDA ,FirstActionType ,FirstActionDate ,FirstIOBTime ,FirstTimeFunded ,FirstFundedDate ,IsFunded …`. (Tier 2 — from `main.bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_customer_daily_status`, `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_date`) |
...
| 53 | CanDeposit | BOOLEAN | YES | Computed in source (transform kind not classified). Formula: `,currentclub FirstClub ,date FirstTimeClubDate ,PlayerLevelID ,Name AS ClubTier ,RegulationID ,Name AS Regulation ,Name AS VerificationLevel…`. (Tier 2 — from `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_verificationlevel`, `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_regulation`, `main.general.gold_sql_dp_prod_we_bi_db_dbo_bi_db_clubchangelogproduct` (+2 more)) |
...
*Generated: 2026-06-19 | Concepts: 15 | Formulas: 67 | Tiers: 2 T1, 65 T2, 0 T3, 0 T4, 0 T5, 0 TN, 0 U | Elements: 67/67 | Source: view_definition*
