...
| 30 | VerificationLevelID | INT | YES | Computed in source (transform kind not classified). Formula: `,OfferID ,OfferName ,Inventorytype ,DeliveryMethod ,Type ,SubType ,Category ,StartDate ,IsEligble ,HasOffer ,CountryCriteria ,RegulationCrite…`. (Tier 2 — from `main.bi_output_stg.bi_output_customer_customer_facing_club_club_offer_eligibilty`) |
...
| 46 | IsPro | INT | NO | `IsPro` discriminator: `MifidCategorizationID IN (2,3)` → set to 1 else 0. Formula: `,ClaimedDate ,IsEligbleOnRequest ,RequestedBy ,Active ,PlayerLevelID ,Name AS ClubTier ,RegulationID ,Name AS Regulation ,VerificationLevelID ,Name AS Ve…`. (Tier 2 — from `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_verificationlevel`, `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_regulation`, `main.bi_output_stg.bi_output_customer_customer_facing_club_club_offer_eligibilty` (+2 more)) |
| 47 | AccountStatusID | INT | YES | Computed in source (transform kind not classified). Formula: `,IsEligbleOnRequest ,RequestedBy ,Active ,PlayerLevelID ,Name AS ClubTier ,RegulationID ,Name AS Regulation ,VerificationLevelID ,Name AS VerificationLevel ,…`. (Tier 2 — from `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_verificationlevel`, `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_regulation`, `main.bi_output_stg.bi_output_customer_customer_facing_club_club_offer_eligibilty` (+2 more)) |
...
| 55 | CanDeposit | BOOLEAN | YES | Computed in source (transform kind not classified). Formula: `,Name AS VerificationLevel ,CountryID ,Name AS Country ,MarketingRegionManualName AS Region ,AccountManagerID ,concat_ws(FirstName, '', LastName) AS AccountManager ,La…`. (Tier 2 — from `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_country`, `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_verificationlevel`, `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_customer_masked` (+1 more)) |
...
*Generated: 2026-06-19 | Concepts: 15 | Formulas: 64 | Tiers: 0 T1, 64 T2, 0 T3, 0 T4, 0 T5, 0 TN, 0 U | Elements: 64/64 | Source: view_definition*
