---
object_fqn: main.bi_output.bi_output_vg_customer_snapshot_v2
object_type: VIEW
...
writer:
  ...
  path: knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql
  source_code_snapshot: knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql
concept_count: 15
formula_count: 62
tier_breakdown:
  tier1_columns: 0
...
> View in `main.bi_output`. 15 business concept(s) in §2; 62 of 62 columns documented from anchored evidence; 0 unverified (see sidecar).
...
`bi_output_vg_customer_snapshot_v2` is a view in `main.bi_output` that composes 1 CASE-based classifier flag(s) computed from upstream IDs, 14 JOIN-enriched dimension lookup(s).
...
**Rules**:
- `GuruStatusID > 1`
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` bi_output.sql L31-L34
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L86
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L88
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L90
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L92
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L94,L116
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L96,L112
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L98
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L100
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L102
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L104
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L106
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L108
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L110
...
**Evidence**: `knowledge/UC_generated/bi_output/_discovery/source_code/bi_output_vg_customer_snapshot_v2.sql` L130
...
| 21 | GuruStatusID | INT | YES | Popular Investor (Guru) program status. FK to Dim_GuruStatus. (Tier 2 — inherited from Fact_SnapshotCustomer wiki) |
...
| 24 | AccountStatusID | INT | YES | Account enabled/suspended status. FK to Dim_AccountStatus. (Tier 2 — inherited from Fact_SnapshotCustomer wiki) |
...
| 26 | PlayerStatusID | INT | YES | Customer lifecycle status. FK to Dim_PlayerStatus. (Tier 2 — inherited from Fact_SnapshotCustomer wiki) |
...
| 38 | ActiveTraded | INT | YES | Direct passthrough from upstream. Formula: `ActiveTraded`. (Tier 2 — computed in source) |
| 39 | BalanceOnlyAccount | INT | YES | Direct passthrough from upstream. Formula: `BalanceOnlyAccount`. (Tier 2 — computed in source) |
| 40 | Portfolio_Only | DECIMAL | YES | Direct passthrough from upstream. Formula: `Portfolio_Only`. (Tier 2 — computed in source) |
| 41 | AccountActive | INT | YES | Direct passthrough from upstream. Formula: `AccountActive`. (Tier 2 — computed in source) |
| 42 | AccountInActive | INT | YES | Direct passthrough from upstream. Formula: `AccountInActive`. (Tier 2 — computed in source) |
| 43 | IsFunded | INT | YES | Direct passthrough from upstream. Formula: `IsFunded`. (Tier 2 — computed in source) |
| 44 | WeekNumberYear | INT | YES | Direct passthrough from upstream. Formula: `WeekNumberYear`. (Tier 2 — from `main.bi_output.bi_output_vg_date`) |
| 45 | CalendarYearMonth | STRING | YES | Direct passthrough from upstream. Formula: `CalendarYearMonth`. (Tier 2 — from `main.bi_output.bi_output_vg_date`) |
| 46 | CalendarQuarter | INT | YES | Direct passthrough from upstream. Formula: `CalendarQuarter`. (Tier 2 — from `main.bi_output.bi_output_vg_date`) |
| 47 | CalendarYear | INT | YES | Direct passthrough from upstream. Formula: `CalendarYear`. (Tier 2 — from `main.bi_output.bi_output_vg_date`) |
| 48 | IsLastDayWeek | INT | NO | Direct passthrough from upstream. Formula: `IsLastDayWeek`. (Tier 2 — from `main.bi_output.bi_output_vg_date`) |
| 49 | IsLastDayMonth | INT | NO | Direct passthrough from upstream. Formula: `IsLastDayMonth`. (Tier 2 — from `main.bi_output.bi_output_vg_date`) |
| 50 | IsLastDayQuarter | INT | NO | Direct passthrough from upstream. Formula: `IsLastDayQuarter`. (Tier 2 — from `main.bi_output.bi_output_vg_date`) |
| 51 | IsLastDayYear | INT | NO | Direct passthrough from upstream. Formula: `IsLastDayYear`. (Tier 2 — from `main.bi_output.bi_output_vg_date`) |
| 52 | CitizenshipCountryID | INT | YES | Direct passthrough from upstream. Formula: `CitizenshipCountryID`. (Tier 2 — from `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_customer_masked`) |
| 53 | CitizenshipCountry | STRING | YES | Computed in source (transform kind not classified). Formula: `Name CitizenshipCountry`. (Tier 2 — from `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_country`) |
| 54 | AffiliateID | INT | YES | Direct passthrough from upstream. Formula: `AffiliateID`. (Tier 2 — from `main.dwh.gold_sql_dp_prod_we_dwh_dbo_dim_customer_masked`) |
| 55 | ClusterDetail | STRING | YES | Direct passthrough from upstream. Formula: `cdl.ClusterDetail`. (Tier 2 — computed in source) |
| 56 | ClusterSF | STRING | YES | Direct passthrough from upstream. Formula: `cdl.ClusterSF`. (Tier 2 — computed in source) |
| 57 | IsLastCluster | INT | YES | Direct passthrough from upstream. Formula: `cdl.IsLastCluster`. (Tier 2 — computed in source) |
| 58 | IsFirstCluster | INT | YES | Direct passthrough from upstream. Formula: `cdl.IsFirstCluster`. (Tier 2 — computed in source) |
| 59 | IsSFCluster | INT | YES | Direct passthrough from upstream. Formula: `cdl.IsSFCluster`. (Tier 2 — computed in source) |
| 60 | UpdateDateIDSF | INT | YES | Direct passthrough from upstream. Formula: `cdl.UpdateDateIDSF`. (Tier 2 — computed in source) |
| 61 | ClusterDynamic | STRING | YES | Direct passthrough from upstream. Formula: `cdl.ClusterDynamic`. (Tier 2 — computed in source) |
...
| `main.bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_customer_daily_status / main.bi_output.bi_output_vg_date` | JOIN/UNION | `(no wiki — see `.review-needed.md`)` |
...
- **Wiki coverage**: 18/19 JOIN/UNION upstreams have a cached upstream wiki (see `_discovery/upstream_wikis/_index.json`)
