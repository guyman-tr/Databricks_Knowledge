...
| 28 | SettlementTypeID | int | YES | Modern settlement type from Dim_Position. Passthrough from BI_DB_PositionPnL. (Tier 1 — BI_DB_PositionPnL) |
| 29 | IsCreditReportValidCB | int | YES | 1 if customer is eligible for CreditBureau credit report validation. ETL-computed in Fact_SnapshotCustomer from PlayerLevelID, AccountTypeID, LabelID, CountryID. Passthrough from Fact_SnapshotCustomer. (Tier 1 — DWH_dbo.Fact_SnapshotCustomer) |
| 30 | IsValidCustomer | int | YES | 1 if the customer is a valid retail customer for analytics purposes. ETL-computed in Fact_SnapshotCustomer from PlayerLevelID, LabelID, CountryID. Passthrough from Fact_SnapshotCustomer. (Tier 1 — DWH_dbo.Fact_SnapshotCustomer) |
