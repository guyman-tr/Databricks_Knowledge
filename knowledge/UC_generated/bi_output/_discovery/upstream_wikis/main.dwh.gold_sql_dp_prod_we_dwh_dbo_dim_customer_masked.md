...
| 22 | CampaignID | int | Marketing campaign ID under which the customer was acquired. FK to BackOffice.Campaign. 0 for organically acquired customers (NULLs are converted to 0 by SP_Dim_Customer). |
...
| 24 | LabelID | int | Internal segment label. FK to Dictionary.Label. LabelID values 26 and 30 are excluded from IsValidCustomer logic. Default=0. IsHedged=0 is triggered by presence in BackOffice.BonusOnlyCustomers, not by LabelID. |
...
| 34 | AccountStatusID | int | Account operational status. Default=0. SP_Dim_Customer converts NULLs to 0 via ISNULL. |
...
| 53 | VerificationLevelID | int | KYC verification level. FK to Dictionary.VerificationLevel. Values: 0=unverified, 1=partial, 2=intermediate, 3=fully verified. DDL default=-1; SP_Dim_Customer converts NULLs to 0 via ISNULL. |
...
| 61 | MifidCategorizationID | int | MiFID II investor classification. FK to Dictionary.MifidCategorization. Values: 1=Retail (97.3%), 4=Eligible Counterparty (2.6%), 5=Professional (0.03%). Default=1. |
...
History.BackOfficeCustomer       →  etoro_History_BackOfficeCustomer   ─┤  14 Ext_ staging tables
STS_Audit.UserOperationsData     →  STS_Audit_UserOperationsData       ─┤       │
ContactVerification.Phone.Customer → ContactVerification_Phone_Customer ─┤       ▼
UserApiDB.Customer.Avatars       →  UserApiDB_Customer_Avatars         ─┤  SP_Dim_Customer
CustomerFinanceDB.FirstTimeDeposits → CustomerFinanceDB_...             ─┤  (JOIN+CDC+UPDATE)
ScreeningService.UserScreening   →  ScreeningService_Screening_...     ─┤       │
SalesForce_DB_Prod.IdMapTopology →  SalesForce_DB_Prod_dbo_...         ─┤       ▼
BackOffice.CustomerDocument      →  etoro_BackOffice_CustomerDocument  ─┤  Dim_Customer
Customer.CustomerStatic (ApexID) →  etoro_Customer_CustomerStatic      ─┤       │
UserApiDB.CustomerIdentification →  UserApiDB_Customer_...             ─┤  UC Override (daily)
ComplianceStateDB.StocksLending  →  ComplianceStateDB_Compliance_...   ─┘       │
                                                                           ┌────┴────┐
                                                                     masked      unmasked PII
```
...
### 6.1 Distribution & Indexing

- **HASH(RealCID)**: Queries filtering on RealCID are single-node. Fact tables distributed on CID (or RealCID) will have co-located JOINs.
- **PK NOT ENFORCED**: Synapse does not enforce the PK constraint; uniqueness is guaranteed by the ETL logic.
- **No NCI besides PK**: Only the PK NONCLUSTERED exists as secondary index.

### 6.2 PII Access

- 14 columns are masked with `FUNCTION = 'default()'`
- Masked UC table (`main.dwh.gold_...dim_customer_masked`) available for general analytics
- Unmasked table (`main.pii_data.gold_...dim_customer`) requires PII access privileges

### 6.3 Common JOIN Pattern

```sql
SELECT dc.RealCID, dc.CountryID, dc.RegulationID, dc.PlayerLevelID, ...
FROM DWH_dbo.Dim_Customer dc
JOIN DWH_dbo.Fact_SnapshotCustomer fsc ON dc.RealCID = fsc.RealCID
WHERE dc.IsValidCustomer = 1
```

### 6.4 DEFAULT Values

Many FK columns default to 0 (not NULL). When JOINing to dimension tables, use `LEFT JOIN` or filter `WHERE column > 0` to avoid ghost dimension rows for ID=0.

## 7. Sample Queries

```sql
-- Customer details with key dimensions
SELECT dc.RealCID, dc.CountryID, dc.RegulationID, dc.PlayerLevelID,
       dc.IsValidCustomer, dc.FirstDepositDate, dc.AccountTypeID
FROM DWH_dbo.Dim_Customer dc
WHERE dc.IsValidCustomer = 1;

-- Depositor analysis by country and regulation
SELECT dc.CountryID, dc.RegulationID,
       COUNT(*) AS customers,
       SUM(CASE WHEN dc.IsDepositor = 1 THEN 1 ELSE 0 END) AS depositors
FROM DWH_dbo.Dim_Customer dc
WHERE dc.IsValidCustomer = 1
GROUP BY dc.CountryID, dc.RegulationID
...
|--------|-----------------|
| Business & Regulatory Undertakings Monitoring Platform | JOINs Dim_Customer on CID=RealCID for country, regulation, and status filtering |
| Temporary Tables in Synapse | Uses Dim_Customer as reference example for HASH distribution optimization |

---
*Generated: 2026-03-19 | Quality: 9.0/10*
*Tiers: 68 T1, 39 T2, 0 T3, 0 T4, 0 T5 | Phases: 1,5,7,8,9,9B,10,10.5,13,11*
*Object: DWH_dbo.Dim_Customer | Type: Table*
