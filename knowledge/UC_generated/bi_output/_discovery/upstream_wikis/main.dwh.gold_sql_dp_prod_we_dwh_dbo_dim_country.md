...
| 5 | IsHighRiskCountry | tinyint | YES | AML/compliance risk flag. 0=standard risk, 1=high risk. Recomputed by SP from RiskGroupID: CASE WHEN RiskGroupID IN (0,4) THEN 0 ELSE 1 END — countries with RiskGroupID 1, 2, or 3 are flagged high-risk (14 countries per upstream Dictionary.Country). Triggers enhanced due diligence. |
| 6 | Region | varchar(50) | NO | Marketing region label for this country. Loaded from Dictionary.MarketingRegion.Name via JOIN on MarketingRegionID. NOT the geographic region from Dictionary.Region. Up to 21 distinct values (e.g., "ROW", "Africa", "French", "Arabic"). Used for marketing campaign grouping. |
