...
|---|---------|------|----------|-------------|
| 1 | Country | varchar(50) | YES | Full country name in English. Unique per row. Used in UI dropdowns, compliance documents, and analytical reports. Passthrough from Dim_Country (c.Name AS Country). CountryID=0 excluded by SP WHERE clause. (Tier 1 — Dictionary.Country) |
| 2 | CountryID | int | YES | Primary key. 0=Not available (fallback/placeholder for users whose country cannot be determined), 1-250=countries ordered roughly alphabetically by ISO code. Referenced by Dim_Customer, Fact_BillingDeposit, Dim_CountryBin, V_Dim_Customer. CountryID=0 excluded by SP WHERE clause. Distribution key for this table. (Tier 1 — Dictionary.Country) |
...
| 8 | AllowedUserTagType | nvarchar(50) | YES | Tag scope that won the restriction-weight priority resolution for the allowedUser resource. Possible values: Default, Country, CountryAndRegion. Currently Default=51,642 (100% of rows). (Tier 2 — SP_EXW_WalletElligibleCountries) |
...
| 12 | FromTagType | nvarchar(50) | YES | Tag scope that won priority resolution for the allowedConvertFrom resource. Values: Default=51,477, Country=165. (Tier 2 — SP_EXW_WalletElligibleCountries) |
...
| 16 | ToTagType | nvarchar(50) | YES | Tag scope that won priority resolution for the allowedConvertTo resource. Values: Default=51,574, Country=68. (Tier 2 — SP_EXW_WalletElligibleCountries) |
...
| 19 | FromConversionAllowed | int | YES | ETL-computed flag: 0=conversion from this crypto not allowed, 1=conversion from this crypto allowed. Derived as CASE WHEN AllowedUserSelectedValue='true' AND FromSelectedValue='true' THEN 1 ELSE 0 END. Currently 0=51,642 (100% — all conversions blocked because AllowedUserSelectedValue is universally 'false'). (Tier 2 — SP_EXW_WalletElligibleCountries) |
| 20 | ToConversionAllowed | int | YES | ETL-computed flag: 0=conversion to this crypto not allowed, 1=conversion to this crypto allowed. Derived as CASE WHEN AllowedUserSelectedValue='true' AND ToSelectedValue='true' THEN 1 ELSE 0 END. Currently 0=51,642 (100% — all conversions blocked because AllowedUserSelectedValue is universally 'false'). (Tier 2 — SP_EXW_WalletElligibleCountries) |
