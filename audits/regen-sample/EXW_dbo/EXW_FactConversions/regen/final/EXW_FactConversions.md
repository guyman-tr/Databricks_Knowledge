...
- **Wallet.Conversions** — the base conversion record (Id, wallets, requested amounts, crypto types, correlation ID, occurred timestamp).
- **Wallet.ConversionTransactions** — the per-leg details (actual amounts, fees, addresses, timestamps) for both the From-leg and To-leg.
...
### 2.5 Requested vs Actual Amounts (Dropped ConversionTypeId Context)

**What**: The upstream `Wallet.Conversions` table has a `ConversionTypeId` column (1=FixedFrom, 2=FixedTo) that determines whether the user specified the sell amount or buy amount. This column was dropped during the ETL denormalization into EXW_FactConversions.

**Columns Involved**: RequestedFromAmount, RequestedToAmount, FromAmount, ToAmount

**Rules**:
- `RequestedFromAmount` and `RequestedToAmount` come from `Wallet.Conversions.FromAmount` and `Wallet.Conversions.ToAmount` — these are the user's original requested amounts at conversion initiation.
- `FromAmount` and `ToAmount` come from `Wallet.ConversionTransactions.Amount` for the respective legs — these are the actual executed amounts after micro-fee deduction and market execution.
- When ConversionTypeId=1 (FixedFrom), the user fixed the sell amount: RequestedFromAmount is nearly exact to FromAmount (differing only by micro-fee ~0.00005). The ToAmount may differ significantly from RequestedToAmount due to market price movement.
- When ConversionTypeId=2 (FixedTo), the user fixed the buy amount: RequestedToAmount is nearly exact to ToAmount. The FromAmount may differ from RequestedFromAmount.
- All recent conversions used FixedFrom (type 1). Without ConversionTypeId in this table, the distinction must be inferred from comparing requested vs actual amount differences.
...
- **RequestedFromAmount vs FromAmount**: RequestedFromAmount is from Wallet.Conversions.FromAmount (user's original request); FromAmount is from Wallet.ConversionTransactions.Amount for the From-leg (actual executed amount after micro-fee deduction). Typical difference is ~0.00005 for FixedFrom conversions.
- **RequestedToAmount vs ToAmount**: Same pattern — RequestedToAmount is the requested buy amount from Conversions; ToAmount is the actual executed amount from ConversionTransactions. Difference can be large (up to ~1,259 units) due to market price movement.
- **Missing ConversionTypeId**: The upstream Wallet.Conversions has ConversionTypeId (1=FixedFrom, 2=FixedTo) which was dropped during ETL. See Section 2.5 for details.
...
| 5 | FromAddress | nvarchar(512) | YES | Destination blockchain address for this conversion leg. NULL when the transfer is internal. Mapped to the From-leg of the conversion (sourced from ConversionTransactions.ToAddress of the From-leg record). (Tier 1 — Wallet.ConversionTransactions) |
...
| 7 | RequestedFromAmount | numeric(38,8) | YES | Amount of source crypto being sold. In native units of FromCryptoId. This is the user's original requested sell amount at conversion initiation; differs from FromAmount which reflects the actual executed amount after micro-fee deduction. (Tier 1 — Wallet.Conversions) |
...
| 12 | FromAmount | numeric(38,8) | YES | Amount of crypto for this conversion leg in native units. From-leg actual executed amount after micro-fee deduction. NULL for failed conversions. (Tier 1 — Wallet.ConversionTransactions) |
...
| 14 | ToEtoroDate | datetime | YES | Timestamp of this transaction record creation. To-leg of the conversion. (Tier 1 — Wallet.ConversionTransactions) |
...
| 19 | RequestedToAmount | numeric(38,8) | YES | Amount of destination crypto being purchased. In native units of ToCryptoId. This is the user's original requested buy amount at conversion initiation; differs from ToAmount which reflects the actual executed amount. (Tier 1 — Wallet.Conversions) |
...
| 22 | ToAmount | numeric(38,8) | YES | Amount of crypto for this conversion leg in native units. To-leg actual executed amount. NULL for failed conversions. (Tier 1 — Wallet.ConversionTransactions) |
...
| 24 | FromEtoroDate | datetime | YES | Timestamp of this transaction record creation. From-leg of the conversion. (Tier 1 — Wallet.ConversionTransactions) |
...
| 29 | SentToEtoroWalletAmount | numeric(38,8) | YES | Amount of crypto for this conversion leg in native units. To-leg amount sent to the destination wallet. (Tier 1 — Wallet.ConversionTransactions) |
...
| 32 | SentFromEtoroWalletAmount | numeric(38,8) | YES | Amount of crypto for this conversion leg in native units. From-leg amount sent from the source wallet. (Tier 1 — Wallet.ConversionTransactions) |
...
| FromAddress | Wallet.ConversionTransactions | ToAddress | From-leg record, rename |
| RequestedFromAmount | Wallet.Conversions | FromAmount | Rename (original user-requested sell amount) |
| FromAmount | Wallet.ConversionTransactions | Amount | From-leg actual executed amount |
| FromCryptoID | Wallet.Conversions | FromCryptoId | Passthrough |
| ToWalletId | Wallet.Conversions | ToWalletId | Passthrough |
| RequestedToAmount | Wallet.Conversions | ToAmount | Rename (original user-requested buy amount) |
| ToAmount | Wallet.ConversionTransactions | Amount | To-leg actual executed amount |
...
| ToEtoroDate | Wallet.ConversionTransactions | Occurred | To-leg timestamp |
| FromEtoroDate | Wallet.ConversionTransactions | Occurred | From-leg timestamp |
...
WalletDB.Wallet.Conversions (50K rows, production)
  ├── Id, CorrelationId, Occurred, FromWalletId, ToWalletId,
  │   FromAmount (→RequestedFromAmount), ToAmount (→RequestedToAmount),
  │   FromCryptoId, ToCryptoId
  │   [ConversionTypeId dropped — 1=FixedFrom, 2=FixedTo]
  │
WalletDB.Wallet.ConversionTransactions (per-leg details)
  ├── From-leg record → FromAmount, FromEtoroDate, FromAddress, SentFromEtoro* columns
  ├── To-leg record → ToAmount, ToEtoroDate, ToAddress, SentToEtoro* columns
  │
EXW_Wallet.CryptoTypes (lookup)
  ├── FromCryptoID → FromCrypto, FromBlockchainCryptoId/Name
  ├── ToCryptoID → ToCrypto, ToBlockchainCryptoId/Name
  │
External enrichment: wallet → GCID mapping → SendingGCID, RecievingGCID
  |-- External pipeline (denormalized join + lookups + GCID mapping) ---|
  v
...
*Generated: 2026-04-27 | Quality: 8.5/10 (Elements: 10/10, Logic: 9/10, Relationships: 7/10, Sources: 7/10)*
*Tiers: 22 T1, 8 T2, 16 T3, 0 T4 | Elements: 46/46, Logic: 5/10, Relationships: 7/10, Sources: 7/10*
*Phases: 11/11*
