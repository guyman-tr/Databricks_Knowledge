...
- **Attempt**: `2`
- **Output directory** (relative to repo root): `audits/regen-sample/EXW_dbo/EXW_FactConversions/regen/attempt_2/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\EXW_dbo\EXW_FactConversions\regen\attempt_2`
...
1. [high] `ToEtoroDate, FromEtoroDate` — Upstream ConversionTransactions.Occurred says 'Timestamp of this transaction record creation.' Wiki rewrites as 'Timestamp of the To/From-leg transaction record creation.' — verbatim quoting rule violated.
2. [high] `SentToEtoroWalletAmount, SentFromEtoroWalletAmount` — Upstream ConversionTransactions.Amount says 'Amount of crypto for this conversion leg in native units.' Wiki rewrites as 'Amount of crypto for the To/From-leg conversion in native units' and appends an extra sentence — verbatim quoting rule violated.
3. [medium] `FromAddress (Element #5)` — Tagged Tier 2 (external pipeline) but lineage traces it to Wallet.ConversionTransactions.ToAddress which has an upstream wiki in the bundle. Should be Tier 1 with the upstream description verbatim, or Tier 2 citing ConversionTransactions rather than 'external pipeline'.
4. [medium] `Section 5.1 lineage — RequestedFromAmount/FromAmount` — Both RequestedFromAmount and FromAmount are mapped to Wallet.Conversions.FromAmount in the lineage table. If they come from the same upstream column, the wiki's claim that one is 'original request' and the other is 'actual after micro-fee deduction' contradicts the single-source mapping.
5. [low] `Section 2 — missing ConversionTypeId context` — Upstream Wallet.Conversions has ConversionTypeId (1=FixedFrom, 2=FixedTo) which determines whether FromAmount or ToAmount was user-specified. This column was dropped during ETL but the business logic implications are not documented.

Tier 1 paraphrasing failures (must be fixed verbatim):

- **ToEtoroDate**:
  - Upstream: `Timestamp of this transaction record creation.`
  - You wrote: `Timestamp of the To-leg transaction record creation.`
  - Loss: Rewrote 'this' as 'the To-leg' — verbatim text altered
- **SentToEtoroWalletAmount**:
  - Upstream: `Amount of crypto for this conversion leg in native units.`
  - You wrote: `Amount of crypto for the To-leg conversion in native units. The actual amount sent to the destination wallet.`
  - Loss: Rewrote 'this conversion leg' as 'the To-leg conversion'; appended extra sentence
- **SentFromEtoroWalletAmount**:
  - Upstream: `Amount of crypto for this conversion leg in native units.`
  - You wrote: `Amount of crypto for the From-leg conversion in native units. The actual amount sent from the source wallet.`
  - Loss: Same rewording pattern as SentToEtoroWalletAmount
- **FromEtoroDate**:
  - Upstream: `Timestamp of this transaction record creation.`
  - You wrote: `Timestamp of the From-leg transaction record creation.`
