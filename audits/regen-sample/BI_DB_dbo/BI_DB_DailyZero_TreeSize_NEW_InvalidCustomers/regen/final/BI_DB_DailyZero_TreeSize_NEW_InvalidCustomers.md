...
| 33 | IsValidCustomer | int | YES | 1 if the customer is a valid retail customer for analytics purposes. ETL-computed from PlayerLevelID, LabelID, CountryID. Approx 98% of current rows = 1 in Fact_SnapshotCustomer. Always 0 in this table (SP filters WHERE IsValidCustomer=0). (Tier 1 — DWH_dbo.Fact_SnapshotCustomer) |
| 34 | IsCreditReportValidCB | int | YES | 1 if customer is eligible for CreditBureau credit report validation. ETL-computed. Passthrough from Fact_SnapshotCustomer. (Tier 1 — DWH_dbo.Fact_SnapshotCustomer) |
