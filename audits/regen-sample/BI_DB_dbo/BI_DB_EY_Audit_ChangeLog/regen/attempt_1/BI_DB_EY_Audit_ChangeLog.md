...
The table contains approximately 86 million rows spanning from 2023-01-01 to 2025-10-27. ChangeTypeID 12 (amount adjustments) accounts for 99.98% of rows (~86M); ChangeTypeID 13 accounts for the remaining ~13K rows. Buy-side positions (IsBuy=1) dominate at ~98.5% in recent data.
...
- ChangeTypeID=13: Purpose not officially documented; accounts for ~13K rows total. Likely settlement-related based on the IsSettled backfill logic applied only to this type.
...
- **ChangeTypeID is almost exclusively 12**: 99.98% of rows are ChangeTypeID=12. ChangeTypeID=13 rows are rare (~13K total). Do not assume even distribution.
...
|---|---------|------|----------|-------------|
| 1 | PositionID | bigint | YES | Primary key. Allocated by Internal.GetPositionID_Bigint. Unique per position. (Tier 1 — Trade.PositionTbl) |
...
| 4 | OccurredDateID | int | YES | ETL-computed YYYYMMDD int from Occurred. Always filter on this for performance. (Tier 1 — Dim_PositionChangeLog) |
...
| 13 | PreviousAmountInUnits | numeric(16,6) | YES | Unit count (shares/coins) before this change. DWH note: for ChangeTypeID=13, backfilled from UnitsOpenStartOfDay when NULL. (Tier 1 — Dim_PositionChangeLog) |
