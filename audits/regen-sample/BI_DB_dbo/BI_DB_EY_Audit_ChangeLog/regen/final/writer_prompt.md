...
- **Attempt**: `2`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_EY_Audit_ChangeLog/regen/attempt_2/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_EY_Audit_ChangeLog\regen\attempt_2`
- **Bundle path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_EY_Audit_ChangeLog\regen\_upstream_bundle.md`
- **DDL path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Tables\BI_DB_dbo.BI_DB_EY_Audit_ChangeLog.sql`
...
2. [high] `CID` — Tagged Tier 1 — Trade.PositionTbl with Dim_Position wiki description, but SP sources CID from dpcl.CID (Dim_PositionChangeLog). Should be Tier 1 — Dim_PositionChangeLog with that wiki's description: 'Customer ID who owns the position. Nullable (some system positions may not have CID).'
3. [medium] `PreviousAmountInUnits` — Upstream Dim_PositionChangeLog description 'Unit count (shares/coins) before this change. Added for futures/unit-based positions.' was replaced with SP transform-specific backfill note, losing the upstream context about futures/unit-based origin.
4. [medium] `OccurredDateID` — Dropped 'Clustered index key.' from the verbatim Dim_PositionChangeLog upstream description.
5. [low] `Property Table` — Missing UC Target row in property table. Should include UC Target or explicitly mark N/A.

Tier 1 paraphrasing failures (must be fixed verbatim):

- **PositionID**:
  - Upstream: `FK to Dim_Position.PositionID. Distribution key -- co-located with Dim_Position for efficient JOINs. (Tier 2 - SP_Dim_PositionChangeLog_DL_To_Synapse)`
  - You wrote: `Primary key. Allocated by Internal.GetPositionID_Bigint. Unique per position. (Tier 1 — Trade.PositionTbl)`
...
- **CID**:
  - Upstream: `Customer ID who owns the position. Nullable (some system positions may not have CID). (Tier 2 - SP_Dim_PositionChangeLog_DL_To_Synapse)`
  - You wrote: `Customer ID. References Customer.Customer. (Tier 1 — Trade.PositionTbl)`
  - Loss: Wrong upstream — used Dim_Position wiki. Lost 'who owns the position' context and nullable caveat for system positions.
- **PreviousAmountInUnits**:
  - Upstream: `Unit count (shares/coins) before this change. Added for futures/unit-based positions.`
  - You wrote: `Unit count (shares/coins) before this change. DWH note: for ChangeTypeID=13, backfilled from UnitsOpenStartOfDay when NULL.`
