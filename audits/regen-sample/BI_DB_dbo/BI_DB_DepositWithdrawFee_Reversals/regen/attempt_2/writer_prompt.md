...
- **Attempt**: `2`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_DepositWithdrawFee_Reversals/regen/attempt_2/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_DepositWithdrawFee_Reversals\regen\attempt_2`
- **Bundle path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_DepositWithdrawFee_Reversals\regen\_upstream_bundle.md`
- **DDL path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Tables\BI_DB_dbo.BI_DB_DepositWithdrawFee_Reversals.sql`
...
# Pre-Resolved Upstream Bundle for `BI_DB_dbo.BI_DB_DepositWithdrawFee_Reversals`

This bundle was assembled deterministically by the regen harness BEFORE the writer claude process started. Use this as your AUTHORITATIVE Tier 1 inheritance source. Quote descriptions VERBATIM from the upstream wikis below for any column that is a passthrough or rename of an upstream column. Do NOT paraphrase. Do NOT generalize vendor names. Do NOT drop NULL semantics.

---

## Source DDL — `BI_DB_dbo.BI_DB_DepositWithdrawFee_Reversals.sql`

```sql
CREATE TABLE [BI_DB_dbo].[BI_DB_DepositWithdrawFee_Reversals]
(
	[DateID] [int] NULL,
	[CID] [int] NULL,
	...
	[IsIBANTrade] [int] NULL,
	[MIDName] [varchar](200) NULL,
	[GuruStatus] [varchar](200) NULL,
	[PreviousTransactionStatus] [varchar](200) NULL,
	[TransactionStatus] [varchar](200) NULL,
	[DepositID] [int] NULL,
	[WithdrawPaymentID] [int] NULL,
	[CreditID] [bigint] NULL,
	[ExchangeFeePercentage] [decimal](20, 5) NULL
)
WITH
(
...
### Upstream `DWH_dbo.Fact_Cashout_State` — synapse
- **Resolved as**: `DWH_dbo.Fact_Cashout_State`
- **Wiki path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\knowledge\synapse\Wiki\DWH_dbo\Tables\Fact_Cashout_State.md`
...
### Upstream `DWH_dbo.Fact_Deposit_State` — synapse
- **Resolved as**: `DWH_dbo.Fact_Deposit_State`
- **Wiki path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\knowledge\synapse\Wiki\DWH_dbo\Tables\Fact_Deposit_State.md`
...
### SP `BI_DB_dbo.SP_DepositWithdrawFee`
- **Path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Stored Procedures\BI_DB_dbo.SP_DepositWithdrawFee.sql`
...
GO
CREATE PROC [BI_DB_dbo].[SP_DepositWithdrawFee] @StartDate [Date] AS
...
Author:      Guy M
Date:        2025-02-06
Description: replaces the old depositwithdraw calculations with output from RnD
...
20250206		Guy M		#SR-299957		completely new proc replacing all the old logic with output from RnD feed
20250507		Guy M		#SR-313302		replaced the unnecessary and misleading credittypeid data with nulls
20250602		Guy M		#SR-xxxxxx		after finding out Fact_BillingWithdraw has ~200 dupe, first deduplicate that data
20250602		Guy M		#SR-xxxxxx		changed all the amounts to ABS() - this will override any RnD logics and they will be redone in the proc at the end
20250727		Guy M		#SR-324538		on 2025-07-27 we switched the old proc entirely with the parallel proc using RnD PIPS data. the old proc for audit trail sits
											here: SP_DepositWithdrawFee_Before_2025_05_For_Audit. the old data sits here: BI_DB_DepositWithdrawFee_JUNK.
20250727		Guy M		#SR-324538		during the switch i made an error to replace @StartDate param with @date param - switching back.
20250728		Guy M		#SR-324697		when rerunning data later than next day, some withdraws disappear in the dedupe stage because Fact_BillingWithdraw is a state
											table not a history table - changed the where clause to always look at the cashout_state population.
20250810		Guy M		#SR-328549		added credit id from cashout state - needed for final validation on the transaction type sign logic :(
20260304		Markos Ch	#SR-359957      Add ExchangeFeePercentage

*****************************************************************************************************************************************************************/
-- -- exec [BI_DB_dbo].[SP_DepositWithdrawFee] '20260325'


/*******Log Start********/
exec BI_DB_dbo.[SP_ProcessStatusLog] 'SP_DepositWithdrawFee_2025', 'Start', @StartDate
/************************/

 -- DECLARE @StartDate DATE = '20250814'
DECLARE @StartDateID INT = CONVERT(VARCHAR(8), @StartDate, 112)

-- dedupe the fact table (has dupes on BankName field)

IF OBJECT_ID('tempdb..#fbwDeduped') IS NOT NULL DROP TABLE #fbwDeduped
CREATE TABLE #fbwDeduped
    WITH (HEAP,DISTRIBUTION = HASH(WithdrawPaymentID))
AS
SELECT distinct
	  fbw.WithdrawPaymentID
	, fbw.FundingTypeID_Funding
	, fbw.ModificationDateID
	, fbw.CardTypeIDAsInteger
	, fbw.BinCountryIDAsInteger
	, fbw.CardCategory
	, fbw.FlowID
FROM DWH_dbo.Fact_BillingWithdraw fbw
WHERE fbw.WithdrawPaymentID IN (SELECT fcs.WPID FROM DWH_dbo.Fact_Cashout_State fcs WHERE fcs.ModificationDateID = @StartDateID)


IF OBJECT_ID('tempdb..#withdraws') IS NOT NULL DROP TABLE #withdraws
CREATE TABLE #withdraws
    WITH (HEAP,DISTRIBUTION = ROUND_ROBIN)
as
SELECT @StartDateID AS DateID
		, fcs.CID
		, fcs.WithdrawID AS DepositWithdrawID
		, fcs.ModificationDate as Occurred
		, null AS [CreditTypeID]
		, cast(fcs.WPID AS VARCHAR (50)) + 'W' AS [TransactionID]
		, CAST(fcs.ModificationDate AS DATE) AS [Date]
		, CAST(dc.ExternalID AS varchar(50)) AS Customer
		, fcs.TransactionType
		, dft.Name as PaymentMethod
		, ABS(fcs.Amount) [Amount]
		, dc1.Abbreviation AS [Currency] -- select * from DWH_dbo.Dim_Currency
		, fcs.ExchangeRate
		, ABS(fcs.AmountInUSD) as [AmountUSD]
		, fsc.RegulationID
		, fsc.[LabelID]
		, fsc.PlayerLevelID
		...
		, fcs.[BaseExchangeRate]
		, fcs.ExchangeFee
		, fcs.ExTransactionID as [ExternalTransactionID]
		, dbd.Name as [Depot]
		, fcs.MID as [MIDValue]
		, dpl.Name as [Club]
		, dps.Name as [PlayerStatus]
		, ABS(ISNULL(fcs.PIPsInUSD,0)) as [PIPsCalculation]
		, dc2.Name as RegCountry
		, dc3.Name as RegCountryByIP
		, dct.CarTypeName as CardType
		, fbw.CardCategory
		, dc4.Name as BinCountry
		, NULL AS MOPCountry
		, NULL AS IsGermanBaFin
		, CASE WHEN fbw.FlowID = 2 THEN 1 ELSE 0 END AS IsIBANTrade
		, fcs.MIDName
		, dgs.GuruStatusName AS GuruStatus
		, fcs.PreviousStatus AS PreviousTransactionStatus
		, fcs.CashoutStatus AS TransactionStatus
		, NULL AS DepositID
		, fbw.WithdrawPaymentID
		, fcs.CreditID,
		fcs.ExchaFeeInPercentage AS ExchangeFeePercentage
FROM DWH_dbo.Fact_Cashout_State fcs
	JOIN DWH_dbo.Dim_Customer dc
		ON fcs.CID = dc.RealCID
	JOIN #fbwDeduped fbw
		ON fcs.WPID = fbw.WithdrawPaymentID
	LEFT JOIN DWH_dbo.Dim_FundingType dft
		ON fbw.FundingTypeID_Funding = dft.FundingTypeID
	JOIN DWH_dbo.Dim_Currency dc1
		ON fcs.CurrencyID = dc1.CurrencyID
	JOIN DWH_dbo.Fact_SnapshotCustomer fsc
		ON dc.RealCID = fsc.RealCID
	JOIN DWH_dbo.Dim_Range dr
		ON fsc.DateRangeID = dr.DateRangeID AND fbw.ModificationDateID BETWEEN dr.FromDateID AND dr.ToDateID
	JOIN DWH_dbo.Dim_Regulation dr1
		ON fsc.RegulationID = dr1.DWHRegulationID
	LEFT JOIN DWH_dbo.Dim_Label dl
		ON fsc.LabelID = dl.LabelID
	LEFT JOIN DWH_dbo.Dim_BillingDepot dbd
		ON fcs.DepotID = dbd.DepotID
	JOIN DWH_dbo.Dim_PlayerLevel dpl
	...
	LEFT JOIN DWH_dbo.Dim_Country dc3
		ON dc.CountryIDByIP = dc3.CountryID
	LEFT JOIN DWH_dbo.Dim_CardType dct
		ON fbw.CardTypeIDAsInteger = dct.CardTypeID
	LEFT JOIN DWH_dbo.Dim_Country dc4
		ON fbw.BinCountryIDAsInteger = dc4.CountryID
	LEFT JOIN DWH_dbo.Dim_GuruStatus dgs
		ON fsc.GuruStatusID = dgs.GuruStatusID
WHERE fcs.TransactionType = 'Withdraw'
AND fcs.ModificationDateID = @StartDateID

--DECLARE @StartDate DATE = '20250814'
--DECLARE @StartDateID INT = CONVERT(VARCHAR(8), @StartDate, 112)

IF OBJECT_ID('tempdb..#withdrawsrollbacks') IS NOT NULL DROP TABLE #withdrawsrollbacks
CREATE TABLE #withdrawsrollbacks
    WITH (HEAP,DISTRIBUTION = ROUND_ROBIN)
as
SELECT @StartDateID AS DateID
		, fcs.CID
...
SELECT @StartDateID AS DateID
		, fds.[CID]
		, fds.DepositID AS DepositWithdrawID
		, fds.ModificationDate as Occurred
		, NULL as [CreditTypeID]
		, CAST(fds.DepositID AS VARCHAR(50)) + 'D' AS [TransactionID]
		, CAST(fds.ModificationDate AS DATE) AS [Date]
		, CAST(dc.[ExternalID] AS VARCHAR(50)) AS Customer
		, fds.TransactionType
		, dft.Name as PaymentMethod
		, ABS(fds.Amount) Amount
		, dc1.Abbreviation as [Currency]
		, fds.ExchangeRate
		, ABS(fds.AmountInUSD) as [AmountUSD]
		, fsc.RegulationID
		, fsc.[LabelID]
		, fsc.[PlayerLevelID]
		, dr1.Name as Regulation
		...
		, fsc.IsValidCustomer
		, GETDATE() AS [UpdateDate]
		, fds.BaseExchangeRate
		, fds.[ExchangeFee]
		, fds.ExTransactionID as [ExternalTransactionID]
		, dbd.Name as [Depot]
		, fds.MID as [MIDValue]
		, dpl.Name as [Club]
		, dps.Name as [PlayerStatus]
		, ABS(ISNULL(fds.PIPsInUSD,0)) as [PIPsCalculation]
		, dc2.Name as RegCountry
		, dc3.Name as RegCountryByIP
		, fds.CardType
		, fds.CardCategory
		...
		, NULL AS MOPCountry
		, NULL AS IsGermanBaFin
		, CASE WHEN fbd.FlowID = 1 THEN 1 ELSE 0 END as IsIBANTrade
		, fds.MIDName
		, dgs.GuruStatusName AS GuruStatus
		, fds.PreviousStatus AS PreviousTransactionStatus
		, fds.DepositStatus AS TransactionStatus
		, fds.DepositID
		, NULL AS WithdrawPaymentID
		, fds.CreditID
		, fds.FeeInPercentage AS ExchangeFeePercentage
FROM DWH_dbo.Fact_Deposit_State fds
	JOIN DWH_dbo.Dim_Customer dc
		ON fds.CID = dc.RealCID
	...
	LEFT JOIN DWH_dbo.Dim_Currency dc1
		ON fds.CurrencyID = dc1.CurrencyID
	JOIN DWH_dbo.Fact_SnapshotCustomer fsc
		ON fsc.RealCID = fds.CID
	JOIN DWH_dbo.Dim_Range dr
		ON fsc.DateRangeID = dr.DateRangeID AND fds.ModificationDateID BETWEEN dr.FromDateID AND dr.ToDateID
	JOIN DWH_dbo.Dim_Regulation dr1
		ON fsc.RegulationID = dr1.DWHRegulationID
	LEFT JOIN DWH_dbo.Dim_Label dl
		ON dc.LabelID = dl.LabelID
	LEFT JOIN DWH_dbo.Dim_BillingDepot dbd
		ON fds.DepotID = dbd.DepotID
	JOIN DWH_dbo.Dim_PlayerLevel dpl
		ON fsc.PlayerLevelID = dpl.PlayerLevelID
	JOIN DWH_dbo.Dim_PlayerStatus dps
		ON fsc.PlayerStatusID = dps.PlayerStatusID
	...
	LEFT JOIN DWH_dbo.Dim_Country dc3
		ON dc.CountryIDByIP = dc3.CountryID
	LEFT JOIN DWH_dbo.Dim_Country dc4
		ON fbd.BinCountryIDAsInteger = dc4.CountryID
	LEFT JOIN DWH_dbo.Dim_GuruStatus dgs
		ON fsc.GuruStatusID = dgs.GuruStatusID
WHERE fds.TransactionType = 'Deposit'
AND fds.ModificationDateID = @StartDateID

--DECLARE @StartDate DATE = '20250814'
--DECLARE @StartDateID INT = CONVERT(VARCHAR(8), @StartDate, 112)

IF OBJECT_ID('tempdb..#depositsRevesals') IS NOT NULL DROP TABLE #depositsRevesals -- select * from #depositsRevesals
CREATE TABLE #depositsRevesals
    WITH (HEAP,DISTRIBUTION = ROUND_ROBIN)
as
SELECT @StartDateID AS DateID
		, fds.[CID]
...
DELETE FROM  BI_DB_dbo.BI_DB_DepositWithdrawFee WHERE DateID = @StartDateID

INSERT INTO BI_DB_dbo.BI_DB_DepositWithdrawFee
	(
	   DateID
	 , CID
	 , DepositWithdrawID
...
SELECT d.DateID
	 ...
	 , d.MIDName
	 , d.GuruStatus
	 , d.PreviousTransactionStatus
	 , d.TransactionStatus
	 , d.DepositID
	 , d.WithdrawPaymentID
	 , d.CreditID
	 , d.ExchangeFeePercentage
FROM #deposits d -- select * from #deposits
UNION ALL
SELECT w.DateID
	 , w.CID
	 , w.DepositWithdrawID
	 ...
	 , w.MIDName
	 , w.GuruStatus
	 , w.PreviousTransactionStatus
	 , w.TransactionStatus
	 , w.DepositID
	 , w.WithdrawPaymentID
	 , w.CreditID
	 , w.ExchangeFeePercentage
FROM #withdraws w -- select * from #withdraws


DELETE FROM BI_DB_dbo.BI_DB_DepositWithdrawFee_Reversals WHERE DateID = @StartDateID;

INSERT INTO BI_DB_dbo.BI_DB_DepositWithdrawFee_Reversals
    (
       DateID
     , CID
     , DepositWithdrawID
...
SELECT w.DateID
	 ...
	 , w.CreditID
	 , w.ExchangeFeePercentage
FROM #withdrawsrollbacks w;


IF OBJECT_ID('tempdb..#amountDirections') IS NOT NULL DROP TABLE #amountDirections

CREATE TABLE #amountDirections
...
INSERT INTO #amountDirections VALUES ('Withdraw', -1)
INSERT INTO #amountDirections VALUES ('Deposit',  1)
INSERT INTO #amountDirections VALUES ('CancelledChargeback', 1)
INSERT INTO #amountDirections VALUES ('CancelledChargebackReversal', -1)
INSERT INTO #amountDirections VALUES ('CancelledRefund', 1)
INSERT INTO #amountDirections VALUES ('CancelledRefundReversal', -1)
INSERT INTO #amountDirections VALUES ('CancelledReversedDeposit', 1)
INSERT INTO #amountDirections VALUES ('Chargeback', -1)
INSERT INTO #amountDirections VALUES ('ChargebackReversal', 1)
INSERT INTO #amountDirections VALUES ('Refund', -1)
INSERT INTO #amountDirections VALUES ('ReverseDeposit', -1)
INSERT INTO #amountDirections VALUES ('ReversedDeposit', -1)
INSERT INTO #amountDirections VALUES ('CashoutRollback', 1)
INSERT INTO #amountDirections VALUES ('CancelledCashoutRollback', -1)

--DECLARE @StartDate DATE = '20250507'
--DECLARE @StartDateID INT = CONVERT(VARCHAR(8), @StartDate, 112)

 UPDATE  t1
 SET
 t1.Amount = t2.Direction *  t1.Amount
 , t1.AmountUSD = t2.Direction *  t1.AmountUSD
 , t1.PIPsCalculation = CASE WHEN t1.TransactionType = 'Withdraw' THEN t1.PIPsCalculation else t2.Direction *  t1.PIPsCalculation end
 FROM BI_DB_dbo.BI_DB_DepositWithdrawFee t1
 INNER JOIN #amountDirections t2
	 ON t1.TransactionType = t2.TransactionType
WHERE t1.DateID = @StartDateID

 UPDATE  t1
 SET
 t1.Amount = t2.Direction *  t1.Amount
 , t1.AmountUSD = t2.Direction *  t1.AmountUSD
 , t1.PIPsCalculation = t2.Direction *  t1.PIPsCalculation
 FROM BI_DB_dbo.BI_DB_DepositWithdrawFee_Reversals t1
 INNER JOIN #amountDirections t2
	 ON t1.TransactionType = t2.TransactionType
WHERE t1.DateID = @StartDateID

-- last heuristic :( :( - join to history credit for ops failures which produce indistinguishable opposite +/- situations


 UPDATE  t1
 SET
 --t1.Amount = -1 * t1.Amount
 --, t1.AmountUSD = -1 * t1.AmountUSD
 t1.PIPsCalculation = -1 * t1.PIPsCalculation
 FROM BI_DB_dbo.BI_DB_DepositWithdrawFee t1
 INNER JOIN DWH_dbo.Fact_CustomerAction t2
	 ON t1.DateID = t2.DateID and t1.CreditID = t2.CreditID
WHERE t1.DateID = @StartDateID
AND t1.TransactionType IN ('CashoutRollback', 'CancelledCashoutRollback')
...
-- fix specific ops human mistake edge cases
 ...
 INNER JOIN DWH_dbo.Fact_CustomerAction t2
	 ON t1.DateID = t2.DateID and t1.CreditID = t2.CreditID
WHERE t1.DateID = @StartDateID
AND t1.TransactionType IN ('CancelledChargebackReversal') AND t2.Amount = -1 * t1.AmountUSD
...
|---|---|---|---|---|
| `BI_DB_dbo.SP_DepositWithdrawFee` | synapse_sp | BI_DB_dbo | SP_DepositWithdrawFee | `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Stored Procedures\BI_DB_dbo.SP_DepositWithdrawFee.sql` |
...
| `DWH_dbo.Fact_Cashout_State` | synapse | DWH_dbo | Fact_Cashout_State | `C:\Users\guyman\Documents\github\Databricks_Knowledge\knowledge\synapse\Wiki\DWH_dbo\Tables\Fact_Cashout_State.md` |
...
| `DWH_dbo.Fact_Deposit_State` | synapse | DWH_dbo | Fact_Deposit_State | `C:\Users\guyman\Documents\github\Databricks_Knowledge\knowledge\synapse\Wiki\DWH_dbo\Tables\Fact_Deposit_State.md` |
...
Previous attempt scored **3.65** (FAIL). The adversarial judge required regeneration with the following specific fixes:

> Re-run with: (1) Discard entire Elements section and rebuild from DDL. Real 45 columns are: DateID, CID, DepositWithdrawID, Occurred, CreditTypeID, TransactionID, Date, Customer, TransactionType, PaymentMethod, Amount, Currency, ExchangeRate, AmountUSD, RegulationID, LabelID, PlayerLevelID, Regulation, Label, IsValidCustomer, UpdateDate, BaseExchangeRate, ExchangeFee, ExternalTransactionID, Depot, MIDValue, Club, PlayerStatus, PIPsCalculation, RegCountry, RegCountryByIP, CardType, CardCategory, BinCountry, MOPCountry, IsGermanBaFin, IsIBANTrade, MIDName, GuruStatus, PreviousTransactionStatus, TransactionStatus, DepositID, WithdrawPaymentID, CreditID, ExchangeFeePercentage. (2) For every Tier 1 column quote upstream wiki verbatim — no paraphrasing whatsoever. (3) Tag Club as Tier 1 (etoro.PlayerLevel.PlayerLevel) with verbatim: 'Tier display name: Bronze, Silver, Gold, Platinum, Platinum Plus, Diamond, Internal, N/A.' (4) Tag Currency as Tier 1 with verbatim from Dim_Currency: 'Ticker symbol. USD, EUR for forex; AAPL.US, TSLA.US for US stocks; BTC for crypto. Unique across all instruments.' (5) Document PIPsCalculation as Tier 2: ABS(ISNULL(PIPsInUSD,0)) at insert, then signed via #amountDirections and Fact_CustomerAction edge-case UPDATEs. (6) Rewrite Business Logic to reference only actual DDL columns. (7) Document RegCountryByIP (Tier 1 — etoro.Country.Country) and BinCountry (Tier 1 — etoro.Country.Country) using verbatim country name description.

Top issues from the judge:
1. [critical] `Section 4 — Elements (phantom columns)` — Wiki documents 18 columns that do not exist in the DDL: Fee, FeeUSD, SpreadFee, SpreadFeeUSD, ExchangeFeeUSD, RegistrationCountry, RegistrationRegulation, CustomerType, AccountType, IsRealAccount, IsFTD, IsDepositor, PlayerStatusID, GuruStatusID, BillingDepotID, FundingTypeID, CountryID, InsertDate. The SP code and DDL confirm none of these are columns in BI_DB_DepositWithdrawFee_Reversals. The writer appears to have confused this table with a more enriched internal version.
2. [critical] `Section 4 — Elements (missing DDL columns)` — 17 real DDL columns are completely absent from the wiki: DepositWithdrawID, Occurred, Date, Customer, ExchangeRate, BaseExchangeRate, ExternalTransactionID, MIDValue, Club, PIPsCalculation, RegCountryByIP, BinCountry, IsIBANTrade, MIDName, PreviousTransactionStatus, WithdrawPaymentID, CreditID. Analytically important columns like PIPsCalculation, MIDValue, RegCountryByIP, and BinCountry are entirely undocumented.
3. [high] `Club` — DDL has [Club] [varchar](200) which holds the player loyalty tier name (Dim_PlayerLevel.Name). The writer neither documented this column nor found its upstream. Instead, a non-existent 'PlayerLevel' varchar column was invented in its place. The correct Tier 1 source is etoro.PlayerLevel.PlayerLevel.
4. [high] `Section 2 — Business Logic (sections 2.2 and 2.3)` — Business Logic sections describe behavior for phantom columns. Section 2.2 ('Amount Sign Correction') lists Fee, FeeUSD, SpreadFee, SpreadFeeUSD, ExchangeFee, ExchangeFeeUSD as amount columns corrected by #amountDirections — Fee/FeeUSD/SpreadFee/SpreadFeeUSD/ExchangeFeeUSD do not exist in the DDL. Section 2.3 describes RegistrationCountry, RegCountry, Label, PlayerLevel, PlayerStatus, GuruStatus, IsFTD, IsDepositor, CustomerType, AccountType, IsRealAccount sourced from Fact_SnapshotCustomer/Dim_Customer — only RegCountry, Label, PlayerStatus, GuruStatus are real columns.
5. [high] `Currency` — Description says 'Currency abbreviation resolved from Dim_Currency.Abbreviation via CurrencyID' — misses that Dim_Currency is the universal instrument registry (15.7K rows covering stocks, ETFs, crypto, forex, commodities), not a currency-only table. The Abbreviation field is the ticker symbol (e.g., 'AAPL.US', 'BTC', 'USD'). Upstream wiki is explicit: 'Despite its name, this is the universal instrument registry.' Verbatim quote required.

Tier 1 paraphrasing failures (must be fixed verbatim):

- **PaymentMethod**:
  - Upstream: `Payment method name (e.g., CreditCard, Wire, PayPal, Skrill, Neteller, ApplePay, GooglePay). (Tier 1 — Dictionary.FundingType)`
  - You wrote: `Payment method name resolved from Dim_FundingType.Name via FundingTypeID.`
  - Loss: Drops all specific method examples (PayPal, Skrill, Neteller, ApplePay, GooglePay)
- **Currency**:
  - Upstream: `Ticker symbol. 'USD', 'EUR' for forex; 'AAPL.US', 'TSLA.US' for US stocks (format: TICKER.EXCHANGE); 'BTC' for crypto. Unique across all instruments. Use this for human-readable instrument identificat`
  - You wrote: `Currency abbreviation resolved from Dim_Currency.Abbreviation via CurrencyID.`
  - Loss: Drops entire instrument-registry context. Dim_Currency is not just currencies — it is the universal instrument registry (stocks, ETFs, crypto, forex). 'Abbreviation' is the ticker symbol field.
- **Regulation**:
  - Upstream: `Short code for the regulation. Used in V_Dim_Customer and analytics dashboards. Values match production Dictionary.Regulation.Name.`
  - You wrote: `Regulatory jurisdiction name resolved from Dim_Regulation.Name via point-in-time RegulationID.`
  - Loss: Drops 'short code' nuance; adds spurious 'point-in-time' qualifier (this column is not point-in-time in the DDL, it comes from the SP join not Fact_SnapshotCustomer)
- **RegCountry**:
  - Upstream: `Full country name in English. Unique per row. Used in UI dropdowns, compliance documents, and analytical reports.`
  - You wrote: `Regulatory country name resolved from Dim_Country.Name via point-in-time CountryID.`
  - Loss: Adds spurious 'point-in-time' qualifier; drops 'Unique per row' and usage context
- **Label**:
  - Upstream: `Brand name displayed in BackOffice interfaces, reports, and internal systems. Multiple LabelIDs can share the same Name (e.g., 0, 1, 9 all = 'eToro').`
  - You wrote: `White-label brand name resolved from Dim_Label.Name via point-in-time LabelID.`
  - Loss: Drops BackOffice context; drops multiple-IDs-same-name note; adds spurious 'point-in-time' qualifier
- **Depot**:
  - Upstream: `Human-readable depot name (e.g., 'MoneyBookers USD', 'Neteller', 'Wire'). Unique across all depots. Used in admin dashboards, routing logs, and discrepancy reports.`
  - You wrote: `Payment gateway endpoint name resolved from Dim_BillingDepot.Name via BillingDepotID.`
  - Loss: Drops all examples; drops 'Unique across all depots'; drops usage context (admin dashboards, routing logs)
- **GuruStatus**:
  - Upstream: `Human-readable PI tier name. Values: No, Certified, Cadet, Rising Star, Champion, Elite, Elite Pro, Removed, Rejected.`
  - You wrote: `Popular Investor programme status resolved from Dim_GuruStatus.Name.`
  - Loss: Drops all 9 distinct values
- **PlayerStatus**:
  ...
  - You wrote: `Account restriction state resolved from Dim_PlayerStatus.Name.`
  - Loss: Drops RTRIM() trailing-spaces gotcha; drops usage context
