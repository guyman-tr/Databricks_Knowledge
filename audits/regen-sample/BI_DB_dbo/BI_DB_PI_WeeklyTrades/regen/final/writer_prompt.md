...
- **Attempt**: `2`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_PI_WeeklyTrades/regen/attempt_2/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_PI_WeeklyTrades\regen\attempt_2`
- **Bundle path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_PI_WeeklyTrades\regen\_upstream_bundle.md`
- **DDL path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Tables\BI_DB_dbo.BI_DB_PI_WeeklyTrades.sql`
...
# Pre-Resolved Upstream Bundle for `BI_DB_dbo.BI_DB_PI_WeeklyTrades`

This bundle was assembled deterministically by the regen harness BEFORE the writer claude process started. Use this as your AUTHORITATIVE Tier 1 inheritance source. Quote descriptions VERBATIM from the upstream wikis below for any column that is a passthrough or rename of an upstream column. Do NOT paraphrase. Do NOT generalize vendor names. Do NOT drop NULL semantics.

---

## Source DDL — `BI_DB_dbo.BI_DB_PI_WeeklyTrades.sql`

```sql
CREATE TABLE [BI_DB_dbo].[BI_DB_PI_WeeklyTrades]
(
	[CID] [int] NOT NULL,
	[Week1] [int] NULL,
	[Year1] [int] NULL,
	[NewTrades] [int] NOT NULL,
...
Found 17 upstream wiki(s). Read EACH one in full.
...
### Upstream `BI_DB_dbo.BI_DB_PI_Positions` — synapse
- **Resolved as**: `BI_DB_dbo.BI_DB_PI_Positions`
- **Wiki path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\knowledge\synapse\Wiki\BI_DB_dbo\Tables\BI_DB_PI_Positions.md`
...
| `BI_DB_dbo.BI_DB_PI_Positions` | synapse | BI_DB_dbo | BI_DB_PI_Positions | `C:\Users\guyman\Documents\github\Databricks_Knowledge\knowledge\synapse\Wiki\BI_DB_dbo\Tables\BI_DB_PI_Positions.md` |
...
Previous attempt scored **6.0** (FAIL). The adversarial judge required regeneration with the following specific fixes:
...
1. [high] `Week1, Year1, NewTrades, FirstDayOfWeek` — All four passthrough columns from BI_DB_CID_WeeklyPanel_FullData are tagged Tier 2 but should be Tier 1. The WeeklyPanel wiki was available in the bundle and documents SSWeekNumberOfYear (#3), CalendarYear (#4), NewTrades_Total (#64), and FirstDayOfWeek (#28) with specific descriptions.
2. [high] `Week1, Year1, NewTrades, FirstDayOfWeek` — Upstream WeeklyPanel descriptions were not quoted verbatim. Writer paraphrased all four columns instead of inheriting the documented descriptions. E.g., WeeklyPanel says 'Total positions opened across all instrument types during the week. SUM.' for NewTrades_Total but wiki says 'Total number of new trades (positions opened) by this PI during the calendar week.'
3. [medium] `NewTrades` — Wiki claims 'Includes manual and copy positions, excludes AirDrop' and 'Range: 0 to 8,400 observed' — neither claim is present in the WeeklyPanel upstream wiki for NewTrades_Total. The 'excludes AirDrop' semantic applies to ActiveOpen flags, not the NewTrades_Total count column. These may be fabricated data claims.
4. [medium] `Section 1` — Section 1 states '~4,419 distinct CIDs across 225 distinct weeks' confidently, but review-needed sidecar confirms DMV row count query failed (permission denied). Numbers should be flagged as estimates, not presented as observed facts.
5. [low] `Footer` — Footer tier breakdown '1 T1, 5 T2' is consistent with the wiki's (incorrect) tier assignments but should be '5 T1, 1 T2' after correcting the tier misclassifications.
