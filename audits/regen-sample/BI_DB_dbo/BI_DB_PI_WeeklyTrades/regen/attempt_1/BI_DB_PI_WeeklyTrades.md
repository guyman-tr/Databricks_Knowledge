...
The table holds ~4,419 distinct CIDs across 225 distinct weeks, covering 2019-12-29 through 2024-04-14. Each row records the total number of new trades (`NewTrades_Total` from WeeklyPanel) opened by a PI during a given calendar week, identified by `FirstDayOfWeek` (Sunday of the target week).
...
HASH(CID) distributed — co-located JOINs with other HASH(CID) tables (e.g., BI_DB_CID_WeeklyPanel_FullData). CLUSTERED INDEX on CID supports efficient per-PI lookups. ~3,220 rows per weekly slice; total table size is modest.
...
- **NewTrades includes all trade types**: Sourced from `NewTrades_Total` in WeeklyPanel, which counts all new positions (manual + copy, excludes AirDrop).
...
- **NewTrades range**: 0 to 8,400 observed. High values reflect CopyFund accounts or very active PIs with automated strategies.
...
| Tier | Meaning |
|------|---------|
| Tier 1 | Upstream wiki verbatim (Dim_Customer, Customer.CustomerStatic) |
| Tier 2 | Passthrough from BI_DB_CID_WeeklyPanel_FullData or ETL-computed |
...
| 2 | Week1 | int | YES | SQL Server week-of-year number (1-53) for the target week. Passthrough from BI_DB_CID_WeeklyPanel_FullData.SSWeekNumberOfYear, which originates from DWH_dbo.Dim_Date. (Tier 2 — BI_DB_CID_WeeklyPanel_FullData) |
| 3 | Year1 | int | YES | Calendar year of the week (e.g., 2024). Passthrough from BI_DB_CID_WeeklyPanel_FullData.CalendarYear, which originates from DWH_dbo.Dim_Date. (Tier 2 — BI_DB_CID_WeeklyPanel_FullData) |
| 4 | NewTrades | int | NO | Total number of new trades (positions opened) by this PI during the calendar week. Passthrough from BI_DB_CID_WeeklyPanel_FullData.NewTrades_Total. Includes manual and copy positions, excludes AirDrop. Range: 0 to 8,400 observed. Used to compute Avg_weekly_trades in the PI Dashboard via AVG(NewTrades) over the last 52 weeks. (Tier 2 — BI_DB_CID_WeeklyPanel_FullData) |
...
| 6 | FirstDayOfWeek | date | YES | Sunday date marking the start of the calendar week. Passthrough from BI_DB_CID_WeeklyPanel_FullData.FirstDayOfWeek. Used as DELETE+INSERT key for daily incremental refresh. Primary grain column alongside CID. (Tier 2 — BI_DB_CID_WeeklyPanel_FullData) |
...
| Week1 | BI_DB_dbo.BI_DB_CID_WeeklyPanel_FullData | SSWeekNumberOfYear | Passthrough |
| Year1 | BI_DB_dbo.BI_DB_CID_WeeklyPanel_FullData | CalendarYear | Passthrough |
| NewTrades | BI_DB_dbo.BI_DB_CID_WeeklyPanel_FullData | NewTrades_Total | Passthrough |
...
*Tiers: 1 T1, 5 T2, 0 T3, 0 T4, 0 T5 | Elements: 6/6, Logic: 8/10, Relationships: 7/10, Sources: 8/10*
