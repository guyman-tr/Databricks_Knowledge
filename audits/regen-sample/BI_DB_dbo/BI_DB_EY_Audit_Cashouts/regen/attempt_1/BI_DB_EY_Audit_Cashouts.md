...
| 8 | Occurred | datetime | NO | UTC timestamp when the action occurred. For credits: when the credit was recorded. (Tier 1 — source-dependent) |
| 9 | Amount | decimal(11,2) | NO | Cashout/refund/chargeback amount in USD. Passthrough from Fact_CustomerAction.Amount for cashout-related ActionTypeIDs. (Tier 1 — History.Credit) |
...
| 13 | CardType | varchar(50) | YES | Card brand name. 0=None, 1=Visa, 2=Master Card, 3=Diners, 4=Amex, 5=Fire Pay, 6=JCB, 7=American Express, 8=Maestro. Resolved from Fact_BillingWithdraw.CardTypeIDAsInteger via Dim_CardType.CarTypeName. NULL for non-card payment methods. (Tier 1 — Dictionary.CardType) |
...
| 15 | IsCreditReportValidCB | int | YES | 1 if customer is eligible for CreditBureau credit report validation. ETL-computed in Fact_SnapshotCustomer from PlayerLevelID, AccountTypeID, LabelID, CountryID. Passthrough from point-in-time snapshot. (Tier 2 — Fact_SnapshotCustomer) |
| 16 | BaseExchangeRate | varchar(50) | YES | Base FX rate from BI_DB_DepositWithdrawFee, originally sourced from Fact_Cashout_State. Stored as varchar despite numeric origin. NULL when no matching DepositWithdrawFee row exists. (Tier 2 — Fact_Cashout_State) |
| 17 | ExchangeFee | varchar(50) | YES | Exchange fee from BI_DB_DepositWithdrawFee, originally sourced from Fact_Cashout_State. Stored as varchar despite numeric origin. NULL when no matching DepositWithdrawFee row exists. (Tier 2 — Fact_Cashout_State) |
...
*Tiers: 8 T1, 11 T2, 0 T3, 0 T4, 0 T5 | Elements: 19/19, Logic: 9/10, Relationships: 7/10, Sources: 9/10*
