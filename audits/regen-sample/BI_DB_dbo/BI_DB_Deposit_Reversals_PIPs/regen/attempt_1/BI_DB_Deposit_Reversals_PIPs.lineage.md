...
| TransactionType | ETL-computed | DepositStatus + PreviousStatus | Complex CASE matrix mapping (current, previous) → transaction type name |
| PaymentMethod | Dim_FundingType | Name | Dim-lookup passthrough via FundingID → FundingPaymentDetails → Dim_FundingType |
...
| ExchangeRate | External_etoro_Billing_DepositRollbackTracking / Fact_BillingDeposit | ExchangeRate | Passthrough |
...
| MIDValue | ETL-computed | MID | Complex CASE: FundingTypeID=2 → BPMSValue; DepotID IN (78,79,80,4,75,86) → maName; ELSE COALESCE chain |
| Club | Dim_PlayerLevel | Name | Dim-lookup passthrough |
| PlayerStatus | Dim_PlayerStatus | Name | Dim-lookup passthrough |
| PIPsCalculation | BI_DB_DepositWithdrawFee + ETL-computed | PIPsCalculation × PIPsRatio | ROUND(DW_fee.PIPsCalculation, 2) * ROUND(RollbackAmountInCurrency / Amount, 32) |
...
| Entity | ETL-computed | MIDName | Complex CASE: FundingTypeID=2 → BPMSDescription; DepotID IN (78,79,80,4,75,86) → maBODescription; ELSE COALESCE chain |
