...
| Tier | Meaning |
|------|---------|
| Tier 1 | Upstream wiki verbatim (Dim_Position via Trade.PositionTbl) |
| Tier 2 | ETL-computed in Dim_Position SP or this SP |
...
| 5 | Amount | money | NOT NULL | Position size in currency. Must be >= 0. Stored in dollars. Synced via UPDATE when value changes in Dim_Position. Passthrough from Dim_Position. (Tier 1 — Trade.PositionTbl) |
...
| 7 | OpenOccurred | datetime | NOT NULL | When position was persisted (mapped from Occurred in production). Passthrough from Dim_Position. (Tier 1 — Trade.PositionTbl) |
...
| 12 | OpenDateID | int | NOT NULL | ETL-computed date int (YYYYMMDD) derived from OpenOccurred. Used as DELETE+INSERT key for daily incremental refresh. Passthrough from Dim_Position. (Tier 2 — Dim_Position) |
| 13 | CloseDateID | int | YES | ETL-computed date int (YYYYMMDD) derived from CloseOccurred. 0 = still open, YYYYMMDD = closed. Synced via UPDATE from Dim_Position. (Tier 2 — Dim_Position) |
| 14 | Volume | int | YES | ETL-computed approximation of USD value: ROUND(AmountInUnitsDecimal * InitForexRate * USD conversion, 0). Passthrough from Dim_Position. (Tier 2 — Dim_Position) |
| 15 | FullCommissionOnCloseOrig | money | YES | Original FullCommissionOnClose before reopen adjustments. ETL: CASE WHEN ReopenForPositionID IS NOT NULL THEN CommissionOnClose ELSE 0. Synced via UPDATE from Dim_Position. (Tier 2 — Dim_Position) |
| 16 | IsSettled | int | YES | 1 = real asset, 0 = CFD asset. Determines settlement type. Passthrough from Dim_Position. (Tier 2 — Dim_Position) |
| 17 | FullCommissionByUnits | decimal(38,6) | YES | Prorated full commission for partial close. Formula: (AmountInUnitsDecimal / InitialUnits) * FullCommission. Synced via UPDATE from Dim_Position. (Tier 2 — Dim_Position) |
...
*Tiers: 11 T1, 7 T2, 0 T3, 0 T4, 0 T5 | Elements: 18/18, Logic: 9/10, Relationships: 8/10, Sources: 9/10*
