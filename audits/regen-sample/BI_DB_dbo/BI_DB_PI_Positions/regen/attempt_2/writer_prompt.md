...
- **Attempt**: `2`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_PI_Positions/regen/attempt_2/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_PI_Positions\regen\attempt_2`
- **Bundle path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_PI_Positions\regen\_upstream_bundle.md`
- **DDL path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Tables\BI_DB_dbo.BI_DB_PI_Positions.sql`
...
Previous attempt scored **6.7** (FAIL). The adversarial judge required regeneration with the following specific fixes:
...
1. [high] `OpenDateID, CloseDateID, Volume, FullCommissionOnCloseOrig, IsSettled, FullCommissionByUnits` — 6 columns tagged Tier 2 — Dim_Position but are direct SELECT dp.X FROM Dim_Position dp passthroughs with an upstream wiki available. Per passthrough rule, all should be Tier 1 with the origin from the Dim_Position wiki.
2. [high] `FullCommissionByUnits` — Dim_Position wiki explicitly marks this column as '(Tier 1 — Trade.Position)'. Writer downgraded to '(Tier 2 — Dim_Position)', losing the production origin Trade.Position entirely. Clear missed Tier 1 inheritance.
3. [medium] `Amount` — Upstream says 'Stored in dollars (PositionOpen divides by 100 from cents)'. Wiki drops the cents conversion detail, replacing with 'Stored in dollars. Synced via UPDATE when value changes in Dim_Position.' Paraphrasing failure on a Tier 1 column.
4. [medium] `IsSettled` — Dim_Position wiki says '(Tier 5 — Expert Review)' signaling low upstream confidence. Writer replaced with '(Tier 2 — Dim_Position)' which misrepresents the upstream's own confidence level. The review-needed sidecar correctly flags this but the wiki itself should carry the Tier 5 designation.
5. [medium] `OpenOccurred` — Upstream says 'Default getutcdate()'. Wiki drops this, losing the default value behavior documentation.

Tier 1 paraphrasing failures (must be fixed verbatim):

- **Amount**:
  - Upstream: `Position size in currency. Must be >= 0. Stored in dollars (PositionOpen divides by 100 from cents).`
  - You wrote: `Position size in currency. Must be >= 0. Stored in dollars. Synced via UPDATE when value changes in Dim_Position. Passthrough from Dim_Position.`
  - Loss: Dropped '(PositionOpen divides by 100 from cents)' — production-layer cents-to-dollars conversion detail lost
- **OpenOccurred**:
  - Upstream: `When position was persisted (mapped from Occurred in production). Default getutcdate().`
  - You wrote: `When position was persisted (mapped from Occurred in production). Passthrough from Dim_Position.`
