...
- **Attempt**: `1`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_EY_Audit_CashoutFees/regen/attempt_1/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_EY_Audit_CashoutFees\regen\attempt_1`
- **Bundle path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_EY_Audit_CashoutFees\regen\_upstream_bundle.md`
- **DDL path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Tables\BI_DB_dbo.BI_DB_EY_Audit_CashoutFees.sql`
...
# Pre-Resolved Upstream Bundle for `BI_DB_dbo.BI_DB_EY_Audit_CashoutFees`

This bundle was assembled deterministically by the regen harness BEFORE the writer claude process started. Use this as your AUTHORITATIVE Tier 1 inheritance source. Quote descriptions VERBATIM from the upstream wikis below for any column that is a passthrough or rename of an upstream column. Do NOT paraphrase. Do NOT generalize vendor names. Do NOT drop NULL semantics.

---

## Source DDL — `BI_DB_dbo.BI_DB_EY_Audit_CashoutFees.sql`

```sql
CREATE TABLE [BI_DB_dbo].[BI_DB_EY_Audit_CashoutFees]
(
	[DateID] [int] NULL,
	[RealCID] [int] NULL,
	...
	[PopularInvestors] [varchar](100) NULL,
...
### SP `BI_DB_dbo.SP_EY_Audit_CashoutFees`
- **Path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Stored Procedures\BI_DB_dbo.SP_EY_Audit_CashoutFees.sql`
...
GO
CREATE PROC [BI_DB_dbo].[SP_EY_Audit_CashoutFees] @date [date] AS
...
-- exec BI_DB_dbo.SP_EY_Audit_CashoutFees '20200513'

BEGIN

--DECLARE @date DATE = cast (getdate()-1 as date)
DECLARE @sdate DATE = @date
...
DECLARE @edateID int =CAST(CONVERT(VARCHAR(8), @edate, 112) AS INT)
		...
		FROM BI_DB_dbo.BI_DB_EY_Audit_CashoutFees ; --- !!!!! MAKE SURE CORRECT Table !!!!!
		...
		BEGIN
		    ...
		    BEGIN
		        ...
		        EXEC BI_DB_dbo.SP_EY_Audit_CashoutFees @date = @DateToProcess; ---- !!!!! MAKE SURE CORRECT SP is RUN with correct @VARIABLE !!!! !!!!!
...
IF OBJECT_ID('tempdb..#cashoutfees') IS NOT NULL DROP TABLE #cashoutfees
CREATE TABLE #cashoutfees
...
SELECT
  ...
  , dgs.GuruStatusName AS PopularInvestors
  , 'CashOut' AS Category
  , -1*SUM (ca.Commission) AS Commission
  , GETDATE() AS UpdateDate
FROM DWH_dbo.Fact_CustomerAction ca
		INNER JOIN DWH_dbo.Fact_SnapshotCustomer f
			ON ca.RealCID=f.RealCID
		INNER JOIN DWH_dbo.Dim_PlayerLevel dpl
			ON f.PlayerLevelID=dpl.PlayerLevelID
		INNER JOIN DWH_dbo.Dim_Regulation dr1
			ON f.RegulationID=dr1.DWHRegulationID
		INNER JOIN DWH_dbo.Dim_AccountType dat
			ON f.AccountTypeID=dat.AccountTypeID
		INNER JOIN DWH_dbo.Dim_Country dc
			ON f.CountryID=dc.CountryID
		INNER JOIN DWH_dbo.Dim_GuruStatus dgs
			ON f.GuruStatusID=dgs.GuruStatusID
		INNER JOIN DWH_dbo.Dim_Range DR
			ON f.DateRangeID=DR.DateRangeID AND ca.DateID BETWEEN DR.FromDateID AND DR.ToDateID
WHERE
	f.IsCreditReportValidCB = 1
	AND ca.ActionTypeID IN (30)
	AND ca.IsRedeem=0
	AND ca.DateID BETWEEN @sdateID AND @edateID
GROUP BY
	ca.WithdrawID
  , ca.RealCID
  , ca.Occurred
...
DELETE FROM BI_DB_dbo.BI_DB_EY_Audit_CashoutFees WHERE DateID = @sdateID
INSERT INTO BI_DB_dbo.BI_DB_EY_Audit_CashoutFees (
[DateID]
,[RealCID]
,[WithdrawID]
,[Occurred]
...
|---|---|---|---|---|
| `BI_DB_dbo.SP_EY_Audit_CashoutFees` | synapse_sp | BI_DB_dbo | SP_EY_Audit_CashoutFees | `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Stored Procedures\BI_DB_dbo.SP_EY_Audit_CashoutFees.sql` |
