...
- **Attempt**: `2`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_PaymentSent_Results/regen/attempt_2/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_PaymentSent_Results\regen\attempt_2`
- **Bundle path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_PaymentSent_Results\regen\_upstream_bundle.md`
- **DDL path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Tables\BI_DB_dbo.BI_DB_PaymentSent_Results.sql`
...
1. [high] `CID` — Tagged Tier 1 — Customer.CustomerStatic but CID is selected from fbd (External_etoro_Billing_Withdraw), not from Dim_Customer. The SP joins Dim_Customer only for RegulationID. Writer borrowed the Dim_Customer.RealCID description to dress up a Tier 2 column. Should be Tier 2 — External_etoro_Billing_Withdraw.
2. [high] `Section 2.2 / Section 3.4 (CAD threshold)` — Wiki claims CAD has no explicit aging threshold and 'may appear regardless of age'. In reality, the #final WHERE clause requires matching one of four OR-branches (USD/EUR, AUD+CySEC, AUD+non-CySEC, GBP). CAD matches NONE of them, so CAD rows are always excluded from the final output. The wiki gives analysts a false expectation that CAD rows can appear.
3. [high] `Section 1 (summary)` — Summary says 'Filtered to wire transfers in USD, EUR, GBP, AUD, CAD only' but CAD is effectively excluded by the #final WHERE clause. The effective output covers only 4 currencies.
4. [low] `Regulation` — Upstream Dim_Regulation.Name says 'Values match production Dictionary.Regulation.Name' — wiki drops this line and substitutes dim-lookup context. Minor semantic loss.
5. [low] `Currency (element #3)` — Currency has exactly 5 values in #cashouts (4 effective in output) but the element description does not include inline key=value pairs mapping ProcessCurrencyID to abbreviation (1=USD, 2=EUR, 3=GBP, 5=AUD, 7=CAD).

Tier 1 paraphrasing failures (must be fixed verbatim):

- **CID**:
  - Upstream: `Customer ID - platform-internal primary key. Assigned at registration. Unique within etoro DB. Used as the universal customer identifier across all tables. (Tier 1 — Customer.CustomerStatic)`
  - You wrote: `Customer ID - platform-internal primary key. Assigned at registration. Unique within etoro DB. Used as the universal customer identifier across all tables. Passthrough from External_etoro_Billing_With`
