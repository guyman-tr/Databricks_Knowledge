...
> Operational monitoring table tracking wire withdrawal requests stuck in "Payment Sent" status beyond currency-specific aging thresholds. Rebuilt daily via TRUNCATE+INSERT by SP_H_PaymentSent_Results from production billing and history external tables. Currently 0 rows (empty when no cashouts exceed the aging rules). Filtered to wire transfers (FundingTypeID=2) in 4 effective currencies (USD, EUR, GBP, AUD) with a 4-week lookback window. Note: CAD (ProcessCurrencyID=7) passes the initial currency filter but is excluded from the final output because no WHERE-branch in #final matches it.
...
3. Filters to wire transfers only (FundingTypeID=2) in 5 currencies at the intermediate stage: USD (1), EUR (2), GBP (3), AUD (5), CAD (7)
4. Applies aging thresholds in #final: USD/EUR require >= 1 day; GBP requires >= 1 day; AUD requires > 2 days under CySEC or > 1 day under other regulations. CAD is effectively excluded — it passes the intermediate #cashouts filter but no OR-branch in the #final WHERE clause matches CAD, so CAD rows are always dropped. The effective output covers only 4 currencies (USD, EUR, GBP, AUD).
...
**What**: Only wire transfer withdrawals in specific currencies are monitored.

**Columns Involved**: `Currency`, `Amount$Withdraw`

**Rules**:
- FundingTypeID must equal 2 (Wire transfer) — all other payment methods are excluded
- ProcessCurrencyID must be one of: 1 (USD), 2 (EUR), 3 (GBP), 5 (AUD), 7 (CAD) — this is the #cashouts intermediate filter
- Currency column shows the Abbreviation from Dim_Currency for the ProcessCurrencyID
- Despite CAD being included in the intermediate filter, it is effectively excluded from the final output (see 2.2)

### 2.2 Payment Sent Aging Thresholds

**What**: Different currencies and regulations have different thresholds for when a "Payment Sent" status is considered overdue. The #final WHERE clause uses four OR-branches — only rows matching at least one branch survive.

**Columns Involved**: `DaysInPaymentSentStatus`, `Currency`, `Regulation`

**Rules**:
- Branch 1 — USD, EUR: flagged when DaysInPaymentSentStatus >= 1
- Branch 2 — AUD + CySEC regulation: flagged when DaysInPaymentSentStatus > 2
- Branch 3 — AUD + non-CySEC regulation: flagged when DaysInPaymentSentStatus > 1
- Branch 4 — GBP: flagged when DaysInPaymentSentStatus >= 1
- **CAD: no branch matches** — CAD rows from #cashouts are always excluded from #final. This is either an SP bug (CAD was intended but the WHERE clause was never updated) or by design (CAD was added to #cashouts speculatively but never operationalized). The net effect is that the final table will never contain CAD rows.
...
- **CAD is effectively excluded**: CAD (ProcessCurrencyID=7) passes the intermediate #cashouts currency filter but no OR-branch in the #final WHERE clause matches it, so CAD rows never appear in the output. This may be an SP bug or intentional.
...
|---|---------|------|----------|-------------|
| 1 | CID | int | YES | Customer ID identifying the account that initiated the withdrawal. Read directly from External_etoro_Billing_Withdraw (fbd.[CID]). The SP joins Dim_Customer only to obtain RegulationID, not to source CID. (Tier 2 — External_etoro_Billing_Withdraw) |
| 2 | Regulation | nvarchar(1000) | YES | Short code for the regulation. Values match production Dictionary.Regulation.Name. Used in V_Dim_Customer and analytics dashboards. Dim-lookup passthrough from Dim_Regulation.Name via Dim_Customer.RegulationID. (Tier 1 — Dictionary.Regulation) |
...
#cashouts (temp)
  ...
  <- Currencies: 1=USD, 2=EUR, 3=GBP, 5=AUD, 7=CAD
  v
#final (temp)
  <- #cashouts JOIN #LastUpdatedDate
  <- WHERE CashoutStatus_Funding = 'Payment Sent'
  <- Aging thresholds: USD/EUR>=1d, GBP>=1d, AUD>2d(CySEC)/AUD>1d(other)
  <- CAD: no matching branch — effectively excluded
  v
...
*Generated: 2026-04-29 | Quality: 8.5/10 | Phases: 10/14 (no Atlassian)*
*Tiers: 2 T1, 9 T2, 0 T3, 0 T4 | Elements: 11/11, Logic: 9/10, Relationships: 7/10, Sources: 8/10*
