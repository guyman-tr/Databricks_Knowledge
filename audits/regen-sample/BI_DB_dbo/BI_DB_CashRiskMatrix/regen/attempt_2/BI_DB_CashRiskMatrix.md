...
> Daily cash-risk scenario matrix: for every open CFD position per customer, captures the instrument's bid/ask prices and 47 price-shock scenarios (±1% through ±100%, plus ±200%/±300%/±400%/±900%) showing the net open position in units that would remain within the customer's stop/limit bounds if the market moved by each shock amount — used by the Risk desk to assess net exposure at hypothetical price moves.
...
### 2.3 Price-Scenario Columns (47 columns)
...
**Upside shock columns** (`UnitsNOP+1%` through `UnitsNOP+900%` — 25 columns):
...
**Downside shock columns** (`UnitsNOP-1%` through `UnitsNOP-100%` — 22 columns):
...
| # | Element | Type | Nullable | Description |
|---|---------|------|----------|-------------|
| 12 | Bid | decimal(16,6) | YES | Raw bid price before spread adjustment. Mid-price reference. Compare to BidSpreaded to derive the spread. (Tier 1 — DWH_dbo.Fact_CurrencyPriceWithSplit.Bid via SP_Fact_CurrencyPriceWithSplit_DL_To_Synapse). NULL for ~208 rows/day where no price record exists. |
...
#### Price-Shock Upside Scenarios — 25 columns
...
#### Price-Shock Downside Scenarios — 22 columns
...
| UnitsNOP+N% (×25) | SP_CashRiskMatrix | UnitsNOP, LimitRate/StopRate, Bid/Ask scenarios | CASE on LimitRate/StopRate vs shocked Bid/Ask; SUM aggregated |
...
etoro.Trade.Position / History.ClosePosition
  ...
  → DWH_dbo.Dim_Position [open CFD positions as of @Date]
...
etoro.Customer + BackOffice → DWH_dbo.Dim_Customer [IsValidCustomer=1 filter]
...
Fact_SnapshotEquity + Fact_CustomerUnrealized_PnL
  → DWH_dbo.V_Liabilities [DateID=@DateID, TotalCash]

All of the above feed SP_CashRiskMatrix(@Date):
  #Prices    ← Fact_CurrencyPriceWithSplit JOIN Dim_Instrument (×2 self-join for cross-currency bridge)
               → ConversionRate routing per instrument
  #Positions ← Dim_Position JOIN #Prices JOIN Dim_Customer JOIN Dim_Regulation JOIN Dim_Country
               → signed UnitsNOP + 94 shocked Bid/Ask price columns
  #Stage     ← #Positions → 47 UnitsNOP scenario columns
               (CASE on LimitRate/StopRate vs shocked Bid/Ask)
  #Final     ← #Stage LEFT JOIN V_Liabilities (TotalCash AVG) → GROUP BY
...
*Generated: 2026-04-28 | Regen Harness Attempt 2 | Quality: 8.0/10 | Phases: 11/14*
