...
- **Attempt**: `2`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_Daily_PI_Performance_COPYDATA_RuningSideBySide/regen/attempt_2/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_Daily_PI_Performance_COPYDATA_RuningSideBySide\regen\attempt_2`
...
Previous attempt scored **5.9** (FAIL). The adversarial judge required regeneration with the following specific fixes:
...
1. [high] `CopyEquity` — Tier 1 description substitutes the verbatim upstream formula from BI_DB_CopyDailyData.CopyAUM — ISNULL(SUM(Cash+Investment+PnL+DetachedPosInvestment+Dit_PnL), 0) — with the intermediate column reference ISNULL(CopyAUM, 0). An analyst cannot derive the AUM composition (Cash, Investment, PnL, DetachedPosInvestment, Dit_PnL) from the wiki.
2. [high] `Country` — Tier 1 origin is wrong. BI_DB_CopyDailyData.Country is itself Tier 1 — DWH_dbo.Dim_Country via Dictionary.Country. Per rubric, dim-lookup passthroughs must use the root origin: (Tier 1 — Dictionary.Country), not the intermediate table BI_DB_CopyDailyData.
3. [medium] `SymbolFull` — Tier 1 origin citation includes the intermediate dimension: (Tier 1 — DWH_dbo.Dim_Instrument via Trade.InstrumentMetaData). Rubric requires citing the dim's root origin directly: (Tier 1 — Trade.InstrumentMetaData). The 'via Dim_X' notation is explicitly prohibited.
4. [medium] `Property table` — Missing required fields: Refresh cadence (should be: Daily DELETE+INSERT on DateINT) and UC Target (should be: _Not_Migrated). These are required property table entries per the golden reference shape.
5. [medium] `Section 4 / Footer` — No Tier Legend table in the Elements section (Section 4). Footer is missing tier breakdown counts (e.g., Tiers: 5 T1, 10 T2, 6 T3), quality score, and phases-completed list. All three are required by the golden reference shape.

Tier 1 paraphrasing failures (must be fixed verbatim):

- **CopyEquity**:
  - Upstream: `Total AUM managed by this PI through copy relationships: ISNULL(SUM(Cash+Investment+PnL+DetachedPosInvestment+Dit_PnL), 0). Source: etoroGeneral_History_GuruCopiers.`
  - You wrote: `Total AUM managed by this PI through copy relationships: ISNULL(CopyAUM, 0); renamed CopyAUM → CopyEquity; wiki: knowledge/synapse/Wiki/BI_DB_dbo/Tables/BI_DB_CopyDailyData.md.`
