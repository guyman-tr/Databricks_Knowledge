...
| Dim_Channel | DWH_dbo | Channel/SubChannel classification (SP-computed CASE) | SubChannelID |
...
| Channel | Dim_Channel | Channel | Dim-lookup via Dim_Affiliate.SubChannelID; ISNULL default 'Direct'. Dim_Channel marks Channel as Tier 2 (SP-computed CASE from AffWizz MarketingExpenseName) | T2 |
| SubChannel | Dim_Channel | SubChannel | Dim-lookup via Dim_Affiliate.SubChannelID; ISNULL default 'Direct'. Dim_Channel marks SubChannel as Tier 2 (derived via parallel CASE expression) | T2 |
...
| Region | Dim_Country | Region | Dim-lookup passthrough via CountryID. Dim_Country.Region traces to Dictionary.MarketingRegion.Name | T1 |
...
| State | Dim_State_and_Province | Name | Dim-lookup via Dim_Customer.RegionID = RegionByIP_ID. Traces to Dictionary.RegionName.Name | T1 |
