...
- **Attempt**: `2`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_CIDFirstDates/regen/attempt_2/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_CIDFirstDates\regen\attempt_2`
...
Previous attempt scored **7.0** (FAIL). The adversarial judge required regeneration with the following specific fixes:
...
1. [high] `Channel (#7), SubChannel (#8)` — Tagged Tier 1 (fiktivo_dbo.tblaff_Affiliates via Dim_Channel) but Dim_Channel marks both columns as Tier 2 — they are SP-computed via a massive CASE expression, not dictionary passthroughs. Should be Tier 2 — SP_CIDFirstDates via Dim_Channel.
2. [high] `Region (#12)` — Tagged Tier 2 (SP_CIDFirstDates via Dim_Country.Region) but Region is a dim-lookup passthrough from Dim_Country.Region which traces to Dictionary.MarketingRegion.Name. Per dim-lookup rules, should be Tier 1 — Dictionary.MarketingRegion.
3. [high] `Email (#14)` — Upstream says 'Unique (case-insensitive via LowerEmail computed column). Email changes trigger Customer.LastChanges update via trigger.' Wiki says only 'Customer email address. Dynamically masked with default().' Dropped uniqueness, computed column, and trigger semantics.
4. [high] `Club (#5)` — Upstream Dim_PlayerLevel.Name says 'Tier display name: Bronze, Silver, Gold, Platinum, Platinum Plus, Diamond, Internal, N/A. Used in BackOffice reporting JOINs and customer-facing UI.' Wiki completely rewrites with distribution counts and lineage, dropping Internal/N/A and BackOffice context.
5. [medium] `State (#32)` — Tagged Tier 2 but is a dim-lookup passthrough from Dim_State_and_Province.Name, which traces to Dictionary.RegionName.Name. Should be Tier 1 — Dictionary.RegionName.
6. [medium] `BirthDate (#25)` — Upstream includes 'Used in LinkedAccountHash1 for duplicate detection and in KYC age verification.' Wiki drops the LinkedAccountHash1 reference — a specific technical use case lost.
7. [medium] `LabelName (#9)` — Upstream says 'Brand name displayed in BackOffice interfaces, reports, and internal systems. Multiple LabelIDs can share the same Name (e.g., 0, 1, 9 all = eToro).' Wiki completely rewrites, dropping the multi-ID sharing note.

Tier 1 paraphrasing failures (must be fixed verbatim):

- **Club**:
  - Upstream: `Tier display name: Bronze, Silver, Gold, Platinum, Platinum Plus, Diamond, Internal, N/A. Used in BackOffice reporting JOINs and customer-facing UI.`
  - You wrote: `eToro Club loyalty tier name. Values: Bronze (45.97M), Silver (287K), Gold (259K), Platinum (129K), Platinum Plus (92K), Diamond (11K). Dim-lookup from Dim_PlayerLevel.Name via PlayerLevelID.`
...
- **Email**:
  - Upstream: `Customer email address. Unique (case-insensitive via LowerEmail computed column). Email changes trigger Customer.LastChanges update via trigger.`
  - You wrote: `Customer email address. Dynamically masked with default().`
  - Loss: Dropped uniqueness constraint, LowerEmail computed column, trigger behavior; replaced with DDL masking info
- **BirthDate**:
  - Upstream: `Customer date of birth. Used in LinkedAccountHash1 for duplicate detection and in KYC age verification.`
  - You wrote: `Customer date of birth. Used in KYC age verification.`
  - Loss: Dropped LinkedAccountHash1 duplicate detection reference
- **Channel**:
  - Upstream: `Top-level marketing channel category. Derived from AffWizz MarketingExpense.MarketingExpenseName with overrides: 'Introducing Agents' -> 'Affiliate', AffiliateID IN (56662,56663) -> 'Direct'. (Tier 2 `
  - You wrote: `Top-level marketing channel category. Dim-lookup via Dim_Affiliate.SubChannelID -> Dim_Channel.Channel. ISNULL default 'Direct' for customers without affiliate mapping. Common values: Direct, SEM, SEO`
...
- **SubChannel**:
  - Upstream: `Granular sub-channel name within the parent Channel. Human-readable label for SubChannelID. Derived via parallel CASE expression alongside SubChannelID. (Tier 2 in Dim_Channel)`
  - You wrote: `Granular sub-channel name within the parent Channel. Dim-lookup via Dim_Affiliate.SubChannelID -> Dim_Channel.SubChannel. ISNULL default 'Direct'.`
...
- **LabelName**:
  ...
  - You wrote: `White-label broker brand name. Dim-lookup from Dim_Label.Name via LabelID. Most customers show 'eToro' (LabelID 0/1).`
