...
- POI reasons (codes 1-7, 14, 17-19, 21): Expired, Incomplete, Clearer Required, Missing, Front/Back side required, Missing expiry date, Missing Name details, Cannot be accepted, Under different Name, Screenshot not accepted
...
- ReasonNumber=0 indicates the rejection reason does not match any of the 22 standardised codes (e.g., "Duplicate", "Underage", "High Risk Country", "Corrupted File")
...
- **ReasonNumber = 0 is overloaded**: It covers all rejection reasons not in the 22-code lookup (Duplicate, Underage, High Risk Country, Corrupted File, SSN Card, Visa issues, etc.). Do not treat 0 as "no reason".
...
| 2 | ReasonNumber | int | YES | Numeric rejection reason code mapped from a hardcoded 22-row lookup in SP_Failed_Verification_MA. COALESCE(POI match, POA match, 0). Codes 1-7 = POI reasons, 8-16 = POA reasons, 17-22 = additional POI/POA reasons, 0 = unmatched (raw reason text preserved in RejectReasonName). (Tier 2 — SP_Failed_Verification_MA) |
...
BI_DB_dbo.BI_DB_Operations_Onboarding_Flow_UserKPIs (9.9M rows)
  ...
  -> SP_Failed_Verification_MA @Date
      ...
      -> INSERT INTO BI_DB_Failed_Verification_MA (~1K rows)
```
