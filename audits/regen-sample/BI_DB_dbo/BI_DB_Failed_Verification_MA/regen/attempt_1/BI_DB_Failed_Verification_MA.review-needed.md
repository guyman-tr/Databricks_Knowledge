...
1. **GCID NOT NULL in DDL but upstream says "NULL for older accounts"**: The DDL defines GCID as NOT NULL, but the upstream BI_DB_Operations_Onboarding_Flow_UserKPIs wiki (inherited from Customer.CustomerStatic) states GCID can be NULL for older accounts. The SP does not apply any ISNULL/COALESCE on GCID. If a customer with NULL GCID passes all WHERE filters, the INSERT would fail. Confirm whether this is an intentional exclusion or a latent bug.

2. **ReasonNumber = 0 covers diverse reasons**: When the rejection reason text does not match any of the 22 hardcoded codes, ReasonNumber defaults to 0. This groups genuinely different reasons (Duplicate, Underage, High Risk Country, Corrupted File, SSN Card issues, Visa issues, Other, Not Needed) under the same code. Marketing Automation workflows relying on ReasonNumber alone will lose granularity for these cases.
