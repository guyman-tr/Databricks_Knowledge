...
- **Attempt**: `2`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_Deposits/regen/attempt_2/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_Deposits\regen\attempt_2`
- **Bundle path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_Deposits\regen\_upstream_bundle.md`
- **DDL path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Tables\BI_DB_dbo.BI_DB_Deposits.sql`
...
Previous attempt scored **6.65** (FAIL). The adversarial judge required regeneration with the following specific fixes:
...
2. [high] `CreditCardType` — Upstream Dim_CardType.CarTypeName verbatim includes full 18-item enum (0=None through 17=GE Capital), uniqueness constraint, usage context, and rename note. Writer condensed to 4 examples and a passthrough note, dropping the complete reference data.
...
4. [high] `DepoName` — Writer used fabricated examples ('Checkout, PayPal, Wire, Tribe') not appearing in Dim_BillingDepot upstream ('MoneyBookers USD', 'Neteller', 'Wire'). 'Checkout' and 'Tribe' are invented. Also drops 'Unique across all depots.' and usage context ('admin dashboards, routing logs, and discrepancy reports').
5. [medium] `RiskManagementStatusID` — Upstream Fact_BillingDeposit description includes '69 distinct risk reason codes.' and 'Key codes: 1=Success, 35=DeclineByRRE, 47=ML, 49=CustomerToFundingViolation.' — all dropped. Analyst has no diagnostic vocabulary for interpreting risk management status values.

Tier 1 paraphrasing failures (must be fixed verbatim):

- **RiskManagementStatusID**:
  - Upstream: `Result of the pre-processing risk management check. 69 distinct risk reason codes. NULL=no risk check recorded. Key codes: 1=Success, 35=DeclineByRRE, 47=ML, 49=CustomerToFundingViolation.`
  - You wrote: `Result of the pre-processing risk management check. NULL=no risk check recorded.`
  - Loss: Drops 69-code count and all key code enumerations (1=Success, 35=DeclineByRRE, 47=ML, 49=CustomerToFundingViolation)
- **PaymentStatus_PaymentStatusID**:
  - Upstream: `Primary key identifying the payment state. 1=Pending, 2=InProcess, 3=Processed, 4=Canceled, 5=Failed, 6=Reversed, 7=CompletedExternally.`
  - You wrote: `Denormalized payment status ID from Dim_PaymentStatus. Passthrough from Dim_PaymentStatus.`
  - Loss: Entire upstream text replaced with generic passthrough statement; all enum values dropped
- **Funnel**:
  - Upstream: `Unique human-readable label for the funnel. Used in marketing reports, BackOffice customer views, and attribution analytics. Describes the campaign/channel/product that drove registration.`
  - You wrote: `Funnel name at deposit time, resolved from Dim_Funnel on fbd.FunnelID. Passthrough from Dim_Funnel.Name.`
  - Loss: Entire upstream description replaced with routing statement; all meaning and usage context lost
- **FunnelFrom**:
  - Upstream: `Unique human-readable label for the funnel. Used in marketing reports, BackOffice customer views, and attribution analytics. Describes the campaign/channel/product that drove registration.`
  - You wrote: `Source funnel variant name for the customer, resolved from Dim_Funnel on Dim_Customer.FunnelFromID. Passthrough from Dim_Funnel.Name.`
  - Loss: Same failure as Funnel — upstream description replaced with routing statement
- **AcquisitionFunnel**:
  - Upstream: `Unique human-readable label for the funnel. Used in marketing reports, BackOffice customer views, and attribution analytics. Describes the campaign/channel/product that drove registration.`
  - You wrote: `Registration funnel name for the customer, resolved from Dim_Funnel on Dim_Customer.FunnelID. Passthrough from Dim_Funnel.Name.`
  - Loss: Same failure as Funnel — upstream description replaced with routing statement
- **CreditCardType**:
  - Upstream: `Card brand name. Unique constraint prevents duplicates in production. Used in payment UI, transaction records, and fraud reporting. Renamed from `Name` in production. 0=None, 1=Visa, 2=Master Card, 3=`
  - You wrote: `Card brand name (e.g., Visa, Master Card, Diners, Maestro). Passthrough from Dim_CardType.CarTypeName.`
...
- **DepoName**:
  ...
  - You wrote: `Human-readable depot name (e.g., Checkout, PayPal, Wire, Tribe). Passthrough from Dim_BillingDepot.Name.`
