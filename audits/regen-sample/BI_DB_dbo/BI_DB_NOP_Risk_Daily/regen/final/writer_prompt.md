...
- **Attempt**: `2`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_NOP_Risk_Daily/regen/attempt_2/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_NOP_Risk_Daily\regen\attempt_2`
...
1. [high] `DateID` — Tagged Tier 2 (BI_DB_PositionPnL) but is a direct passthrough from BI_DB_PositionPnL which has a wiki documenting DateID. Should be Tier 1 with verbatim description from BI_DB_PositionPnL.
2. [high] `IsSettled` — Tagged Tier 5 (Expert Review) but is a passthrough from BI_DB_PositionPnL which has a wiki documenting IsSettled as '1 = real asset, 0 = CFD asset. Rewound via PositionChangeLog (ChangeTypeID = 13) when applicable.' Should be Tier 1 from BI_DB_PositionPnL.
3. [medium] `InstrumentID` — Source relay: cited (Tier 1 — Trade.PositionTbl) tracing 3 hops back through Dim_Position. Immediate upstream with wiki is BI_DB_PositionPnL. Should cite BI_DB_PositionPnL as the source.
4. [medium] `InstrumentDisplayName` — DDL declares varchar(200) but upstream Dim_Instrument wiki shows varchar(100). The wider target column is not called out as a discrepancy.
5. [low] `Section 4 — Tier Legend` — Tier legend only lists Tiers 1, 2, and 5. Standard shape requires full tier legend with star ratings for all tiers.
