...
- **Attempt**: `1`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_NOP_Risk_Daily/regen/attempt_1/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_NOP_Risk_Daily\regen\attempt_1`
- **Bundle path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_NOP_Risk_Daily\regen\_upstream_bundle.md`
- **DDL path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Tables\BI_DB_dbo.BI_DB_NOP_Risk_Daily.sql`
...
# Pre-Resolved Upstream Bundle for `BI_DB_dbo.BI_DB_NOP_Risk_Daily`

This bundle was assembled deterministically by the regen harness BEFORE the writer claude process started. Use this as your AUTHORITATIVE Tier 1 inheritance source. Quote descriptions VERBATIM from the upstream wikis below for any column that is a passthrough or rename of an upstream column. Do NOT paraphrase. Do NOT generalize vendor names. Do NOT drop NULL semantics.

---

## Source DDL — `BI_DB_dbo.BI_DB_NOP_Risk_Daily.sql`

```sql
CREATE TABLE [BI_DB_dbo].[BI_DB_NOP_Risk_Daily]
(
	[DateID] [int] NOT NULL,
	[InstrumentID] [int] NOT NULL,
	[IsSettled] [int] NULL,
	[InstrumentType] [varchar](50) NULL,
	[InstrumentDisplayName] [varchar](200) NULL,
	[SellBuy] [varchar](50) NULL,
	[NOP] [bigint] NULL,
...
### SP `BI_DB_dbo.SP_NOP_TradingActivity_Risk_Daily`
- **Path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Stored Procedures\BI_DB_dbo.SP_NOP_TradingActivity_Risk_Daily.sql`
...
GO
CREATE PROC [BI_DB_dbo].[SP_NOP_TradingActivity_Risk_Daily] @Date1 [DATE] AS
BEGIN
SET NOCOUNT ON;
--DECLARE @Date1 DATE='2022-02-01'
DECLARE @Date2 DATE=DATEADD(DAY,-1,@Date1)
DECLARE @Date3 DATE=DATEADD(MONTH,-1,@Date1)
DECLARE @DateINT1 INT = CAST(replace(CONVERT(VARCHAR(10), @Date1),'-','') AS INT)
DECLARE @DateINT2 INT = CAST(replace(CONVERT(VARCHAR(10), @Date2),'-','') AS INT)
DECLARE @DateINT3 INT = CAST(replace(CONVERT(VARCHAR(10), @Date3),'-','') AS INT)

/********** Step 01 **********/
/********** all opened and closed positions at @Date**********/
...
AS
 ...
 SELECT CAST(dp.OpenOccurred AS Date)  as 'Date',

	   dp.OpenDateID AS 'DateID',
	   CASE WHEN di.InstrumentTypeID IN (5,6) AND dp.IsSettled=1 THEN 'RealStocksETF'
	   WHEN di.InstrumentTypeID IN (5,6) AND dp.IsSettled=0 THEN 'CFDStocksETF'
	   ...
	   WHEN	di.InstrumentTypeID IN (1) THEN 'Currencies'
	   WHEN	di.InstrumentTypeID IN (2) then 'Commodities'
	   WHEN di.InstrumentTypeID IN (4) then 'Indecies'
	   ELSE 'Check' END AS 'InstrumentType'	,
	   di.InstrumentDisplayName,
	   CASE WHEN dp.IsBuy=1 THEN 'Buy' ELSE 'Sell' END  AS 'SellBuy',
	   di.InstrumentID,

	   'Opened' as 'Type',
	   'Not Relevant' AS 'CloseReason',
	   CAST(SUM(ISNULL(dp.Volume,0)) AS BIGINT) as 'Volume',
	   COUNT_BIG(* ) AS Trades,
	   COUNT(DISTINCT(CID)) AS 'Customers'
	   

from DWH_dbo.Dim_Position dp
inner join DWH_dbo.Dim_Instrument di on di.DWHInstrumentID=dp.InstrumentID
INNER JOIN DWH_dbo.Fact_SnapshotCustomer fsc ON fsc.RealCID=dp.CID	  AND  fsc.IsValidCustomer=1
INNER JOIN DWH_dbo.Dim_Range dr ON dr.DateRangeID=fsc.DateRangeID AND dp.OpenDateID BETWEEN dr.FromDateID AND dr.ToDateID

 where OpenDateID =@DateINT1
 ...
 /**************************prices for run date and day before**********************/
...
CREATE TABLE #prices 
 ...
 SELECT fcpws.InstrumentID,
		fcpws.OccurredDateID,
		(fcpws.Ask+fcpws.Bid)/2	 AS 'Price'
 FROM	DWH_dbo.Fact_CurrencyPriceWithSplit fcpws
 WHERE fcpws.OccurredDateID IN (@DateINT2,@DateINT1)

/********** Step 02 **********/
/*Positions that Closed at @Date agg by date,dateid,CID,Instrument, Closed */
 IF OBJECT_ID('tempdb..#cp1') IS NOT NULL Drop TABLE #cp1

CREATE TABLE #cp1 
	WITH (HEAP,DISTRIBUTION=ROUND_ROBIN)
 AS
 SELECT CAST(dp.CloseOccurred AS DATE)  as 'Date',
	   ...
	   CAST(SUM(ISNULL(dp.VolumeOnClose,0)) AS BIGINT) as 'Volume',
	   COUNT_BIG(* ) AS Trades,
	   COUNT(DISTINCT(CID)) AS 'Customers'
...
INNER JOIN DWH_dbo.Dim_ClosePositionReason dcpr ON dp.ClosePositionReasonID = dcpr.ClosePositionReasonID
 ...
 GROUP BY  CAST(dp.CloseOccurred AS DATE)  ,
	   ...
	   di.InstrumentID
/********** Step 03 **********/
/*union tables with opened and closed positions */
 IF OBJECT_ID('tempdb..#unioned') IS NOT NULL Drop TABLE #unioned

CREATE TABLE #unioned 
	WITH (HEAP,DISTRIBUTION=ROUND_ROBIN)
 AS
SELECT a.*,
       p.Price 'ClosedPrice',
	   p1.Price 'OpenedPrice',
	   p.Price -p1.Price  AS 'PriceChange'
...
LEFT JOIN #prices p ON p.InstrumentID=a.InstrumentID AND p.OccurredDateID=@DateINT1
LEFT JOIN #prices p1 ON p1.InstrumentID=a.InstrumentID AND p1.OccurredDateID=@DateINT2

/********** Step 04 **********/
/*Daily Nop */
 IF OBJECT_ID('tempdb..#dailynop') IS NOT NULL Drop TABLE #dailynop

CREATE TABLE #dailynop 
	WITH (HEAP,DISTRIBUTION=ROUND_ROBIN)
 AS
 SELECT dp.DateID,
		dp.InstrumentID,
 ...
 FROM BI_DB_dbo.BI_DB_PositionPnL dp 
 INNER JOIN DWH_dbo.Dim_Instrument di on di.DWHInstrumentID=dp.InstrumentID
 WHERE dp.DateID =@DateINT1
 GROUP BY  dp.DateID,
		   dp.InstrumentID,
		   dp.IsSettled,
		   CASE WHEN di.InstrumentTypeID IN (5,6) AND dp.IsSettled=1 THEN 'RealStocksETF'
 ...
 /********** Step 05 **********/
/*delete and insert - both tables */
DELETE FROM BI_DB_dbo.[BI_DB_TradingActivity_Risk_Daily]
WHERE DateID=@DateINT1 OR DateID<@DateINT3

INSERT INTO   BI_DB_dbo.[BI_DB_TradingActivity_Risk_Daily]
	  (Date
	  ,DateID
	  ,InstrumentType
	  ,InstrumentDisplayName
...
DELETE FROM BI_DB_dbo.[BI_DB_NOP_Risk_Daily]
WHERE DateID=@DateINT1	OR DateID<@DateINT3

INSERT INTO  BI_DB_dbo.[BI_DB_NOP_Risk_Daily]
      ( DateID
	  ,InstrumentID
	  ,IsSettled
	  ,InstrumentType
...
--	[NOP] [bigint] NULL,
--	[UpdateDate] [datetime] NOT NULL
--)  WITH (CLUSTERED INDEX ([DateID]),DISTRIBUTION=ROUND_ROBIN)
...
|---|---|---|---|---|
| `BI_DB_dbo.SP_NOP_TradingActivity_Risk_Daily` | synapse_sp | BI_DB_dbo | SP_NOP_TradingActivity_Risk_Daily | `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Stored Procedures\BI_DB_dbo.SP_NOP_TradingActivity_Risk_Daily.sql` |
...
| `BI_DB_dbo.BI_DB_TradingActivity_Risk_Daily` | unresolved | BI_DB_dbo | BI_DB_TradingActivity_Risk_Daily | `—` |
