...
- **Attempt**: `1`
- **Output directory** (relative to repo root): `audits/regen-sample/BI_DB_dbo/BI_DB_Social_Activity/regen/attempt_1/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_Social_Activity\regen\attempt_1`
- **Bundle path**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\BI_DB_dbo\BI_DB_Social_Activity\regen\_upstream_bundle.md`
- **DDL path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Tables\BI_DB_dbo.BI_DB_Social_Activity.sql`
...
# Pre-Resolved Upstream Bundle for `BI_DB_dbo.BI_DB_Social_Activity`

This bundle was assembled deterministically by the regen harness BEFORE the writer claude process started. Use this as your AUTHORITATIVE Tier 1 inheritance source. Quote descriptions VERBATIM from the upstream wikis below for any column that is a passthrough or rename of an upstream column. Do NOT paraphrase. Do NOT generalize vendor names. Do NOT drop NULL semantics.

---

## Source DDL — `BI_DB_dbo.BI_DB_Social_Activity.sql`

```sql
CREATE TABLE [BI_DB_dbo].[BI_DB_Social_Activity]
(
	[ActionTypeID] [smallint] NOT NULL,
	[ActionDate] [datetime] NOT NULL,
	[PostID] [varchar](300) NULL,
	[CommentID] [varchar](150) NULL,
	[Username] [varchar](100) NULL,
	[RealCID] [int] NULL,
	[MessageText] [nvarchar](max) NULL,
	[MessageSize] [int] NULL,
	[MessageWordNum] [int] NULL,
	[ActionDateID] [int] NULL,
...
### SP `BI_DB_dbo.SP_Social_Activity`
- **Path**: `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Stored Procedures\BI_DB_dbo.SP_Social_Activity.sql`
...
GO
CREATE PROC [BI_DB_dbo].[SP_Social_Activity] @dt [DATE] AS 
 BEGIN


 EXECUTE [BI_DB_dbo].[SP_Create_External_Streams_dbo_Entries_Range] @dt,@dt,'BI_DB_dbo.Streams_dbo_Entries_Social_Activity_Daily'
/*Delete run date activity*/

DECLARE @DateID  INT = CAST(CONVERT(CHAR(8),@dt,112) AS INT) 


DELETE FROM BI_DB_dbo.BI_DB_Social_Activity
WHERE ActionDateID = @DateID

IF OBJECT_ID('tempdb..#Entries') IS NOT NULL DROP TABLE #Entries
	CREATE TABLE #Entries  
    WITH (DISTRIBUTION = ROUND_ROBIN,CLUSTERED INDEX 
	(OccurredAt) )
	--(Id,OccurredAt,Username,TypeName,RootId,ParentId,SharedEntryId) )
	AS
/*Get all relevant data of @dt*/
SELECT 
	Id,
	OccurredAt,
	SUBSTRING([Username], 2, 50) as Username,
	MessageBody,
	TypeName,
	RootId,
	ParentId,
	SharedEntryId,
	[RichMediaScrapDataType]
FROM BI_DB_dbo.Streams_dbo_Entries_Social_Activity_Daily


IF OBJECT_ID('tempdb..#Dim_Customer') IS NOT NULL DROP TABLE #Dim_Customer
CREATE TABLE #Dim_Customer  
WITH (DISTRIBUTION = HASH(RealCID),CLUSTERED INDEX(LOWER_UserName)) 
AS
select RealCID
	  ,LOWER(DC.UserName)  as LOWER_UserName
from DWH_dbo.Dim_Customer DC with (nolock)
/*Insert Post Actions*/
INSERT INTO BI_DB_dbo.BI_DB_Social_Activity
           (
            [ActionTypeID]
           ,[ActionDate]
           ,[PostID]
...
SELECT 
	...
	CASE WHEN TypeName = 'Discussion' THEN 1 ELSE 5 END as [ActionTypeID]
   ...
   ,LEN(MessageBody) AS [MessageSize]
   ,LEN(MessageBody) - LEN(REPLACE(MessageBody, ' ', '')) + 1  [MessageWordNum]
   ,CAST(CONVERT(varchar, @dt, 112) AS INT)
   ,getdate()
   ,E.ParentId
   ,TypeName
   ,[RichMediaScrapDataType] MediaTypeID
FROM 
	#Entries E
	INNER JOIN 
	#Dim_Customer DC with (nolock)
	ON E.[Username] = LOWER_UserName COLLATE Latin1_General_100_BIN 
WHERE 
	TypeName in 
	(
	 'Discussion'
...
/*Insert Likes Data for Comments*/ 
INSERT INTO BI_DB_dbo.BI_DB_Social_Activity
           ([ActionTypeID]
           ,[ActionDate]
           ,[PostID]
...
|---|---|---|---|---|
| `BI_DB_dbo.SP_Social_Activity` | synapse_sp | BI_DB_dbo | SP_Social_Activity | `c:\Users\guyman\Documents\github\DataPlatform\SynapseSQLPool1\sql_dp_prod_we\BI_DB_dbo\Stored Procedures\BI_DB_dbo.SP_Social_Activity.sql` |
| `BI_DB_dbo.Streams_dbo_Entries_Social_Activity_Daily` | unresolved | BI_DB_dbo | Streams_dbo_Entries_Social_Activity_Daily | `—` |
