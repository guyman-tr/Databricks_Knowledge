...
|---|---|---|---|
| `SP_Client_Balance_Check_Opening_Balance` | BI_DB_dbo | Stored Procedure | Writer — DELETE + INSERT pattern, called from SP_Client_Balance_New daily |
| `BI_DB_Client_Balance_Aggregate_Level_New` | BI_DB_dbo | Table | Source of `Deposits` and `InternalTransferDeposits` used to compute `Deposits_CB` |
...
| 4 | Error_Message | Tier 2 | SP_Client_Balance_Check_Opening_Balance | Constructed string if `Balance_diff_deposit <> 0`; NULL when check passes (variable declared but never SET in success branch) | ETL-generated diagnostic message; NULL on clean run |
| 5 | UpdateDate | Tier 2 | SP_Client_Balance_Check_Opening_Balance | `GETDATE()` at INSERT time | Timestamp of last SP execution; can be stale if RAISERROR aborts batch before DELETE/INSERT |
