...
- **Attempt**: `2`
- **Output directory** (relative to repo root): `audits/regen-sample/Dealing_dbo/Dealing_Islamic_Daily_Administrative_Fee/regen/attempt_2/`
- **Absolute output directory**: `C:\Users\guyman\Documents\github\Databricks_Knowledge\audits\regen-sample\Dealing_dbo\Dealing_Islamic_Daily_Administrative_Fee\regen\attempt_2`
...
2. [high] `AmountInUnitsDecimal (#33)` — Upstream says 'Position size in units/shares. Fractional lots.' but wiki paraphrases to 'Position size in instrument units (e.g., shares, crypto coins). Fractional lots.' — rewording and injection of 'crypto coins' not in upstream.
3. [medium] `Footer` — Footer claims '8 T1, 34 T2' but actual element count is 11 T1, 31 T2. Three Tier 1 columns are miscounted.
4. [medium] `OpenDateID, CloseDateID (columns 7-8)` — Tagged Tier 2 but pass through from Dim_Position whose wiki is in the bundle. Should be Tier 1. OpenDateID drops 'E.g., 20260310' and 'NOT a FK to Dim_Date by default'. CloseDateID drops 'Partition column. Always include in WHERE clause.'
5. [low] `Admin_Fee_USD, GracePeriod, InstrumentGroup, InstrumentName_InstrumentGroup, InstrumentType_ID_InstrumentGroup, Units_per_Contract` — Config table passthrough columns have upstream wikis in the bundle (Dealing_Islamic_Admin_Fee_Per_Group, Dealing_Islamic_Instruments_Groups, Dealing_Islamic_Units_Per_Contract) but are tagged Tier 2 instead of Tier 1.

Tier 1 paraphrasing failures (must be fixed verbatim):

- **AmountInUnitsDecimal**:
  - Upstream: `Position size in units/shares. Fractional lots.`
  - You wrote: `Position size in instrument units (e.g., shares, crypto coins). Fractional lots. Passthrough from Dim_Position.`
