...
23 Tier 1 columns (passthrough from upstream wikis), 19 Tier 2 columns (ETL-computed or passthrough from tables without upstream wikis in the bundle). No Tier 3 or Tier 4 columns.
...
### 2. Dim_Instrument and Dim_ExchangeInfo Columns — Tier 2

Columns sourced from `Dim_Instrument` (InstrumentTypeID, InstrumentType, InstrumentName, Exchange) and `Dim_ExchangeInfo` (ExchangeID) are marked Tier 2 because no upstream wiki was available in the bundle for these dimension tables. If Dim_Instrument or Dim_ExchangeInfo wikis become available, these could be upgraded to Tier 1 with their production origins.
