ALTER PROC [BI_DB_dbo].[SP_DDR_Fact_Fact_MIMO_AllPlatforms] @date [DATE] AS 
...
2026-06-01		Guy M			REQ-24699: $1 synthetic bad-FTD cohort fix.
								(a) Added REMOVE_BAD_FTDS predicate to both recovery UPDATEs (eMoney + TP).
								(b) Added final demotion UPDATE that zeros IsPlatformFTD + IsGlobalFTD for the bad 
								    cohort rows that entered the table via the upstream platform SPs (whose IsFTD 
								    flag is set via Dim_Customer.FTDTransactionID match ג€” no filter at that layer).
								Bad dates: 2025-08-18/19/20 + 2026-05-22/23/25.
								Un-blacklist via HAVING COUNT > 1 preserved (legit later FTDs are NOT demoted).
----------    ----------   ------------------------------------*/      
...
/*----------------------------------------------------------------
2026-06-01 / REQ-24699
Final demotion of $1 synthetic bad-FTD cohort.

Required because:
  1. Upstream BI_DB_DDR_Fact_MIMO_Trading_Platform.IsFTD is set via
     Dim_Customer.FTDTransactionID match (no REMOVE_BAD_FTDS filter),
     so the bad cohort enters AllPlatforms with IsPlatformFTD=1.
  2. Even after the recovery-UPDATE filter (above) is applied, the row
     keeps IsPlatformFTD=1 from the initial INSERT.

This UPDATE demotes BOTH flags to 0 so downstream (Customer_Daily_Status,
Customer_Periodic_Status, DDR reports) see the cohort as non-FTD without
having to fan out the filter into every upstream platform SP. The row
itself is kept (transaction is real, just not credited as an FTD).

Un-blacklist via HAVING COUNT > 1 preserved ג€” users who later make a
legitimate non-$1 FTD on the platform are NOT demoted.
-----------------------------------------------------------------*/

UPDATE t1
SET IsPlatformFTD = 0, IsGlobalFTD = 0
FROM BI_DB_dbo.BI_DB_DDR_Fact_MIMO_AllPlatforms t1
WHERE t1.DateID = @dateID
  AND t1.MIMOAction = 'Deposit'
  AND (t1.IsPlatformFTD = 1 OR t1.IsGlobalFTD = 1)
  AND t1.RealCID IN (
      SELECT dc.RealCID
      FROM DWH_dbo.Dim_Customer dc
      WHERE CAST(dc.FirstDepositDate AS DATE) IN (
