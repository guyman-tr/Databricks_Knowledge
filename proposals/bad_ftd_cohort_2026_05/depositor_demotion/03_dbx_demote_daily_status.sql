/*===========================================================================
  03_dbx_demote_daily_status.sql

  Purpose : One-shot UPDATE on
            main.bi_db.gold_sql_dp_prod_we_bi_db_dbo_bi_db_ddr_customer_daily_status
            to zero out depositor / FTD anchor columns for all 30,980 bad
            $1 FTD cohort RealCIDs across every snapshot DateID.
  ...
  Pre-req : DO NOT RUN until both
              (a) REQ-25250 (PR #3875) is merged and Synapse rerun has run
              (b) The DBX going-forward SP patch (05_dbx_sp_patch_daily_status.sql)
                  is also deployed, otherwise tonight's run will re-introduce
                  IsDepositor=1 for the current etr_ymd row.

  Estimated rows : ~30,980 RealCIDs x avg ~285 days = ~8.8M rows.
